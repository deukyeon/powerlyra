...
struct scheduler_factory {
  ...
  static ischeduler_type* new_scheduler_impl(size_t num_vertices,
                                             const graphlab_options& opts) {
    ischeduler_type* scheduler_ptr = new Scheduler(num_vertices, opts);
   ...
   */
  static ischeduler_type* new_scheduler(size_t num_vertices,
                                        const graphlab_options& opts) {
...
#define __GENERATE_NEW_SCHEDULER__(r_unused, data_unused, i, elem) \
  ...
  if (scheduler_str == BOOST_PP_TUPLE_ELEM(3, 0, elem)) {          \
    typedef BOOST_PP_TUPLE_ELEM(3, 1, elem) scheduler_type;        \
    return new_scheduler_impl<scheduler_type>(num_vertices, opts); \
  }
...
#undef __GENERATE_NEW_SCHEDULER__
    logstream(LOG_FATAL) << "Invalid scheduler type: " << scheduler_str
                         << std::endl;
    return NULL;
  }  // end of new_scheduler
