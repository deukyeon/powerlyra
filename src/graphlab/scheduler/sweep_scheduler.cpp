...
sweep_scheduler::sweep_scheduler(size_t num_vertices,
  ...
  if (strict_round_robin) {
    logstream(LOG_INFO) << "Using a strict round robin schedule." << std::endl;
    // Max iterations only applies to strict round robin
    if (max_iterations != std::numeric_limits<size_t>::max()) {
      logstream(LOG_INFO) << "Using maximum iterations: " << max_iterations
                          << std::endl;
    }
    rr_index = 0;
  } else {
...
sched_status::status_enum sweep_scheduler::get_next(const size_t cpuid,
  ...
  for (size_t idx = get_and_inc_index(cpuid), fails = 0; fails <= max_fails;  //
    ...
    if (__builtin_expect(idx >= num_vertices, false))
      return sched_status::EMPTY;
