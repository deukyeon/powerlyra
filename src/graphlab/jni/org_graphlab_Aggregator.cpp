...
JNIEXPORT void JNICALL Java_org_graphlab_Aggregator_initNative(JNIEnv *env,
                                                               jclass clazz) {
  // Aggregator#exec
  ...
  // Aggregator#finalize
  proxy_aggregator::java_finalize = env->GetMethodID(clazz, "finalize", "(J)V");
...
proxy_aggregator::proxy_aggregator(JNIEnv *env, jobject &obj)
    : java_any(env, obj) {}

proxy_aggregator::proxy_aggregator() : java_any() {}
...
void proxy_aggregator::operator()(icontext_type &context) {
  ...
  JNIEnv *env = core::get_jni_env();
  env->CallVoidMethod(obj(), java_exec, &context, context.vertex_data().obj());
