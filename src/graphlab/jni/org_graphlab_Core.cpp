...
JNIEXPORT jlong JNICALL Java_org_graphlab_Core_createCore__(JNIEnv *env,
                                                            jobject obj) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_destroyCore(JNIEnv *env,
                                                          jobject obj,
                                                          jlong ptr) {
  if (NULL == env || 0 == ptr) {
    proxy_updater::core::throw_exception(
        env, "java/lang/IllegalArgumentException", "ptr must not be null.");
    return;
  }
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_resizeGraph(JNIEnv *env,
                                                          jobject obj,
                                                          jlong ptr,
                                                          jint count) {
  if (NULL == env || 0 == ptr) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_addVertex(JNIEnv *env,
  ...
  // add to graph
  (*jni_core)().graph().add_vertex(vertex_id, proxy_vertex(env, app_vertex));
}

JNIEXPORT void JNICALL Java_org_graphlab_Core_addEdge(JNIEnv *env, jobject obj,
                                                      jlong ptr, jint source,
                                                      jint target,
                                                      jobject app_edge) {
  if (NULL == env || 0 == ptr) {
    proxy_updater::core::throw_exception(
...
JNIEXPORT jdouble JNICALL Java_org_graphlab_Core_start(JNIEnv *env, jobject obj,
                                                       jlong ptr) {
  if (NULL == env || 0 == ptr) {
    proxy_updater::core::throw_exception(
        env, "java/lang/IllegalArgumentException", "ptr must not be null.");
...
JNIEXPORT jlong JNICALL Java_org_graphlab_Core_lastUpdateCount(JNIEnv *env,
                                                               jobject obj,
                                                               jlong ptr) {
  if (NULL == env || 0 == ptr) {
    proxy_updater::core::throw_exception(
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_addGlobal(JNIEnv *env,
                                                        jobject obj, jlong ptr,
                                                        jstring key,
                                                        jobject to_store) {
  if (NULL == env || 0 == ptr) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_setGlobal(JNIEnv *env,
                                                        jobject obj, jlong ptr,
                                                        jstring key,
                                                        jobject to_store) {
  if (NULL == env || 0 == ptr) {
...
JNIEXPORT jobject JNICALL Java_org_graphlab_Core_getGlobal(JNIEnv *env,
                                                           jobject obj,
                                                           jlong ptr,
                                                           jstring key) {
  if (NULL == env || 0 == ptr) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_setNCpus(JNIEnv *env, jobject obj,
                                                       jlong ptr, jlong ncpus) {
  if (NULL == env || 0 == ptr) {
    proxy_updater::core::throw_exception(
        env, "java/lang/IllegalArgumentException", "ptr must not be null.");
    return;
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_setScopeType(JNIEnv *env,
                                                           jobject obj,
                                                           jlong ptr,
                                                           jstring scope_str) {
  if (NULL == env || 0 == ptr) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_schedule(JNIEnv *env, jobject obj,
                                                       jlong core_ptr,
                                                       jobject updater,
                                                       jint vertex_id) {
  if (NULL == env || 0 == core_ptr) {
    proxy_updater::core::throw_exception(env,
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_scheduleAll(JNIEnv *env,
                                                          jobject obj,
                                                          jlong core_ptr,
                                                          jobject updater) {
  if (NULL == env || 0 == core_ptr) {
...
JNIEXPORT void JNICALL Java_org_graphlab_Core_addAggregator(
    JNIEnv *env, jobject obj, jlong core_ptr, jstring key, jobject aggregator,
    jlong frequency) {
  if (NULL == env || 0 == core_ptr) {
    proxy_updater::core::throw_exception(
        env, "java/lang/IllegalArgumentException",
  ...
  (*jni_core)().add_aggregator(std::string(key_str),
                               proxy_aggregator(env, aggregator), frequency);
  env->ReleaseStringUTFChars(key, key_str);
}

JNIEXPORT void JNICALL Java_org_graphlab_Core_aggregateNow(JNIEnv *env,
                                                           jobject obj,
                                                           jlong core_ptr,
                                                           jstring key) {
  if (NULL == env || 0 == core_ptr) {
