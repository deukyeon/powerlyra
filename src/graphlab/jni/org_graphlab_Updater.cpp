...
JNIEXPORT void JNICALL Java_org_graphlab_Updater_initNative(JNIEnv *env,
                                                            jclass clazz) {
  ...
  proxy_updater::java_priority = env->GetMethodID(clazz, "priority", "()D");
  ...
  proxy_updater::java_scatter =
      env->GetMethodID(clazz, "scatter", "(JLjava/lang/Object;)V");
}
...
proxy_updater::proxy_updater(JNIEnv *env, jobject &obj) : java_any(env, obj) {}
...
void proxy_updater::operator()(icontext_type &context) {
  ...
  JNIEnv *env = core::get_jni_env();
  env->CallVoidMethod(obj(), java_update, &context, vertex);
...
void proxy_updater::scatter(icontext_type &context, const edge_type &edge) {
  JNIEnv *env = core::get_jni_env();
  env->CallVoidMethod(obj(), java_scatter, &context,
                      context.const_edge_data(edge).obj());
  handle_exception(env);
}
