 ...
 *                  std::equal_to<Key>
 */
template <typename Key, typename Hash = _HOPSCOTCH_SET_DEFAULT_HASH,
...
class hopscotch_set {
 public:
  ...
  typedef hopscotch_table<storage_type, Hash, KeyEqual> container_type;
 ...
 public:
  hopscotch_set(size_t initialsize = 32, Hash hashfun = Hash(),
  ...
  ~hopscotch_set() { destroy_all(); }

  hasher hash_function() const { return hashfun; }

  KeyEqual key_eq() const { return equalfun; }
  ...
  iterator find(value_type const& v) { return container->find(v); }

  const_iterator find(value_type const& v) const { return container->find(v); }

  size_t count(value_type const& v) const { return container->count(v); }

  bool erase(iterator iter) { return container->erase(iter); }

  bool erase(value_type const& v) { return container->erase(v); }
  ...
  float load_factor() const { return container->load_factor(); }
