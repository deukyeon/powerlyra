...
class union_find {
 private:
  ...
  bool is_root(IDType i) { return setid[i].first == (IDType)i; }
...
class concurrent_union_find {
 private:
  ...
  bool is_root(uint32_t i) { return setid[i].d.next == i; }

  bool updateroot(uint32_t x, uint32_t oldrank, uint32_t y, uint32_t newrank) {
    elem old;
    old.d.next = x;
    old.d.rank = oldrank;
    elem newval;
    newval.d.next = y;
    newval.d.rank = newrank;
