...
class small_map {
 ...
 public:
  typedef small_set<MAX_DIM, std::pair<KeyT, ValueT>, less> small_set_type;
 ...
 public:
  ...
  bool contains(const value_type& pair) const { return set.contains(pair); }

  //! test whether the set contains the given element
  bool contains(const key_type& key) const {
    const_iterator iter = std::lower_bound(set.begin(), set.end(),
                                           std::make_pair(key, mapped_type()));
    return (iter != set.end()) && (iter->first == key);
  }

  //! test whether the set has the given key
  inline bool has_key(const key_type& key) { return contains(key); }
  ...
  inline const mapped_type& operator[](const key_type& key) const {
    value_type* const ptr = std::lower_bound(
        set.begin(), set.end(), std::make_pair(key, mapped_type()), less());
  ...
  inline mapped_type& operator[](const key_type& key) {
    value_type* ptr = std::lower_bound(
        set.begin(), set.end(), std::make_pair(key, mapped_type()), less());
    if (ptr != end() && (key == ptr->first)) {
      return ptr->second;
    }
    // Add the entry to the map
    set += std::make_pair(key, ValueT());
    ptr = std::lower_bound(set.begin(), set.end(),
                           std::make_pair(key, mapped_type()), less());
    ASSERT_NE(ptr, set.end());
  ...
  template <size_t OtherDim>
  inline small_map<max_type<OtherDim, MAX_DIM>::value, KeyT, ValueT> operator+(
      const small_map<OtherDim, KeyT, ValueT>& other) const {
...
std::ostream& operator<<(
    std::ostream& out, const graphlab::small_map<MAX_DIM, KeyT, ValueT>& map) {
