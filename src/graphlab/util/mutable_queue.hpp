...
class mutable_queue {
 ...
 protected:
  ...
  size_t left(size_t i) const { return 2 * i; }

  //! Returns the index of the right child of the supplied index.
  size_t right(size_t i) const { return 2 * i + 1; }
  ...
  Priority priority_at(size_t i) { return heap[i].second; }

  //! Compares the priorities at two heap locations.
  bool less(size_t i, size_t j) { return heap[i].second < heap[j].second; }
  ...
  void heapify(size_t i) {
    ...
    size_t s = size();
    size_t largest = i;
    if ((l <= s) && less(i, l)) largest = l;
    if ((r <= s) && less(largest, r)) largest = r;
    if (largest != i) {
      swap(i, largest);
      heapify(largest);
    }
 ...
 public:
  //! Default constructor.
  mutable_queue() : heap(1, std::make_pair(T(), Priority())) {}
  ...
  bool contains(const T& item) const { return index_map.count(item) > 0; }
  ...
  Priority operator[](T item) const { return get(item); }
  ...
  const std::vector<heap_element>& values() const { return heap; }
