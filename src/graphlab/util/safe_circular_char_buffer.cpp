...
safe_circular_char_buffer::~safe_circular_char_buffer() { free(buffer); }
...
bool safe_circular_char_buffer::empty() const { return (head == tail); }
...
std::streamsize safe_circular_char_buffer::write_unsafe(const char* c,
                                                        std::streamsize clen) {
...
std::streamsize safe_circular_char_buffer::introspective_read(
  ...
  if (loop_around)
    available_readlen = bufsize - head;
  else
    available_readlen = curtail - head;
  ASSERT_GE(available_readlen, 0);
  const std::streamsize actual_readlen = std::min(available_readlen, clen);
  ASSERT_GT(actual_readlen, 0);
  return actual_readlen;
}

std::streamsize safe_circular_char_buffer::blocking_introspective_read(
    char*& s, std::streamsize clen) {
  // try to read
...
void safe_circular_char_buffer::advance_head(
