...
void all_gather(const T& elem, std::vector<T>& results) {
  ...
  int error = MPI_Allgather(&send_buffer_size,  // Send buffer
                            ...
                            MPI_COMM_WORLD);
  assert(error == MPI_SUCCESS);
  for (size_t i = 0; i < recv_sizes.size(); ++i) assert(recv_sizes[i] >= 0);
...
template <typename T>
void all2all(const std::vector<T>& send_data, std::vector<T>& recv_data) {
#ifdef HAS_MPI
  // Get the mpi rank and size
  size_t mpi_size(size());
  ...
  int error = MPI_Alltoall(&(send_buffer_sizes[0]), 1, MPI_INT,
                           &(recv_buffer_sizes[0]), 1, MPI_INT, MPI_COMM_WORLD);
  ...
  error =
      MPI_Alltoallv(send_buffer, &(send_buffer_sizes[0]), &(send_offsets[0]),
                    MPI_BYTE, &(recv_buffer[0]), &(recv_buffer_sizes[0]),
                    &(recv_offsets[0]), MPI_BYTE, MPI_COMM_WORLD);
  ASSERT_EQ(error, MPI_SUCCESS);
...
void recv(T& elem, const size_t id, const int tag = 0) {
  ...
  MPI_Status status;
  // recv the size
  int error = MPI_Recv(&recv_buffer_size, 1, MPI_INT, dest, tag, MPI_COMM_WORLD,
                       &status);
  assert(error == MPI_SUCCESS);
  assert(recv_buffer_size > 0);
  ...
  error = MPI_Recv(&(recv_buffer[0]), recv_buffer_size, MPI_BYTE, dest, tag,
