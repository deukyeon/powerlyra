...
template <typename T>
std::pair<std::set<T>, std::set<T> > set_partition(
    const std::set<T>& s, const std::set<T>& partition) {
  std::set<T> a, b;
...
std::map<NewKey, T> rekey(const std::map<OldKey, T>& map,
...
std::map<Key, NewT> remap(const std::map<Key, OldT>& map,
 ...
 */
template <typename Key, typename T>
std::map<Key, T> map_union(const std::map<Key, T>& a,
                           const std::map<Key, T>& b) {
  // Initialize the output map
  std::map<Key, T> output;
  std::set_union(a.begin(), a.end(), b.begin(), b.end(),
                 std::inserter(output, output.begin()), output.value_comp());
  return output;
}
 ...
 */
template <typename Key, typename T>
std::map<Key, T> map_intersect(const std::map<Key, T>& a,
                               const std::map<Key, T>& b) {
  // Initialize the output map
  std::map<Key, T> output;
 ...
 */
template <typename Key, typename T>
std::map<Key, T> map_intersect(const std::map<Key, T>& m,
                               const std::set<Key>& keys) {
  std::map<Key, T> output;
  foreach (const Key& key, keys) {
    typename std::map<Key, T>::const_iterator it = m.find(key);
    if (it != m.end()) output[key] = it->second;
  }
  return output;
}
 ...
 */
template <typename Key, typename T>
std::map<Key, T> map_difference(const std::map<Key, T>& a,
                                const std::map<Key, T>& b) {
  // Initialize the output map
  std::map<Key, T> output;
  // compute the intersection
...
std::set<Key> keys(const std::map<Key, T>& map) {
  std::set<Key> output;
  typedef std::pair<Key, T> pair_type;
  foreach (const pair_type& pair, map) { output.insert(pair.first); }
  return output;
}
...
std::vector<Key> keys_as_vector(const std::map<Key, T>& map) {
  ...
  typedef std::pair<Key, T> pair_type;
  size_t i = 0;
  foreach (const pair_type& pair, map) { output[i++] = pair.first; }
  return output;
}
...
std::set<T> values(const std::map<Key, T>& map) {
  std::set<T> output;
  typedef std::pair<Key, T> pair_type;
  foreach (const pair_type& pair, map) { output.insert(pair.second); }
  return output;
}
...
std::vector<T> values(const std::map<Key, T>& m, const std::set<Key>& keys) {
  ...
  foreach (const Key& i, keys) { output.push_back(safe_get(m, i)); }
  return output;
}
...
*/
inline std::string pad_number(const size_t number, const size_t npad,
                              const char pad_value = '0') {
  std::stringstream strm;
  strm << std::setw((int)npad) << std::setfill(pad_value) << number;
  return strm.str();
}
