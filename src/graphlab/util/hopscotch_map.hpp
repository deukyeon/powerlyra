...
class hopscotch_map {
 public:
  ...
  struct hash_redirect {
    ...
    size_t operator()(const storage_type& v) const { return hashfun(v.first); }
  };
  ...
  typedef hopscotch_table<storage_type, hash_redirect, key_equal_redirect>
      container_type;
  ...
  struct const_iterator {
    ...
    reference operator*() {
      if (!in_spill)
        ...
        return *reinterpret_cast<pointer>(&(*iter2));
    }
    ...
    bool operator==(const const_iterator it) const {
      return ptr == it.ptr && ((!in_spill && iter == it.iter) ||
  ...
  struct iterator {
   ...
   private:
    iterator(hopscotch_map* map, typename container_type::iterator iter,
 ...
 public:
  hopscotch_map(Hash hashfun = Hash(), KeyEqual equalfun = KeyEqual())
      : container(NULL), hashfun(hashfun), equalfun(equalfun) {
  ...
  ~hopscotch_map() { destroy_all(); }

  hasher hash_function() const { return hashfun.hashfun; }

  KeyEqual key_eq() const { return equalfun.equalfun; }
  ...
  size_type size() const { return container->size() + spill.size(); }

  iterator begin() { return iterator(this, container->begin(), spill.begin()); }

  iterator end() { return iterator(this, container->end(), spill.end()); }
  ...
  std::pair<iterator, bool> insert(const value_type& v) {
    ...
    if (i != end())
      ...
      return std::make_pair(do_insert(v), true);
  }
  ...
  size_t capacity() const { return container->capacity() + spill.size(); }
  ...
  void put(const Key& k, const Value& v) { (*this)[k] = v; }
