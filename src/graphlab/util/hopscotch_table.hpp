 ...
 *                  std::equal_to<T>
 */
template <typename T, typename Hash = _HOPSCOTCH_TABLE_DEFAULT_HASH,
...
class hopscotch_table {
 ...
 public:
   ...
   */
  hopscotch_table(size_t len, Hash hashfun = Hash(),
  ...
  equality_function key_eq() const { return equalfun; }
 ...
 private:
   ...
   */
  iterator try_find_and_overwrite(const value_type& newdata, size_t target,
                                  bool overwrite) {
  ...
  iterator insert_impl(const value_type& newdata, bool overwrite = true) {
    ...
    iterator ret = try_find_and_overwrite(newdata, target, overwrite);
  ...
  const_iterator find_impl(const value_type& key, size_t target) const {
    ...
    while (field > 0) {
      ...
      if (data[target + r].hasdata && key_eq()(data[target + r].elem, key)) {
 ...
 public:
   ...
   */
  iterator insert(const value_type& newdata) { return insert_impl(newdata); }
  ...
  bool erase(const value_type& key) { return erase(find(key)); }
  ...
  /// Returns an iterator to the end of the table
  const_iterator end() const { return const_iterator(this, data.end()); }

  /// Returns 1 if the table contains a given element. 0 otherwise.
  size_t count(const value_type& v) const { return find(v) != end(); }
