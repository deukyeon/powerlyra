...
struct uniform {
  ...
  static inline IntType sample(RealRNG& real_rng, DiscreteRNG& discrete_rng,
...
struct uniform<double> {
  ...
  template <typename RealRNG, typename DiscreteRNG>
  static inline double sample(RealRNG& real_rng, DiscreteRNG& discrete_rng,
...
struct uniform<float> {
  ...
  template <typename RealRNG, typename DiscreteRNG>
  static inline float sample(RealRNG& real_rng, DiscreteRNG& discrete_rng,
...
class generator {
 public:
  ...
  inline void time_seed() { seed(graphlab::timer::usec_of_day()); }
  ...
  inline NumType uniform(const NumType min, const NumType max) {
    mut.lock();
    const NumType result = distributions::uniform<NumType>::sample(
        real_rng, discrete_rng, min, max);
    mut.unlock();
    return result;
  }  // end of uniform
  ...
  inline NumType fast_uniform(const NumType min, const NumType max) {
    mut.lock();
    const NumType result = distributions::uniform<NumType>::sample(
        real_rng, fast_discrete_rng, min, max);
    mut.unlock();
    return result;
  }  // end of fast_uniform
  ...
  size_t multinomial(const std::vector<Double>& prb) {
    ...
    size_t ind = 0;
    for (Double cumsum(prb[ind] / sum); rnd > cumsum && (ind + 1) < prb.size();
         cumsum += (prb[++ind] / sum))
      ;
    return ind;
  }  // end of multinomial
  ...
  inline size_t multinomial_cdf(const std::vector<Double>& cdf) {
    return std::upper_bound(cdf.begin(), cdf.end(), uniform<Double>(0, 1)) -
           cdf.begin();
 ...
 private:
  ...
  struct shuffle_functor {
    ...
    inline std::ptrdiff_t operator()(std::ptrdiff_t end) {
      return distributions::uniform<ptrdiff_t>::sample(
          gen.real_rng, gen.fast_discrete_rng, 0, end - 1);
    }
  };
