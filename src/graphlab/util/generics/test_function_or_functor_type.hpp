 ...
 * \li F is the same type as PreferredFunctionForm*
 * \li F implements an F::operator() with the prototype RetType
 * F::operator()(Arg1)
 *
 * For instance,
 * \code
 ...
 * \code
 * test_function_or_const_functor_1<F, void(const int&), void, const
 * int&>::value \endcode
 *
 * is true if any of the following is true:
 * \li F is the same type as PreferredFunctionForm
 ...
 */
template <typename F, typename PreferredFunctionForm, typename RetType,
          typename Arg1>
struct test_function_or_functor_1 {
  ...
  static const bool value =
      ((sizeof(test1<F>(0)) == sizeof(char)) ||
       ...
       boost::is_same<typename boost::remove_pointer<F>::type,
                      PreferredFunctionForm>::value);
};
