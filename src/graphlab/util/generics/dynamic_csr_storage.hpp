 ...
 * The key has type size_t and can be assolicated with multiple values of
 * valuetype.
 *
 * The core operation of is querying the list of values associated with the
 * query key and returns the begin and end iterators via <code>begin(id)</code>
 * and <code>end(id)</code>.
 *
 * Also, this class supports insert (and batch insert) values associated with
 * any key.
 */
template <typename valuetype, typename sizetype = size_t,
          uint32_t blocksize =
              (4096 - 20) /
              (4 * sizeof(valuetype))>  // the block size makes the block fit in
                                        // a memory page
 ...
 public:
  ...
  /**
   * Create the storage with given keys and values. The id_vec and value_vec
  ...
  void insert(const idtype& key, InputIterator first, InputIterator last) {
    ...
    std::pair<iterator, iterator> iter_pair =
        values.insert(ins_iter, first, last);
...
#pragma omp parallel for
  ...
  void print(std::ostream& out) const {
    for (size_t i = 0; i < num_keys(); ++i) {
      ...
      out << i << ": ";
      // out << "begin: " << iter.get_blockptr() << " " << iter.get_offset() <<
      // std::endl; out << "end: " << end(i).get_blockptr() << " " <<
      // end(i).get_offset() << std::endl;
      while (iter != end(i)) {
        out << *iter << " ";
        ++iter;
      }
 ...
 public:
  ...
  size_t estimate_sizeof() const {
    return sizeof(value_ptrs) + sizeof(values) +
           sizeof(sizetype) * value_ptrs.size() +
           sizeof(valuetype) * values.size();
  }
  ...
  void meminfo(std::ostream& out) {
    ...
    out << "utilization: "
        << (float)num_values() / (values.num_blocks() * blocksize) << std::endl;
  }
 ...
 private:
  ...
  void init(const std::vector<idtype>& id_vec,
    ...
    typedef boost::permutation_iterator<
        ...
        permute_iterator;
    permute_iterator _begin = boost::make_permutation_iterator(
        value_vec.begin(), permute_index.begin());
    permute_iterator _end =
        boost::make_permutation_iterator(value_vec.end(), permute_index.end());
