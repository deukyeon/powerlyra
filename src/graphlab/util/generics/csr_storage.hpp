 ...
 * valuetype. The core operation of is querying the list of values associated
 * with the query key *  and returns the begin and end iterators via
 * <code>begin(id)</code> and <code>end(id)</code>.
 */
...
class csr_storage {
 ...
 public:
  ...
  inline iterator end(size_t id) {
    return (id + 1) < num_keys() ? values.begin() + value_ptrs[id + 1]
                                 : values.end();
  }
 ...
 public:
  ...
  void save(oarchive& oarc) const { oarc << value_ptrs << values; }

  size_t estimate_sizeof() const {
    return sizeof(value_ptrs) + sizeof(values) +
           sizeof(sizetype) * value_ptrs.capacity() +
           sizeof(valuetype) * values.capacity();
  }
