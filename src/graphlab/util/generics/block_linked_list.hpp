...
class block_linked_list {
 ...
 public:
  ...
  static const size_t get_blocksize() { return blocksize; }
 ...
 public:
  ...
  class value_iterator
   ...
   public:
    ...
    value_iterator(
        value_iterator<othervalue> const& other,
   ...
   private:
    ...
    value& dereference() const { return blockptr->values[offset]; }
    ...
    ptrdiff_t distance_to(const value_iterator& other) const {
      ...
      if (blockptr == other.blockptr) {
        ...
        if (cur != NULL || other.blockptr == NULL) {
          ...
          logstream(LOG_INFO)
              << "block list iterator reverse direction!!" << std::endl;
    ...
    // For internal use only. Get access to the internal block pointer and
    // offset.
   public:
    // returns block pinter
    blocktype*& get_blockptr() { return blockptr; }
  ...
  const_iterator end() const { return const_iterator(NULL, 0); }
   ...
   */
  template <typename InputIterator>
  std::pair<iterator, iterator> insert(iterator iter, InputIterator first,
                                       InputIterator last) {
 ...
 private:
  ...
  blocktype* append_block() { return insert_block(tail); }
   ...
   */
  template <typename InputIterator>
  std::pair<iterator, iterator> append_to_block(blocktype* ibegin_ptr,
                                                InputIterator first,
                                                InputIterator last) {
    ...
    return std::pair<iterator, iterator>(iterator(ibegin_ptr, ibegin_offset),
...
//     memcpy(splitblk->values, &(ibegin_ptr->values[nfirst]),
//     padsize*sizeof(valuetype)); std::copy(first, first+nnew,
//     &(splitblk->values[padsize])); ibegin_ptr = splitblk; ibegin_offset =
//     padsize;
//  }
// if (!add_new_blocks) {
//    iend_ptr = splitblk;
