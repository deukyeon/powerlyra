...
class lru {
 ...
 public:
  lru(size_t cache_reserve = 1024) { cache_map.left.rehash(cache_reserve); }
  ...
  std::pair<key_type, value_type> evict() {
    ...
    const std::pair<key_type, value_type> result(iter->get_left(),
                                                 iter->get_right());
...
class associative {
 ...
 public:
  associative(size_t cache_size = 1024)
      : keys(cache_size), values(cache_size), is_set(cache_size), size_(0) {}

  size_t size() { return size_; }

  bool evict_slot(const key_type& key, key_type& ret_key,
