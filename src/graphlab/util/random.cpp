...
class nondet_generator {
 public:
  ...
  BOOST_STATIC_CONSTANT(
      result_type, min_value = boost::integer_traits<result_type>::const_min);
  BOOST_STATIC_CONSTANT(
      result_type, max_value = boost::integer_traits<result_type>::const_max);
...
void destroy_tls_data(void* ptr) {
  generator* tls_rnd_ptr = reinterpret_cast<generator*>(ptr);
  if (tls_rnd_ptr != NULL) {
...
struct tls_key_creator {
  ...
  tls_key_creator() : TLS_RANDOM_SOURCE_KEY(0) {
    pthread_key_create(&TLS_RANDOM_SOURCE_KEY, destroy_tls_data);
  }
};
...
generator& get_source() {
  ...
  generator* tls_rnd_ptr = reinterpret_cast<generator*>(
  ...
  if (tls_rnd_ptr == NULL) {
    ...
    pthread_setspecific(get_random_source_key(), tls_rnd_ptr);
  }
  // assert(tls_rnd_ptr != NULL);
  return *tls_rnd_ptr;
