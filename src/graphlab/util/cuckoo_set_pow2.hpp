 ...
 */
template <typename Key, size_t CuckooK = 3, typename IndexType = size_t,
...
class cuckoo_set_pow2 {
 ...
 private:
  ...
  map_container_iterator data_begin() { return data; }

  map_container_iterator data_end() { return data + datalen; }

  map_container_const_iterator data_begin() const { return data; }

  map_container_const_iterator data_end() const { return data + datalen; }

  // bypass the const key_type with a placement new
  void replace_in_vector(map_container_iterator iter, const key_type& key) {
    // delete
    iter->~value_type();
    // placement new
  ...
  struct const_iterator {
    ...
    const_iterator operator++() {
      if (!in_stash) {
        ...
        while (vec_iter != cmap->data_end() &&
               cmap->key_eq()(*vec_iter, cmap->illegal_key()))
          ++vec_iter;
    ...
    bool operator==(const const_iterator iter) const {
      return in_stash == iter.in_stash &&
             (in_stash == false ? vec_iter == iter.vec_iter
   ...
   private:
    ...
    const_iterator(
        const cuckoo_set_pow2* cmap,
        typename cuckoo_set_pow2::stash_container_type::const_iterator
            stash_iter)
        : cmap(cmap),
          in_stash(true),
          vec_iter(cmap->data_begin()),
          stash_iter(stash_iter) {}
  };
  ...
  iterator do_insert(const value_type& v_) {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      if (insertpos == (index_type)(-1))
        insertpos = idx;
      else if (insertpos == idx)
        insertpos = (index_type)(-1);
      // there is room here
    ...
    typename stash_container_type::iterator stashiter =
        stash.insert(stash.end(), v);
 ...
 public:
  cuckoo_set_pow2(key_type illegalkey, index_type stashsize = 8,
                  index_type reserve_size = 128, hasher const& h = hasher(),
  ...
  const key_type& illegal_key() const { return illegalkey; }

  ~cuckoo_set_pow2() { destroy_all(); }

  cuckoo_set_pow2& operator=(const cuckoo_set_pow2& other) {
    if (&other == this) return *this;
    if (other.numel == 0 && numel == 0)
      return *this;
      ...
      std::uninitialized_copy(other.data_begin(), other.data_end(),
                              data_begin());
      // copy the stash
      stash = other.stash;
      numel = other.numel;
  ...
  index_type size() const { return numel; }
  ...
  iterator begin() {
    ...
    while (iter.vec_iter != data_end() && keyeq(*(iter.vec_iter), illegalkey))
      ++iter.vec_iter;
  ...
  iterator end() { return iterator(this, stash.end()); }
  ...
  const_iterator end() const { return const_iterator(this, stash.end()); }
  ...
  void reserve(size_t newlen) {
    ...
    std::uninitialized_fill(data_end(), data + newlen,
                            non_const_value_type(illegalkey));
    datalen = newlen;
    rehash();
  }
  ...
  std::pair<iterator, bool> insert(const value_type& v_) {
    ...
    if (i != end())
      ...
      return std::make_pair(do_insert(v_), true);
  }
  ...
  float load_factor() const { return (float)numel / (datalen + stash.size()); }
  ...
  void load(iarchive& iarc) {
    ...
    deserialize_iterator<iarchive, non_const_value_type>(iarc,
