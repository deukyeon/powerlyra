...
void graphlab::fs_util::list_files_with_suffix(
    const std::string& pathname, const std::string& suffix,
  ...
  if (fs::exists(dir_path) && fs::is_directory(dir_path)) {
    for (fs::directory_iterator dir_iter(dir_path); dir_iter != end_iter;
         ++dir_iter) {
...
#endif
        if (suffix.size() > 0 && !boost::ends_with(filename, suffix)) continue;
...
void graphlab::fs_util::list_files_with_prefix(
    const std::string& pathname, const std::string& prefix,
    std::vector<std::string>& files) {
  namespace fs = boost::filesystem;
  fs::path dir_path(pathname);
  fs::directory_iterator end_iter;
...
std::string graphlab::fs_util::change_suffix(const std::string& fname,
