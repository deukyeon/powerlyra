...
class dense_bitset {
  ...
  */
  ...
  bit_pos_iterator end() const { return bit_pos_iterator(this, (size_t)(-1)); }
...
class fixed_dense_bitset {
 public:
  /// Constructs a bitset of 0 length
  fixed_dense_bitset() { clear(); }

  /// Make a copy of the bitset db
  fixed_dense_bitset(const fixed_dense_bitset<len>& db) { *this = db; }
  ...
  */
  ...
  /// Sets all bits to 0
  inline void clear() { memset((void*)array, 0, sizeof(size_t) * arrlen); }
  ...
  */
  ...
  struct bit_pos_iterator {
    ...
    bit_pos_iterator() : pos(-1), db(NULL) {}
    bit_pos_iterator(const fixed_dense_bitset* const db, size_t pos)
        : pos(pos), db(db) {}
...
template <int len>
const size_t fixed_dense_bitset<len>::arrlen = len / (sizeof(size_t) * 8) +
                                               (len % (sizeof(size_t) * 8) > 0);
}  // namespace graphlab
#endif
