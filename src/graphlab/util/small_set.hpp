...
class small_set {
 ...
 public:
  ...
  small_set(const std::set<OtherT>& other) : nelems(other.size()) {
    ...
    size_t index = 0;
    foreach (const OtherT& elem, other)
      values[index++] = elem;
  }
  ...
  small_set(const small_set<OtherSize, T, Less>& other) : nelems(other.size()) {
    ASSERT_LE(nelems, MAX_DIM);
    size_t index = 0;
  ...
  bool contains(const small_set<OtherDim, T, Less>& other) const {
    return std::includes(begin(), end(), other.begin(), other.end(), Less());
  }
  ...
  inline void insert(const T& elem) { *this += elem; }
  ...
  //! Take the union of two sets
  template <size_t OtherDim>
  inline small_set<max_type<OtherDim, MAX_DIM>::value, T, Less> operator+(
      const small_set<OtherDim, T, Less>& other) const {
    typedef small_set<max_type<OtherDim, MAX_DIM>::value, T, Less> result_type;
    result_type result;
    const T* new_end =
        std::set_union(begin(), end(), other.begin(), other.end(),
                       safe_iterator(result.begin(), result.absolute_end()),
                       Less())
            .begin;
  ...
  small_set operator-(const small_set<OtherDim, T, Less>& other) const {
    ...
    T* const new_end =
        std::set_difference(
            ...
            safe_iterator(result.begin(), result.absolute_end()), Less())
            .begin;
    result.nelems = new_end - result.begin();
 ...
 private:
  ...
  struct safe_iterator : public std::iterator<std::input_iterator_tag, T> {
    T* begin;
    const T* end;
    safe_iterator(const safe_iterator& other)
        : begin(other.begin), end(other.end) {}
    safe_iterator(T* begin, const T* end) : begin(begin), end(end) {
      ASSERT_NE(begin, NULL);
      ASSERT_NE(end, NULL);
      ASSERT_LE(begin, end);
    }
    inline safe_iterator& operator++() {
      ++begin;
      return *this;
    }
...
std::ostream& operator<<(std::ostream& out,
                         const graphlab::small_set<MAX_DIM, T>& set) {
  out << "{";
