...
class fast_multinomial {
  ...
  void propagate_change(size_t loc) {
    ...
    for (; !is_root(loc); loc = parent(loc)) {
      ...
      //        while(!atomic_compare_and_swap(tree[parent_loc], old_value,
      //        new_value)) {
      //          old_value = *parent;
 ...
 public:
  ...
  fast_multinomial(size_t num_asg, size_t ncpus)
      : first_leaf_index(0), num_asg(num_asg) {
  ...
  bool pop(size_t& ret_asg, size_t cpuid) {
    ...
    std::cout << "Queue emptied!: " << tree[0] << ", " << num_support.value
              << std::endl;
    print_tree();
    return false;
  }  // end of pop
  ...
  size_t positive_support() { return num_support.value; }

  /** print the tree */
  void print_tree() {
    for (size_t i = 0; i < std::min(tree.size(), size_t(1000)); ++i) {
      if (is_leaf(i)) {
        std::cout << "Leaf(" << asg_from_tree_loc(i) << ", [" << parent(i)
                  << "], " << tree[i] << ") ";
      } else {
        std::cout << "Node(" << i << ", "
