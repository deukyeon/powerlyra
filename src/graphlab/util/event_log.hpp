...
class event_log {
 public:
  enum event_print_type { NUMBER, DESCRIPTION, RATE_BAR, LOG_FILE };
 ...
 public:
  ...
  void initialize(std::ostream& ostrm, size_t flush_interval_ms,
  ...
  inline void accumulate_event(unsigned char eventid, size_t count)
 ...
 * printdesc is either graphlab::event_log::NUMBER, or
 * graphlab::event_log::DESCRIPTION or graphlab::event_log::RATE_BAR. There is
 * also graphlab::event_log::LOG_FILE which outputs to a centralized log. If
 * this is used, the ostrm argument is ignored and an output of the DESCRIPTION
 * format will be written to an eventlog.txt file in the current directory.
 *
...
#define ADD_IMMEDIATE_EVENT_TYPE(name, id, desc) \
  name.add_immediate_event_type(id, desc);
...
#define PERMANENT_ADD_IMMEDIATE_EVENT_TYPE(name, id, desc) \
  name.add_immediate_event_type(id, desc);
#define PERMANENT_ACCUMULATE_EVENT(name, id, count) \
  name.accumulate_event(id, count);
