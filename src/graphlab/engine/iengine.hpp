...
class iengine {
 public:
   ...
   * }
   *
   * void print_finalize(engine_type::icontext_type& context, const float&
   * total) { std::cout << total << "\n";
   * }
   * \endcode
   ...
   *                        operator+= defined, and must be \ref
   * sec_serializable. \tparam VertexMapperType The type of the map function.
   ...
   *                               argument.
   */
  template <typename ReductionType, typename VertexMapType,
            typename FinalizerType>
  bool add_vertex_aggregator(const std::string& key, VertexMapType map_function,
    ...
    return aggregator->template add_vertex_aggregator<ReductionType>(
   ...
   *                               argument.
   */
  template <typename VertexMapType, typename FinalizerType>
  bool add_vertex_aggregator(const std::string& key, VertexMapType map_function,
                             FinalizerType finalize_function) {
    aggregator_type* aggregator = get_aggregator();
    if (aggregator == NULL) {
  ...
  template <typename ReductionType, typename EdgeMapType,
            typename FinalizerType>
  bool add_edge_aggregator(const std::string& key, EdgeMapType map_function,
                           FinalizerType finalize_function) {
    BOOST_CONCEPT_ASSERT((graphlab::Serializable<ReductionType>));
    BOOST_CONCEPT_ASSERT((graphlab::OpPlusEq<ReductionType>));
    aggregator_type* aggregator = get_aggregator();
   ...
   *                               argument.
   */
  template <typename EdgeMapType, typename FinalizerType>
  bool add_edge_aggregator(const std::string& key, EdgeMapType map_function,
                           FinalizerType finalize_function) {
    aggregator_type* aggregator = get_aggregator();
    if (aggregator == NULL) {
