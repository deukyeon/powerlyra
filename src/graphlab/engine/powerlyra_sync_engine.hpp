...
class powerlyra_sync_engine : public iengine<VertexProgram> {
 public:
  /**
 ...
 private:
  ...
  void run_synchronous(MemberFunction member_fun) {
    ...
    for (size_t i = 0; i < ncpus; ++i) {
      ...
      threads.launch(
          boost::bind(
              &powerlyra_sync_engine::thread_launch_wrapped_event_counter, this,
              invoke),
          affinity);
    }
 ...
 */
template <typename VertexProgram>
powerlyra_sync_engine<VertexProgram>::powerlyra_sync_engine(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::signal(vertex_id_type gvid,
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::signal_all(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::signal_vset(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::internal_signal(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::internal_signal_gvid(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::internal_signal_rpc(
    vertex_id_type gvid, const message_type& message) {
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::internal_post_delta(
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::internal_clear_gather_cache(
    const vertex_type& vertex) {
  const bool caching_enabled = !gather_cache.empty();
  const lvid_type lvid = vertex.local_id();
...
template <typename VertexProgram>
size_t powerlyra_sync_engine<VertexProgram>::num_updates() const {
  return completed_applys.value;
}

template <typename VertexProgram>
float powerlyra_sync_engine<VertexProgram>::elapsed_seconds() const {
  return timer::approx_time_seconds() - start_time;
}

template <typename VertexProgram>
int powerlyra_sync_engine<VertexProgram>::iteration() const {
...
template <typename VertexProgram>
execution_status::status_enum powerlyra_sync_engine<VertexProgram>::start() {
  if (vlocks.size() != graph.num_local_vertices()) resize();
  completed_gathers = 0;
  completed_applys = 0;
  completed_scatters = 0;
...
template <typename VertexProgram>
inline bool powerlyra_sync_engine<VertexProgram>::high_lvid(
    const lvid_type lvid) {
  return graph.l_degree_type(lvid) == graph_type::HIGH;
}

template <typename VertexProgram>
inline bool powerlyra_sync_engine<VertexProgram>::low_lvid(
    const lvid_type lvid) {
  return graph.l_degree_type(lvid) == graph_type::LOW;
}

template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::exchange_messages(
    const size_t thread_id) {
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::receive_messages(
    const size_t thread_id) {
  context_type context(*this, graph);
  fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // a word-size = 64 bit
  const size_t TRY_RECV_MOD = 100;
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::execute_gathers(
    const size_t thread_id) {
  context_type context(*this, graph);
  const bool caching_enabled = !gather_cache.empty();
  fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // a word-size = 64 bit
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::execute_applys(
    const size_t thread_id) {
  context_type context(*this, graph);
...
template <typename VertexProgram>
void powerlyra_sync_engine<VertexProgram>::execute_scatters(
    const size_t thread_id) {
  context_type context(*this, graph);
  fixed_dense_bitset<8 * sizeof(size_t)>
...
// Data Synchronization ===================================================
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::send_activs(
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::recv_activs() {
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::send_updates_activs(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_TRUE(graph.l_is_master(lvid));
  const vertex_id_type vid = graph.global_vid(lvid);
  local_vertex_type vertex = graph.l_vertex(lvid);
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::recv_updates_activs() {
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::send_updates(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_TRUE(graph.l_is_master(lvid));
  const vertex_id_type vid = graph.global_vid(lvid);
  local_vertex_type vertex = graph.l_vertex(lvid);
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::recv_updates() {
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::send_accum(
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::recv_accums() {
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::send_message(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_FALSE(graph.l_is_master(lvid));
  const procid_t master = graph.l_master(lvid);
  const vertex_id_type vid = graph.global_vid(lvid);
...
template <typename VertexProgram>
inline void powerlyra_sync_engine<VertexProgram>::recv_messages() {
