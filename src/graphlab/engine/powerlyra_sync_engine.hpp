...
namespace graphlab {
  ...
  private:
    ...
    */
    ...
    /**
     * \brief The counter to track the number of bytes for graph data to be received.
     */
  ...
  powerlyra_sync_engine(distributed_control& dc,
    ...
    accum_exchange(dc),
    message_exchange(dc),
    graph_data_bytes_sent(0), graph_data_bytes_recv(0),
...
#ifdef TUNING
    logstream(LOG_EMPH) << rmi.procid() << " Graph Data Sent: " << graph_data_bytes_sent << std::endl;
    logstream(LOG_EMPH) << rmi.procid() << " Graph Data Received: " << graph_data_bytes_recv << std::endl;
#endif
  ...
  send_updates_activs(lvid_type lvid, const size_t thread_id) {
    ...
    foreach(const procid_t& mirror, vertex.mirrors()) {
      ...
      graph_data_bytes_sent.inc(sizeof(vid) + sizeof(vertex.data()) + sizeof(vertex_programs[lvid]));
    }
  } // end of send_update
    ...
    while(update_activ_exchange.recv(recv_buffer)) {
      for (size_t i = 0;i < recv_buffer.size(); ++i) {
        ...
        foreach(const vid_vdata_vprog_triple_type& t, buffer) {
          ...
          active_minorstep.set_bit(lvid);
          graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(t.second) + sizeof(t.third));
        }
      }
    }
  } // end of recv_updates
