...
void extended_default_combiner(T& self, const T& other,
                               const ExtraArgs& unused) {
  self += other;
}
...
struct map_reduce_neighborhood_impl {
   ...
   * Which then issues calls to basic_local_mapper on each machine with a
   * replica.
   */

  static conditional_combiner_wrapper<RetType> basic_local_mapper(
      GraphType& graph, edge_dir_type edge_direction,
      RetType (*mapper)(edge_type edge, vertex_type other),
      void (*combiner)(RetType&, const RetType&), vertex_id_type vid) {
  ...
  static conditional_combiner_wrapper<RetType> basic_local_mapper_from_remote(
      size_t objid, edge_dir_type edge_direction, size_t mapper_ptr,
      size_t combiner_ptr, vertex_id_type vid) {
    ...
    return basic_local_mapper(
        *reinterpret_cast<GraphType*>(
            distributed_control::get_instance()->get_registered_object(objid)),
        edge_direction, mapper, combiner, vid);
  }

  static RetType basic_map_reduce_neighborhood(
      typename GraphType::vertex_type current, edge_dir_type edge_direction,
      RetType (*mapper)(edge_type edge, vertex_type other),
      void (*combiner)(RetType& self, const RetType& other)) {
    // get a reference to the graph
    GraphType& graph = current.graph_ref;
    // get the object ID of the graph
    size_t objid = graph.get_rpc_obj_id();
    typename GraphType::vertex_record vrecord =
        graph.l_get_vertex_record(current.local_id());
    ...
    std::vector<request_future<conditional_combiner_wrapper<RetType> > >
        requests(vrecord.num_mirrors());

    size_t ctr = 0;
    foreach (procid_t proc, vrecord.mirrors()) {
      // issue the communication
      requests[ctr] = fiber_remote_request(
          proc,
          map_reduce_neighborhood_impl<
              RetType, GraphType>::basic_local_mapper_from_remote,
          objid, edge_direction, reinterpret_cast<size_t>(mapper),
          reinterpret_cast<size_t>(combiner), current.id());
      ++ctr;
    }
    // compute the local tasks
    conditional_combiner_wrapper<RetType> accum = basic_local_mapper(
        graph, edge_direction, mapper, combiner, current.id());
 ...
 * Which then issues calls to extended_local_mapper on each machine with a
 * replica. The extended mapreduce neighborhood allows the mapper and combiner
...
struct map_reduce_neighborhood_impl2 {
  ...
  static conditional_combiner_wrapper<RetType>
  extended_local_mapper_from_remote(size_t objid, edge_dir_type edge_direction,
                                    size_t mapper_ptr, size_t combiner_ptr,
                                    vertex_id_type vid, const ExtraArg extra) {
    // cast the mappers and combiners back into their pointer types
    RetType (*mapper)(edge_type edge, vertex_type other, const ExtraArg) =
        reinterpret_cast<RetType (*)(edge_type, vertex_type, const ExtraArg)>(
            mapper_ptr);
    void (*combiner)(RetType&, const RetType&, const ExtraArg) =
        reinterpret_cast<void (*)(RetType&, const RetType&, const ExtraArg)>(
            combiner_ptr);
    return extended_local_mapper(
        *reinterpret_cast<GraphType*>(
            distributed_control::get_instance()->get_registered_object(objid)),
        edge_direction, mapper, combiner, vid, extra);
  }

  static RetType extended_map_reduce_neighborhood(
      typename GraphType::vertex_type current, edge_dir_type edge_direction,
    ...
    foreach (procid_t proc, vrecord.mirrors()) {
      ...
      requests[ctr] = fiber_remote_request(
          ...
          objid, edge_direction, reinterpret_cast<size_t>(mapper),
          reinterpret_cast<size_t>(combiner), current.id(), extra);
      ++ctr;
    }
    // compute the local tasks
    conditional_combiner_wrapper<RetType> accum = extended_local_mapper(
        graph, edge_direction, mapper, combiner, current.id(), extra);
 ...
 *    // computes an aggregate over the neighborhood using
 * map_reduce_neighborhood
 *    // The pagerank_map function will be executed over every in-edge of the
 * graph,
 ...
 * \param mapper The map function that will be executed. Must be a function
 * pointer. \param combiner The combine function that will be executed. Must be
 * a function pointer. Optional. Defaults to using "+=" on the output of the
 * mapper
 *
...
RetType map_reduce_neighborhood(
    VertexType current, edge_dir_type edge_direction,
    RetType (*mapper)(typename VertexType::graph_type::edge_type edge,
                      VertexType other),
    void (*combiner)(RetType& self, const RetType& other) =
        warp_impl::default_combiner<RetType>) {
  return warp_impl::map_reduce_neighborhood_impl<
      RetType, typename VertexType::graph_type>::
      basic_map_reduce_neighborhood(current, edge_direction, mapper, combiner);
}
 ...
 * Here is an example which implements the PageRank computation, using
 * the parfor_all_vertices function to create a parallel for loop using fibers.
 * We demonstrate the additional argument by passing on the 0.85 scaling value
 * to be computed in the map. \code float pagerank_map(graph_type::edge_type
 * edge, graph_type::vertex_type other, const float scale) {
...
RetType map_reduce_neighborhood(
    VertexType current, edge_dir_type edge_direction, const ExtraArg extra,
    RetType (*mapper)(typename VertexType::graph_type::edge_type edge,
                      VertexType other, const ExtraArg extra),
    void (*combiner)(RetType& self, const RetType& other,
                     const ExtraArg extra) =
        warp_impl::extended_default_combiner<RetType, ExtraArg>) {
  return warp_impl::map_reduce_neighborhood_impl2<
      RetType, typename VertexType::graph_type,
      ExtraArg>::extended_map_reduce_neighborhood(current, edge_direction,
                                                  extra, mapper, combiner);
}
