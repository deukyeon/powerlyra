...
struct transform_neighborhood_impl {
  ...
  static void basic_local_transform_neighborhood(
  ...
  static void basic_local_transform_neighborhood_from_remote(
      size_t objid, edge_dir_type edge_direction, size_t transform_ptr,
      vertex_id_type vid) {
    ...
    basic_local_transform_neighborhood(
        *reinterpret_cast<GraphType*>(
        ...
        edge_direction, transform_fn, vid);
  }

  static void basic_transform_neighborhood(
    ...
    foreach (procid_t proc, vrecord.mirrors()) {
      ...
      requests[ctr] = fiber_remote_request(
          proc,
          transform_neighborhood_impl<
              GraphType>::basic_local_transform_neighborhood_from_remote,
          objid, edge_direction, reinterpret_cast<size_t>(transform_fn),
          current.id());
      ++ctr;
    }
    // compute the local tasks
    basic_local_transform_neighborhood(graph, edge_direction, transform_fn,
                                       current.id());
...
struct transform_neighborhood_impl2 {
  ...
  static void extended_local_transform_neighborhood(
      GraphType& graph, edge_dir_type edge_direction,
      void (*transform_fn)(edge_type edge, vertex_type other,
  ...
  static void extended_local_transform_neighborhood_from_remote(
      size_t objid, edge_dir_type edge_direction, size_t transform_ptr,
      vertex_id_type vid, const ExtraArg extra) {
    // cast the mappers and combiners back into their pointer types
    void (*transform_fn)(edge_type edge, vertex_type other, const ExtraArg) =
        reinterpret_cast<void (*)(edge_type, vertex_type, const ExtraArg)>(
            transform_ptr);
    extended_local_transform_neighborhood(
        *reinterpret_cast<GraphType*>(
            distributed_control::get_instance()->get_registered_object(objid)),
        edge_direction, transform_fn, vid, extra);
  }

  static void extended_transform_neighborhood(
      typename GraphType::vertex_type current, edge_dir_type edge_direction,
      void (*transform_fn)(edge_type edge, vertex_type other,
                           const ExtraArg extra),
      const ExtraArg extra) {
    ...
    foreach (procid_t proc, vrecord.mirrors()) {
      ...
      requests[ctr] = fiber_remote_request(
          proc,
          transform_neighborhood_impl2<GraphType, ExtraArg>::
              extended_local_transform_neighborhood_from_remote,
          objid, edge_direction, reinterpret_cast<size_t>(transform_fn),
          current.id(), extra);
      ++ctr;
    }
    // compute the local tasks
    extended_local_transform_neighborhood(graph, edge_direction, transform_fn,
                                          current.id(), extra);
    // now, wait for everyone
    for (size_t i = 0; i < requests.size(); ++i) {
      requests[i]();
 ...
 * modifications to the edge data, and not the data on the other vertex.
 *
 ...
 * transform_neighborhood (below) if you want to pass on additional information
 * to the mapper.
 *
 * Here is an example which clears all the in edge values of some set of
 * vertices. \code void clear_value(graph_type::edge_type edge,
 * graph_type::vertex_type other) { edge.data() = 0;
 * }
 *
 *
...
void transform_neighborhood(
    VertexType current, edge_dir_type edge_direction,
    void (*transform_fn)(typename VertexType::graph_type::edge_type edge,
                         VertexType other)) {
  warp_impl::transform_neighborhood_impl<typename VertexType::graph_type>::
      basic_transform_neighborhood(current, edge_direction, transform_fn);
}
 ...
 * edge.data() and other.data() will be valid. The value of the vertex
 * on the "self" end of the edge will not reflect changes you made to the vertex
 * immediately before calling warp::transform_neighborhood() . Use the overload
 * of warp::transform_neighborhood() if you want to pass on additional
 ...
 * functions. \param transform_fn The transform function to run on all the
 * selected edges.
 *
 * \see warp_engine
 * \see warp::parfor_all_vertices()
...
void transform_neighborhood(
    ...
    const ExtraArg extra) {
  warp_impl::transform_neighborhood_impl2<
      typename VertexType::graph_type,
      ExtraArg>::extended_transform_neighborhood(current, edge_direction,
                                                 transform_fn, extra);
}
