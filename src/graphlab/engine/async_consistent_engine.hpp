 ...
 *   graphlab::async_consistent_engine<pagerank_vprog> engine(dc, graph,
 * clopts); engine.signal_all(); engine.start(); std::cout << "Runtime: " <<
 * engine.elapsed_seconds(); graphlab::mpi_tools::finalize();
 * }
 * \endcode
 *
...
class async_consistent_engine : public iengine<VertexProgram> {
 ...
 public:
   ...
   */
  async_consistent_engine(distributed_control& dc, graph_type& graph,
 ...
 private:
  ...
  void set_options(const graphlab_options& opts) {
    ...
    scheduler_ptr =
        scheduler_factory::new_scheduler(graph.num_local_vertices(), opts_copy);
    rmi.barrier();

    // create initial fork arrangement based on the alternate vid mapping
    if (factorized_consistency == false) {
      cmlocks = new distributed_chandy_misra<graph_type>(
          rmi.dc(), graph, boost::bind(&engine_type::lock_ready, this, _1));
 ...
 public:
  ...
  size_t num_updates() const { return programs_executed.value; }
 ...
 private:
   ...
   */
  void rpc_signal(vertex_id_type vid, const message_type& message) {
  ...
  void internal_signal(const vertex_type& vtx,
    ...
    if (force_stop) return;
    if (started) {
      const typename graph_type::vertex_record& rec =
          graph.l_get_vertex_record(vtx.local_id());
 ...
 private:
   ...
   */
  bool try_to_quit(size_t threadid, bool& has_sched_msg, lvid_type& sched_lvid,
                   message_type& msg) {
    ...
    if (stat == sched_status::EMPTY || force_stop) {
      ...
      if (!endgame_mode)
        logstream(LOG_EMPH)
            << rmi.procid() << " Endgame mode "
            << (timer::approx_time_seconds() - engine_start_time) << std::endl;
      endgame_mode = true;
      ...
      if (ret == false) {
        ...
        logstream(LOG_DEBUG)
            << rmi.procid() << "-" << threadid << ": "
            << "\tDying"
            << " (" << fiber_control::get_tid() << ")" << std::endl;
      }
      return ret;
    } else {
      logstream(LOG_DEBUG) << rmi.procid() << "-" << threadid << ": "
  ...
  void perform_scatter_local(lvid_type lvid, vertex_program_type& vprog) {
    local_vertex_type local_vertex(graph.l_vertex(lvid));
    vertex_type vertex(local_vertex);
    context_type context(*this, graph);
  ...
  void perform_scatter(vertex_id_type vid, vertex_program_type& vprog_,
   ...
   */
  void eval_sched_task(const lvid_type lvid, const message_type& msg) {
    const typename graph_type::vertex_record& rec =
        graph.l_get_vertex_record(lvid);
    ...
    foreach (procid_t mirror, local_vertex.mirrors()) {
      gather_futures.push_back(object_fiber_remote_request(
          rmi, mirror, &async_consistent_engine::perform_gather, vid, vprog));
    }
    ...
    foreach (procid_t mirror, local_vertex.mirrors()) {
      scatter_futures.push_back(object_fiber_remote_request(
          rmi, mirror, &async_consistent_engine::perform_scatter, vid, vprog,
          local_vertex.data()));
    }
    perform_scatter_local(lvid, vprog);
    for (size_t i = 0; i < scatter_futures.size(); ++i) scatter_futures[i]();
 ...
 public:
  ...
  execution_status::status_enum start() {
    ...
    if (rmi.procid() == 0) {
      logstream(LOG_INFO) << "Total Allocated Bytes: " << allocatedmem
                          << std::endl;
    }
    thrgroup.set_stacksize(stacksize);

    size_t effncpus =
        std::min(ncpus, fiber_control::get_instance().num_workers());
