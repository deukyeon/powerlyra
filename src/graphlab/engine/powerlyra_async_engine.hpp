...
class powerlyra_async_engine : public iengine<VertexProgram> {
 ...
 public:
   ...
   */
  powerlyra_async_engine(distributed_control& dc, graph_type& graph,
 ...
 private:
  ...
  void internal_signal_gvid(vertex_id_type gvid,
    ...
    if (graph.is_master(gvid)) {
      ...
      rmi.remote_call(proc, &powerlyra_async_engine::internal_signal_gvid, gvid,
                      message);
    }
  }
  ...
  void eval_sched_task(const lvid_type lvid, const message_type& msg) {
    ...
    if (high || (gather_dir == graphlab::ALL_EDGES) ||
      ...
      foreach (procid_t mirror, local_vertex.mirrors()) {
        gather_futures.push_back(object_fiber_remote_request(
            rmi, mirror, &powerlyra_async_engine::perform_gather, vid, vprog));
      }
    }
    ...
    foreach (procid_t mirror, local_vertex.mirrors()) {
      scatter_futures.push_back(object_fiber_remote_request(
          rmi, mirror, &powerlyra_async_engine::perform_scatter, vid, vprog,
          local_vertex.data()));
    }
    perform_scatter_local(lvid, vprog);
