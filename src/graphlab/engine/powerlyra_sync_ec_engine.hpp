...
class powerlyra_sync_ec_engine : public iengine<VertexProgram> {
 public:
  /**
 ...
 private:
   ...
   */
  typedef
      typename iengine<vertex_program_type>::aggregator_type aggregator_type;
   ...
   */
  typedef fiber_buffered_exchange<vid_vprog_pair_type> activ_exchange_type;
   ...
   */
  typedef
      typename update_activ_exchange_type::buffer_type update_activ_buffer_type;
  ...
  /**
   * \brief The counter to track the number of bytes for graph data to be sent.
   */
  atomic<size_t> graph_data_bytes_sent;

  /**
   * \brief The counter to track the number of bytes for graph data to be
   * received.
   */
  atomic<size_t> graph_data_bytes_recv;
 ...
 public:
  ...
  /**
   * \brief Initialize the engine and allocate datastructures for vertex, and
   * lock, clear all the messages.
   */
  void init();

 private:
  /**
   * \brief Resize the datastructures to fit the graph size (in case of dynamic
   * graph). Keep all the messages and caches.
   */
  void resize();

  /**
   * \brief This internal stop function is called by the \ref graphlab::context
   * to terminate execution of the engine.
   */
  void internal_stop();
   ...
   */
  void internal_signal(const vertex_type& vertex, const message_type& message);
 ...
 */
template <typename VertexProgram>
powerlyra_sync_ec_engine<VertexProgram>::powerlyra_sync_ec_engine(
    distributed_control& dc, graph_type& graph, const graphlab_options& opts)
    : rmi(dc, this),
      graph(graph),
      ...
      accum_exchange(dc),
      message_exchange(dc),
      graph_data_bytes_sent(0),
      graph_data_bytes_recv(0),
  ...
  if (snapshot_interval >= 0 && snapshot_path.length() == 0) {
    logstream(LOG_FATAL) << "Snapshot interval specified, but no snapshot path"
                         << std::endl;
  }
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::signal(
    vertex_id_type gvid, const message_type& message) {
  if (vlocks.size() != graph.num_local_vertices()) resize();
  rmi.barrier();
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::signal_all(
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::signal_vset(
    const vertex_set& vset, const message_type& message,
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::internal_signal(
    const vertex_type& vertex, const message_type& message) {
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::internal_signal_gvid(
    vertex_id_type gvid, const message_type& message) {
  ...
  if (proc == rmi.procid())
    ...
    rmi.remote_call(
        proc, &powerlyra_sync_ec_engine<VertexProgram>::internal_signal_rpc,
        gvid, message);
}  // end of internal_signal_gvid

template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::internal_signal_rpc(
    vertex_id_type gvid, const message_type& message) {
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::internal_post_delta(
    const vertex_type& vertex, const gather_type& delta) {
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::internal_clear_gather_cache(
    const vertex_type& vertex) {
  const bool caching_enabled = !gather_cache.empty();
  const lvid_type lvid = vertex.local_id();
...
template <typename VertexProgram>
size_t powerlyra_sync_ec_engine<VertexProgram>::num_updates() const {
  return completed_applys.value;
}

template <typename VertexProgram>
float powerlyra_sync_ec_engine<VertexProgram>::elapsed_seconds() const {
  return timer::approx_time_seconds() - start_time;
}

template <typename VertexProgram>
int powerlyra_sync_ec_engine<VertexProgram>::iteration() const {
...
template <typename VertexProgram>
execution_status::status_enum powerlyra_sync_ec_engine<VertexProgram>::start() {
  if (vlocks.size() != graph.num_local_vertices()) resize();
  completed_gathers = 0;
  completed_applys = 0;
  completed_scatters = 0;
...
#ifdef TUNING
  exec_time = exch_time = recv_time = gather_time = apply_time = scatter_time =
      0.0;
  ...
  if (rmi.procid() == 0) {
    logstream(LOG_EMPH) << "Iteration counter will only output every "
                        << print_interval << " seconds." << std::endl;
  }
  ...
  while (iteration_counter < max_iterations && !force_abort) {
    ...
    if (rmi.procid() == 0 && print_this_round)
      logstream(LOG_EMPH) << "\tActive vertices: " << total_active_vertices
                          << std::endl;
  ...
  if (rmi.procid() == 0) {
    logstream(LOG_EMPH) << iteration_counter << " iterations completed."
                        << std::endl;
  }
...
#ifdef TUNING
    logstream(LOG_INFO) << "Total Calls(G|A|S): " << completed_gathers.value
                        << "|" << completed_applys.value << "|"
                        << completed_scatters.value << std::endl;
    logstream(LOG_INFO) << std::endl;
    logstream(LOG_EMPH) << "      Execution Time: " << exec_time << std::endl;
    logstream(LOG_EMPH) << "Breakdown(X|R|G|A|S): " << exch_time << "|"
                        << recv_time << "|" << gather_time << "|" << apply_time
                        << "|" << scatter_time << std::endl;
#endif
  }

#ifdef TUNING
  logstream(LOG_EMPH) << rmi.procid()
                      << " Graph Data Sent: " << graph_data_bytes_sent
                      << std::endl;
  logstream(LOG_EMPH) << rmi.procid()
                      << " Graph Data Received: " << graph_data_bytes_recv
                      << std::endl;
#endif
...
template <typename VertexProgram>
inline bool powerlyra_sync_ec_engine<VertexProgram>::high_lvid(
    const lvid_type lvid) {
  return graph.l_degree_type(lvid) == graph_type::HIGH;
}

template <typename VertexProgram>
inline bool powerlyra_sync_ec_engine<VertexProgram>::low_lvid(
    const lvid_type lvid) {
  return graph.l_degree_type(lvid) == graph_type::LOW;
}

template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::exchange_messages(
    const size_t thread_id) {
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::receive_messages(
    const size_t thread_id) {
  context_type context(*this, graph);
  fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // a word-size = 64 bit
  const size_t TRY_RECV_MOD = 100;
  ...
  while (1) {
    ...
    foreach (size_t lvid_block_offset, local_bitset) {
      ...
      if (gather_dir != graphlab::NO_EDGES) {
        ...
        // send Gx1 msgs
        if (high_lvid(lvid) || (low_lvid(lvid)  // only if gather via out-edge
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::execute_gathers(
    const size_t thread_id) {
  context_type context(*this, graph);
  const bool caching_enabled = !gather_cache.empty();
  fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // a word-size = 64 bit
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::execute_applys(
    const size_t thread_id) {
  context_type context(*this, graph);
  fixed_dense_bitset<8 * sizeof(size_t)>
      local_bitset;  // allocate a word size = 64bits
  const size_t TRY_RECV_MOD = 1000;
  size_t vcount = 0;
  size_t napply_inc = 0;
  timer ti;
...
template <typename VertexProgram>
void powerlyra_sync_ec_engine<VertexProgram>::execute_scatters(
    const size_t thread_id) {
  context_type context(*this, graph);
  fixed_dense_bitset<8 * sizeof(size_t)>
      local_bitset;  // allocate a word size = 64 bits
...
// Data Synchronization ===================================================
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::send_activs(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_TRUE(graph.l_is_master(lvid));
  ...
  foreach (const procid_t& mirror, vertex.mirrors()) {
    activ_exchange.send(mirror, std::make_pair(vid, vertex_programs[lvid]));
    graph_data_bytes_sent.inc(sizeof(vid) + sizeof(vertex_programs[lvid]));
  }
}  // end of send_activ

template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::recv_activs() {
  ...
  while (activ_exchange.recv(recv_buffer)) {
    for (size_t i = 0; i < recv_buffer.size(); ++i) {
      ...
      foreach (const vid_vprog_pair_type& pair, buffer) {
        ...
        active_minorstep.set_bit(lvid);
        graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(vertex_programs[lvid]));
      }
    }
  }
...
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::send_updates_activs(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_TRUE(graph.l_is_master(lvid));
  const vertex_id_type vid = graph.global_vid(lvid);
  local_vertex_type vertex = graph.l_vertex(lvid);
  foreach (const procid_t& mirror, vertex.mirrors()) {
    update_activ_exchange.send(
        mirror, make_triple(vid, vertex.data(), vertex_programs[lvid]));
    graph_data_bytes_sent.inc(sizeof(vid) + sizeof(vertex.data()) +
                              sizeof(vertex_programs[lvid]));
  }
}  // end of send_update

template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::recv_updates_activs() {
  ...
  while (update_activ_exchange.recv(recv_buffer)) {
    for (size_t i = 0; i < recv_buffer.size(); ++i) {
      ...
      foreach (const vid_vdata_vprog_triple_type& t, buffer) {
        ...
        active_minorstep.set_bit(lvid);
        graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(t.second) +
                                  sizeof(t.third));
      }
    }
  }
...
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::send_updates(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_TRUE(graph.l_is_master(lvid));
  const vertex_id_type vid = graph.global_vid(lvid);
  local_vertex_type vertex = graph.l_vertex(lvid);
...
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::recv_updates() {
  ...
  while (update_exchange.recv(recv_buffer)) {
    for (size_t i = 0; i < recv_buffer.size(); ++i) {
      ...
      foreach (const vid_vdata_pair_type& pair, buffer) {
        ...
        graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(pair.second));
      }
    }
  }
}  // end of recv_updates
...
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::send_accum(
    lvid_type lvid, const gather_type& accum, const size_t thread_id) {
  if (graph.l_is_master(lvid)) {
    vlocks[lvid].lock();
    ...
    graph_data_bytes_sent.inc(sizeof(vid) + sizeof(accum));
  }
}  // end of send_accum

template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::recv_accums() {
  ...
  while (accum_exchange.recv(recv_buffer)) {
    for (size_t i = 0; i < recv_buffer.size(); ++i) {
      ...
      foreach (const vid_gather_pair_type& pair, buffer) {
        ...
        graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(acc));
        vlocks[lvid].unlock();
      }
    }
  }
...
template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::send_message(
    lvid_type lvid, const size_t thread_id) {
  ASSERT_FALSE(graph.l_is_master(lvid));
  const procid_t master = graph.l_master(lvid);
  const vertex_id_type vid = graph.global_vid(lvid);
  message_exchange.send(master, std::make_pair(vid, messages[lvid]));
  graph_data_bytes_sent.inc(sizeof(vid) + sizeof(messages[lvid]));
}  // end of send_message

template <typename VertexProgram>
inline void powerlyra_sync_ec_engine<VertexProgram>::recv_messages() {
  ...
  while (message_exchange.recv(recv_buffer)) {
    for (size_t i = 0; i < recv_buffer.size(); ++i) {
      ...
      foreach (const vid_message_pair_type& pair, buffer) {
        ...
        graph_data_bytes_recv.inc(sizeof(lvid) + sizeof(msg));
        vlocks[lvid].unlock();
      }
    }
  }
