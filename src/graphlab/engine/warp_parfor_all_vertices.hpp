...
struct parfor_all_vertices_impl {
  ...
  parfor_all_vertices_impl(
      GraphType& graph,
      boost::function<void(typename GraphType::vertex_type)> fn,
      vertex_set& vset)
      : graph(graph), fn(fn), vset(vset), ctr(0) {}
 ...
 * map_reduce_neighborhood vertex.data() = 0.15 + 0.85 *
 ...
 * \param fn A function to run on each vertex. Has the prototype
 * void(GraphType::vertex_type). Can be a boost::function \param vset A set of
 * vertices to run on \param nfibers Number of fiber threads to use. Defaults to
 * 10000 \param stacksize Size of each fiber stack in bytes. Defaults to 16384
 * bytes
 *
...
void parfor_all_vertices(GraphType& graph, FunctionType fn,
                         vertex_set vset = GraphType::complete_set(),
                         size_t nfibers = 10000, size_t stacksize = 16384) {
  distributed_control::get_instance()->barrier();
  bool old_fast_track =
      distributed_control::get_instance()->set_fast_track_requests(false);
  fiber_group group;
  group.set_stacksize(stacksize);
  warp_impl::parfor_all_vertices_impl<GraphType> parfor(graph, fn, vset);

  for (size_t i = 0; i < nfibers; ++i) {
    group.launch(boost::bind(
        &warp_impl::parfor_all_vertices_impl<GraphType>::run_fiber, &parfor));
  }
  group.join();
  distributed_control::get_instance()->barrier();
