...
class distributed_chandy_misra {
 public:
  ...
  enum { OWNER_BIT = 1, DIRTY_BIT = 2, REQUEST_0 = 4, REQUEST_1 = 8 };
  enum { OWNER_SOURCE = 0, OWNER_TARGET = 1 };
  inline unsigned char request_bit(bool owner) {
    return owner ? REQUEST_1 : REQUEST_0;
  }

  enum { COLLISIONS = 0, CANCELLATIONS = 1, ACCEPTED_CANCELLATIONS = 2 };
   ...
   */
  enum { THINKING = 0, HUNGRY = 1, HORS_DOEUVRE = 2, EATING = 3 };
  ...
  inline bool fork_owner(size_t forkid) { return forkset[forkid] & OWNER_BIT; }
  ...
  void compute_initial_fork_arrangement() {
    for (lvid_type i = 0; i < graph.num_local_vertices(); ++i) {
      ...
      philosopherset[i].num_edges =
          lvertex.num_in_edges() + lvertex.num_out_edges();
   ...
   */
  void try_acquire_edge_with_backoff(lvid_type v1, lvid_type v2) {
    if (v1 < v2) {
   ...
   ***************************************************************************/
  inline bool advance_fork_state_on_lock(size_t forkid, lvid_type source,
                                         lvid_type target) {
    ...
    if (currentowner == OWNER_SOURCE) {
      ...
      if (philosopherset[source].state != EATING &&
          (forkset[forkid] & DIRTY_BIT) && (forkset[forkid] & REQUEST_1)) {
      ...
      if (philosopherset[target].state != EATING &&
          (forkset[forkid] & DIRTY_BIT) && (forkset[forkid] & REQUEST_0)) {
        //  change the owner and clean the fork)
  ...
  void cancellation_request_unlocked(lvid_type lvid, procid_t requestor,
                                     bool lockid) {
    ...
    if (philosopherset[lvid].lockid == lockid) {
      if (philosopherset[lvid].counter > 0) {
        ...
        logstream(LOG_DEBUG)
            << rmi.procid() << ": Cancellation accepted on " << gvid << "("
            << (int)philosopherset[lvid].counter << ")" << std::endl;
        philosopherset[lvid].lock.unlock();

        if (requestor != rmi.procid()) {
          unsigned char pkey =
              rmi.dc().set_sequentialization_key(gvid % 254 + 1);
          rmi.remote_call(requestor, &dcm_type::rpc_cancellation_accept, gvid,
                          lockid);
        ...
        philosopherset[lvid].lock.unlock();
        logstream(LOG_DEBUG)
            << rmi.procid() << ": Cancellation on " << graph.global_vid(lvid)
            << " denied due to lock completion" << std::endl;
      }
    } else {
      philosopherset[lvid].lock.unlock();
      logstream(LOG_DEBUG) << rmi.procid() << ": Cancellation on "
                           << graph.global_vid(lvid)
                           << " denied to invalid lock ID" << std::endl;
    }
  }

  void rpc_cancellation_request(vertex_id_type gvid, procid_t requestor,
                                bool lockid) {
  ...
  void issue_cancellation_request_unlocked(lvid_type lvid, bool lockid) {
    // signal the master
    logstream(LOG_DEBUG) << rmi.procid() << ": Requesting cancellation on "
    ...
    if (lvertex.owner() == rmi.procid()) {
      ...
      rmi.remote_call(lvertex.owner(), &dcm_type::rpc_cancellation_request,
                      lvertex.global_id(), rmi.procid(), lockid);
      rmi.dc().set_sequentialization_key(pkey);
    }
  }
  ...
  void cancellation_accept_unlocked(lvid_type p_id, bool lockid) {
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": Cancellation accept received on "
                         << lvertex.global_id() << " "
    ...
    foreach (local_edge_type edge, lvertex.in_edges()) {
      ...
      if (philosopherset[p_id].state == HUNGRY) {
        // std::cout << "\t" << graph.edge_id(edge) << ": " << edge.source() <<
        // "->" << edge.target() << std::endl;
        lvid_type other = edge.source().id();
        size_t edgeid = edge.id();
        if (fork_owner(edgeid) == OWNER_TARGET && fork_dirty(edgeid)) {
          if (advance_fork_state_on_lock(edgeid, edge.source().id(),
                                         edge.target().id()) &&
    ...
    foreach (lvid_type lvid, retval) { enter_hors_doeuvre_unlocked(lvid); }
  }
  ...
  void rpc_make_philosopher_hungry(vertex_id_type gvid, bool newlockid) {
    lvid_type lvid = graph.local_vid(gvid);
    logstream(LOG_DEBUG) << rmi.procid() << ": Local HUNGRY Philosopher  "
                         << gvid << std::endl;
    philosopherset[lvid].lock.lock();
    ...
    foreach (local_edge_type edge, lvertex.in_edges()) {
      ...
      if (philosopherset[p_id].state == HUNGRY) {
        ...
        if (fork_owner(edgeid) == OWNER_SOURCE) {
          ...
          advance_fork_state_on_lock(edgeid, edge.source().id(),
                                     edge.target().id());
        }
        philosopherset[edge.source().id()].lock.unlock();
      } else {
  ...
  void enter_hors_doeuvre_unlocked(lvid_type p_id) {
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": Local HORS_DOEUVRE Philosopher  "
                         << graph.global_vid(p_id) << std::endl;
    // signal the master
    local_vertex_type lvertex(graph.l_vertex(p_id));
    ...
    if (lvertex.owner() == rmi.procid()) {
      ...
      rmi.remote_call(lvertex.owner(), &dcm_type::rpc_signal_ready,
  ...
  void signal_ready_unlocked(lvid_type lvid, bool lockid) {
    ...
    if (!(philosopherset[lvid].state == (int)HUNGRY ||
      ...
      logstream(LOG_ERROR) << rmi.procid() << ": Bad signal ready state!!!! : "
                           << (int)philosopherset[lvid].state << std::endl;
      logstream(LOG_ERROR) << rmi.procid()
                           << " Lock IDs : " << (int)philosopherset[lvid].lockid
                           << " " << (int)lockid << std::endl;
      logstream(LOG_ERROR) << rmi.procid() << ": BAD Global HORS_DOEUVRE "
                           << graph.global_vid(lvid) << "("
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": Global HORS_DOEUVRE "
                         << graph.global_vid(lvid) << "("
                         << (int)philosopherset[lvid].counter << ")"
                         << " " << (int)(philosopherset[lvid].state)
                         << std::endl;
  ...
  void set_eating(lvid_type lvid, bool lockid) {
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": EATING "
                         << graph.global_vid(lvid) << "("
                         << (int)philosopherset[lvid].counter << ")"
                         << std::endl;
    ...
    if (graph.l_vertex(lvid).owner() == rmi.procid()) {
      logstream(LOG_DEBUG) << rmi.procid() << ": CALLBACK "
                           << graph.global_vid(lvid) << std::endl;
  ...
  void rpc_set_eating(vertex_id_type gvid, bool lockid) {
    logstream(LOG_DEBUG) << rmi.procid() << ": Receive Set EATING " << gvid
                         << std::endl;
  ...
  inline bool advance_fork_state_on_unlock(size_t forkid, lvid_type source,
                                           lvid_type target) {
    unsigned char currentowner = forkset[forkid] & OWNER_BIT;
    if (currentowner == OWNER_SOURCE) {
      // if the current owner is not eating, and the
      // fork is dirty and other side has placed a request
      if ((forkset[forkid] & DIRTY_BIT) && (forkset[forkid] & REQUEST_1)) {
        //  change the owner and clean the fork)
      ...
      // if the current owner is not eating, and the
      // fork is dirty and other side has placed a request
      if ((forkset[forkid] & DIRTY_BIT) && (forkset[forkid] & REQUEST_0)) {
        //  change the owner and clean the fork)
        // keep my request bit if any
        clean_fork_count.inc();
        forkset[forkid] = OWNER_SOURCE;
  ...
  void local_philosopher_stops_eating(lvid_type p_id) {
    ...
    foreach (local_edge_type edge, lvertex.in_edges()) {
      ...
      if (philosopherset[p_id].state == THINKING) {
        size_t edgeid = edge.id();
        advance_fork_state_on_unlock(edgeid, edge.source().id(),
                                     edge.target().id());
  ...
  void rpc_philosopher_stops_eating(vertex_id_type gvid) {
    logstream(LOG_DEBUG) << rmi.procid() << ": Receive STOP eating on " << gvid
                         << std::endl;
    local_philosopher_stops_eating(graph.local_vid(gvid));
  }

 public:
  inline distributed_chandy_misra(
      distributed_control &dc, GraphType &graph,
      boost::function<void(lvid_type)> callback,
  ...
  size_t num_clean_forks() const { return clean_fork_count.value; }
  ...
  void make_philosopher_hungry(lvid_type p_id) {
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": Global HUNGRY "
                         << lvertex.global_id() << "("
    ...
    unsigned char pkey =
        rmi.dc().set_sequentialization_key(lvertex.global_id() % 254 + 1);
    rmi.remote_call(lvertex.mirrors().begin(), lvertex.mirrors().end(),
                    &dcm_type::rpc_make_philosopher_hungry, lvertex.global_id(),
                    newlockid);
    rmi.dc().set_sequentialization_key(pkey);
  ...
  void philosopher_stops_eating(lvid_type p_id) {
    ...
    logstream(LOG_DEBUG) << rmi.procid() << ": Global STOP Eating "
    ...
    unsigned char pkey =
        rmi.dc().set_sequentialization_key(lvertex.global_id() % 254 + 1);
    rmi.remote_call(lvertex.mirrors().begin(), lvertex.mirrors().end(),
                    &dcm_type::rpc_philosopher_stops_eating,
                    lvertex.global_id());
    rmi.dc().set_sequentialization_key(pkey);
  ...
  void no_locks_consistency_check() {
    ...
    for (size_t i = 0; i < philosopherset.size(); ++i)
      ASSERT_TRUE(philosopherset[i].state == THINKING);
  }
  ...
  void print_out() {
    ...
    for (lvid_type v = 0; v < graph.num_local_vertices(); ++v) {
      ...
      std::cout << graph.global_vid(v) << ": " << (int)philosopherset[v].state
                << " " << philosopherset[v].forks_acquired << " "
                << philosopherset[v].num_edges << " ";
      if (philosopherset[v].forks_acquired == philosopherset[v].num_edges)
        std::cout << "---------------!";
      std::cout << "\n";
      std::cout << "\tin: ";
      foreach (local_edge_type edge, lvertex.in_edges()) {
        size_t edgeid = edge.id();
        if (fork_dirty(forkset[edgeid]))
          std::cout << edgeid << ":" << (int)forkset[edgeid] << " ";
      }
      std::cout << "\n\tout: ";
      foreach (local_edge_type edge, lvertex.out_edges()) {
        size_t edgeid = edge.id();
  ...
  void complete_consistency_check() {
    for (lvid_type v = 0; v < graph.num_local_vertices(); ++v) {
      ...
      if (philosopherset[v].state == THINKING) {
        ...
        ASSERT_NE(philosopherset[v].num_edges,
                  philosopherset[v].forks_acquired);
        ...
        ASSERT_EQ(philosopherset[v].forks_acquired,
                  philosopherset[v].num_edges);
      }
    }
  }
