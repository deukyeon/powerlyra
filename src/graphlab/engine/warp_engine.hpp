...
class warp_engine {
 public:
  ...
  struct context {
    ...
    context(warp_engine& engine, graph_type& graph, vertex_type vtx)
        : engine(engine), graph(graph), vtx(vtx), vtx_set(true) {
      set_synchronized();
    }

    context(warp_engine& engine, graph_type& graph)
        : engine(engine), graph(graph), vtx(graph, 0), vtx_set(false) {}
  ...
  typedef boost::function<void(context_type&, vertex_type)>
 ...
 public:
   ...
   */
  warp_engine(distributed_control& dc, graph_type& graph,
   ...
   */
  size_t get_rpc_obj_id() { return rmi.get_obj_id(); }
 ...
 private:
  ...
  void internal_signal_gvid(vertex_id_type gvid,
    ...
    if (graph.is_master(gvid)) {
      ...
      rmi.remote_call(proc, &warp_engine::internal_signal, gvid, message);
    }
  }
 ...
 private:
  ...
  void update_vertex_value(vertex_id_type vid, vertex_data_type& vdata) {
  ...
  void synchronize_one_vertex(vertex_type vtx) {
    ...
    foreach (procid_t mirror, lvtx.mirrors()) {
      rmi.remote_call(mirror, &warp_engine::update_vertex_value, vtx.id(),
                      vtx.data());
    }
  }
  ...
  void synchronize_one_vertex_wait(vertex_type vtx) {
    ...
    foreach (procid_t mirror, lvtx.mirrors()) {
      futures.push_back(object_fiber_remote_request(
          rmi, mirror, &warp_engine::update_vertex_value, vtx.id(),
          vtx.data()));
    }
