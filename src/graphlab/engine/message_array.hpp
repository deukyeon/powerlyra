...
class message_array {
 ...
 private:
  ...
  static size_t get_lock_idx(size_t i) { return i % 65536; }

 public:
  /** Initialize the per vertex task set */
  message_array(size_t num_vertices = 0) : message_vector(num_vertices) {
   ...
   */
  void resize(size_t num_vertices) { message_vector.resize(num_vertices); }
   ...
   * stored at the index.
   */
  bool get(const size_t idx, value_type& ret_val) {
    bool has_val = false;
    size_t lockidx = get_lock_idx(idx);
    lock_array[lockidx].lock();
  ...
  bool empty(const size_t idx) const { return message_vector[idx].empty; }
  ...
  size_t size() const { return message_vector.size(); }
