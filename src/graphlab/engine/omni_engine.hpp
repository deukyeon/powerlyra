...
class omni_engine : public iengine<VertexProgram> {
 ...
 public:
  ...
  omni_engine(distributed_control& dc, graph_type& graph,
    ...
    } else if (engine_type == "plsync" ||
               engine_type == "powerlyra_synchronous") {
      logstream(LOG_INFO) << "Using the PowerLyra Synchronous engine."
                          << std::endl;
      engine_ptr = new powerlyra_sync_engine_type(dc, graph, new_options);
    } else if (engine_type == "plsyncec" ||
               engine_type == "powerlyra_synchronous_ec") {
      logstream(LOG_INFO) << "Using the PowerLyra Synchronous Edge-cut engine"
                          << std::endl;
      engine_ptr = new powerlyra_sync_ec_engine_type(dc, graph, new_options);
    } else if (engine_type == "plasync" ||
               engine_type == "powerlyra_asynchronous") {
      logstream(LOG_INFO) << "Using the PowerLyra Asynchronous engine."
                          << std::endl;
      engine_ptr = new powerlyra_async_engine_type(dc, graph, new_options);
    } else {
      logstream(LOG_FATAL) << "Invalid engine type: " << engine_type
                           << std::endl;
    }
  }  // end of constructor
