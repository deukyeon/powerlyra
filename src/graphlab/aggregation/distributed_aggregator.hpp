...
class distributed_aggregator {
 ...
 private:
  ...
  struct default_map_types {
    typedef ReductionType (*vertex_map_type)(icontext_type&,
                                             const vertex_type&);
  ...
  template <typename ReductionType, typename VertexMapperType,
            typename EdgeMapperType, typename FinalizerType>
  struct map_reduce_type : public imap_reduce_base {
    ...
    map_reduce_type(EdgeMapperType map_edge_function,
                    FinalizerType finalize_function, bool)
    ...
    void perform_map_vertex(icontext_type& context, vertex_type& vertex) {
       ...
       *
       *  ReductionType mapfun(icontext_type& context, const vertex_type&
       * vertex);
       *
    ...
    bool is_vertex_map() const { return vertex_map; }

    any get_accumulator() const { return any(acc); }
    ...
    void clear_accumulator() { acc.clear(); }
    ...
    imap_reduce_base* clone_empty() const {
      ...
      if (is_vertex_map()) {
        copy = new map_reduce_type(map_vtx_function, finalize_function);
      } else {
        copy = new map_reduce_type(map_edge_function, finalize_function, true);
      }
      return copy;
    }
  };
  ...
  static void test_vertex_mapper_type(std::string key = "") {
    bool test_result = test_function_or_const_functor_2<
        F, ReductionType(icontext_type&, const vertex_type&), ReductionType,
        icontext_type&, const vertex_type&>::value;
    if (!test_result) {
      ...
      if (boost::is_function<typename boost::remove_pointer<F>::type>::value) {
        ...
        strm << "Functor's operator() prototype should be \n"
             << "\t ReductionType operator()(icontext_type&, const "
                "vertex_type&) const\n";
      }
  ...
  static void test_edge_mapper_type(std::string key = "") {
    bool test_result = test_function_or_const_functor_2<
        F, ReductionType(icontext_type&, const edge_type&), ReductionType,
        icontext_type&, const edge_type&>::value;
    ...
    if (!test_result) {
      ...
      if (boost::is_function<typename boost::remove_pointer<F>::type>::value) {
        ...
        strm << "Functor's operator() prototype should be "
             << "\t ReductionType operator()(icontext_type&, const edge_type&) "
                "const\n";
      }
      strm << "If you are not intentionally violating the abstraction,"
           << " we recommend fixing your function for safety reasons";
 ...
 public:
  distributed_aggregator(distributed_control& dc, graph_type& graph,
                         icontext_type* context)
      : rmi(dc, this), graph(graph), context(context), ncpus(0) {}
  ...
  bool add_vertex_aggregator(const std::string& key,
    ...
    if (aggregators.count(key) == 0) {
      ...
      aggregators[key] = new map_reduce_type<
          ReductionType, VertexMapperType,
          typename default_map_types<ReductionType>::edge_map_type,
          FinalizerType>(map_function, finalize_function);
      return true;
    } else {
   ...
   */
  template <typename VertexMapperType, typename FinalizerType>
  bool add_vertex_aggregator(const std::string& key,
                             VertexMapperType map_function,
                             FinalizerType finalize_function) {
  ...
  template <typename ReductionType, typename EdgeMapperType,
            typename FinalizerType>
  bool add_edge_aggregator(const std::string& key, EdgeMapperType map_function,
                           FinalizerType finalize_function) {
    if (key.length() == 0) return false;
    if (aggregators.count(key) == 0) {
      if (rmi.procid() == 0) {
      ...
      aggregators[key] = new map_reduce_type<
          ...
          EdgeMapperType, FinalizerType>(map_function, finalize_function, true);
      return true;
    } else {
      // aggregator already exists. fail
      return false;
   ...
   */
  template <typename EdgeMapperType, typename FinalizerType>
  bool add_edge_aggregator(const std::string& key, EdgeMapperType map_function,
                           FinalizerType finalize_function) {
    // an edge_type is actually hard to get
    typedef decltype(map_function(
        *context, edge_type(graph.l_vertex(0).in_edges()[0]))) ReductionType;
    if (key.length() == 0) return false;
    if (aggregators.count(key) == 0) {
      aggregators[key] = new map_reduce_type<
          ReductionType,
...
#pragma omp critical
  ...
  void start(size_t ncpus = 0) {
    ...
    if (ncpus > 0) {
      ...
      while (iter != aggregate_period.end()) {
        ...
        async_state[iter->first].distributed_count_down = (int)rmi.numprocs();
  ...
  void tick_asynchronous_compute(size_t cpuid, const std::string& key) {
    ...
    if (countdown_val == 0) {
      ...
      // - reset the counters
      for (size_t i = 0; i < iter->second.per_thread_aggregation.size(); ++i) {
      ...
      if (rmi.procid() != 0) {
        ...
        rmi.remote_call(0, &distributed_aggregator::rpc_key_merge, key, acc);
      } else {
  ...
  void decrement_distributed_counter(const std::string& key) {
    ...
    if (countdown_val == 0) {
      ...
      for (procid_t i = 1; i < rmi.numprocs(); ++i) {
        rmi.remote_call(i, &distributed_aggregator::rpc_perform_finalize, key,
                        acc_val);
      }
  ...
  void decrement_finalize_counter(const std::string& key) {
    ...
    if (countdown_val == 0) {
      ...
      float next_time =
          timer::approx_time_seconds() + aggregate_period[key] - start_time;
      logstream(LOG_INFO) << rmi.procid() << "Reschedule of " << key << " at "
                          << next_time << std::endl;
      rpc_schedule_key(key, next_time);
      for (procid_t i = 1; i < rmi.numprocs(); ++i) {
        rmi.remote_call(i, &distributed_aggregator::rpc_schedule_key, key,
                        next_time);
      }
    }
  }
  ...
  void stop() {
    ...
    // clear the asynchronous state
    {
      ...
      while (iter != async_state.end()) {
        ...
        for (size_t i = 0; i < iter->second.per_thread_aggregation.size();
             ++i) {
...
#pragma omp critical
    ...
    conditional_addition_wrapper<ResultType> wrapper(global_result,
                                                     global_result_set);
...
#pragma omp parallel for
  ...
  ~distributed_aggregator() { delete context; }
};
