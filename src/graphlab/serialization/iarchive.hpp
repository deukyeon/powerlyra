...
class iarchive_soft_fail {
 public:
   ...
   */
  inline iarchive_soft_fail(iarchive& iarc) : iarc(&iarc), mine(false) {}
...
inline iarchive& operator>>(iarchive& iarc, T& t) {
  archive_detail::deserialize_impl<iarchive, T, gl_is_pod<T>::value>::exec(iarc,
                                                                           t);
  return iarc;
}
...
inline iarchive_soft_fail& operator>>(iarchive_soft_fail& iarc, T& t) {
  archive_detail::deserialize_impl<iarchive_soft_fail, T,
...
/**
   deserializes an arbitrary pointer + length from an archive
*/
inline iarchive_soft_fail& deserialize(iarchive_soft_fail& iarc, void* str,
                                       const size_t length) {
  iarc.read(reinterpret_cast<char*>(str), (std::streamsize)length);
  assert(!iarc.fail());
  return iarc;
