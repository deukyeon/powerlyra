...
class spinlock {
 ...
 public:
  ...
  inline bool try_lock() const { return pthread_spin_trylock(&m_spin) == 0; }
  ~spinlock() {
...
class simple_spinlock {
 ...
 public:
  /// constructs a spinlock
  simple_spinlock() { spinner = 0; }

  /** Copy constructor which does not copy. Do not use!
  Required for compatibility with some STL implementations (LLVM).
  which use the copy constructor for vector resize,
  rather than the standard constructor.    */
  simple_spinlock(const simple_spinlock&) { spinner = 0; }
  ...
  ~simple_spinlock() { ASSERT_TRUE(spinner == 0); }
};
...
class padded_simple_spinlock {
 ...
 public:
  /// constructs a spinlock
  padded_simple_spinlock() { spinner = 0; }

  /** Copy constructor which does not copy. Do not use!
  Required for compatibility with some STL implementations (LLVM).
  which use the copy constructor for vector resize,
  rather than the standard constructor.    */
  padded_simple_spinlock(const padded_simple_spinlock&) { spinner = 0; }
  ...
  ~padded_simple_spinlock() { ASSERT_TRUE(spinner == 0); }
};
...
class spinrwlock {
 ...
 public:
  spinrwlock() { memset(const_cast<rwticket*>(&l), 0, sizeof(rwticket)); }
  ...
  inline void rdunlock() const { atomic_inc(&l.s.write); }
...
struct spinrwlock2 {
  ...
  void wrunlock() const { atomic_add(&l, -RW_WRITE); }
  ...
  void rdunlock() const { atomic_add(&l, -RW_READ); }
};
...
class rwlock {
 ...
 public:
  ...
  inline void rdunlock() const { unlock(); }
  inline void wrunlock() const { unlock(); }
};  // End rwlock
...
class cancellable_barrier {
 ...
 public:
  ...
  void resize_unsafe(size_t numthreads) { needed = numthreads; }
...
class thread {
 public:
  ...
  class tls_data {
   public:
    ...
    size_t erase(const size_t& id) { return local_data.erase(id); }
  ...
  /** Set the id of the calling thread.  This will typically be the
      index in the thread group. Between 0 to ncpus. */
  static inline void set_thread_id(size_t t) {
    get_tls_data().set_thread_id(t);
  }

  /**
   * Get a reference to an any object
   */
  static inline any& get_local(const size_t& id) { return get_tls_data()[id]; }
 ...
 private:
  struct invoke_args {
    ...
    invoke_args(size_t m_thread_id,
                const boost::function<void(void)>& spawn_routine)
 ...
 public:
  ...
  inline bool active() const { return thread_started; }

  inline ~thread() {}

  /// Returns the pthread thread id
  inline pthread_t pthreadid() { return m_p_thread; }
...
class thread_group {
 private:
  ...
  static void invoke(boost::function<void(void)> spawn_function,
                     thread_group* group);
...
/// Runs f in a new thread. convenience function for creating a new thread
/// quickly.
