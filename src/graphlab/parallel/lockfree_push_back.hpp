...
struct idx_ref {
  ...
  inline void dec_ref() { __sync_fetch_and_sub(&reference_count, 1); }

  inline void flag_ref() { __sync_fetch_and_xor(&reference_count, MAX_REF); }

  inline size_t inc_idx() { return idx.inc_ret_last(); }

  inline size_t inc_idx(size_t n) { return idx.inc_ret_last(n); }
};
}  // namespace lockfree_push_back_impl
...
class lockfree_push_back {
 ...
 public:
  lockfree_push_back(Container& container, size_t startidx,
                     float scalefactor = 2)
      : container(container), cur(startidx), scalefactor(scalefactor) {}

  size_t size() const { return cur.idx.value; }

  void set_size(size_t s) { cur.idx.value = s; }
    ...
    while (1) {
      ...
      if (endidx <= container.size()) {
        ...
        if (mut.try_lock()) {
          ...
          if (endidx > container.size()) {
            container.resize(
                std::max<size_t>(endidx, container.size() * scalefactor));
          }
          while (putpos < endidx) {
            container[putpos] = (*begin);
            ++putpos;
    ...
    while (1) {
      ...
      if (putpos < container.size()) {
        ...
        if (mut.try_lock()) {
          ...
          if (putpos >= container.size()) {
            container.resize(
                std::max<size_t>(putpos + 1, container.size() * scalefactor));
          }
          container[putpos] = t;
          cur.flag_ref();
          mut.unlock();
