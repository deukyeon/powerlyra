...
key_value::key_value(std::vector<std::string> zkhosts, std::string _prefix,
  ...
  if (prefix != "/")
    create_dir(handle, prefix.substr(0, prefix.length() - 1), "zk_key_value");
  if (prefix != "/") {
  ...
  create_dir(handle, masters_path.substr(0, masters_path.length() - 1),
             "zk_key_value");

  create_dir(handle, values_path.substr(0, values_path.length() - 1),
             "zk_key_value");
  ...
  while (iter != my_values.end()) {
    ...
    if (version >= 0) {
      std::string value_node =
          get_sequence_node_path(values_path + (*iter) + "-", version);
...
bool key_value::insert(const std::string& key, const std::string& value) {
  ...
  int ret = create_ephemeral_node(handle, masters_path + key, serveridentifier,
                                  "zk_key_value insert");
  if (ret == ZNODEEXISTS)
    return false;
  else {
...
bool key_value::modify(const std::string& key, const std::string& value) {
  ...
  datalock.lock();
  // add a - to the end
  std::pair<int, int> ret = create_ephemeral_sequence_node(
      handle, values_path + key + "-", value, "zk_key_value modify");
  ...
  if (prev_remote_version >= 0) {
    std::string old_node_path =
        get_sequence_node_path(values_path + key + "-", prev_remote_version);
...
bool key_value::erase(const std::string& key) {
  ...
  if (cur_remote_version >= 0) {
    std::string old_node_path =
        get_sequence_node_path(values_path + key + "-", cur_remote_version);
...
bool key_value::get_all_keys_locked(
  ...
  fill_data_locked(allkeys, masterkeys, out_newkeys, out_deletedkeys,
                   out_modifiedkeys);
  return true;
}
...
std::pair<bool, std::string> key_value::get(const std::string& key) {
  ...
  std::string node =
      get_sequence_node_path(values_path + key + "-", remote_version);
  datalock.unlock();
  // ok. try to query the node
  // std::cout << "Getting value for " << node << "\n";
  std::pair<bool, std::string> value =
      get_node_value(handle, node, "zk_key_value get");
...
void key_value::fill_data_locked(const std::vector<std::string>& keys,
  ...
  // modified keys
  {
    ...
    while (iter != key_and_version.end()) {
      std::map<std::string, lazy_value>::iterator data_iter =
          data.find(iter->first);
      if (data_iter == data.end()) {
        ...
        data[iter->first].has_value = (data[iter->first].stored_version ==
                                       data[iter->first].remote_version);
      } else {
        ...
        data_iter->second.remote_version =
            std::max(data_iter->second.remote_version, iter->second);
        data[iter->first].has_value = (data[iter->first].stored_version ==
                                       data[iter->first].remote_version);
      }
      ++iter;
...
void key_value::watcher(zhandle_t* zh, int type, int state, const char* path,
                        void* watcherCtx) {
