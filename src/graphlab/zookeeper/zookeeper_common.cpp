...
void print_stat(int stat, const std::string& prefix, const std::string& path) {
  if (stat == ZNONODE) {
    ...
    std::cerr << prefix << ": No permission to list children of node " << path
              << std::endl;
    ...
    std::cerr << prefix << ": Unexpected error " << stat << " on path " << path
              << std::endl;
  }
}
...
int create_dir(zhandle_t* handle, const std::string& name,
  ...
  int stat = zoo_create(handle, name.c_str(), NULL, -1, &ZOO_OPEN_ACL_UNSAFE, 0,
                        NULL, 0);
...
int create_ephemeral_node(zhandle_t* handle, const std::string& path,
  ...
  if (stat != ZOK)
    print_stat(stat, stat_message + " create_ephemeral_node", path);
  return stat;
}
...
int delete_sequence_node(zhandle_t* handle, const std::string& path,
                         const int version, const std::string& stat_message) {
  std::string actualpath = get_sequence_node_path(path, version);
  int stat = zoo_delete(handle, actualpath.c_str(), -1);
  //  if (stat == ZOK) return stat;
  if (stat != ZOK)
    print_stat(stat, stat_message + " delete_sequence_node", actualpath);
  return stat;
}
...
std::pair<int, int> create_ephemeral_sequence_node(
  ...
  // if (stat == ZOK) return stat;
  if (stat != ZOK)
    print_stat(stat, stat_message + " create_ephemeral_sequence_node", path);
...
std::pair<bool, std::string> get_node_value(zhandle_t* handle,
  ...
  if (length <= 1024) {
    ...
    if (length < 0)
      ...
      return std::pair<bool, std::string>(true, std::string(buffer, length));
  } else {
    while (1) {
      // buffer not long enough. The length parameter constains the actual
      // length try again. keep looping until we succeed
      ...
      if (length < 0)
        ...
        return std::pair<bool, std::string>(true, retval);
    }
  }
}
