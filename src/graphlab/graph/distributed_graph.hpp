 ...
 *                but currently only works with perfect square number of
 * machines.
 *
 * \li \c "pds"  Runs at roughly the speed of random. Randomly places
 *                edges on machines with a sparser constraint generated by
 *                perfect difference set.  This obtains the highest quality
 * partition, reducing runtime memory consumption significantly, without
 * load-time penalty. Currently only works with p^2+p+1 number of machines (p
 * prime).
 *
 ...
 * \ref save(const std::string& prefix, writer writer, bool gzip, bool
 * save_vertex, bool save_edge, size_t files_per_machine) "save()" provides
 * generalized writing capabilities allowing you to write your own graph output
 * to disk or HDFS.
 *
...
class distributed_graph {
 public:
  ...
  /**
     ...
     See \ref graphlab::distributed_graph::load(std::string path,
     line_parser_type line_parser) "load()" for details.
   */
  ...
  struct vertex_type {
    ...
    bool operator==(vertex_type& v) const { return lvid == v.lvid; }
    ...
    vertex_id_type id() const { return graph_ref.global_vid(lvid); }

    /// \cond GRAPHLAB_INTERNAL

    /// \brief Returns a list of in edges (not implemented)
    edge_list_type in_edges() __attribute__((noreturn)) { ASSERT_TRUE(false); }

    /// \brief Returns a list of out edges (not implemented)
    edge_list_type out_edges() __attribute__((noreturn)) { ASSERT_TRUE(false); }
    /// \endcond

    /**
     *  \brief Returns the local ID of the vertex
     */
    lvid_type local_id() const { return lvid; }
  };
   ...
   *                grid with ( |m-n| <= 2). "pds" uses requires P = p^2+p+1
  ...
  lock_manager_type& get_lock_manager() { return lock_manager; }
  ...
  void set_options(const graphlab_options& opts) {
    ...
    foreach (std::string opt, keys) {
      if (opt == "ingress") {
        ...
        if (rpc.procid() == 0)
          logstream(LOG_EMPH)
              << "Graph Option: ingress = " << ingress_method << std::endl;
 ...
 public:
  // METHODS ===============================================================>
  bool is_dynamic() const { return local_graph.is_dynamic(); }
  ...
  void set_duplicate_vertex_strategy(
      boost::function<void(vertex_data_type&, const vertex_data_type&)>
          combine_strategy) {
  ...
  bool add_vertex(const vertex_id_type& vid,
    ...
    if (vid == vertex_id_type(-1)) {
      logstream(LOG_ERROR)
          ...
          << "\n\tThe -1 vertex id is reserved for internal use." << std::endl;
      return false;
    }
    ASSERT_NE(ingress_ptr, NULL);
  ...
  bool add_edge(vertex_id_type source, vertex_id_type target,
    ...
    if (source == target) {
      logstream(LOG_ERROR) << "\n\tTrying to add self edge (" << source << "->"
                           << target << ")."
                           << "\n\tSelf edges are not allowed." << std::endl;
      return false;
    }
    ASSERT_NE(ingress_ptr, NULL);
  ...
  ReductionType map_reduce_vertices(MapFunctionType mapfunction,
    ...
    if (!finalized) {
      logstream(LOG_FATAL)
          ...
          << "\n\tbefore calling graph.finalize()." << std::endl;
    }
...
#pragma omp critical
    ...
    conditional_addition_wrapper<ReductionType> wrapper(global_result,
                                                        global_result_set);
...
#pragma omp critical
   ...
   * value of the data on the vertex.
   * \code
   * void absolute_vertex_data(const graph_type::vertex_type& vertex, float&
   * total) { total += std::fabs(vertex.data());
   * }
   * \endcode
   * After which calling:
   * \code
  ...
  void transform_vertices(TransformType transform_functor,
    ...
    if (!finalized) {
      logstream(LOG_FATAL)
          ...
          << "\n\tbefore finalizing the graph." << std::endl;
    }
...
#pragma omp parallel for
    ...
    for (int i = 0; i < (int)accfunction.size(); ++i) {
      for (int j = i; j < (int)local_graph.num_vertices();
           j += numaccfunctions) {
...
#pragma omp parallel for
  ...
  void load(iarchive& arc) {
    // read the vertices
    arc >> nverts >> nedges >> local_own_nverts >> nreplicas >> vid2lvid >>
        lvid2record >> local_graph;
    finalized = true;
   ...
   * Like \ref save(const std::string& prefix, writer writer, bool gzip, bool
   * save_vertex, bool save_edge, size_t files_per_machine) "save()" but only
   * saves to local filesystem.
   */
  template <typename Writer>
  void save_to_posixfs(const std::string& prefix, Writer writer,
                       bool gzip = true, bool save_vertex = true,
                       bool save_edge = true, size_t files_per_machine = 4) {
    ...
    for (size_t i = 0; i < files_per_machine; ++i) {
      // graph_files[i] = prefix + "_" + tostr(1 + i + rpc.procid() *
      // files_per_machine)
      graph_files[i] = prefix + "." +
                       tostr(1 + i + rpc.procid() * files_per_machine) +
    ...
    for (size_t i = 0; i < graph_files.size(); ++i) {
      ...
      base_fstream_type* out_file = new base_fstream_type(
          graph_files[i].c_str(), std::ios_base::out | std::ios_base::binary);
      ...
      vertex_callbacks[i] = boost::bind(
          &graph_type::template save_vertex_to_stream<boost_fstream_type,
                                                      Writer>,
          this, _1, boost::ref(*fout), boost::ref(writer));
      edge_callbacks[i] = boost::bind(
          &graph_type::template save_edge_to_stream<boost_fstream_type, Writer>,
          this, _1, boost::ref(*fout), boost::ref(writer));
    }
   ...
   */
  template <typename Writer>
  void save_to_hdfs(const std::string& prefix, Writer writer, bool gzip = true,
                    bool save_vertex = true, bool save_edge = true,
    ...
    if (!hdfs::has_hadoop()) {
      logstream(LOG_FATAL)
          ...
          << "\n\twas built without HDFS." << std::endl;
    }
    ...
    for (size_t i = 0; i < graph_files.size(); ++i) {
      ...
      base_fstream_type* out_file =
          new base_fstream_type(hdfs, graph_files[i], true);
      // attach gzip if the file is gzip
      boost_fstream_type* fout = new boost_fstream_type;
      // Using gzip filter
      if (gzip) fout->push(boost::iostreams::gzip_compressor());
   ...
   */
  template <typename Writer>
  void save(const std::string& prefix, Writer writer, bool gzip = true,
            bool save_vertex = true, bool save_edge = true,
            size_t files_per_machine = 4) {
    if (boost::starts_with(prefix, "hdfs://")) {
      save_to_hdfs(prefix, writer, gzip, save_vertex, save_edge,
                   files_per_machine);
    } else {
      save_to_posixfs(prefix, writer, gzip, save_vertex, save_edge,
                      files_per_machine);
    }
  }  // end of save
  ...
  void save_format(const std::string& prefix, const std::string& format,
    ...
    if (format == "snap" || format == "tsv") {
      save(prefix, builtin_parsers::tsv_writer<distributed_graph>(), gzip,
           false, true, files_per_machine);
    } else if (format == "graphjrl") {
      save(prefix, builtin_parsers::graphjrl_writer<distributed_graph>(), gzip,
           true, true, files_per_machine);
    } else if (format == "bin") {
      save_binary(prefix);
    } else if (format == "bintsv4") {
      save_direct(prefix, gzip, &graph_type::save_bintsv4_to_stream);
    } else {
      logstream(LOG_FATAL) << "Unrecognized Format \"" << format << "\"!"
                           << std::endl;
      return;
    }
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < graph_files.size(); ++i) {
      if ((parallel_ingress && (i % rpc.numprocs() == rpc.procid())) ||
          (!parallel_ingress && (rpc.procid() == 0)) || (data_affinity)) {
...
#pragma omp parallel for
  ...
  void load_format(const std::string& path, const std::string& format) {
    ...
    if (format == "snap") {
      ...
      logstream(LOG_ERROR) << "Unrecognized Format \"" << format << "\"!"
                           << std::endl;
      return;
    }
  }  // end of load
   ...
   */
  static vertex_set empty_set() { return vertex_set(false); }

  /**
   *  \brief Retuns a full set of vertices
   */
  static vertex_set complete_set() { return vertex_set(true); }
...
#pragma omp for
  ...
  struct vertex_record {
    ...
    void clear() { _mirrors.clear(); }

    void load(iarchive& arc) {
      clear();
      arc >> owner >> dtype >> gvid >> num_in_edges >> num_out_edges >>
          _mirrors;
    }
    ...
    bool operator==(const vertex_record& other) const {
      return ((owner == other.owner) && (dtype == other.dtype) &&
              (gvid == other.gvid) && (num_in_edges == other.num_in_edges) &&
  ...
  procid_t procid() const { return rpc.procid(); }

  procid_t numprocs() const { return rpc.numprocs(); }

  distributed_control& dc() { return rpc.dc(); }
   ...
   */
  local_graph_type& get_local_graph() { return local_graph; }

  /** \internal
   *  \brief Returns a const reference to the internal graph representation
   */
  const local_graph_type& get_local_graph() const { return local_graph; }
...
#pragma omp parallel for
  ...
  struct local_vertex_type {
    ...
    explicit local_vertex_type(vertex_type v)
        : graph_ref(v.graph_ref), lvid(v.lvid) {}
    /// \brief Can be casted to vertex_type using an explicit cast
    operator vertex_type() const { return vertex_type(graph_ref, lvid); }

    bool operator==(local_vertex_type& v) const { return lvid == v.lvid; }
    ...
    lvid_type id() const { return lvid; }

    /// \brief Returns the global ID of this local vertex
    vertex_id_type global_id() const { return graph_ref.global_vid(lvid); }

    /** \brief Returns a list of all in edges on the
     *         local graph of this local vertex
     */
    local_edge_list_type in_edges() { return graph_ref.l_in_edges(lvid); }

    /** \brief Returns a list of all out edges on the
     *         local graph of this local vertex
     */
    local_edge_list_type out_edges() { return graph_ref.l_out_edges(lvid); }

    /** \brief Returns the owner of this local vertex
     */
    procid_t owner() const { return graph_ref.l_get_vertex_record(lvid).owner; }
  ...
  struct local_edge_list_type {
    ...
    typedef boost::transform_iterator<
        make_local_edge_type_functor,
        typename local_graph_type::edge_list_type::iterator>
        iterator;
    ...
    iterator begin() const {
      return boost::make_transform_iterator(elist.begin(), me_functor);
    }
     ...
     */
    iterator end() const {
      return boost::make_transform_iterator(elist.end(), me_functor);
    }
 ...
 public:
  ...
  size_t get_rpc_obj_id() { return rpc.get_obj_id(); }
 ...
 private:
  ...
  buffered_exchange<std::pair<vertex_id_type, vertex_data_type> >
      vertex_exchange;

  /** Buffered Exchange used by vertex sets */
  buffered_exchange<vertex_id_type> vset_exchange;

  /** Command option to disable parallel ingress. Used for simulating single
   * node ingress */
  bool parallel_ingress;

  /** Command option to enable data affinity. Currently only supported by
   * bipartite */
  bool data_affinity;

  lock_manager_type lock_manager;

  void set_ingress_method(const std::string& method, size_t bufsize = 50000,
                          bool usehash = false, bool userecent = false,
                          std::string favorite = "source",
                          size_t threshold = 100, size_t nedges = 0,
                          size_t nverts = 0,
                          size_t interval = std::numeric_limits<size_t>::max(),
                          std::string metis_lookup_file = "") {
    if (ingress_ptr != NULL) {
    ...
    if (method == "oblivious") {
      ...
      ingress_ptr = new distributed_oblivious_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, usehash, userecent);
    } else if (method == "random") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use random ingress" << std::endl;
      ingress_ptr =
          new distributed_random_ingress<VertexData, EdgeData>(rpc.dc(), *this);
    } else if (method == "random_ec") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use random-edgecut ingress" << std::endl;
      ingress_ptr = new distributed_random_ec_ingress<VertexData, EdgeData>(
          rpc.dc(), *this);
    } else if (method == "ldg") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Use LDG ingress, edge balanced multiple loaders" << std::endl;
      ingress_ptr = new distributed_ldg_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, nedges, nverts, true, false);
    } else if (method == "ldg_vertex") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Use LDG ingress, vertex balanced multiple loaders" << std::endl;
      ingress_ptr = new distributed_ldg_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, nedges, nverts, false, false);
    } else if (method == "fennel") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Use Fennel ingress, edge balanced multiple loaders"
            << std::endl;
      ingress_ptr = new distributed_fennel_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, nedges, nverts, true, false);
    } else if (method == "fennel_vertex") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Use Fennel ingress, vertex balanced multiple loaders"
            << std::endl;
      ingress_ptr = new distributed_fennel_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, nedges, nverts, false, false);
      ...
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Use METIS ingress with lookup: " << metis_lookup_file
            << std::endl;
      ingress_ptr = new distributed_metis_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, metis_lookup_file);
    } else if (method == "hdrf") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use HDRF ingress" << std::endl;
      ingress_ptr = new distributed_hdrf_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, usehash, userecent);
    } else if (method == "dbh") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use DBH ingress" << std::endl;
      ingress_ptr =
          new distributed_dbh_ingress<VertexData, EdgeData>(rpc.dc(), *this);
    } else if (method == "grid") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use grid ingress" << std::endl;
      ingress_ptr =
          new distributed_constrained_random_ingress<VertexData, EdgeData>(
              rpc.dc(), *this, "grid");
    } else if (method == "pds") {
      if (rpc.procid() == 0)
      ...
      if (data_affinity) {
        if (rpc.procid() == 0)
          logstream(LOG_EMPH)
              << "Use bipartite ingress w/ affinity" << std::endl;
        ingress_ptr =
            new distributed_bipartite_affinity_ingress<VertexData, EdgeData>(
                rpc.dc(), *this, favorite);
      } else {
        if (rpc.procid() == 0)
          logstream(LOG_EMPH)
              << "Use bipartite ingress w/o affinity" << std::endl;
        ingress_ptr =
            new distributed_bipartite_random_ingress<VertexData, EdgeData>(
                rpc.dc(), *this, favorite);
      }
    } else if (method == "bipartite_aweto") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use bipartite_aweto ingress" << std::endl;
      ingress_ptr =
          new distributed_bipartite_aweto_ingress<VertexData, EdgeData>(
              rpc.dc(), *this, favorite);
    } else if (method == "hybrid") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use hybrid ingress" << std::endl;
      ingress_ptr = new distributed_hybrid_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, threshold);
      set_cuts_type(HYBRID_CUTS);
    } else if (method == "hybrid_ginger") {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use hybrid ginger ingress" << std::endl;
      ASSERT_GT(nedges, 0);
      ASSERT_GT(nverts, 0);
      ingress_ptr = new distributed_hybrid_ginger_ingress<VertexData, EdgeData>(
          rpc.dc(), *this, threshold, nedges, nverts, interval);
      ...
      if (sharding_constraint::is_pds_compatible(num_shards, p)) {
        ...
        ingress_ptr =
            new distributed_constrained_random_ingress<VertexData, EdgeData>(
                rpc.dc(), *this, "pds");
      } else if (sharding_constraint::is_grid_compatible(num_shards, nrow,
                                                         ncol)) {
        ingress_auto = "grid";
        ingress_ptr =
            new distributed_constrained_random_ingress<VertexData, EdgeData>(
      ...
      if (rpc.procid() == 0)
        logstream(LOG_EMPH)
            << "Automatically determine ingress method: " << ingress_auto
            << std::endl;
    }
    // batch ingress is deprecated
    // if (method == "batch") {
    //   logstream(LOG_EMPH) << "Use batch ingress, bufsize: " << bufsize
    //     << ", usehash: " << usehash << ", userecent" << userecent <<
    //     std::endl;
    //   ingress_ptr = new distributed_batch_ingress<VertexData,
    //   EdgeData>(rpc.dc(), *this,
    //                                                    bufsize, usehash,
    //                                                    userecent);
    // } else
  ...
  bool load_from_stream(std::string filename, Fstream& fin,
    ...
    while (fin.good() && !fin.eof()) {
      ...
      if (!success) {
        logstream(LOG_WARNING) << "Error parsing line " << linecount << " in "
                               << filename << ": " << std::endl
                               << "\t\"" << line << "\"" << std::endl;
        return false;
      }
  ...
  void save_vertex_to_stream(vertex_type& vertex, Fstream& fout,
                             Writer writer) {
  ...
  void save_direct(const std::string& prefix, bool gzip,
    ...
    savetime.start();
    std::string fname =
        prefix + "_" + tostr(rpc.procid() + 1) + "_of_" + tostr(rpc.numprocs());
