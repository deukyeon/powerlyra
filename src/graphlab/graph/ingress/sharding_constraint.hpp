 ...
 * construction is based on a grid, and the other is based on perfect difference
 * set. Both algorithms guarentees that Ai \intersect Aj is non-empty.
 *
 * \note: grid methods requires the number of shards to be a perfect square
 * number. pds requires the number of shards to be p^2 + p + 1 where p is a
 * prime number.
 *
 */
namespace graphlab {
 ...
 public:
  sharding_constraint(size_t num_shards, std::string method) {
    ...
    if (method == "grid") {
      ...
      logstream(LOG_FATAL) << "Unknown sharding constraint method: " << method
                           << std::endl;
    }
  ...
  bool get_neighbors(procid_t shard, std::vector<procid_t>& neighbors) {
    ...
    for (size_t i = 0; i < ls.size(); ++i) neighbors.push_back(ls[i]);
    return true;
  }

  const std::vector<procid_t>& get_joint_neighbors(procid_t shardi,
                                                   procid_t shardj) {
 ...
 private:
  void make_grid_constraint() {
    ...
    if (!is_grid_compatible(nshards, nrows, ncols)) {
      logstream(LOG_FATAL) << "Num shards: " << nshards
                           << " cannot be used for grid ingress." << std::endl;
    };
  ...
  void make_pds_constraint() {
    ...
    if (!is_pds_compatible(nshards, p)) {
      logstream(LOG_FATAL) << "Num shards: " << nshards
                           << " cannot be used for pdsingress." << std::endl;
    };
    pds pds_generator;
  ...
  bool compute_neighbors(procid_t shardi, procid_t shardj,
                         std::vector<procid_t>& neighbors) {
