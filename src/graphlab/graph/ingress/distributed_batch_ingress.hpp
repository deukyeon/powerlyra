...
class distributed_batch_ingress
 ...
 public:
  distributed_batch_ingress(distributed_control& dc, graph_type& graph,
                            size_t bufsize = 50000, bool usehash = false,
    ...
    INITIALIZE_TRACER(batch_ingress_compute_assignments,
                      "Time spent in compute assignment");
    INITIALIZE_TRACER(batch_ingress_request_degree_table,
                      "Time spent in requesting assignment");
    INITIALIZE_TRACER(batch_ingress_get_degree_table,
                      "Time spent in retrieve degree table");
    INITIALIZE_TRACER(batch_ingress_update_degree_table,
 ...
 private:
  ...
  void add_edges(const std::vector<vertex_id_type>& source_arr,
    ...
    std::vector<std::vector<vertex_id_type> > local_degree_count(
        rpc.numprocs());
    ...
    for (size_t i = 0; i < source_arr.size(); ++i) {
      ...
      // lvid_type>::iterator vid2lvid_iter;
      ...
      max_lvid = std::max(std::max(lvid_source, lvid_target), max_lvid);

      local_degree_count[graph_hash::hash_vertex(source) % rpc.numprocs()]
          .push_back(source);
      local_degree_count[graph_hash::hash_vertex(target) % rpc.numprocs()]
          .push_back(target);
    }
    ...
    for (size_t i = 0; i < rpc.numprocs(); ++i) {
      if (i != rpc.procid()) {
        rpc.remote_call(i, &distributed_batch_ingress::block_add_degree_counts,
                        rpc.procid(), local_degree_count[i]);
      } else {
    ...
    base_type::graph.local_graph.add_edges(local_source_arr, local_target_arr,
                                           edata_arr);
  ...
  void block_add_degree_counts(procid_t pid,
                               std::vector<vertex_id_type>& whohas) {
  ...
  dht_degree_table_type block_get_degree_table(
  ...
  void assign_edges(std::vector<std::vector<vertex_id_type> >& proc_src,
    ...
    for (size_t i = 0; i < rpc.numprocs(); ++i) {
      if (i == rpc.procid()) {
        ...
        degree_table[i] = rpc.remote_request(
            i, &distributed_batch_ingress::block_get_degree_table,
            query_set[i]);
      }
      query_set[i].clear();
    }
    ...
    for (size_t i = 0; i < num_edges; ++i) {
      std::pair<vertex_id_type, vertex_id_type>& e = edgesend[i];
      ...
      procid_t proc = base_type::edge_decision.edge_to_proc_greedy(
          e.first, e.second, src_degree, dst_degree, proc_num_edges, usehash,
          userecent);
  ...
  void flush() {
    ...
    for (size_t i = 0; i < proc_src.size(); ++i) {
      if (proc_src[i].size() == 0) continue;
      if (i == rpc.procid()) {
        add_edges(proc_src[i], proc_dst[i], proc_edata[i]);
        num_edges -= proc_src[i].size();
      } else {
        rpc.remote_call(i, &distributed_batch_ingress::add_edges, proc_src[i],
                        proc_dst[i], proc_edata[i]);
        num_edges -= proc_src[i].size();
      }  // end if
    }    // end for
  }      // end flush
