  ...
  void add_vertex(vertex_id_type vid,
    ...
    for (size_t i = 0; i < nprocs; i++) {
      ...
      float partition_score =
          neighbour_count[i] * (1 - (current_partition_capacity /
                                     (double)vertex_capacity_constraint));
      ...
      for (size_t i = 0; i < nprocs; i++) {
        ...
        if (i != self_pid) {
          ...
          ldg_rpc.remote_request(
              i, &distributed_ldg_reverse_ingress::block_add_placement_pair,
 ...
 private:
  ...
  void block_add_placement_pair(
    ...
    foreach (placement_pair_type& placement, placement_buffer) {
      ...
      std::cout << "From " << pid << " to " << this->self_pid << " assignment"
                << placement.first << " : " << placement.second << std::endl;
    }
