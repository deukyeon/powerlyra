...
class distributed_hybrid_ginger_ingress
 ...
 public:
  ...
  typedef typename std::pair<procid_t, size_t> proc_score_pair_type;
 ...
 public:
  distributed_hybrid_ginger_ingress(
      distributed_control& dc, graph_type& graph, size_t threshold = 100,
...
#endif
        high_edge_exchange(dc),
        low_edge_exchange(dc),
        resend_vertex_exchange(dc),
        mht_exchange(dc),
        proc_balance(dc.numprocs()),
        proc_score_incr(dc.numprocs()),
        proc_score_exchange(dc),
        tot_nedges(tot_nedges),
        tot_nverts(tot_nverts),
        interval(interval) {
  ...
  void add_vertex(vertex_id_type vid, const VertexData& vdata) {
    ...
    const procid_t owning_proc =
        standalone ? 0 : graph_hash::hash_vertex(vid) % hybrid_rpc.numprocs();
#ifdef _OPENMP
  ...
  procid_t ginger_to_proc(const vertex_id_type target,
    ...
    for (size_t i = 0; i < nprocs; ++i) {
      proc_score[i] =
          proc_degrees[i] - alpha * gamma * pow(proc_balance[i], (gamma - 1));
    }
  ...
  void assign_hybrid_edges() {
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "collect raw map: " << ti.current_time() << " secs"
                          << std::endl;
    }
    logstream(LOG_INFO) << "receive " << raw_map.size() << " vertices done."
                        << std::endl;
#endif
    ...
    // logstream(LOG_INFO) << "] ";
    logstream(LOG_INFO) << "nsyncs(" << (vcount / interval) << ") using "
                        << ti.current_time() << " secs "
                        << "#mht=" << mht.size() << std::endl;
#endif
  }
  ...
  void finalize() {
    ...
    {
      size_t changed_size =
          hybrid_edge_exchange.size() + hybrid_vertex_exchange.size();
      hybrid_rpc.all_reduce(changed_size);
      if (changed_size == 0) {
        logstream(LOG_INFO)
            << "Skipping Graph Finalization because no changes happened..."
            << std::endl;
        return;
      }
    }
...
#ifdef TUNING
      if (l_procid == 0) {
        logstream(LOG_INFO) << "low-degree edges: " << ti.current_time()
                            << " secs" << std::endl;
      }
#endif
    ...
    if (l_procid == 0) {
      ...
      logstream(LOG_EMPH) << "prepare ginger finalizing. (" << ti.current_time()
                          << " secs)" << std::endl;
    }
    ...
    if (l_procid == 0) {
      logstream(LOG_EMPH) << "ginger finalizing graph. (" << ti.current_time()
                          << " secs)" << std::endl;
    }
  }  // end of finalize
  ...
  void modified_base_finalize(size_t nedges) {
    ...
    {  // Add all the edges to the local graph
...
#ifdef TUNING
      if (l_procid == 0) {
        logstream(LOG_INFO) << "populating local graph: " << ti.current_time()
                            << " secs" << std::endl;
      }
#endif
      ...
      if (l_procid == 0) {
        logstream(LOG_INFO) << "finalizing local graph: " << ti.current_time()
                            << " secs" << std::endl;
      }
#endif
    }
    ...
    /*                                                                        */
    /**************************************************************************/
    // Setup the map containing all the vertices being negotiated by this
    // machine
    {
      if (standalone) {
...
#ifdef TUNING
      if (l_procid == 0) {
        logstream(LOG_INFO) << "adding vertex data: " << ti.current_time()
                            << " secs" << std::endl;
      }
#endif
    ...
    /**************************************************************************/
    {
      ...
      const size_t local_nverts =
          graph.vid2lvid.size() + vid2lvid_buffer.size();
...
#ifdef TUNING
      if (l_procid == 0) {
        logstream(LOG_INFO) << "allocating lvid2record: " << ti.current_time()
                            << " secs" << std::endl;
      }
#endif
      mht.clear();
...
#pragma omp parallel
      ...
      if (!flying_vids.empty()) {
        logstream(LOG_INFO)
            << "#flying-own-nverts=" << flying_vids.size() << std::endl;
        ...
        for (typename boost::unordered_map<vertex_id_type,
                                           mirror_type>::iterator it =
                 flying_vids.begin();
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "master handshake: " << ti.current_time()
                          << " secs" << std::endl;
    }
#endif
    ...
    /**************************************************************************/
    {
      if (graph.vid2lvid.size() == 0) {
        ...
        foreach (const typename vid2lvid_map_type::value_type& pair,
                 vid2lvid_buffer) {
    ...
    // TODO:  optimization for standalone
    {
      ...
      graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
          vrecord_sync_gas(
              graph,
              boost::bind(&distributed_hybrid_ginger_ingress::finalize_gather,
                          this, _1, _2),
              boost::bind(&distributed_hybrid_ginger_ingress::finalize_apply,
                          this, _1, _2, _3));
      ...
      if (l_procid == 0) {
        logstream(LOG_INFO)
            << "synchrionizing vertex (meta)data: " << ti.current_time()
            << " secs" << std::endl;
      }
#endif
    }
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "exchange global info: " << ti.current_time()
                          << " secs" << std::endl;
    }
#endif
    ...
    if (l_procid == 0) {
      ...
      logstream(LOG_EMPH) << "base finalizing. (" << ti.current_time()
                          << " secs)" << std::endl;
    }
 ...
 private:
   ...
   */
  void finalize_apply(lvid_type lvid, const vertex_negotiator_record& accum,
                      graph_type& graph) {
