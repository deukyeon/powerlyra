...
class distributed_constrained_batch_ingress
 ...
 public:
  distributed_constrained_batch_ingress(distributed_control& dc,
                                        graph_type& graph,
  ...
  ~distributed_constrained_batch_ingress() { delete constraint; }
 ...
 private:
  ...
  void add_edges(const std::vector<vertex_id_type>& source_arr,
    ...
    for (size_t i = 0; i < source_arr.size(); ++i) {
      ...
      typedef typename cuckoo_map_pow2<vertex_id_type, lvid_type, 3,
                                       uint32_t>::iterator vid2lvid_iter;
      vid2lvid_iter iter;
    ...
    for (size_t i = 0; i < rpc.numprocs(); ++i) {
      if (i != rpc.procid()) {
        rpc.remote_call(
            i, &distributed_constrained_batch_ingress::block_add_degree_counts,
  ...
  void assign_edges(std::vector<std::vector<vertex_id_type> >& proc_src,
    ...
    for (size_t i = 0; i < rpc.numprocs(); ++i) {
      if (i == rpc.procid()) {
        ...
        degree_table[i] = rpc.remote_request(
            i, &distributed_constrained_batch_ingress::block_get_degree_table,
            query_set[i]);
      }
      query_set[i].clear();
    }
    ...
    for (size_t i = 0; i < num_edges; ++i) {
      ...
      const std::vector<procid_t>& candidates = constraint->get_joint_neighbors(
          get_master(e.first), get_master(e.second));

      procid_t proc = base_type::edge_decision.edge_to_proc_greedy(
          e.first, e.second, src_degree, dst_degree, candidates, proc_num_edges,
          usehash, userecent);
