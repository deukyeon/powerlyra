...
class ingress_edge_decision {
 ...
 public:
  ...
  procid_t edge_to_proc_random(const vertex_id_type source,
                               const vertex_id_type target, size_t numprocs) {
   ...
   *  bitset<MAX_MACHINE> src_degree : the degree presence of source over
   * machines bitset<MAX_MACHINE> dst_degree : the degree presence of target
   * over machines vector<size_t>      proc_num_edges : the edge counts over
   * machines
   * */
  procid_t edge_to_proc_greedy(const vertex_id_type source,
    ...
    size_t minedges =
        *std::min_element(proc_num_edges.begin(), proc_num_edges.end());
    size_t maxedges =
        *std::max_element(proc_num_edges.begin(), proc_num_edges.end());

    for (size_t i = 0; i < numprocs; ++i) {
      size_t sd = src_degree.get(i) + (usehash && (source % numprocs == i));
      size_t td = dst_degree.get(i) + (usehash && (target % numprocs == i));
      double bal =
          (maxedges - proc_num_edges[i]) / (epsilon + maxedges - minedges);
    ...
    for (size_t i = 0; i < numprocs; ++i)
      if (std::fabs(proc_score[i] - maxscore) < 1e-5) top_procs.push_back(i);
   ...
   *  bitset<MAX_MACHINE> src_degree : the degree presence of source over
   * machines bitset<MAX_MACHINE> dst_degree : the degree presence of target
   * over machines size_t              src_true_degree : the degree of source
   * vertex over machines size_t              dst_true_degree : the degree of
   * target vertex over machines vector<size_t>      proc_num_edges : the edge
   * counts over machines
   *
  ...
  procid_t edge_to_proc_hdrf(const vertex_id_type source,
                             ...
                             bin_counts_type& src_degree,
                             bin_counts_type& dst_degree,
                             size_t& src_true_degree, size_t& dst_true_degree,
                             std::vector<size_t>& proc_num_edges,
                             bool usehash = false, bool userecent = false) {
    size_t numprocs = proc_num_edges.size();
