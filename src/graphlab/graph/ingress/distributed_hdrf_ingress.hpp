...
class distributed_hdrf_ingress
 ...
 public:
   ...
   * a map from vertex id to a bitset of length num_procs.
   */
  typedef cuckoo_map_pow2<vertex_id_type, size_t, 3, uint32_t>
      true_degree_hash_table_type;
 ...
 public:
  distributed_hdrf_ingress(distributed_control& dc, graph_type& graph,
      ...
      : base_type(dc, graph),
        ...
        usehash(usehash),
        userecent(userecent) {
    // INITIALIZE_TRACER(ob_ingress_compute_assignments, "Time spent in compute
    // assignment");
  }
  ...
  void add_edge(vertex_id_type source, vertex_id_type target,
    ...
    const procid_t owning_proc = base_type::edge_decision.edge_to_proc_hdrf(
        source, target, dht[source], dht[target], degree_dht[source],
        degree_dht[target], proc_num_edges, usehash, userecent);
  ...
  virtual void finalize() {
    ...
    size_t count = 0;
    for (std::vector<size_t>::iterator it = proc_num_edges.begin();
         it != proc_num_edges.end(); ++it) {
      count = count + *it;
    }
