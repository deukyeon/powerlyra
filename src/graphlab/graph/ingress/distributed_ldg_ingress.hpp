...
class distributed_ldg_ingress
 ...
 public:
  ...
  typedef typename std::pair<vertex_id_type, procid_t> placement_pair_type;
 ...
 public:
  distributed_ldg_ingress(distributed_control& dc, graph_type& graph,
                          ...
                          bool edge_balanced = true, bool single_loader = false)
      : base_type(dc, graph),
        ldg_rpc(dc, this),
        nprocs(dc.numprocs()),
        tot_nedges(tot_nedges),
        tot_nverts(tot_nverts),
        partition_capacity(dc.numprocs(), 0),
    ...
    logstream(LOG_INFO) << "LDG Ingress edge balanced: " << edge_balanced
                        << " single loader (no sync): " << single_loader
                        << std::endl;
  }  // end of constructor
  ...
  void add_vertex(vertex_id_type vid,
    ...
    // std::cout << "### Process ID" << self_pid << "    Vertex Id" << vid <<
    // std::endl;

    // query partition id of each neighbour and count neighbours in each
    // partition
    for (size_t i = 0; i < adjacency_list.size(); i++) {
      procid_t neighbour_owner = get_vertex_partition(adjacency_list[i]);
      // std::cout << "Neighbourhood : " << adjacency_list[i] << "   partition:"
      // << neighbour_owner << std::endl;
    ...
    for (size_t i = 0; i < nprocs; i++) {
      ...
      float partition_score =
          neighbour_count[i] *
          (1 - (current_partition_capacity / (double)capacity_constraint));
      ...
      // std::cout << "Partition:" << i << "     Score:" << partition_score <<
      // std::endl;
    }
    ...
    const procid_t owning_proc =
        candidate_partitions[graph_hash::hash_vertex(vid) %
                             candidate_partitions.size()];
    ...
    if (!placement_buffer.empty()) {
      for (size_t i = 0; i < nprocs; i++) {
        ...
        if (i != self_pid) {
          ...
          ldg_rpc.remote_request(
              i, &distributed_ldg_ingress::block_add_placement_pair, self_pid,
              placement_buffer);
        }
      }
 ...
 protected:
  virtual void determine_master(vid2lvid_map_type& vid2lvid_buffer) {
    ...
    const size_t local_nverts =
        base_type::graph.vid2lvid.size() + vid2lvid_buffer.size();
    ...
    ASSERT_EQ(base_type::graph.lvid2record.size(),
 ...
 private:
  /**
   * Acquires read lock on the distributed table and returns partition procid
   * for given vertex
   * @param vid
  ...
  /**
   * Acquires write lock on the table and populated the partition entry for
   * given vertex
   * @param vid
   * @param procid
   */
  void set_vertex_partition(vertex_id_type vid,
                            std::vector<vertex_id_type>& adjacency_list,
                            procid_t procid) {
    ...
    if (placement_buffer.size() > PLACEMENT_BUFFER_THRESHOLD) {
      for (size_t i = 0; i < nprocs; i++) {
        ...
        if (i != self_pid) {
          // need remote call to populate dht
          // use unblocking calls for single loader case, so that loader process
          // can continue
          ldg_rpc.remote_request(
              i, &distributed_ldg_ingress::block_add_placement_pair, self_pid,
              placement_buffer);
  ...
  void block_add_placement_pair(
      procid_t pid, std::vector<placement_pair_type>& placement_buffer) {
    dht_placement_table_lock.writelock();
    ...
    foreach (placement_pair_type& placement, placement_buffer) {
      ...
      // std::cout << "From " << pid << " to " << this->self_pid << "
      // assignment" << placement.first << " : " << placement.second <<
      // std::endl;
    }
