...
class distributed_metis_ingress
 ...
 public:
  distributed_metis_ingress(distributed_control& dc, graph_type& graph,
                            std::string metis_lookup_file)
      : base_type(dc, graph),
        metis_rpc(dc, this),
        metis_lookup_file(metis_lookup_file) {
    ...
    logstream(LOG_INFO) << "Lookup table populated using: " << metis_lookup_file
                        << std::endl;
  ...
  void add_vertex(vertex_id_type vid,
    ...
    if (lookup_table.find(vid) == lookup_table.end()) {
      ...
      logstream(LOG_WARNING)
          << "Lookup entry cannot be found for vertex: " << vid << std::endl;
    } else {
