...
class distributed_fennel_ingress
 ...
 public:
  distributed_fennel_ingress(distributed_control& dc, graph_type& graph,
    ...
    logstream(LOG_INFO) << "Fennel Ingress edge balanced: " << edge_balanced
  ...
  void add_vertex(vertex_id_type vid,
    ...
    for (size_t i = 0; i < nprocs; i++) {
      ...
      partition_score =
          neighbour_count[i] -
          (alpha * gamma * pow(current_partition_capacity, (gamma - 1)));
    ...
    if (!placement_buffer.empty()) {
      for (size_t i = 0; i < nprocs; i++) {
        ...
        if (i != self_pid) {
          ...
          fennel_rpc.remote_request(
              i, &distributed_fennel_ingress::block_add_placement_pair,
              self_pid, placement_buffer);
        }
      }
 ...
 private:
  ...
  procid_t get_vertex_partition(vertex_id_type vid) {
    procid_t partition;
    // TODO: removed the read_lock as knowing exact partition does not have
