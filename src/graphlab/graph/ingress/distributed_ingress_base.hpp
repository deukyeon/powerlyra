...
class distributed_ingress_base {
 public:
  ...
  struct vertex_negotiator_record {
    ...
    vertex_negotiator_record()
        : num_in_edges(0), num_out_edges(0), has_data(false) {}
 ...
 public:
   ...
   *
   * 2. Construct lvid2record map (of empty entries) using the received
   * vertices.
   *
  ...
  virtual void finalize() {
    ...
    if (rpc.procid() == 0) memory_info::log_usage("Post Flush");
    ...
    {  // Add all the edges to the local graph
      logstream(LOG_INFO) << "Graph Finalize: constructing local graph"
                          << std::endl;
      ...
      while (edge_exchange.recv(proc, edge_buffer)) {
        foreach (const edge_buffer_record& rec, edge_buffer) {
          ...
          // std::cout << "add edge " << rec.source << "\t" << rec.target <<
          // std::endl;
      ...
      logstream(LOG_INFO) << rpc.procid() << " Local graph info: " << std::endl
...
#pragma omp parallel
      ...
      for (typename boost::unordered_map<vertex_id_type, mirror_type>::iterator
               it = flying_vids.begin();
        ...
        // std::cout << "proc " << rpc.procid() << " recevies flying vertex " <<
        // gvid << std::endl;
      }
    ...
    /**************************************************************************/
    {
      ...
      graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
          vrecord_sync_gas(
              graph,
              boost::bind(&distributed_ingress_base::finalize_gather, this, _1,
                          _2),
              boost::bind(&distributed_ingress_base::finalize_apply, this, _1,
                          _2, _3));
  ...
  void exchange_global_info(bool standalone) {
    ...
    logstream(LOG_INFO) << "Graph Finalize: exchange global statistics "
                        << std::endl;
    ...
    if (standalone) {
      ...
      graph.nreplicas = 0;
      foreach (size_t count, swap_counts)
        graph.nreplicas += count;
      if (rpc.procid() == 0) {
        size_t max = *std::max_element(swap_counts.begin(), swap_counts.end());
        logstream(LOG_EMPH)
            << "local vertices balance: "
  ...
  void print_network_statistics() {
    logstream(LOG_INFO) << "Graph Finalize: network statistics" << std::endl;
    logstream(LOG_INFO) << rpc.procid()
                        << " Bytes Sent(Ingress): " << rpc.dc().bytes_sent()
                        << std::endl;
    logstream(LOG_INFO) << rpc.procid()
                        << " Calls Sent(Ingress): " << rpc.dc().calls_sent()
                        << std::endl;
    logstream(LOG_INFO) << rpc.procid() << " Network Sent(Ingress): "
                        << rpc.dc().network_bytes_sent() << std::endl;
    logstream(LOG_INFO) << rpc.procid() << " Bytes Received(Ingress): "
                        << rpc.dc().bytes_received() << std::endl;
    logstream(LOG_INFO) << rpc.procid() << " Calls Received(Ingress): "
                        << rpc.dc().calls_received() << std::endl;

    std::vector<size_t> bytessents = rpc.dc().bytes_sent_vector();
    ...
    oss.str();
    logstream(LOG_INFO) << rpc.procid()
                        << " Bytes Sent Vector(Ingress): " << oss.str()
                        << std::endl;
  }
