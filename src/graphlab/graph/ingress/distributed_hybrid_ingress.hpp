...
class distributed_hybrid_ingress
 ...
 public:
  distributed_hybrid_ingress(distributed_control& dc, graph_type& graph,
  ...
  void finalize() {
    ...
    /**************************************************************************/
    {
      ...
      if (standalone) { /* fast pass for standalone */
        ...
        if (l_procid == 0) {
          logstream(LOG_INFO)
              << "save local edges and count in-degree: " << ti.current_time()
              << " secs" << std::endl;
        }
#endif
        ...
        if (l_procid == 0) {
          logstream(LOG_INFO)
              << "resend edges of high-degree vertices: " << ti.current_time()
              << " secs" << std::endl;
        }
#endif
...
#ifdef TUNING
        logstream(LOG_INFO)
            << "receive high-degree edges: " << hybrid_edge_exchange.size()
            << std::endl;
#endif
        proc = -1;
        while (hybrid_edge_exchange.recv(proc, edge_buffer)) {
        ...
        if (l_procid == 0) {
          logstream(LOG_INFO)
              << "receive high-degree edges: " << ti.current_time() << " secs"
              << std::endl;
        }
#endif
      }
    ...
    if (l_procid == 0) {
      ...
      logstream(LOG_EMPH) << "prepare hybrid finalizing. (" << ti.current_time()
                          << " secs)" << std::endl;
    }
    ...
    if (l_procid == 0) {
      ...
      logstream(LOG_EMPH) << "hybrid finalizing graph. (" << ti.current_time()
                          << " secs)" << std::endl;
    }
  }  // end of finalize
...
#ifdef TUNING
        logstream(LOG_INFO)
            << "base::#vert-msgs=" << hybrid_vertex_exchange.size()
            << std::endl;
        if (l_procid == 0) {
...
#pragma omp parallel
    ...
    // TODO:  optimization for standalone
    {
      ...
      graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
          vrecord_sync_gas(
              graph,
              boost::bind(&distributed_hybrid_ingress::finalize_gather, this,
                          _1, _2),
              boost::bind(&distributed_hybrid_ingress::finalize_apply, this, _1,
                          _2, _3));
