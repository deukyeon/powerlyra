...
class distributed_bipartite_aweto_ingress
 ...
 public:
  distributed_bipartite_aweto_ingress(distributed_control& dc,
                                      graph_type& graph,
                                      const std::string& favorite)
      : base_type(dc, graph),
        bipartite_rpc(dc, this),
        graph(graph),
#ifdef _OPENMP
        bipartite_vertex_exchange(dc, omp_get_max_threads()),
        bipartite_edge_exchange(dc, omp_get_max_threads())
#else
        bipartite_vertex_exchange(dc),
        bipartite_edge_exchange(dc)
#endif
  {
  ...
  /** accumulate edges temporal rally point using random of "favorite"
   * assignment. */
  ...
  void finalize() {
    ...
    {
      size_t changed_size =
          bipartite_edge_exchange.size() + bipartite_vertex_exchange.size();
    ...
    for (typename boost::unordered_map<vertex_id_type,
      ...
      // heuristic score
      double best_score =
          (it->second)[best_proc] - sqrt(1.0 * proc_num_edges[best_proc]);

      for (size_t i = 0; i < bipartite_rpc.numprocs(); i++) {
        double score = (it->second)[i] - sqrt(1.0 * proc_num_edges[i]);
        if (score > best_score) {
          best_proc = i;
          best_score = score;
        }
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "hold " << own_vid_set.size()
                          << " masters: " << ti.current_time() << " secs"
                          << std::endl;
    }
#endif
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "exchange edges: " << ti.current_time() << " secs"
                          << std::endl;
    }
#endif
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "exchange vertex data: " << ti.current_time()
                          << " secs" << std::endl;
    }
#endif
