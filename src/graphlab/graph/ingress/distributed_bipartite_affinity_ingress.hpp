...
class distributed_bipartite_affinity_ingress
 ...
 public:
  distributed_bipartite_affinity_ingress(distributed_control& dc,
                                         graph_type& graph,
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "add " << local_nverts
                          << " vertex: " << ti.current_time() << " secs"
                          << std::endl;
    }
#endif
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "exchange mapping: " << ti.current_time()
                          << " secs" << std::endl;
    }
#endif
...
#ifdef TUNING
    if (l_procid == 0) {
      logstream(LOG_INFO) << "distribute edges: " << ti.current_time()
                          << " secs" << std::endl;
    }
#endif
