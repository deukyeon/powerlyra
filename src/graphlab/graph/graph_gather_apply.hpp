...
class graph_gather_apply {
 public:
   ...
   */
  typedef typename boost::function<gather_type(lvid_type, graph_type&)>
      gather_fun_type;

  /**
   * \brief The gather operation which will be called on each vertex (master and
   * mirrors) and send to the master vertex.
   */
   ...
   */
  typedef typename boost::function<void(lvid_type, const gather_type& accum,
                                        graph_type&)>
      apply_fun_type;

  /**
   * \brief The apply operation which will be called on each vertex (master and
   * mirrors) with the result of gather.
   */
 ...
 private:
  ...
  /**
   * \brief  The vertex locks protect access to vertex specific data-structrues
   * including \ref graphlab::graph_gather_apply::gather_accum.
   */
   ...
   *
   * While dense bitsets are thread safe the value of this bit must change
   * concurrently with the \ref graphlab::graph_gather_apply and therefore is
   * set while holding the lock in \ref graphlab::graph_gather_apply::vlocks
   */
  dense_bitset has_gather_accum;

  /**
   * \brief Gather accumulator used for each master vertex to merge the result
   * of all the machine specific accumulators.
   *
   * The gather accumulator can be accessed by multiple threads at once and
   * therefore must be guarded by a vertex locks in \ref
   * graphlab::graph_gather_apply::vlocks
   */
 ...
 public:
   ...
   */
  graph_gather_apply(graph_type& graph, gather_fun_type gather_fun,
                     apply_fun_type apply_fun,
 ...
 private:
   ...
   *
   * \code
   * void graph_gather_apply::member_fun(size_t threadid, const vertex_set&
   * vset); \endcode
   *
  ...
  void run_synchronous(MemberFunction member_fun, const vertex_set& vset) {
    ...
    if (threads.size() <= 1) {
      ...
      for (size_t i = 0; i < threads.size(); ++i) {
        boost::function<void(void)> invoke =
            boost::bind(member_fun, this, i, vset);
  ...
  /**
   * \brief Execute the \ref graphlab::graph_gather_apply::gather_fun function
   * on all vertices in the vset. The result of the gather will be send to the
   * master proc of the vertex.
   *
   * The accumulators are stored in \ref
   * graphlab::graph_gather_apply::gather_accum if the the vertex is
   * pre-allocated in the local graph of the proc. Otherwise, it wil be stored
   * in \ref graphlab::graph_gather_apply::temporary_gather_map and be merged in
   * the graph.
   *
   * @param thread_id the thread to run this as which determines
   * @param vset the vertex set specifying the set of vertex to run the gather
   * operation.
   */
  ...
  /**
   * \brief Execute the \ref graphlab::graph_gahter_apply::apply_fun function on
   * all all vertices (masters and mirrors) with the synrhonized gather
   * accumulators.
   *
   * @param thread_id the thread to run this as which determines
   * @param vset the vertex set specifying the set of vertex to run the gather
   ...
   */
  void scatter_gather(lvid_type lvid, const gather_type& accum,
                      size_t thread_id);
...
graph_gather_apply<Graph, GatherType>::graph_gather_apply(
    graph_type& graph, gather_fun_type gather_fun, apply_fun_type apply_fun,
    const graphlab_options& opts)
    : gather_fun(gather_fun),
      apply_fun(apply_fun),
      rmi(graph.dc(), this),
      graph(graph),
...
void graph_gather_apply<Graph, GatherType>::exec(const vertex_set& vset) {
  if (vset.lazy && !vset.is_complete_set) return;
...
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::execute_gathers(
    const size_t thread_id, const vertex_set& vset) {
  ...
  while (1) {
    ...
    foreach (size_t lvid_block_offset, local_bitset) {
      ...
      // std::cout << "proc " << rmi.procid() << " gather on lvid " << lvid <<
      // std::endl;
...
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::execute_scatters(
    const size_t thread_id, const vertex_set& vset) {
  const bool TRY_TO_RECV = true;
  const size_t TRY_RECV_MOD = 1000;
  size_t vcount = 0;
...
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::execute_applys(
    const size_t thread_id, const vertex_set& vset) {
  fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;
  while (1) {
    // increment by a word at a time
    ...
    foreach (size_t lvid_block_offset, local_bitset) {
      ...
      if (graph.l_is_master(lvid)) continue;
...
// Data Synchronization ===================================================
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::sync_gather(
...
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::scatter_gather(
    lvid_type lvid, const gather_type& accum, const size_t thread_id) {
...
template <typename Graph, typename GatherType>
void graph_gather_apply<Graph, GatherType>::recv_gathers(
...
/**
 * \brief Temporary map storing the gather accumulators that is not preallocated
 * in the \ref graphlab::graph_gather_apply::gather_accum. The key of the map is
 * the global vertex id.
 *
 *  This map can be accessed by multiple threads at once and therefore must be
 * guarded by a lock \ref graphlab::graph_gather_apply::tmp_gather_map_lock.
 */
// boost::unordered_map<vertex_id_type, gather_type> tmp_gather_map;

/**
 * \brief Lock that protects access to the temporaroy gather accumulator map
 * \ref graphlab::graph_gather_apply::tmp_gather_map.
 */
// mutex tmp_gather_map_lock;

// /**
//  * \brief Merge the gather accumulators in \ref
//  graphlab::graph_gather_apply::tmp_gather_accum
//  * in to \ref graphlab::graph_gather_apply::gahter_accum. Resize the local
...
//       typename boost::unordered_map<vertex_id_type,
//       gather_type>::const_iterator
...
// // Merge in the temporaroy gather
// map-------------------------------------------
// // with the side effect of resizing the graph and associate vertex
// datastructures. merge_temporary_gather_map();

// if the vid does not exist on the proc, put in a temporary map and merge in
// later if (graph.vid2lvid.find(pair.first) == graph.vid2lvid.end()) {
