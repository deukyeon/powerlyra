...
template <typename Graph>
bool csv_parser(Graph& graph, const std::string& filename,
...
bool adj_parser(Graph& graph, const std::string& srcfilename,
  ...
  const bool success = qi::phrase_parse(
       ...
       *(qi::ulong_[phoenix::push_back(phoenix::ref(targets), qi::_1)] %
         -qi::char_(","))),
      //  End grammar
      ascii::space);
...
bool adj_ec_parser(Graph& graph, const std::string& srcfilename,
  ...
  if (ntargets == 0 || targets.size() == 0) {
    logstream(LOG_ERROR) << "Parse error in vertex, empty adjacency list"
                         << std::endl;
    return true;
  }
  if (!success || ntargets != targets.size()) {
