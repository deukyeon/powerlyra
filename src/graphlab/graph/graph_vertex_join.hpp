 ...
 *   // ... serializers omitted ...
 * }
 * typedef distributed_graph<right_vertex_data, some_edge_data>
right_graph_type;
 * \endcode
...
class graph_vertex_join {
 ...
 public:
  graph_vertex_join(distributed_control& dc, left_graph_type& left,
  ...
  void prepare_injective_join(LeftEmitKey left_emit_key,
    ...
    reset_and_fill_injective_index(left_inj_index, left_graph, left_emit_key,
                                   "left graph");

    reset_and_fill_injective_index(right_inj_index, right_graph, right_emit_key,
                                   "right graph");
    rmi.barrier();
  ...
  void left_injective_join(JoinOp join_op) {
    injective_join(left_inj_index, left_graph, right_inj_index, right_graph,
                   join_op);
  }
  ...
  void right_injective_join(JoinOp join_op) {
    injective_join(right_inj_index, right_graph, left_inj_index, left_graph,
                   join_op);
  }

 private:
  template <typename Graph, typename EmitKey>
  void reset_and_fill_injective_index(injective_join_index& idx, Graph& graph,
                                      EmitKey& emit_key, const char* message) {
    // clear the data
  ...
  void compute_injective_join() {
    ...
    for (size_t p = 0; p < left_keys.size(); ++p) {
      for (size_t i = 0; i < left_keys[p].size(); ++i) {
        ASSERT_MSG(left_key_to_procs.count(left_keys[p][i]) == 0,
                   "Duplicate keys not permitted for left graph keys in "
                   "injective join");
    ...
    std::vector<std::vector<std::pair<size_t, procid_t> > > left_match(
        rmi.numprocs());
    std::vector<std::vector<std::pair<size_t, procid_t> > > right_match(
        rmi.numprocs());
      ...
      for (size_t i = 0; i < right_keys[p].size(); ++i) {
        ...
        if (iter != left_key_to_procs.end()) {
          ASSERT_MSG(iter->second != (procid_t)(-1),
                     "Duplicate keys not permitted for right graph keys in "
                     "injective join");
          // we have a match
...
#pragma omp parallel for
    ...
    for (size_t p = 0; p < left_match.size(); ++p) {
      for (size_t i = 0; i < left_match[p].size(); ++i) {
        ...
        // fill in the match
        left_inj_index.opposing_join_proc[iter->second] =
            left_match[p][i].second;
      }
    }
    left_match.clear();
...
#pragma omp parallel for
    ...
    for (size_t p = 0; p < right_match.size(); ++p) {
      for (size_t i = 0; i < right_match[p].size(); ++i) {
        ...
        // fill in the match
        right_inj_index.opposing_join_proc[iter->second] =
            right_match[p][i].second;
      }
    }
    right_match.clear();
    // ok done.
  ...
  std::vector<std::vector<size_t> > get_procs_with_keys(
  ...
  void injective_join(injective_join_index& target, TargetGraph& target_graph,
                      injective_join_index& source, SourceGraph& source_graph,
                      JoinOp joinop) {
    ...
    for (size_t i = 0; i < source.opposing_join_proc.size(); ++i) {
      if (source_graph.l_vertex(i).owned()) {
        ...
        if (target_proc >= 0 && target_proc < rmi.numprocs()) {
          source_data[target_proc].push_back(std::make_pair(
              source.vtx_to_key[i], source_graph.l_vertex(i).data()));
        }
      }
    }
    // exchange
...
#pragma omp parallel for
    ...
    for (size_t p = 0; p < source_data.size(); ++p) {
      for (size_t i = 0; i < source_data[p].size(); ++i) {
        ...
        // found it!
        typename TargetGraph::local_vertex_type lvtx =
            target_graph.l_vertex(iter->second);
