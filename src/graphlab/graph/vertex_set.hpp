 ...
 * \ref graphlab::iengine::signal_all(const vertex_set& vset, const
 * message_type& message, const std::string& order) "signal_all()".
 *
 * \ref distributed_graph::complete_set() and \ref
 * distributed_graph::empty_set() provide two convenient functions to obtain a
 * full or an empty set of vertices. \code vertex_set all =
 * graph.complete_set(); vertex_set empty = graph.empty_set(); \endcode
 *
...
class vertex_set {
 public:
   ...
   */
  template <typename DGraphType>
  void synchronize_master_to_mirrors(
      DGraphType& dgraph, buffered_exchange<vertex_id_type>& exchange) {
    if (lazy) {
    ...
    foreach (size_t lvid, localvset) {
      ...
      if (lvtx.owned()) {
        ...
        foreach (size_t proc, lvtx.mirrors()) { exchange.send(proc, gvid); }
      } else {
   ...
   */
  template <typename DGraphType>
  void synchronize_mirrors_to_master_or(
      DGraphType& dgraph, buffered_exchange<vertex_id_type>& exchange) {
    if (lazy) {
      make_explicit(dgraph);
      return;
 ...
 public:
  ...
  inline vertex_set& operator|=(const vertex_set& other) {
    if (lazy) {
      if (is_complete_set)
        (*this) = vertex_set(true);
      else
        (*this) = other;
    } else if (other.lazy) {
      if (other.is_complete_set)
        (*this) = vertex_set(true);
      else /* no op */
        ;
    } else {
