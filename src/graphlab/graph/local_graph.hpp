...
class local_graph {
 ...
 public:
  ...
  class vertex_type {
   public:
    vertex_type(local_graph& lgraph_ref, lvid_type vid)
        : lgraph_ref(lgraph_ref), vid(vid) {}

    /// \brief Returns a constant reference to the data on the vertex.
    const vertex_data_type& data() const { return lgraph_ref.vertex_data(vid); }
    /// \brief Returns a reference to the data on the vertex.
    vertex_data_type& data() { return lgraph_ref.vertex_data(vid); }
    /// \brief Returns the number of in edges of the vertex.
    size_t num_in_edges() const { return lgraph_ref.num_in_edges(vid); }
    /// \brief Returns the number of out edges of the vertex.
    size_t num_out_edges() const { return lgraph_ref.num_out_edges(vid); }
    /// \brief Returns the ID of the vertex.
    lvid_type id() const { return vid; }
    /// \brief Returns a list of in edges.
    edge_list_type in_edges() { return lgraph_ref.in_edges(vid); }
    /// \brief Returns a list of out edges.
    edge_list_type out_edges() { return lgraph_ref.out_edges(vid); }
  ...
  class edge_type {
   public:
    edge_type(local_graph& lgraph_ref, lvid_type _source, lvid_type _target,
              edge_id_type _eid)
        : lgraph_ref(lgraph_ref),
          _source(_source),
          _target(_target),
          _eid(_eid) {}

    /// \brief Returns a constant reference to the data on the edge.
    const edge_data_type& data() const { return lgraph_ref.edge_data(_eid); }
    /// \brief Returns a reference to the data on the edge.
    edge_data_type& data() { return lgraph_ref.edge_data(_eid); }
    /// \brief Returns the source vertex of the edge.
    vertex_type source() const { return vertex_type(lgraph_ref, _source); }
    /// \brief Returns the target vertex of the edge.
    vertex_type target() const { return vertex_type(lgraph_ref, _target); }
 ...
 public:
  ...
  local_graph(size_t nverts) : vertices(nverts), finalized(false) {}
  ...
  void finalize() {
...
#ifdef DEBUG_GRAPH
    logstream(LOG_DEBUG) << "Graph2 finalize: Sort by source vertex"
                         << std::endl;
#endif
...
#ifdef DEBUG_GRAPH
    logstream(LOG_DEBUG) << "Graph2 finalize: Inplace permute by source id"
                         << std::endl;
#endif
    lvid_type swap_src;
...
#ifdef DEBUG_GRAPH
    logstream(LOG_DEBUG) << "Graph2 finalize: Outofplace permute by dest id"
                         << std::endl;
#endif
    ...
    std::vector<std::pair<lvid_type, edge_id_type> > csc_value =
        vector_zip(edge_buffer.source_arr, permute);
  ...
  size_t num_edges() const { return edges.size(); }  // end of num edges
  ...
  void reserve_edge_space(size_t n) { edge_buffer.reserve_edge_space(n); }
  /**
  ...
  edge_id_type add_edge(lvid_type source, lvid_type target,
    ...
    if (finalized) {
      logstream(LOG_FATAL) << "Attempting add edge to a finalized local_graph."
                           << std::endl;
      ASSERT_MSG(false, "Add edge to a finalized local_graph.");
    }

    if (source == target) {
      logstream(LOG_FATAL) << "Attempting to add self edge (" << source
                           << " -> " << target << ").  "
                           << "This operation is not permitted in GraphLab!"
                           << std::endl;
  ...
  void add_edges(const std::vector<lvid_type>& src_arr,
    ...
    if (finalized) {
      logstream(LOG_FATAL) << "Attempting add edges to a finalized local_graph."
                           << std::endl;
    }
    ...
    for (size_t i = 0; i < src_arr.size(); ++i) {
      ...
      if (source >= vertices.size() || target >= vertices.size()) {
        logstream(LOG_FATAL) << "Attempting add_edge (" << source << " -> "
                             << target << ") when there are only "
  ...
  void load(iarchive& arc) {
    clear();
    // read the vertices
    arc >> vertices >> edges >> _csr_storage >> _csc_storage >> finalized;
  }  // end of load
  ...
  edge_list_type out_edges(lvid_type v) {
    ...
    edge_iterator begin = edge_iterator(
        *this, csr_edge_iterator(csr_iterator_tuple(base_begin, counter_begin)),
        v);

    edge_iterator end = edge_iterator(
        *this, csr_edge_iterator(csr_iterator_tuple(base_end, counter_end)), v);
  ...
  size_t estimate_sizeof() const {
    ...
    size_t elist_size = _csr_storage.estimate_sizeof() +
                        _csc_storage.estimate_sizeof() + sizeof(edges) +
                        sizeof(EdgeData) * edges.capacity();
    size_t ebuffer_size = edge_buffer.estimate_sizeof();
    // std::cerr << "local_graph: tmplist size: " <<
    // (double)elist_size/(1024*1024)
 ...
 private:
  ...
  typedef csr_storage<std::pair<lvid_type, edge_id_type>, edge_id_type>
      csc_type;
  ...
  class edge_iterator
      : public boost::iterator_facade<edge_iterator, edge_type,
                                      boost::random_access_traversal_tag,
                                      edge_type> {
   public:
    edge_iterator(local_graph& lgraph_ref, csc_edge_iterator iter,
                  lvid_type sourceid)
        : lgraph_ref(lgraph_ref), _type(CSC), csc_iter(iter), vid(sourceid) {}
    edge_iterator(local_graph& lgraph_ref, csr_edge_iterator iter,
                  lvid_type destid)
   ...
   private:
    ...
    edge_type dereference() const { return make_value(); }
    void decrement() {
      switch (_type) {
        case CSC:
          --csc_iter;
   ...
   private:
    edge_type make_value() const {
      switch (_type) {
        case CSC: {
          typename csc_edge_iterator::reference val = *csc_iter;
          return edge_type(lgraph_ref, val.first, vid, val.second);
        }
        case CSR: {
          typename csr_edge_iterator::reference val = *csr_iter;
          return edge_type(lgraph_ref, vid, val.template get<0>(),
                           val.template get<1>());
        }
        default:
