...
class dynamic_local_graph {
 ...
 public:
  ...
  dynamic_local_graph(size_t nverts) : vertices(nverts) {}
  ...
  void finalize() {
    ...
    logstream(LOG_DEBUG) << "Graph2 finalize: Sort by source vertex"
                         << std::endl;
#endif
    counting_sort(edge_buffer.source_arr, dest_permute,
                  &src_counting_prefix_sum);
#ifdef DEBUG_GRAPH
    logstream(LOG_DEBUG) << "Graph2 finalize: Sort by dest id" << std::endl;
#endif
    counting_sort(edge_buffer.target_arr, src_permute,
                  &dest_counting_prefix_sum);
    ...
    for (size_t i = 0; i < dest_permute.size(); ++i) {
      csr_values.push_back(std::pair<lvid_type, edge_id_type>(
          edge_buffer.target_arr[dest_permute[i]], begineid + dest_permute[i]));
    }
    ...
    for (size_t i = 0; i < src_permute.size(); ++i) {
      csc_values.push_back(std::pair<lvid_type, edge_id_type>(
          edge_buffer.source_arr[src_permute[i]], begineid + src_permute[i]));
    }
    ...
    if (edges.size() == 0) {
      ...
      edges.insert(edges.end(), edge_buffer.data.begin(),
                   edge_buffer.data.end());
      ...
      for (size_t i = 0; i < src_counting_prefix_sum.size(); ++i) {
        ...
        if (end > begin) {
          _csr_storage.insert(i, csr_values.begin() + begin,
                              csr_values.begin() + end);
        }
      }
      for (size_t i = 0; i < dest_counting_prefix_sum.size(); ++i) {
        ...
        if (end > begin) {
          _csc_storage.insert(i, csc_values.begin() + begin,
                              csc_values.begin() + end);
        }
      }
      _csr_storage.repack();
      _csc_storage.repack();
  ...
  void load(iarchive& arc) {
    clear();
    // read the vertices
    arc >> vertices >> edges >> _csr_storage >> _csc_storage;
  }  // end of load
  ...
  edge_list_type in_edges(lvid_type v) {
    edge_iterator begin =
        edge_iterator(*this, edge_iterator::CSC, _csc_storage.begin(v), v);
    edge_iterator end =
        edge_iterator(*this, edge_iterator::CSC, _csc_storage.end(v), v);
  ...
  edge_list_type out_edges(lvid_type v) {
    edge_iterator begin =
        edge_iterator(*this, edge_iterator::CSR, _csr_storage.begin(v), v);
    edge_iterator end =
        edge_iterator(*this, edge_iterator::CSR, _csr_storage.end(v), v);
    return boost::make_iterator_range(begin, end);
  }
...
class dynamic_local_graph<VertexData, EdgeData>::vertex_type {
 public:
  vertex_type(dynamic_local_graph& lgraph_ref, lvid_type vid)
...
class dynamic_local_graph<VertexData, EdgeData>::edge_type {
 public:
  edge_type(dynamic_local_graph& lgraph_ref, lvid_type _source,
            lvid_type _target, edge_id_type _eid)
...
// #ifdef DEBUG_GRAPH
//         logstream(LOG_INFO) << "Insert edges into finalized graph..." <<
//         std::endl;
// #endif
