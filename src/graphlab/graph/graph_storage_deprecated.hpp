...
class graph_storage {
 public:
  ...
  /* -----------------------------------------------------------------------------
   */
  /* helper data field and structures: edge_data_list, class edge, class
   * edge_list */
  /* -----------------------------------------------------------------------------
   */
  class edge_type {
  ...
  csr_zip_iterator make_csr_zip_iterator(typename csr_type::iterator iter,
    ...
    return csr_zip_iterator(csr_iterator_tuple(
        iter, boost::counting_iterator<edge_id_type>(beginid)));
  }
  ...
  class edge_list {
   public:
    ...
    inline edge_type operator[](size_t i) const { return *(_begin + i); }
 ...
 public:
  ...
  edge_list out_edges(const lvid_type v) {
    ...
    csr_zip_iterator beginiter = make_csr_zip_iterator(
        _csr_storage.begin(v), _csr_storage.begin(v) - _csr_storage.begin(0));
    csr_zip_iterator enditer = make_csr_zip_iterator(
        _csr_storage.end(v), _csr_storage.end(v) - _csr_storage.begin(0));

    csr_edge_iterator begin =
        boost::make_transform_iterator(beginiter, functor);
    csr_edge_iterator end = boost::make_transform_iterator(enditer, functor);
    return edge_list(edge_iterator(begin), edge_iterator(end));
  }
  ...
  void finalize(local_edge_buffer<VertexData, EdgeData>& edges) {
    ...
    // Unfortunately the zip iterator does not work as expected. Need more
    // debugging... typedef
    //     boost::tuple< std::vector<lvid_type>::iterator,
    //                   std::vector<lvid_type>::iterator,
    //                   typename std::vector<EdgeData>::iterator>
    //                   iterator_tuple;
    ...
    std::vector<std::pair<lvid_type, edge_id_type> > csc_value =
        vector_zip(edges.source_arr, permute);
 ...
 public:
  ...
  void load(iarchive& arc) {
    clear();
    arc >> _csr_storage >> _csc_storage >> _edata_storage;
  }
