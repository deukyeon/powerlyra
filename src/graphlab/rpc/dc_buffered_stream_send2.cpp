...
void dc_buffered_stream_send2::register_send_buffer(
    thread_local_buffer* buffer) {
  lock.lock();
...
void dc_buffered_stream_send2::unregister_send_buffer(
    thread_local_buffer* buffer) {
  lock.lock();
  for (size_t i = 0; i < send_buffers.size(); ++i) {
...
size_t dc_buffered_stream_send2::get_outgoing_data(
    circular_iovec_buffer& outdata) {
  lock.lock();
  size_t sendlen = 0;
  for (size_t i = 0; i < send_buffers.size(); ++i) {
    std::pair<buffer_elem*, buffer_elem*> bufs =
        send_buffers[i]->extract(target);
    if (bufs.first != NULL) {
