...
struct uses {
  ...
  bool operator()(const ValueType& current, const DeltaType& delta,
...
template <typename KeyType, typename ValueType, typename DeltaType = ValueType>
class delta_dht {
 public:
 ...
 public:
  delta_dht(distributed_control& dc, size_t max_cache_size = 2056)
      : rpc(dc, this), max_cache_size(max_cache_size), max_uses(10) {
    rpc.barrier();
  }
  ...
  size_t size() const {
    ...
    for (size_t i = 0; i < rpc.numprocs(); ++i) {
      if (i == rpc.procid())
        ...
        sum += rpc.remote_request(i, &delta_dht::local_size);
    }
    return sum;
  }
  ...
  value_type get_master(const key_type& key) {
    ...
    if (is_local(key)) {
      ...
      return rpc.remote_request(owning_cpu(key), &delta_dht::get_master, key);
    }
  }  // end of direct get
 ...
 private:
  void send_delta(const key_type& key, const delta_type& delta) {
    ...
    rpc.remote_call(owning_cpu(key), &delta_dht::send_delta_rpc, calling_procid,
                    key, delta);

  }  // end of send_delta

  void send_delta_rpc(const size_t& calling_procid, const key_type& key,
    ...
    data_lock.unlock();
    rpc.remote_call(calling_procid, &delta_dht::send_delta_rpc_callback, key,
                    ret_value);

  }  // end of send_delta_rpc

  void send_delta_rpc_callback(const key_type& key,
