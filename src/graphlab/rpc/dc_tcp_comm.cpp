...
void dc_tcp_comm::construct_events() {
  ...
  if (!outevbase)
    logstream(LOG_FATAL) << "Unable to construct libevent base" << std::endl;
  ...
  send_all_event = event_new(outevbase, -1, EV_TIMEOUT | EV_PERSIST,
                             on_send_event, &(send_all_timeout));
  assert(send_all_event != NULL);
  struct timeval t = {SEND_POLL_TIMEOUT / 1000000, SEND_POLL_TIMEOUT % 1000000};
  event_add(send_all_event, &t);
  send_triggered_event = event_new(outevbase, -1, EV_TIMEOUT | EV_PERSIST,
                                   on_send_event, &(send_triggered_timeout));
  ...
  for (size_t i = 0; i < sock.size(); ++i) {
    sock[i].inevent =
        event_new(inevbase, sock[i].insock, EV_READ | EV_PERSIST | EV_ET,
                  on_receive_event, &(sock[i]));
    if (sock[i].inevent == NULL) {
      logstream(LOG_FATAL) << "Unable to register socket read event"
                           << std::endl;
    }

    sock[i].outevent =
        event_new(outevbase, sock[i].outsock, EV_WRITE | EV_PERSIST | EV_ET,
                  on_send_event, &(sock[i]));
    if (sock[i].outevent == NULL) {
      logstream(LOG_FATAL) << "Unable to register socket write event"
                           << std::endl;
    }
...
void dc_tcp_comm::open_listening(int sockhandle) {
  ...
  logstream(LOG_INFO) << "Proc " << procid() << " listening on "
                      << portnums[curid] << "\n";
...
void dc_tcp_comm::connect(size_t target) {
  if (sock[target].outsock != -1) {
    ...
    // Connect!
    logstream(LOG_INFO) << "Trying to connect from " << curid << " -> "
                        << target << " on port " << portnums[target] << "\n";
    ...
    for (size_t i = 0; i < 10; ++i) {
      if (::connect(newsock, (sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
        logstream(LOG_INFO) << "connect " << curid << " to " << target << ": "
        ...
        sendtosock(newsock, reinterpret_cast<char*>(&msg),
                   sizeof(initial_message));
      ...
      while (msglen != sizeof(initial_message)) {
        ...
        if (retval < 0) {
          if (errno == EWOULDBLOCK || errno == EAGAIN) {
            ...
            logstream(LOG_FATAL)
                << "error: " << errno << " receive error: " << strerror(errno)
                << std::endl;
          }
        } else if (retval > 0) {
          msglen += retval;
      ...
      if (newsock != -1) {
        ...
        if (other_md5 != program_md5) {
          logstream(LOG_FATAL)
              ...
              << " \n "
              << "\tGraphLab requires all machines to run exactly the same "
                 "binary."
              << std::endl;
        }
...
#ifdef COMM_DEBUG
        logstream(LOG_INFO)
            << msglen << " bytes <-- " << sockinfo->id << std::endl;
#endif
