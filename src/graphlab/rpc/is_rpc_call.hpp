...
struct check_first_arg {
  typedef
      typename boost::is_same<typename get_args<F, __GLRPC_FARITY>::arg1_type,
                              distributed_control>::type type;
};

template <typename F>
struct check_second_arg {
  typedef typename boost::is_integral<
      typename get_args<F, __GLRPC_FARITY>::arg2_type>::type type;
};
...
struct is_rpc_call {
  typedef typename boost::mpl::if_<
      typename is_rpc_call_detail::less_than_2_args<F>::type, boost::false_type,
      typename boost::mpl::and_<
          typename is_rpc_call_detail::check_first_arg<F>::type,
          typename is_rpc_call_detail::check_second_arg<F>::type>::type>::type
      type;
};

// Varargs are all none RPC calls
#define BLOCK_VAR_ARGS(Z, N, _)                                  \
  template <typename RetType BOOST_PP_COMMA_IF(N)                \
                BOOST_PP_ENUM_PARAMS(N, typename T)>             \
  struct is_rpc_call<RetType(BOOST_PP_ENUM_PARAMS(N, T)          \
                                 BOOST_PP_COMMA_IF(N)...)> {     \
...
#define GEN_GET_USER_ARG(Z, N, _)                                \
  ...
  template <typename F>                                          \
  struct BOOST_PP_CAT(get_cleaned_rpc_or_basic_arg,              \
                      N)<F, boost::mpl::bool_<true> > {          \
    typedef BOOST_PP_CAT(__GLRPC_F, N) arg_type;                 \
  };                                                             \
  template <typename F>                                          \
  struct BOOST_PP_CAT(get_cleaned_user_arg, N) {                 \
    typedef typename BOOST_PP_CAT(                               \
        get_cleaned_rpc_or_basic_arg,                            \
        N)<F, typename is_rpc_call<F>::type>::arg_type arg_type; \
  };
