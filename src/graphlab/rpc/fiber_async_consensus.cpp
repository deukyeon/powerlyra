...
namespace graphlab {
fiber_async_consensus::fiber_async_consensus(
    distributed_control &dc, size_t required_fibers_in_done,
...
bool fiber_async_consensus::end_done_critical_section(size_t cpuid) {
  ...
  */
  if (numactive == 0) {
    logstream(LOG_INFO) << rmi.procid() << ": Termination Possible"
                        << std::endl;
...
void fiber_async_consensus::pass_the_token() {
  ...
  if (cur_token.last_change == rmi.procid() &&
    ...
    logstream(LOG_INFO) << "Completed Token: " << cur_token.total_calls_received
    ...
    for (procid_t i = 0; i < rmi.numprocs(); ++i) {
      if (i != rmi.procid()) {
        rmi.control_call(i, &fiber_async_consensus::force_done);
      }
    }
    ...
    if (callssent != last_calls_sent || callsrecv != last_calls_received) {
    ...
    rmi.control_call((procid_t)((rmi.procid() + 1) % rmi.numprocs()),
                     &fiber_async_consensus::receive_the_token, cur_token);
  }
}
}  // namespace graphlab
