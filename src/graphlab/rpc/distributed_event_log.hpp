...
class distributed_event_logger {
 ...
 public:
   ...
   * out of scope.
   */
  size_t create_callback_entry(std::string name, std::string units,
                               boost::function<double(void)> callback,
                               log_type::log_type_enum logtype);
  ...
  inline double get_current_time() const { return ti.current_time(); }

  inline log_group** get_logs_ptr() { return logs; }
...
#define ADD_CUMULATIVE_EVENT(name, desc, units)      \
  name = graphlab::get_event_log().create_log_entry( \
      desc, units, graphlab::log_type::CUMULATIVE);

#define ADD_INSTANTANEOUS_EVENT(name, desc, units)   \
  name = graphlab::get_event_log().create_log_entry( \
      desc, units, graphlab::log_type::INSTANTANEOUS);

#define ADD_CUMULATIVE_CALLBACK_EVENT(name, desc, units, callback) \
  name = graphlab::get_event_log().create_callback_entry(          \
      desc, units, callback, graphlab::log_type::CUMULATIVE);

#define ADD_INSTANTANEOUS_CALLBACK_EVENT(name, desc, units, callback) \
  name = graphlab::get_event_log().create_callback_entry(             \
      desc, units, callback, graphlab::log_type::INSTANTANEOUS);

#define FREE_CALLBACK_EVENT(name) \
  graphlab::get_event_log().free_callback_entry(name);

#define INCREMENT_EVENT(name, count) \
  graphlab::get_event_log().thr_inc_log_entry(name, count);
#define DECREMENT_EVENT(name, count) \
  graphlab::get_event_log().thr_dec_log_entry(name, count);
