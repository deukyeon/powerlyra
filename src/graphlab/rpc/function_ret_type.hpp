...
Firstly, it provides a consistent interface through a function called
::fcallN<F> to complete a function call with a variable number of arguments.
...
#define FCALL(Z, N, _)                                                   \
  template <typename F>                                                  \
  static RetType BOOST_PP_CAT(                                           \
      fcall, N)(F f BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM(N, GENARGS, _)) { \
...
This struct performs two duties.
Firstly, it provides a consistent interface through a function called
::fcallN<F> to complete a \b member function call with a variable number of
arguments. Next, it provides the type of the return value of the function in
::type. If the return type is void, it is promoted to an int. This makes the
output type of the function call be always serializable, simplifying the
...
#define FCALL(Z, N, _)                                              \
  ...
  static RetType BOOST_PP_CAT(fcall, N)(                            \
      F f, T t BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM(N, GENARGS, _)) { \
...
#define FCALL(Z, N, _)                                              \
  template <typename F, typename T>                                 \
  static size_t BOOST_PP_CAT(fcall, N)(                             \
      F f, T t BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM(N, GENARGS, _)) { \
