...
class circular_char_buffer {
  ...
  */
  ...
  inline std::streamsize size() const { return len; }

  /** Gets the size of the buffer.
      \note: The useable space is reserved_size() - 1 */
  inline std::streamsize reserved_size() const { return bufsize; }

 private:
  inline bool buffer_full() const { return len == bufsize; }
