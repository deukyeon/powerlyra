...
class buffered_exchange {
 ...
 public:
   ...
   *                  the exchange process, but there are performance /
   * contention advantages if this matches. \ref max_buffer_size The size of the
   * per thread and per target send buffer.
   */
  buffered_exchange(
      distributed_control& dc, const size_t num_threads = 1,
   ...
   *
   * \param ret_proc On successful return, will contain a valid procid
   * indicating that the values in the buffer were sent by that process. \param
   * ret_buffer A sequence of values sent by ret_proc \param try_lock If true,
   * will not acquire the lock if the lock is contended. Useful for polling the
   * receive buffer while sending is occuring. \return True on success and there
   * are values in the buffer. False if the receive buffer is empty. Or if
   * try_lock is set, False may also indicate the buffer lock is being
   * contended.
   */
 ...
 private:
  void rpc_recv(size_t len, wild_pointer w) {
    ...
    iarchive numel_iarc(
        reinterpret_cast<const char*>(w.ptr) + len - sizeof(size_t),
        sizeof(size_t));
    size_t numel = 0;
  ...
  oarchive* swap_buffer(size_t index) {
    ...
    (*swaparc).write(reinterpret_cast<char*>(&send_buffers[index].numinserts),
                     sizeof(size_t));
