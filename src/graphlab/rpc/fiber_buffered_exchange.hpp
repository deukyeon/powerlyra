 ...
 *  .. In parallel in fibers .. {
 *    procid_t proc;
 *    std::vector<buffered_exchange<int>::buffer_record> buffer; // (an array of
 * buffers) while(exchange.recv(buffer)) { for each buffer_record in buffer:
 *        buffer_record.proc is the machine which sent the contents of this
 * record buffer_record.buffer is an array containing values sent by the machine
 * buffer_record.proc
 *    }
 *  }
 *
 ...
 *      buffer_record.buffer is an array containing values sent by the machine
 * buffer_record.proc
 *  }
 * \endcode
 *
...
class fiber_buffered_exchange {
 ...
 private:
  ...
  void flush_buffer(size_t wid, procid_t proc) {
    if (send_buffers[wid][proc].oarc) {
      ...
      send_buffers[wid][proc].oarc->write(
          reinterpret_cast<char*>(&send_buffers[wid][proc].numinserts),
          sizeof(size_t));
      rpc.split_call_end(proc, send_buffers[wid][proc].oarc);
      //         logstream(LOG_DEBUG) << rpc.procid() << ": Sending exchange of
      //         length "
      //                              << send_buffers[wid][proc].oarc->off << "
      //                              to "
      //                              << proc << std::endl;
 ...
 public:
  ...
  fiber_buffered_exchange(
      ...
      : rpc(dc, this), max_buffer_size(max_buffer_size) {
  ...
  ~fiber_buffered_exchange() {
    ...
    for (size_t i = 0; i < send_buffers.size(); ++i) {
      for (size_t j = 0; j < send_buffers[i].size(); ++j) {
        if (send_buffers[i][j].oarc)
          rpc.split_call_cancel(send_buffers[i][j].oarc);
      }
    }
  }
  ...
  void send(const procid_t proc, const T& value) {
    ...
    if (send_buffers[wid][proc].oarc == NULL) {
      send_buffers[wid][proc].oarc =
          rpc.split_call_begin(&fiber_buffered_exchange::rpc_recv);
      // write a header
 ...
 private:
  void rpc_recv(size_t len, wild_pointer w) {
    ...
    procid_t src_proc;
    iarc >> src_proc;
    //       logstream(LOG_DEBUG) << rpc.procid() << ": Receiving exchange of
    //       length "
