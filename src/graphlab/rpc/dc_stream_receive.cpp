...
char* dc_stream_receive::advance_buffer(char* c, size_t wrotelength,
  ...
  if (write_buffer_written >= sizeof(packet_hdr)) {
    ...
    if (offset > 0) {
      ...
      // since we are going to give this buffer away, we need to prepare a new
      // buffer
      ...
      if (offset + sizeof(packet_hdr) <= write_buffer_written)
        incomplete_message_len = hdr->len;

      size_t new_buflen = std::max<size_t>(
          sizeof(packet_hdr) + incomplete_message_len, RECEIVE_BUFFER_SIZE);
      ...
      if (write_buffer_len - offset > 0) {
        ...
        memcpy(new_writebuffer, writebuffer + offset,
