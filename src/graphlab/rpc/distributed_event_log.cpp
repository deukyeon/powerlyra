...
static std::pair<std::string, std::string> metric_names_json(
    std::map<std::string, std::string>& vars);

static std::pair<std::string, std::string> metric_aggregate_json(
    std::map<std::string, std::string>& vars);

static std::pair<std::string, std::string> metric_by_machine_json(
    std::map<std::string, std::string>& vars);

static size_t time_to_index(double t) { return std::floor(t / 5); }

static double index_to_time(size_t t) { return 5 * t; }
...
size_t distributed_event_logger::allocate_log_entry(log_group* group) {
  ...
  if (has_log_entry.first_zero_bit(id) == false) {
    logger(LOG_FATAL,
           "More than 256 Log entries created. "
...
distributed_event_logger::get_thread_counter_ref() {
 ...
 */
void distributed_event_logger::rpc_collect_log(size_t srcproc,
                                               size_t record_ctr,
    ...
    if (logs[log]->logtype == log_type::INSTANTANEOUS) {
      ...
      if (logs[log]->is_callback_entry) {
        ...
        foreach (size_t thr, thread_local_count_slots) {
          logs[log]->sum_of_instantaneous_entries +=
              thread_local_count[thr]->values[log];
        }
        ++logs[log]->count_of_instantaneous_entries;
      }
      logs[log]->lock.unlock();
...
void distributed_event_logger::set_dc(distributed_control& dc) {
  if (rmi == NULL) {
    ...
    dc.register_deletion_callback(
        boost::bind(&distributed_event_logger::destroy_event_logger, this));
    ...
    tick_thread.launch(
        boost::bind(&distributed_event_logger::periodic_timer, this));
#endif
    ...
    add_metric_server_callback("metrics_by_machine.json",
                               metric_by_machine_json);
  }
}

size_t distributed_event_logger::create_log_entry(
    std::string name, std::string units, log_type::log_type_enum logtype) {
...
size_t distributed_event_logger::create_callback_entry(
    std::string name, std::string units, boost::function<double(void)> callback,
...
*/
std::pair<std::string, std::string> static metric_names_json(
    std::map<std::string, std::string>& vars) {
...
std::pair<std::string, std::string> static metric_aggregate_json(
    std::map<std::string, std::string>& vars) {
  double tstart = 0;
  double tend = DBL_MAX;
  bool rate = false;
...
std::pair<std::string, std::string> static metric_by_machine_json(
    std::map<std::string, std::string>& vars) {
  double tstart = 0;
  double tend = DBL_MAX;
  bool rate = false;
