  ...
  class BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 0, FNAME_AND_CALL), N) {           \
   public:                                                                     \
    static void exec(dc_dist_object_base* rmi, dc_send* sender,                \
      ...
      uint32_t beginoff = arc.off;                                             \
      dispatch_type d =                                                        \
          BOOST_PP_CAT(dc_impl::OBJECT_NONINTRUSIVE_DISPATCH,                  \
 ...
 * arguments into the oarchive returned. The split call can provide performance
 * gains when the contents of the message are large, since this allows the user
 * to control the serialization process. For examples, see \ref
 * dc_dist_object::split_call_begin
 */
...
class object_split_call {
 public:
  static oarchive* split_call_begin(dc_dist_object_base* rmi, size_t objid,
    ...
    dispatch_type d =
        dc_impl::OBJECT_NONINTRUSIVE_DISPATCH2<distributed_control, T, F,
                                               size_t, wild_pointer>;
    arc << reinterpret_cast<size_t>(d);
    serialize(arc, (char*)(&remote_function), sizeof(F));
    arc << objid;
   ...
   * pointer is consumed.
   */
  static void split_call_end(dc_dist_object_base* rmi, oarchive* oarc,
                             dc_send* sender, procid_t target,
                             unsigned char flags) {
    // header points to the location of the blob size argument
    size_t blobsize_offset = *reinterpret_cast<size_t*>(oarc->buf);
    (*reinterpret_cast<size_t*>(oarc->buf + blobsize_offset)) =
        oarc->off - blobsize_offset - sizeof(size_t);
    ...
    size_t len = hdr->len;
    write_thread_local_buffer(target, oarc->buf, oarc->off,
                              flags & CONTROL_PACKET);
    if ((flags & CONTROL_PACKET) == 0) {
...
/**
Generates a function call issue. 3rd argument is a tuple (issue name, dispacther
name)
*/
