...
form of a function call (that is the function call must take a
distributed_control and a "procid_t source" as its first 2 arguments.
...
>::type >::fcall3 (f, dc, source, (f0)); charstring_free (f0);
...
#define DISPATCH_GENERATOR(Z, N, _)                                           \
  template <typename DcType, typename F BOOST_PP_COMMA_IF(N)                  \
  ...
  void BOOST_PP_CAT(REQUESTDISPATCH, N)(DcType & dc, procid_t source,         \
    ...
    typename function_ret_type<__GLRPC_FRESULT>::type ret =                   \
        function_ret_type<__GLRPC_FRESULT>::BOOST_PP_CAT(fcall,               \
                                                         BOOST_PP_ADD(N, 2))( \
...
#define NONINTRUSIVE_DISPATCH_GENERATOR(Z, N, _)                              \
  template <typename DcType, typename F BOOST_PP_COMMA_IF(N)                  \
                                 BOOST_PP_ENUM_PARAMS(N, typename T)>         \
  void BOOST_PP_CAT(NONINTRUSIVE_REQUESTDISPATCH, N)(                         \
    ...
    BOOST_PP_REPEAT(N, GENPARAMS, _)                                          \
    typename function_ret_type<__GLRPC_FRESULT>::type ret =                   \
        function_ret_type<__GLRPC_FRESULT>::BOOST_PP_CAT(                     \
            fcall, N)(f BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM(N, GENNIARGS, _)); \
    BOOST_PP_REPEAT(N, CHARSTRINGFREE, _)                                     \
    boost::iostreams::stream<resizing_array_sink> retstrm(128);               \
    oarchive oarc(retstrm);                                                   \
    oarc << ret;                                                              \
