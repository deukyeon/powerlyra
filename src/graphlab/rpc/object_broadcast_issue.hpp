...
#define REMOTE_BROADCAST_ISSUE_GENERATOR(Z, N, FNAME_AND_CALL)                 \
  ...
  class BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(2, 0, FNAME_AND_CALL), N) {           \
   public:                                                                     \
    static void exec(dc_dist_object_base* rmi, std::vector<dc_send*> sender,   \
                     unsigned char flags, Iterator target_begin,               \
                     Iterator target_end, size_t objid,                        \
...
name)
*/
BOOST_PP_REPEAT(7, REMOTE_BROADCAST_ISSUE_GENERATOR,
                (object_broadcast_issue, _))
