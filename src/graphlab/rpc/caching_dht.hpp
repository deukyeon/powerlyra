...
class lru_list {
 public:
  ...
  ~lru_list() {}
  explicit lru_list(const KeyType &k = KeyType(),
 ...
 * \internal
 * \ingroup rpc
 This implements a limited distributed key -> value map with caching
 capabilities It is up to the user to determine cache invalidation policies.
 User explicitly calls the invalidate() function to clear local cache entries
*/
template <typename KeyType, typename ValueType>
class caching_dht {
 public:
  ...
  typedef boost::intrusive::member_hook<
      lru_entry_type, typename lru_entry_type::lru_member_hook_type,
      &lru_entry_type::member_hook_>
      MemberOption;
  /// datatype of the intrusive LRU list embedded in the cache map
  typedef boost::intrusive::list<lru_entry_type, MemberOption,
 ...
 private:
  ...
  mutable lru_list_type
      lruage;  /// THe LRU linked list associated with the cache
 ...
 public:
  ...
  caching_dht(distributed_control &dc, size_t max_cache_size = 1024)
      : rpc(dc, this), data(11) {
  ...
  void set(const KeyType &key, const ValueType &newval) {
    ...
    if (owningmachine == rpc.dc().procid()) {
      ...
      rpc.remote_call(owningmachine, &caching_dht<KeyType, ValueType>::set, key,
                      newval);
  ...
  std::pair<bool, ValueType> get(const KeyType &key) const {
    ...
    if (owningmachine == rpc.dc().procid()) {
      ...
      ret = rpc.remote_request(owningmachine,
                               &caching_dht<KeyType, ValueType>::get, key);
      if (ret.first)
        update_cache(key, ret.second);
      else
        invalidate(key);
    }
    return ret;
  ...
  double cache_miss_rate() { return double(misses) / double(reqs); }

  size_t num_gets() const { return reqs; }
  size_t num_misses() const { return misses; }
 ...
 private:
  ...
  void update_cache(const KeyType &key, const ValueType &val) const {
    ...
    if (i == cache.end()) {
      ...
      std::pair<typename cache_type::iterator, bool> ret =
          cache.insert(std::make_pair(key, new lru_entry_type(key, val)));
      if (ret.second) lruage.push_front(*(ret.first->second));
    } else {
      // modify entry in place
      i->second->value = val;
      // swap to front of list
      // boost::swap_nodes(lru_list_type::s_iterator_to(i->second),
      // lruage.begin());
      lruage.erase(lru_list_type::s_iterator_to(*(i->second)));
      lruage.push_front(*(i->second));
    }
