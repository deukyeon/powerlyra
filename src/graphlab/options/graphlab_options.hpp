...
class graphlab_options {
 public:
  ...
  const std::string& get_scheduler_type() const { return scheduler_type; }

  //! Get the engine arguments
  const options_map& get_engine_args() const { return engine_args; }

  //! Get the engine arguments
  options_map& get_engine_args() { return engine_args; }

  const options_map& get_graph_args() const { return graph_args; }

  options_map& get_graph_args() { return graph_args; }

  const options_map& get_scheduler_args() const { return scheduler_args; }

  options_map& get_scheduler_args() { return scheduler_args; }
