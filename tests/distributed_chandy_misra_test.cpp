...
struct vertex_data {
  ...
  vertex_data(double value = 1) : nupdates(0), value(value), old_value(0) {}
};  // End of vertex data
...
void thread_stuff() {
  ...
  while (1) {
    ...
    if (deq.second == false)
      ...
      if (nlocks_to_acquire == 0 &&
          nlocksacquired ==
              INITIAL_NLOCKS_TO_ACQUIRE + lockable_vertices.size())
        cond.signal();
      mt.unlock();
...
int main(int argc, char** argv) {
  ...
  std::cout
      << " ==============================================================="
      << std::endl;
  std::cout << dc.procid() << ": Finished in " << timer.current_time()
            << std::endl;
  ...
  //   std::cout << graph.l_get_vertex_record(v).gvid << ": " <<
  //   graph.l_get_vertex_record(v).owner << ":"; foreach(graphlab::procid_t
  //   pid,  graph.l_get_vertex_record(v).get_replicas()) {
  //     std::cout << pid << " ";
  //   }
  //   std::cout << "\n";
  // }
  dc.barrier();
  locks =
      new graphlab::distributed_chandy_misra<graph_type>(dc, graph, callback);
  nlocksacquired = 0;
  ...
  for (graphlab::vertex_id_type v = 0; v < graph.num_local_vertices(); ++v) {
    if (graph.l_get_vertex_record(v).owner == dc.procid()) {
      // std::cout << dc.procid() << ": Lock Req for " <<
      // graph.l_get_vertex_record(v).gvid << std::endl;
      locks->make_philosopher_hungry(v);
    }
  }
  mt.lock();
  while (nlocksacquired != INITIAL_NLOCKS_TO_ACQUIRE + lockable_vertices.size())
    cond.wait(mt);
  mt.unlock();
  dc.barrier();
  locked_elements.stop_blocking();
  thrs.join();
  std::cout << INITIAL_NLOCKS_TO_ACQUIRE + lockable_vertices.size()
            << " Locks to acquire\n";
  std::cout << nlocksacquired << " Locks Acquired in total\n";
  boost::unordered_map<graphlab::vertex_id_type, size_t>::const_iterator iter =
      demand_set.begin();
  bool bad =
      (nlocksacquired != INITIAL_NLOCKS_TO_ACQUIRE + lockable_vertices.size());
  while (iter != demand_set.end()) {
    if (locked_set[iter->first] != iter->second) {
      std::cout << graph.l_get_vertex_record(iter->first).gvid
                << " mismatch: " << locked_set[iter->first] << ", "
                << iter->second << "\n";
      bad = true;
    }
    ++iter;
