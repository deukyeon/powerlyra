...
size_t count_edges(graph_type::edge_type e) { return 1; }

void set_to_one(graph_type::vertex_type vtx) { vtx.data() = 1; }

int vertex_data_identity(graph_type::vertex_type vtx) { return vtx.data(); }
...
int main(int argc, char** argv) {
  ...
  graphlab::vertex_set small =
      graph.select(boost::bind(select_out_degree_le, _1, 1));
  dc.cout() << graph.vertex_set_size(small)
            << " vertices with out degree <= 1\n";

  // all vertices which have  > 1 neighbors
  graphlab::vertex_set connected = graph.complete_set() - small;
  dc.cout() << graph.vertex_set_size(connected)
            << " vertices with out degree > 1\n";
  ...
  graphlab::vertex_set even_id =
      graph.select(boost::bind(select_vid_modulo, _1, 2));
  // select all vertices with ID divisible by 3
  graphlab::vertex_set div_3_id =
      graph.select(boost::bind(select_vid_modulo, _1, 3));
  ...
  size_t num_div_6 =
      graph.map_reduce_vertices<size_t>(boost::bind(is_divisible, _1, 6));
  ...
  graphlab::vertex_set out_deg_one =
      graph.select(boost::bind(select_out_degree_eq, _1, 1));
  // test edge mapreduce
  ...
  size_t total =
      graph.map_reduce_vertices<size_t>(vertex_data_identity, out_deg_one);
  ...
  graphlab::vertex_set out_nbrs =
      graph.neighbors(out_deg_one, graphlab::OUT_EDGES);

  dc.cout() << graph.vertex_set_size(out_nbrs) << " nbr size\n";
  // extract the set of in neighbors of these out neighbors
  graphlab::vertex_set out_nbrs_in_nbrs =
      graph.neighbors(out_nbrs, graphlab::IN_EDGES);

  dc.cout() << graph.vertex_set_size(out_nbrs_in_nbrs) << " nbr nbr size\n";
  // this set must contain the original out_deg_one set
  ASSERT_TRUE(
      graph.vertex_set_empty((out_deg_one & out_nbrs_in_nbrs) - out_deg_one));
