...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("graph", graphpath, "The graph path \n");

  clopts.attach_option("prefix", prefix,
                       "The prefix for load/save binary file\n");

  clopts.attach_option("gzip", gzip, "The input is in gzip format\n");

  clopts.attach_option("format", format, "format of the graph: {adj, snap}\n");

  if (!clopts.parse(argc, argv)) {
    logstream(LOG_FATAL) << "Error in parsing command line arguments."
                         << std::endl;
    return EXIT_FAILURE;
  }
  ...
  // if (dc.procid() == 0) {
  //   std::cout << "==========================================" << std::endl;
  //   std::cout << "Heap Size (before finalize): " <<
  //   (double)heap_size_load/(1024*1024) << "MB" << "\n"; std::cout <<
  //   "Allocated Size (before finalize): " <<
  //   (double)allocate_size_load/(1024*1024) << "MB" << "\n"; std::cout <<
  //   "==========================================" << std::endl;
  // }
  ...
  std::cout << dc.procid() << ": Finished in " << mytimer.current_time()
            << std::endl;
  ...
  if (dc.procid() == 0) {
    ...
    foreach (std::string opt, keys) {
      if (opt == "ingress") {
        ...
        clopts.get_graph_args().get_option("constrained_graph",
                                           constraint_graph);
      }
    }
    ...
    fout << "Replication factor: "
         << (float)graph.num_replicas() / graph.num_vertices() << std::endl;
    fout << "Balance factor: "
         << (float)graph.num_local_edges() / graph.num_edges() << std::endl;
    // fout << "Heap size (load): " << (double)heap_size_load/(1024*1024) <<
    // std::endl; fout << "Allocated size (load): " <<
    // (double)allocate_size_load/(1024*1024) << std::endl; fout << "Heap size
    // (finalize): " << (double)heap_size/(1024*1024) << std::endl; fout <<
    // "Allocated size (finalize): " << (double)allocate_size/(1024*1024) <<
    // std::endl;
