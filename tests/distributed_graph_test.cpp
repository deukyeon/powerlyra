...
class distributed_graph_test {
 public:
  ...
  void test_dynamic_add_edge() {
    ...
    if (g.is_dynamic()) {
      ...
      dc->cout() << "\n- Graph does not support dynamic. Please compile with "
                    "-DUSE_DYNAMIC_GRAPH \n";
    }
  }
 ...
 private:
  ...
  void test_add_edge_impl(Graph& g, size_t nedges, bool use_dynamic = false) {
    ...
    boost::unordered_map<vertex_id_type, std::vector<vertex_id_type> >
        out_edges;
    ...
    typedef typename boost::unordered_set<
        std::pair<vertex_id_type, vertex_id_type> >::value_type pair_type;
    int count = 0;
    ...
    if (!use_dynamic) ASSERT_EQ(g.num_edges(), 0);
    ...
    if (create_directory(ph)) {
      ...
      for (size_t i = 0; i < g.num_local_vertices(); ++i) {
        ...
        for (size_t j = 0; j < out_edge_size; ++j) {
          ...
          ASSERT_TRUE(g.l_out_edges(i)[j].data() ==
                      g2.l_out_edges(i)[j].data());
        }
      }
      dc->cout() << "Remove path: " << ph.string() << std::endl;
      remove_all(ph);
    } else {
      dc->cout() << "Unable to create tmp directory:" << ph.string()
                 << std::endl;
    }
  }
  ...
  void check_adjacency(
      ...
      boost::unordered_map<typename Graph::vertex_id_type,
                           std::vector<typename Graph::vertex_id_type> >&
          in_edges,
      boost::unordered_map<typename Graph::vertex_id_type,
                           std::vector<typename Graph::vertex_id_type> >&
          out_edges,
    ...
    typedef map_reduce<vertex_id_type, std::vector<vertex_id_type> >
        dist_adj_type;
    ...
    typedef typename boost::unordered_map<
        vertex_id_type, std::vector<vertex_id_type> >::const_iterator iter_type;
    ...
    for (iter_type it = out_edges.begin(); it != out_edges.end(); ++it) {
      ...
      if (g.vid2lvid.count(id)) ASSERT_EQ(g.num_out_edges(id), expected.size());
    ...
    for (iter_type it = in_edges.begin(); it != in_edges.end(); ++it) {
      ...
      std::sort(expected.begin(), expected.end());
      ASSERT_EQ(actual.size(), expected.size());
      if (g.vid2lvid.count(id)) ASSERT_EQ(g.num_in_edges(id), expected.size());
      for (size_t i = 0; i < actual.size(); ++i) {
        ASSERT_EQ(actual[i], expected[i]);
      }
    }
  ...
  struct vertex_info {
    ...
    bool operator==(const vertex_info& other) {
      return ((master == other.master) && (vid == other.vid) &&
              (data == other.data) && (mirrors == other.mirrors) &&
              (num_in_edges == other.num_in_edges) &&
              (num_out_edges == other.num_out_edges));
    }

    void load(graphlab::iarchive& arc) {
      arc >> vid >> master >> mirrors >> num_in_edges >> num_out_edges >> data;
    }
  ...
  void check_vertex_info(Graph& g) {
    ...
    typedef typename boost::unordered_map<vertex_id_type, vinfo_type>
        vinfo_map_type;
    ...
    for (size_t i = 0; i < g.num_local_vertices(); ++i) {
      ...
      vid2info[v.id()] = info;
      if (lv.owned()) vids.push_back(v.id());
    }
    ...
    foreach (vertex_id_type vid, vids) {
      ...
      for (size_t i = 0; i < mirror_expected.size(); ++i) {
        ...
        ASSERT_TRUE(records[0].mirrors.get(procid) ||
                    (records[0].master == procid));
      }
...
class map_reduce {
 public:
  ...
  void save(graphlab::oarchive& oarc) const { oarc << data; }
  void load(graphlab::iarchive& iarc) { iarc >> data; }
  map_reduce& operator+=(const map_reduce& other) {
    for (typename boost::unordered_map<K, V>::const_iterator it =
             other.data.begin();
