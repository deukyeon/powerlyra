...
class edge_data {
 ...
 public:
  edge_data() {}
  edge_data(graphlab::vertex_id_type v1, graphlab::vertex_id_type v2,
            size_t ncolors) {
 ...
 */
void create_synthetic_mrf(graphlab::distributed_control& dc, graph_type& graph,
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("bound", &BOUND, BOUND, "Residual termination bound");
  clopts.attach_option("damping", &DAMPING, DAMPING,
                       "The amount of message damping (higher = more damping)");
  clopts.attach_option("ncolors", &NCOLORS, NCOLORS,
                       "The number of colors in the noisy image");
  clopts.attach_option("sigma", &SIGMA, SIGMA, "Standard deviation of noise.");
  clopts.attach_option("nrows", &nrows, nrows,
                       "The number of rows in the noisy image");
  clopts.attach_option("ncols", &ncols, ncols,
                       "The number of columns in the noisy image");
  clopts.attach_option("lambda", &lambda, lambda,
                       "Smoothness parameter (larger => smoother).");
  clopts.attach_option("smoothing", &smoothing, smoothing,
                       "Options are {square, laplace}");
  clopts.attach_option("orig", &orig_fn, orig_fn, "Original image file name.");
  clopts.attach_option("noisy", &noisy_fn, noisy_fn, "Noisy image file name.");
  clopts.attach_option("pred", &pred_fn, pred_fn, "Predicted image file name.");
  ...
  if (dc.procid() == 0) std::cout << EDGE_FACTOR << std::endl;
...
graphlab::vertex_id_type sub2ind(size_t rows, size_t cols, size_t r, size_t c) {
  return r * cols + c;
};  // end of sub2ind
