...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("model", &model_filename, model_filename,
                       "model file name");
  clopts.add_positional("model");

  clopts.attach_option("experiment", &experiment_type, experiment_type,
                       "the type of experiment to run "
                       "{chromatic, jtsplash}");
  clopts.add_positional("experiment");

  clopts.attach_option("runtimes", &runtimes, runtimes,
                       "total runtime in seconds");

  clopts.attach_option("draw_images", &draw_images, draw_images,
                       "draw pictures (assume sqrt(numvert) rows)");

  clopts.attach_option("treesize", &treesize, treesize,
                       "The maximum number of variables in a junction tree");

  clopts.attach_option("treewidth", &treewidth, treewidth,
                       "The maximum treewidth.");

  clopts.attach_option("treeheight", &treeheight, treeheight,
                       "The maximum height of the trees. ");

  clopts.attach_option("factorsize", &factorsize, factorsize,
                       "The maximum factorsize");

  clopts.attach_option("subthreads", &subthreads, subthreads,
                       "The number of threads to use inside each tree "
                       "(zero means not used)");

  clopts.attach_option("priorities", &priorities, priorities,
                       "Use priorities?");
  ...
  std::cout << "model:          " << model_filename << std::endl
            ...
            << "runtime:        " << boost::lexical_cast<std::string>(runtimes)
            << std::endl;
  ...
  if (experiment_type == "chromatic") {
    run_chromatic_sampler(mrf_core, chromatic_results_fn, runtimes,
                          draw_images);
  }
  if (experiment_type == "jtsplash") {
    ...
    run_jtsplash_sampler(mrf_core.graph(), jtsplash_results_fn, runtimes,
                         draw_images, settings);
