...
void construct_denoise_graph(image& img, size_t num_rings, double sigma,
                             const std::string& corruption,
                             factor_t edge_factor, factorized_model& model) {

}  // End of construct graph

int main(int argc, char** argv) {
  std::cout << "make the image denoising alchemy problem" << std::endl;
  ...
  clopts.attach_option("drawing", &drawing, drawing, "drawing type");
  clopts.attach_option("corruption", &corruption, corruption,
                       "corruption type");
  clopts.attach_option("smoothing", &smoothing, smoothing, "smoothing type");
  clopts.attach_option("lambda", &lambda, lambda, "edge parameter");
  clopts.attach_option("sigma", &sigma, sigma, "noise parameter");
  clopts.attach_option("rows", &rows, rows, "number of rows and cols");
  clopts.attach_option("rings", &rings, rings, "number of rings");
    ...
    for (size_t j = 0; j < noisy.cols(); ++j) {
      ...
      if (corruption == "gaussian") {
        for (size_t pred = 0; pred < rings; ++pred) {
          factor.logP(pred) = -(obs - pred) * (obs - pred) / (2.0 * sigmaSq);
        }
      } else if (corruption == "flip") {
        for (size_t pred = 0; pred < rings; ++pred) {
