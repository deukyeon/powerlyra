...
void save_beliefs(const mrf_graph_type& mrf, const std::string& filename) {
...
void save_asg(const mrf_graph_type& mrf, const std::string& filename) {
  std::ofstream fout(filename.c_str());
  for (size_t v = 0; v < mrf.num_vertices(); ++v)
  ...
  for (vertex_id_type vid = 0; vid < mrf.num_vertices(); ++vid) {
    ...
    foreach (const factor_id_t fid, vdata.factor_ids) {
      ...
      for (size_t n = 0; n < args.num_vars(); ++n) {
        ...
        if (vdata.variable != neighbor_var) neighbors.insert(neighbor_var);
      }
    }
...
void draw_mrf(const size_t experiment_id, const std::string& base_name,
  ...
  img.save(
      make_filename(base_name + "_updates_", ".pgm", experiment_id).c_str());

  for (vertex_id_type vid = 0; vid < mrf.num_vertices(); ++vid) {
    img.pixel(vid) = mrf.vertex_data(vid).nsamples == 0;
  }
  img.save(
      make_filename(base_name + "_unsampled_", ".pgm", experiment_id).c_str());
  ...
  img.pixel(0) = 0;
  img.pixel(1) = mrf.vertex_data(0).variable.size() - 1;
  img.save(make_filename(base_name + "_final_sample_", ".pgm", experiment_id)
               .c_str());
}  // end of draw_mrf
