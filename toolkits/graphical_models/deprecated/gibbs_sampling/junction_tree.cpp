 ...
 *
 **/
bool jtree_list::extend(const mrf_graph_type::vertex_id_type elim_vertex,
  ...
  size_t factor_size =
      std::max(mrf.vertex_data(elim_vertex).variable.size(), uint32_t(1));
  ...
  for (vertex_id_t parent_vid = clique.parent;
    ...
    foreach (vertex_id_t vid, tmp_verts) {
      new_parent_vid = std::max(new_parent_vid, elim_time_lookup(vid));
    }
 ...
 */
void jtree_list::load_graph(const mrf_graph_type& mrf, const size_t num_factors,
  ...
  {  // Construct the junction tree
    ...
    foreach (const jtree_list::elim_clique& clique, cliques) {
      ...
      if (parent_id != NULL_VID) {
        ...
        const jtree_vertex_data& parent_vdata = jt_graph.vertex_data(parent_id);
        jtree_edge_data edata;
        edata.variables = vdata.variables.intersect(parent_vdata.variables);
...
std::ostream& operator<<(std::ostream& out, const std::set<vertex_id_t>& set) {
...
void test_rip(const jtgraph_type& graph) {
  ...
  std::map<vertex_id_t, std::map<vertex_id_t, std::set<vertex_id_t> > >
      edgedata;
  ...
  foreach (const jtgraph_node& node, graph) {
    ...
    foreach (const vertex_id_t n1, node.neighbors) {
      //      std::cout << "\t" << n1 << "--" << edgedata[n1][node.vid] <<
      //      std::endl;
      foreach (const vertex_id_t n2, node.neighbors) {
        if (n1 != n2) {
          local_sep_set = gl::set_union(
              local_sep_set, gl::set_intersect(edgedata[n1][node.vid],
 ...
 */
void jtree_list::validate() const {
