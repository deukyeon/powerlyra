...
struct options {
  ...
  options(matwrap args = matwrap(NULL))
    ...
    safe_assert(args.is_struct(), "Additional arguments must be in a struct");
    {  // parse the sampler algorithm type
      ...
      if (!arg.is_null()) {
        ...
        if (sampler_type_str == "CHROMATIC")
          alg_type = CHROMATIC;
        else if (sampler_type_str == "SPLASH")
          alg_type = SPLASH;
        else {
  ...
  void print() {
    std::cout << "Generating " << nsamples << " samples " << std::endl;
    std::cout << "Skipping every " << nskip << " samples." << std::endl;
    switch (alg_type) {
      case CHROMATIC:
        mexPrintf("Using the chromatic sampler.\n");
        break;
      case SPLASH:
        mexPrintf("Using the splash sampler.\n");
        ...
        break;
      default:
        mexErrMsgTxt("No algorithm selected!\n");
        break;
    }
    flush_screen();
  }
 ...
 */
void add_factor(factorized_model& model, matwrap matlab_factor) {
  ...
  safe_assert(num_vars <= MAX_DIM, "Too many (>32) variables in factor.");
...
struct result_collector {
  ...
  result_collector(matwrap samples = NULL, matwrap beliefs = NULL,
                   matwrap nsamples = NULL, matwrap nchanges = NULL)
      : sample_id(0),
        samples(samples),
        beliefs(beliefs),
        nsamples(nsamples),
...
void collector_sync(mrf_gl::iscope& scope, graphlab::any& accumulator) {
  ...
  if (!collector.samples.is_null() && sample_id < collector.samples.cols()) {
    collector.samples.mat_index2d(vdata.variable.id(), sample_id) =
        vdata.asg + 1;
  }
  if (!collector.beliefs.is_null() && sample_id < collector.beliefs.cols()) {
    matwrap blf(
        collector.beliefs.get_cell_index2d(vdata.variable.id(), sample_id));
    double sum = 0;
  ...
  if (!collector.nsamples.is_null() && sample_id < collector.nsamples.cols()) {
    collector.nsamples.mat_index2d(vdata.variable.id(), sample_id) =
        vdata.nsamples;
  }
  if (!collector.nchanges.is_null() && sample_id < collector.nchanges.cols()) {
    collector.nchanges.mat_index2d(vdata.variable.id(), sample_id) =
        vdata.nchanges;
  }
}

void collector_apply(graphlab::any& current_data, const graphlab::any& param) {
...
bool nsamples_terminator() {
  return glshared_collector.get_val().sample_id >= global_termination_nsamples;
}

void run_chromatic_sampler(mrf_gl::core& core, const options& opts) {
...
void run_jt_splash_sampler(mrf_gl::core& core, const options& opts) {
  ...
  jt_splash_sampler jtsplash_sampler(core.graph(), settings);

  const size_t samples_per_iteration = core.graph().num_vertices() * opts.nskip;
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  {  // Seed the random number generator using rand from within matlab
    ...
    const size_t seed_value = matwrap(plhs[0]).get_data<size_t>()[0];
  ...
  if (opts.alg_type == options::CHROMATIC) {
    ...
    std::cout << "Finished coloring graph with " << ncolors << " colors."
              << std::endl;
    flush_screen();
  ...
  if (nlhs > 0) {
    matlab_samples =
        matwrap::create_matrix(model.variables().size(), opts.nsamples);
    plhs[0] = matlab_samples.array;
    safe_assert(!matlab_samples.is_null(), "Error initializing return samples");
  }
  ...
  if (nlhs > 3) {
    matlab_beliefs =
        matwrap::create_cell(model.variables().size(), opts.nsamples);
    plhs[3] = matlab_beliefs.array;
    safe_assert(!matlab_beliefs.is_null(), "Error initializing return beliefs");
  }
  if (!matlab_beliefs.is_null()) {
    // populate each of the entries
    for (size_t i = 0; i < mrf_core.graph().num_vertices(); ++i) {
      const mrf_vertex_data& vdata = mrf_core.graph().vertex_data(i);
      for (size_t j = 0; j < opts.nsamples; ++j) {
        matwrap blf = matwrap::create_matrix(vdata.variable.size(), 1);
  ...
  glshared_collector.set(result_collector(matlab_samples, matlab_beliefs,
                                          matlab_nsamples, matlab_nchanges));

  // if(!matlab_samples.is_null()) {
  //   std::cout << "Enabling sample collection." << std::endl;
  //   flush_screen();
  //    last_tic = graphlab::lowres_time_seconds();
  const size_t sync_interval = opts.nskip * mrf_core.graph().num_vertices();
  mrf_core.set_sync(glshared_collector, collector_sync, collector_apply,
                    graphlab::any(size_t(0)), sync_interval, collector_merge);
  //}
