...
void run_jtsplash_sampler(mrf_graph_type& mrf_graph,
  ...
  foreach (const double experiment_runtime, runtimes) {
    ...
    std::cout << "Actual Experiment Runtime:" << actual_experiment_runtime
              << std::endl;
    actual_total_runtime += actual_experiment_runtime;
    std::cout << "Total Experiment Runtime (actual): " << total_runtime << "("
              << actual_total_runtime << ")" << std::endl;
    ...
    save_asg(mrf_graph, make_filename("jtsplash_asg_", ".asg", experiment_id));
    // Save the experiment
    std::ofstream fout(jtsplash_results_fn.c_str(), std::ios::app);
    fout.precision(8);
    fout << experiment_id << '\t' << total_runtime << '\t'
         << actual_total_runtime << '\t' << settings.ntrees << '\t'
         << stats.nsamples << '\t' << stats.nchanges << '\t' << stats.loglik
         << '\t' << stats.min_samples << '\t' << stats.max_samples << '\t'
         << settings.max_tree_size << '\t' << settings.max_tree_width << '\t'
         << settings.max_factor_size << '\t' << settings.max_tree_height << '\t'
         << settings.subthreads << '\t' << settings.priorities << '\t'
         << jtsplash_sampler.total_trees() << '\t'
         << jtsplash_sampler.total_collisions() << '\t' << std::endl;
    fout.close();
...
void jtree_update::operator()(base::icontext_type& context) {
  ...
  if (vdata.factor.args() != vdata.variables) {
    ...
    foreach (size_t factor_id, vdata.factor_ids) {
      ...
      for (size_t i = 0; i < conditional_args.num_vars(); ++i) {
        ...
        conditional_asg &= assignment_t(mrf_vdata.variable, mrf_vdata.asg);
      }
  ...
  if (!vdata.calibrated) {
    ...
    vdata.calibrated = received_neighbors == context.in_edge_ids().size();
  } else {
  ...
  if (vdata.calibrated && !vdata.sampled) {
    ...
    foreach (edge_id_t out_eid, context.out_edge_ids()) {
      if (out_eid != to_parent_eid) {
        ...
        callback.add_task(neighbor_vid, jtree_sample_update, 1.0);
      }
    }
...
jt_worker::jt_worker(size_t worker_id, const splash_settings& settings,
...
void jt_worker::run() {
  ...
  while (!terminator_ptr->finished()) {
    ...
    advance_root();
    //    std::cout << "Worker " << worker_id << ": " << current_root <<
    //    std::endl;
...
void jt_worker::grow_bfs_jtree() {
  ...
  while (!bfs_queue.empty() &&
    ...
    bool extended_jtree =
        ...
        jt_list.extend(next_vertex, mrf, settings.max_tree_width,
...
size_t jt_worker::splash_once() {
  ...
  for (vertex_id_t vid = 0; vid < jt_core.graph().num_vertices(); ++vid) {
...
jt_splash_sampler::jt_splash_sampler(mrf_graph_type& mrf,
                                     const splash_settings& settings)
