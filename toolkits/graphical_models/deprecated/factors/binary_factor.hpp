...
class binary_factor {
 public:
  binary_factor(uint32_t var1 = 0, uint16_t arity1 = 0, uint32_t var2 = 0,
                uint16_t arity2 = 0)
      : _var1(var1),
        _arity1(arity1),
        _var2(var2),
  ...
  /** Get the value of the factor.  In var1 == var2 the variables
      are ignored. */
  double& logP(uint32_t x1, uint16_t asg1, uint32_t x2, uint16_t asg2) {
    // If the factor is not symmetric then we may have to match the
    // arguments
    if (_var1 != _var2) {
      assert((x1 == var1() && x2 == var2()) || (x2 == var1() && x1 == var2()));
  ...
  const double& logP(uint32_t x1, uint16_t asg1, uint32_t x2,
                     uint16_t asg2) const {
    // If the factor is not symmetric then we may have to match the
    // arguments
    if (_var1 != _var2) {
  ...
  inline void normalize() {
    ...
    for (uint16_t asg1 = 0; asg1 < arity1(); ++asg1)
      for (uint16_t asg2 = 0; asg2 < arity2(); ++asg2)
        max_value = std::max(max_value, logP(asg1, asg2));
    ...
    // Normalize
    for (uint16_t asg1 = 0; asg1 < arity1(); ++asg1)
      for (uint16_t asg2 = 0; asg2 < arity2(); ++asg2) logP(asg1, asg2) -= logZ;
  }  // End of normalize
  ...
  void printP(std::ostream& out) const {
    out << "Binary Factor(v_" << var1() << " in {1..." << arity1() << "}, "
        << ", v_ " << var2() << " in {1..." << arity2() << "})" << std::endl;
    for (uint16_t i = 0; i < arity1(); ++i) {
      for (uint16_t j = 0; j < arity2(); ++j) {
  ...
  void save(graphlab::oarchive& oarc) const {
    oarc << _var1 << _arity1 << _var2 << _arity2 << _data;
  }

  //! Load the factor from a file
  void load(graphlab::iarchive& iarc) {
    iarc >> _var1 >> _arity1 >> _var2 >> _arity2 >> _data;
  }
...
inline std::ostream& operator<<(std::ostream& out, const binary_factor& fact) {
