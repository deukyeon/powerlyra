...
class unary_factor {
 public:
   ...
   */
  inline void resize(uint16_t arity) { _data.resize(arity); }
  ...
  inline void normalize() {
    ...
    // Normalize
    for (uint16_t asg = 0; asg < arity(); ++asg) logP(asg) -= logZ;
  }  // End of normalize

  inline unary_factor& operator*=(const unary_factor& other) {
    ASSERT_EQ(arity(), other.arity());
    for (uint16_t asg = 0; asg < arity(); ++asg) logP(asg) += other.logP(asg);
    return *this;
  }
  ...
  inline void convolve(const binary_factor& bin_fact,
    ...
    for (uint16_t x = 0; x < arity(); ++x) {
      ...
      for (uint16_t y = 0; y < other.arity(); ++y) {
        sum +=
            std::exp(bin_fact.logP(var(), x, other.var(), y) + other.logP(y));
      }
      assert(!(sum < 0.0));
      // Gaurd against zeros
  ...
  inline void condition(const binary_factor& bin_fact, uint16_t asg) {
    uint32_t other_var =
  ...
  inline double residual(const unary_factor& other) const {
    ...
    double residual = 0;
    for (uint16_t asg = 0; asg < arity(); ++asg)
      residual += std::abs(std::exp(logP(asg)) - std::exp(other.logP(asg)));
  ...
  void save(graphlab::oarchive& oarc) const { oarc << _var << _data; }
  void load(graphlab::iarchive& iarc) { iarc >> _var >> _data; }
...
inline std::ostream& operator<<(std::ostream& out, const unary_factor& fact) {
