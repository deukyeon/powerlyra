...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if ((nrhs < 3) || (nrhs > 4)) mexErrMsgTxt("Takes 3-4 input arguments");
  if (nlhs != 1) mexErrMsgTxt("Outputs one result (a structure)");
  ...
  if (nrhs == 3)  //                          points, weights, bandwidths
    ...
    plhs[0] = BallTreeDensity::createInMatlab(
        prhs[0], prhs[1], prhs[2], (BallTreeDensity::KernelType)ktype);
  }
}
#else
