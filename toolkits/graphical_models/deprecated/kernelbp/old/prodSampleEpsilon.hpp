...
#define SIGVALSMIN(a, b, c) (SigValsMin + a + Ndim * b + Ndim * Ndens * c)
  ...
  void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!allGaussians) mexErrMsgTxt("Sorry -- only Gaussian kernels supported");
    ...
    double *rsizeP = mxGetPr(rsize);
    rsizeP[0] = 1;
    rsizeP[1] = Nsamp + 1;
    rUnif1 = mxCreateDoubleMatrix(1, Nsamp + 1, mxREAL);
    mexCallMATLAB(1, &rNorm, 1, &rsize, "rand");
    randunif1 = mxGetPr(rNorm);
    randunif1[Nsamp] = 100;
    mexCallMATLAB(1, &rUnif1, 1, &rNorm, "sort");
    randunif1 = mxGetPr(rUnif1);
    mxDestroyArray(rNorm);
    rsizeP[0] = Ndens;
    rsizeP[1] = Nsamp;
    mexCallMATLAB(1, &rUnif2, 1, &rsize, "rand");
    randunif2 = mxGetPr(rUnif2);
    rsizeP[0] = Ndim;
    rsizeP[1] = Nsamp;
    ...
    SigValsMax = (double *)mxMalloc(Ndim * Ndens * Ndens *
                                    sizeof(double));  // precalc'd constants
    SigValsMin = (double *)mxMalloc(Ndim * Ndens * Ndens *
                                    sizeof(double));  // precalc'd constants
    ...
    total = -1;
    soFar = soFarMin = 0;
    multiEval();  // calculate total weight
    total = soFar;
    soFar = soFarMin = 0;
    multiEval();  //   then sample
    ...
    mxDestroyArray(rUnif1);
    mxDestroyArray(rUnif2);
    mxDestroyArray(rNorm);
    mxDestroyArray(rsize);
  }
#else
  //////////////////////////////////////////////////////////////////////
  // MEX WRAPPER
  //////////////////////////////////////////////////////////////////////
  kde prodSampleEpsilonRun(
      unsigned int _Ndens,  // number of densities to product
  ...
  double normConstant(void) {
    ...
    for (j = 0; j < Ndim; j++) {
      ...
      if (bwUniform) {
        for (i = 0; i < Ndens; i++)
          tmp += 1 / trees[i].bwMin(0)[j];  // compute result bandwidth
  ...
  double minDistProd(const BallTreeDensity &bt1, BallTree::index i,
                     ...
                     const double *SigValIJ,
                     const double *SigNIJ)  //  precomp'd weighting factors
  {
    double result = 0;
    const double *center1, *center2;

    center1 = bt1.center(i);
    center2 = bt2.center(j);
  ...
  // Compute (1 over) the \Lambda_(i,j) values needed for distance-weight
  // computations
  //
  void computeSigVals(void) {
    ...
    for (i = 0; i < Ndim; i++) {
      ...
      for (j = 0; j < Ndens; j++)
        SigNormMin[i] += 1 / trees[j].bwMin(ind[j])[i];  // compute \Lambda_L
      for (j = 0; j < Ndens; j++)
        SigNormMax[i] += 1 / trees[j].bwMax(ind[j])[i];  //
      SigNormMax[i] = 1 / SigNormMax[i];
      SigNormMin[i] = 1 / SigNormMin[i];
    }
    for (i = 0; i < Ndim; i++) {
      for (j = 0; j < Ndens; j++)      //  then compute pairwise leave-
        for (k = j; k < Ndens; k++) {  //  two-out normalized values
          *SIGVALSMIN(i, k, j) = SigNormMax[i] / (trees[j].bwMin(ind[j])[i] *
                                                  trees[k].bwMin(ind[k])[i]);
          *SIGVALSMAX(i, k, j) = SigNormMin[i] / (trees[j].bwMax(ind[j])[i] *
                                                  trees[k].bwMax(ind[k])[i]);
  ...
  void multiEvalRecursive(void) {
    ...
    for (i = 0; i < Ndens; i++) {        // For each pair of densities, bound
      for (j = i + 1; j < Ndens; j++) {  //   the total weight of their product:
        double maxValT =
            minDistProd(trees[i], ind[i], trees[j], ind[j], SIGVALSMAX(0, i, j),
                        SIGVALSMIN(0, i, j));  // compute min & max
        double minValT =
            maxDistProd(trees[i], ind[i], trees[j], ind[j], SIGVALSMIN(0, i, j),
                        SIGVALSMAX(0, i, j));  // dist = max/min values
        maxVal += maxValT;
        minVal += minValT;
    ...
    if (allLeaves ||
        fabs(maxVal - minVal) <= maxErr * (soFarMin + minVal)) {  // APPROXIMATE
      ...
      while (*randunif1 <=
             soFar / total) {  // for all the samples coming from this block
        randunif1++;
        for (j = 0; j < Ndim; j++) M[j] = 0;  // clear out M
        if (!bwUniform)
          for (j = 0; j < Ndim; j++) C[j] = 0;  // clear out C if necc.

        for (i = 0; i < Ndens; i++) {  // find an index within this block
          double SumTmp = 0;
          BallTree::index index =
              trees[i].leafFirst(ind[i]);  // start with 1st leaf and
          ...
          *(indices++) =
              trees[i].getIndexOf(index) + 1;  // and save selected indices
          // assert(trees[i].getIndexOf(index)+1 <= Ndens*Nsamp);
          if (!bwUniform)
            for (j = 0; j < Ndim; j++)            // compute covariance
              C[j] += 1 / trees[i].bw(index)[j];  //  contribution of each dens.
        }
        if (!bwUniform)
          for (j = 0; j < Ndim; j++) {  // finish computing covar and
      ...
      unsigned int split;
      double size0 = trees[maxInd0].range(
          ind[maxInd0])[0];  // from the pair with the largest
      double size1 = trees[maxInd1].range(
          ind[maxInd1])[0];  // pairwise max-min discrepancy term,
  ...
  void multiEval(void) {
    ...
    assert(Ndens > 0);
    ind = (BallTree::index *)mxMalloc(
        Ndens * sizeof(BallTree::index));  // construct index array
    memset(ind, 0, Ndens * sizeof(BallTree::index));
    for (i = 0; i < Ndens; i++)
      ind[i] = trees[i].root();  //  & init to root node
