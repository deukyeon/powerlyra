 ...
 * See the paper: Nonparametric Belief Propagation. E. Sudderth, A. Ihler, W.
 * Freeman, and A. Willsky. CVPR, June 2003.
 */
...
double image_compare_mae(image& trueimg, image& infered) {
  ...
  while (curi != colors.end()) {
    int low = (previval + (*curi)) / 2;
    if (low < 0) low = 0;
    int high = (nextival + (*curi)) / 2;
  ...
  for (size_t i = 0; i < infered.rows(); ++i) {
    for (size_t j = 0; j < infered.cols(); ++j) {
      // err  += (infered.pixel(i,j) - trueimg.pixel(i,j)) * (infered.pixel(i,j)
      // - trueimg.pixel(i,j)) ;
...
double image_compare_rmse(image& trueimg, image& infered) {
  ...
  for (size_t i = 0; i < infered.rows(); ++i) {
    for (size_t j = 0; j < infered.cols(); ++j) {
      err += (infered.pixel(i, j) - trueimg.pixel(i, j)) *
 ...
 */
void nbp_update(gl_types::iscope& scope, gl_types::icallback& scheduler) {
  bool debug = true;
  ...
  if (v_data.rounds == MAX_ITERATIONS) {
    if (debug && vid % 100000 == 0) printf("computing belief node %d\n", vid);
...
void construct_graph(image& img, kde& edge_pot, gl_types::graph& graph) {
  for (size_t i = 0; i < img.rows(); ++i) {
    for (size_t j = 0; j < img.cols(); ++j) {
      ...
      vdat.bel = vdat.obs;
      if (i == 0 && j == 0) vdat.obs.matlab_print();
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("epsilon", &EPSILON, EPSILON,
                       "epsilon - product accuracy");
  clopts.attach_option("gmmfile", &gmmfile, std::string(""),
                       "true image + self and edge potential file");
  clopts.attach_option("inputfile", &inputfile, std::string(""),
                       "the input noisy image");
  clopts.attach_option(
      "max_iter", &MAX_ITERATIONS, MAX_ITERATIONS,
      "maximum number of iterations. In this round the belief is compted");
  ...
  f >> Name("edge_ce") >> integermat;
  edgecenter = to_mat(integermat);
  f >> Name("edge_alpha") >> doublevec;
  edgeweight = doublevec;
  f >> Name("edge_sigma") >> doublevec;
  edgesigma = doublevec;

  // read self potential
  f >> Name("like_ce") >> nodecenter;
  f >> Name("like_alpha") >> doublevec;
  nodeweight = doublevec;
  f >> Name("like_sigma") >> doublevec;
  nodesigma = doublevec;
