...
struct dd_global_vars {
  double old_dual;  // stores the value of dual objective for the previous
                    // iteration
  double
      primal_best;  //  stores the value of best primal objective found so far.
  bool converged;   // true if dual objective value has converged to required
                    // tolerance level, otherwise false
  int dual_inc_count;  // keeps track of the number of times the value of dual
                       // objective increased
  vector<vector<double> > history;  // stores dual and primal objective values
  int sq_norm_g;          //  stores the value of the square of the norm of the
                          //  subgradient vector
...
struct vertex_data {
  ...
  int degree;       // Degree of this factor (same as nvars for higher-order
                    // factors).
  ...
  int best_configuration;  // Index of the best configuration at a subgradient
                           // step.
                           // TODO: Maybe replace best_configuration by beliefs
                           // for the high order variables? In which case,
                           // beliefs would be vector<vec> beliefs.

  double dual_contrib;    // Contribution of this factor to the dual. We can
                          // compute this during the applies.
  double primal_contrib;  // Contribution of this factor to the primal. We can
                          // compute this during the applies. NOTENOTE: ONLY
                          // true for sync engine. For async, we need to write
                          // an aggregator function.
  double dual_res_contrib;  // Contribution of this factor to the dual residual.
                            // (Used only for ADMM)
  double primal_res_contrib;  // Contribution of this factor to the primal
                              // residual. (Used only for ADMM)
  double primal_rel_contrib;  // Contribution of this factor to the relaxed
                              // primal. (Used only for ADMM)

  vec beliefs;  // Posterior values for the configurations after averaging
                // (projected DD, unary variables only).
  vec factor_beliefs;  // Posterior value for factor variables
  int apply_count;     // No. of times apply has been called on this vertex
  int sum_sq_norm_g;   // Sum of square of norm of subgradient for each vertex
                       // (used only for factor vertices)

  bool schedule_vertex;  // Decides if vertex is to be scheduled for further
                         // iterations or not
  ...
  vertex_data()
      : nvars(0),
        ...
        primal_contrib(0),
        dual_res_contrib(0),
        primal_res_contrib(0),
        primal_rel_contrib(0),
        sum_sq_norm_g(0),
        apply_count(0),
        schedule_vertex(true) {}

  void load(graphlab::iarchive& arc) {
    arc >> nvars >> degree >> cards >> neighbors >> potentials >>
        dual_contrib >> primal_contrib >> best_configuration >> beliefs >>
        apply_count >> factor_beliefs >> sum_sq_norm_g >> primal_res_contrib >>
        primal_rel_contrib >> dual_res_contrib >> schedule_vertex >>
        factor_type >> budget >> bound_states >> unary_degree >> active_set_ >>
        inverse_A_ >> last_sort_;
  }
  void save(graphlab::oarchive& arc) const {
    arc << nvars << degree << cards << neighbors << potentials << dual_contrib
        << primal_contrib << best_configuration << beliefs << apply_count
        << factor_beliefs << sum_sq_norm_g << primal_res_contrib
        << primal_rel_contrib << dual_res_contrib << schedule_vertex
        << factor_type << budget << bound_states << unary_degree << active_set_
        << inverse_A_ << last_sort_;
  }
...
/**
 * \brief gather_type is a structure that will be used as the return type of
 * gather function. It includes messages (used both for unary and factor
 * vertices), neighbor_best_conf, neighbor_distribution (used only for factor
 * vertices) and sq_norm_g (for storing square of norm of subgradient) for each
 * edge.
 */
...
struct gather_type {
  ...
  gather_type(factor_type f, vector<int> nc = vector<int>(), int sg = 0,
      ...
      : messages(f),
        neighbor_conf(nc),
        neighbor_distribution(nd),
        sq_norm_g(sg){};
  void load(graphlab::iarchive& arc) {
    arc >> messages >> neighbor_conf >> sq_norm_g >> neighbor_distribution >>
        multipliers;
  }
  void save(graphlab::oarchive& arc) const {
    arc << messages << neighbor_conf << sq_norm_g << neighbor_distribution
...
/**
 * \brief objective is a structure that is used as the summable data type for
 * computing dual, primal objectives, residuals etc with aggregator map and
 * reduce functions.
 */

struct objective {
  double primal, dual, primal_rel, sum_sq_norm_g, total_confs, dual_res,
      primal_res;
  ...
  void load(graphlab::iarchive& arc) {
    arc >> dual >> primal >> sum_sq_norm_g >> primal_rel >> total_confs >>
        dual_res >> primal_res;
  }
  void save(graphlab::oarchive& arc) const {
    arc << dual << primal << sum_sq_norm_g << primal_rel << total_confs
...
struct dd_vertex_program
  ...
  void get_configuration_states(const graph_type::vertex_type& vertex,
    ...
    const vertex_data& vdata = vertex.data();
    int tmp = 1;
    for (size_t i = 1; i < states->size(); ++i) tmp *= vdata.cards[i];
  ...
  double update_stepsize(icontext_type& context, int type, double old_dual,
                         double primal_best, int norm_g_sq, int dual_inc_count,
                         int iter_since_aggregate) const {
    switch (type) {
      case 0:
      ...
      case 2:
        return (
            2 * opts.step_size * (old_dual - primal_best) /
            ((norm_g_sq + 1) * (iter_since_aggregate + dual_inc_count + 1)));
        break;
      case 3:
        return (opts.step_size / dual_inc_count + 1);
        break;
      case 4:
        if (context.iteration() + 2 < 300)
          return (opts.step_size / (context.iteration() + 2));
        else
          return (opts.step_size / (300));
    }
  }
...
struct dd_vertex_program_symmetric : public dd_vertex_program {
   ...
   * \brief The gather function takes a vertex and an edge as inputs and outputs
   ...
   we can just return the vector of Lagrange multipliers stored in
   "edge.messages". Otherwise (if vertex is a general factor), things are a
   little more tricky. Suppose the factor is linked to K variables, with
   cardinalities C_1, ..., C_K. Suppose this edge is with respect to the k-th
   variable. Then, we return a vector of size C_1 + ... + C_K which is zero
   everywhere except in the k-th slot, where the Lagrange multipliers in
   "edge.messages" will be copied to. This way, when the "gather sum" takes
   place, and since all these slots are disjoint, we will just get the Lagrange
   multipliers of all the variables. It also gathers the best_configuration of
   neighbors for the factors and norm of subgradient value for each edge.
   */
  gather_type gather(icontext_type& context, const vertex_type& vertex,
                     edge_type& edge) const {
    if (opts.verbose > 1) cout << "gather begin" << endl;
    ...
    if (vdata.nvars == 1) {
      ...
      if (opts.verbose > 1)
        cout << "This unary factor has " << vertex.num_in_edges()
             << " in edges and " << vertex.num_out_edges() << " out edges"
             << endl;
      ...
      int sq_norm_g =
          (states[index_neighbor] == other_vertex.data().best_configuration)
              ? 0
              : 2;
    ...
    if (opts.verbose > 2) cout << "gather end" << endl;
   ...
   * \brief The apply function takes a vertex and a vector of numeric values
   ...
   the argmax. For general factors, the vector of numeric values, as stated
   above, will contain all the Lagrange multipliers of the neighboring
   variables. So we need to loop through all possible factor configurations, get
   the sequence of states of each configuration, fetch the Lagrange multipliers
   for those states, and add them to the factor potential. Then we compute the
   argmax. It also computes dual and primal contribution for finding dual and
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (!opts.debug) {
      ...
      if (vdata.nvars == 1) {
        ...
        for (int index_configuration = 0;
             index_configuration < num_configurations; ++index_configuration) {
          vector<int> states(vdata.nvars, -1);
          // This could be made more efficient by defining an iterator over
    ...
    if (opts.verbose > 1) cout << "end apply" << endl;
  };  // end of apply
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    if (!opts.debug) {
      ...
      if (opts.verbose > 1) cout << "begin scatter" << endl;

      int iter_since_aggregate =
          (context.iteration() + 2) - global_vars.iter_at_aggregate;
      double stepsize =
          update_stepsize(context, 1, global_vars.old_dual,
                          global_vars.primal_best, global_vars.sq_norm_g,
                          global_vars.dual_inc_count, iter_since_aggregate);
      ...
      get_configuration_states(*factor_vertex, other_vdata.best_configuration,
                               &states);
      int offset = 0;
      int index_neighbor = -1;
      ...
      // if (opts.verbose > 1)
      if (opts.verbose > 1) {
        cout << "Scatter on (" << unary_vertex->id() << ","
             << factor_vertex->id() << ") called from " << vertex.id() << "\n";
        cout << "unary best config = " << vdata.best_configuration << "\n"
             << "factor best config = " << states[index_neighbor] << "\n";
        cout << "Message: " << edata.multiplier_messages << "\n---\n";
      }
    }
    if (opts.verbose > 1) cout << "end scatter" << endl;

    // Signalling the other vertex and yourself to start.
    if ((context.iteration() + 1) < opts.maxiter &&
        global_vars.converged == false) {
...
struct dd_vertex_program_projected : public dd_vertex_program {
   ...
   * \brief The gather function takes a vertex and an edge as inputs and outputs
   ...
   If the vertex is a unary factor, compute the sum of all the local MAP
   variables, which in the "apply" function will serve to compute the global
   MAP. Otherwise (if vertex is a general factor), things are a little more
   tricky. Suppose the factor is linked to K variables, with cardinalities C_1,
   ..., C_K. Suppose this edge is with respect to the k-th variable. Then, we
   return a vector of size C_1 + ... + C_K which is zero everywhere except in
   the k-th slot, where the Lagrange multipliers in "edge.messages" will be
   copied to. This way, when the "gather sum" takes place, and since all these
   slots are disjoint, we will just get the Lagrange multipliers of all the
   variables.
   */
  gather_type gather(icontext_type& context, const vertex_type& vertex,
                     edge_type& edge) const {
    ...
    if (vdata.factor_type == VAR) {  // Unary factor.
      if (opts.verbose > 1) {
        cout << "This unary factor has " << vertex.num_in_edges()
             << " in edges and " << vertex.num_out_edges() << " out edges"
             << endl;
      ...
      neighbor_conf[index_neighbor] = other_vertex.data().best_configuration;
      for (int state = 0; state < vdata.cards[index_neighbor]; ++state) {
        messages[offset + state] =
            edata.multiplier_messages[state] + edata.potentials[state];
      }
      gather_type gather_data(messages, neighbor_conf);
      return gather_data;
    }
      ...
      if (vdata.factor_type == VAR) {
        ...
        if (context.iteration() % 2 == 0) {
          ...
          for (int index_configuration = 0;
            ...
            int offset = 0;
            for (int k = 0; k < vdata.nvars; ++k) {
              beliefs[index_configuration] +=
                  total.messages[offset + states[k]];
              offset += vdata.cards[k];
            }
          }
          // Save the best configuration for this factor and find dual contrib
  ...
  /**
   * \brief The scatter function takes a vertex and an edge as input.
   (1) If the vertex is a unary factor, we update the messages (Lagrange
   multipliers) by subtracting the global MAP by the local MAP. (2) If the
   vertex is a higher order factor, this function will take the best
    ...
    if (!opts.debug) {
      if (vdata.factor_type == VAR) {
        if (context.iteration() % 2 != 0) {
          ...
          double stepsize = update_stepsize(
              context, 1, global_vars.old_dual, global_vars.primal_best,
              global_vars.sq_norm_g, global_vars.dual_inc_count, 0);

          edata.multiplier_messages +=
              (vdata.beliefs - edata.local_messages) * stepsize;
        }
      } else {
        if (context.iteration() % 2 == 0) {
          ...
          // CHECK_EQ(vdata.cards[index_neighbor],
          // unary_vertex.data().cards[0]);
...
///////////////////////////////////////////////////////////////////////////////////
// This class implements the general Alternating Directions Method of
// Multipliers.
//
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    if (vdata.factor_type == VAR) {
      ...
      for (int state = 0; state < vdata.cards[index_neighbor]; ++state) {
        ...
        neighbor_distribution[offset + state] =
            other_vertex.data().beliefs[state];
      }
      ...
      messages[index_neighbor] =
          edata.multiplier_messages[0] +
          edata.potentials[vdata.bound_states[index_neighbor]];

      neighbor_distribution[index_neighbor] =
          other_vertex.data().beliefs[vdata.bound_states[index_neighbor]];
      ...
      if (vdata.factor_type == VAR) {
        if (context.iteration() % 2 != 0) {
          ...
          for (int i = 0; i < vdata.cards[0]; i++) {
            dual_res_contrib[i] = (total.messages[i] /
                                   static_cast<double>(vdata.unary_degree[i])) -
                                  vdata.beliefs[i];
            dual_res_contrib[i] = dual_res_contrib[i] * dual_res_contrib[i];
            // update global MAP
            vdata.beliefs[i] =
                total.messages[i] / static_cast<double>(vdata.unary_degree[i]);
          }
        ...
        if (context.iteration() % 2 == 0) {
          ...
          // Find dual contrib
          SolveMAP(vertex, total, variable_posteriors, additional_posteriors,
                   vdata.dual_contrib);
          // Find relaxed primal contribution
          if (vdata.factor_type == DENSE) {
            vdata.primal_rel_contrib =
                vdata.potentials.dot(vdata.factor_beliefs);
          ...
          for (int i = 0; i < vdata.beliefs.size(); i++) {
            primal_res_contrib[i] =
                primal_res_contrib[i] * primal_res_contrib[i];
          }
          ...
          // cout<<vertex.id()<<" "<<vdata.factor_type<<"
          // "<<vdata.beliefs<<endl;
          // Find primal contrib
          if (vdata.factor_type == DENSE) {
            int conf_index =
                get_configuration_index(vertex, total.neighbor_conf);
            vdata.primal_contrib = vdata.potentials[conf_index];
          }
        }
      }
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    if (!opts.debug) {
      ...
      if (vdata.factor_type == VAR) {
        if (context.iteration() % 2 != 0) {
          double stepsize = update_stepsize(
              context, 0, global_vars.old_dual, global_vars.primal_best,
              global_vars.sq_norm_g, global_vars.dual_inc_count, 0);
          if (other_vertex.data().factor_type == BUDGET) {
            ...
            edata.multiplier_messages[0] +=
                (vdata.beliefs[other_vertex.data()
                                   .bound_states[index_neighbor]] -
                 edata.local_messages[other_vertex.data()
                                          .bound_states[index_neighbor]]) *
                stepsize;
...
objective sum(dd_vertex_program::icontext_type& context,
              const dd_vertex_program::vertex_type& vertex) {
  objective retval;
  ...
  retval.total_confs =
      (vertex.data().nvars == 1)
          ? (std::accumulate(vertex.data().unary_degree.begin(),
                             vertex.data().unary_degree.end(), 0))
          : 0;
...
void print_obj(dd_vertex_program::icontext_type& context,
               objective total) {  // TODO make this function part of
                                   // dd_vertex_program class
  if (context.iteration() % 2 == 0 || opts.algorithm == 0) {
    ...
    if (opts.verbose > 0) {
      cout << "iteration: " << context.iteration()
           << " Dual Objective: " << total.dual << " "
      ...
      if (opts.algorithm == 2) {
        ...
        cout << "Dual Residual:" << sqrt(total.dual_res / total.total_confs)
             << " "
             << "Primal Residual:" << sqrt(total.primal_res / total.total_confs)
             << endl;
      }
    }
    ...
    if ((std::fabs(total.dual - global_vars.old_dual) < opts.dualimprovthres) &&
      ...
      cout << "Dual Objective: " << total.dual << " "
           << "Primal Objective: " << total.primal << endl;
      cout << " Number of iteration at convergence:" << context.iteration() + 2
           << endl;
    }
    ...
    if ((sqrt(total.dual_res / total.total_confs) < opts.dualimprovthres &&
        ...
        (std::fabs(total.dual - global_vars.old_dual) < opts.dualimprovthres) &&
        opts.algorithm == 2) {
      global_vars.converged = true;
      cout << "Dual Objective: " << total.dual << " "
           << "Primal Objective: " << total.primal << endl;
