...
class sparse_index {
 ...
 public:
  ...
  explicit sparse_index(size_t num_vars) : _num_vars(num_vars) {
    DCHECK_LE(_num_vars, MAX_DIM);
    for (size_t i = 0; i < _num_vars; ++i) _asg[i] = 0;
  }
  sparse_index(size_t const* const begin, size_t const* const end) {
    _num_vars = end - begin;
    DCHECK_LE(_num_vars, MAX_DIM);
    for (size_t i = 0; i < _num_vars; ++i) _asg[i] = begin[i];
  }
  explicit sparse_index(std::vector<size_t> asg) : _num_vars(asg.size()) {
    DCHECK_LE(_num_vars, MAX_DIM);
    for (size_t i = 0; i < _num_vars; ++i) _asg[i] = asg[i];
  }
 ...
 public:
  ...
  bool operator<(const sparse_index& other) const {
    ...
    for (size_t i = 0; i < _num_vars; ++i) {
      if (_asg[i] > other._asg[i])
        return false;
      else if (_asg[i] < other._asg[i])
        return true;
    }
    return false;
  }
  ...
  friend std::ostream& operator<<(std::ostream& out, const sparse_index& sa) {
