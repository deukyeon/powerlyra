...
class sparse_table : public table_base<MAX_DIM> {
 ...
 private:
  ...
  typedef std::vector<std::pair<const sparse_index_t*, double*> >
      compact_view_t;
  typedef std::vector<std::pair<const sparse_index_t*, const double*> >
      compact_const_view_t;
 ...
 public:
  ...
  sparse_table(const domain_t& dom) { set_domain(dom); }
  ...
  sparse_table(const std::vector<variable_t>& vars,
    ...
    // create a faux domain with the size of the dimensions ordered correctly.
    // this is essentially a permute operation.
    domain_t dom;
 ...
 public:
  ...
  sparse_table& copy_onto(const sparse_table& other) {
    ...
    for (; entry != end; ++entry, ++oentry) {
      // ensure the two assignments are equivilent (std::map should sort them
      // similarly)
  ...
  const domain_t& domain() const { return args(); }
 ...
 public:
   ...
   */
  // NOTE the assignment is not removed from the domain if val is
  // APPROX_LOG_ZERO. in the future, if these values are removed, it could
  // invalidate any iterator over the list of sparse assignments.
 ...
 private:
  ...
  template <class Func>
  inline sparse_table& for_each_assignment(const dense_table_t& other,
                                           const Func& f) {
  ...
  sparse_table& for_each_assignment(const sparse_table& other, const Func& f) {
    ...
    if (args() == other.args()) {
      ...
      typename sparse_data_t::iterator it = _dataAtAsg.begin();
      typename sparse_data_t::const_iterator end = _dataAtAsg.end();
      typename sparse_data_t::const_iterator other_it =
          other._dataAtAsg.begin();
    ...
    // else, broadcast the sub-domain across the full domain
      ...
      // Loop over x
      // NOTE the assignments are sorted the same. ie. our assignments share the
      // same ordering over the sub-domain spaned by msg as the assignments in
      // msg.
      typename compact_const_view_t::const_iterator x_fastasg =
          other_compact_view.begin();
      typename compact_const_view_t::const_iterator x_end =
          other_compact_view.end();
  ...
  */
  ...
  virtual void times(const table_base_t& base, table_base_t& out_base) const {
    // ensure we are dealing with a sparse_table
    sparse_table* const out = dynamic_cast<sparse_table* const>(&out_base);
    if (out == NULL) {
      std::cout << "ERROR: std::bad_cast" << std::endl;
      // REVIEW should probably raise an exception
      ASSERT_TRUE(false);
  ...
  //! (out(x) = this(x)) / other(x);
  virtual void divide(const table_base_t& base, table_base_t& out_base) const {
    // ensure we are dealing with a sparse_table
    sparse_table* const out = dynamic_cast<sparse_table* const>(&out_base);
    if (out == NULL) {
      std::cout << "ERROR: std::bad_cast" << std::endl;
  ...
  friend std::ostream& operator<<(std::ostream& out,
    ...
    for (; val != end; ++val) {
      out << "\tLogP({" << val->first << "}=" << tbl.linear_index(val->first)
          << ")=" << val->second << std::endl;
    }
    out << "}";
    return out;
 ...
 private:
  ...
  struct less_than_by_index {
    ...
    inline bool operator()(const std::pair<sparse_index_t, double>& a,
  ...
  void permute(T& data) const {
    ...
    for (size_t i = 0; i < args().num_vars(); ++i) sorting_inds.push_back(i);
  ...
  sparse_index_t restrict(const sparse_index_t& asg,
    ...
    size_t index = 0;
    // Map the variables
    for (size_t i = 0; i < args().num_vars() && index < sub_domain.num_vars();
         ++i) {
  ...
  // O(n)
  // from
  // http://stackoverflow.com/questions/3772664/intersection-of-two-stl-maps and
  // http://stackoverflow.com/questions/1773526/in-place-c-set-intersection
  // REVIEW is there any problem with invalidated iterators?
  template <typename KeyType, typename LeftValue, typename RightValue>
  void map_left_intersection(std::map<KeyType, LeftValue>& left,
