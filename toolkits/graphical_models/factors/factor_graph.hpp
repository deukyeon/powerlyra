...
class factor_graph {
 ...
 public:
  ...
  variable_type add_variable(const size_t n_labels,
    ...
    //    if(it != _var_map.end()) {
    //      std::cout << "WARNING: variable already exists for that name" <<
    //      std::endl; return it->second;
    //    }
    ...
    logstream(LOG_INFO) << "var_id=" << id << " description='" << vert.name
                        << "'" << std::endl;
  ...
  const dense_table_t& prior_for_variable(const variable_type& var) const {
    ...
    dense_table_t const* const potential =
        dynamic_cast<dense_table_t const* const>(
            factors()[var.id()].potential.table());
    DCHECK_NE(potential, NULL);
  ...
  void set_prior_for_variable(const variable_type& var,
    ...
    factors()[var.id()].potential =
        factor_type(factor_type::DENSE_TABLE, table);
  }
 ...
 public:
  ...
  void add_factor(const table_base_t& factor,
    ...
    for (size_t i = 0; i < factor.ndims(); ++i) {
      logstream(LOG_INFO) << "  factor.var(" << i
                          << ").id()=" << factor.var(i).id() << std::endl;
  ...
  // utils
 public:
  size_t num_factors() const { return factors().size(); }

  // FIXME O(n)
  size_t num_variables() const {
    size_t ndims = 0;
    for (typename std::vector<vertex_data_t>::const_iterator factor =
             factors().begin();
  ...
  void save_graph_summary(const std::string& filename) {
    ...
    // Iterate all the factors and all the edges. NOTE all variables are also
    // factors
    typename std::vector<vertex_data_t>::const_iterator factor;
    size_t factor_idx = 0;
    for (factor = factors().begin(); factor != factors().end();
         ++factor_idx, ++factor) {
  ...
  void make_bp_graph(graph_type_t& graph, double bound, double damping,
    ...
    if (graph.dc().procid() == 0) {
      // Add all the factors and all the edges. NOTE all variables are also
      // factors
      ...
      for (; factor != end; ++factor_idx, ++factor) {
        ...
        for (size_t i = 0; i < factor->potential.table()->ndims(); ++i) {
          ...
          msg.zero();  // using shift normalization, this is equivalent to
                       // uniform()
          graph.add_edge(factor_idx, variable.id(), edge_data_t(msg));
        }
      }
      graph.dc().cout() << "Loading graph. Finished in " << timer.current_time()
                        << std::endl;
    }
    timer.start();
    graph.finalize();
    ...
    graph.dc().cout() << "================ "
                      << "Graph statistics on proc " << graph.dc().procid()
                      << " of " << graph.dc().numprocs() << " ==============="
 ...
 private:
  struct aggregate_vertex_data {
    ...
    aggregate_vertex_data operator()(
        const typename graph_type_t::vertex_type& vertex) {
      const vertex_data_t& ovdata = vertex.data();

      aggregate_vertex_data out;
      // variables are always dense and 1D, although not all 1D vertices are
      // variables...
 ...
 public:
  void print_variable(const variable_type& var,
                      const std::vector<double>& labels,
                      std::ostream& out = std::cout) {
    const dense_table_t& table = belief_for_variable(var);
    DCHECK_EQ(table.size(), labels.size());

    // std::ios::fmtflags f(out.flags()); // i cant believe this is how you do
    // this

    out << "var_" << var.id() << ": " << factors()[var.id()].name << std::endl;
    out << std::setw(8) << "index" << std::setw(16) << "logP" << std::setw(16)
        << "label" << std::endl;
    size_t end = table.size();
    for (size_t i = 0; i < end; ++i) {
      out << std::setw(8) << i << std::setw(16) << table.logP(i)
