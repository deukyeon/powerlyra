...
struct clopts_vals {
  clopts_vals(double bound = 1E-4, double damping = 0.3,
              std::string exec_t = "sync")
...
int main(int argc, char** argv) {
  ...
  cv::Mat_<uchar> image =
      cv::imread("noisy_img.png", 0);  // force to grayscale with '0'
  ...
  for (unsigned i = 0; i < ni; ++i) {
    for (unsigned j = 0; j < nj; ++j) {
      ...
      for (domain_t::const_iterator asg = prior.domain().begin(); asg != end;
           ++asg) {
  ...
  for (unsigned i = 0; i < ni; ++i) {
    for (unsigned j = 0; j < nj; ++j) {
      if (j != 0) {
        ...
        for (domain_t::const_iterator asg = factor.domain().begin(); asg != end;
             ++asg) {
  ...
  if (dc.procid() == 0) {
    ...
    cv::Mat_<uchar> gm =
        cv::imread("denoised_gm.png", 0);  // force to grayscale with '0'
...
int setup_cli(graphlab::command_line_options& clopts, clopts_vals& clvals,
              int argc, char** argv) {
  clopts.attach_option("bound", clvals.BOUND, "Residual termination bound");
...
void run_engine(graphlab::distributed_control& dc,
  ...
  std::cout << "Finished Running engine in " << runtime << " seconds with "
            << clopts.get_ncpus() << " cpus." << std::endl
