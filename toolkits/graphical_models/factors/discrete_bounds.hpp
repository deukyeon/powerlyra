...
class discrete_bounds {
 public:
  ...
  discrete_bounds(const discrete_variable& v1) : _num_vars(1) {
  ...
  explicit discrete_bounds(const std::set<discrete_variable>& vars)
    ...
    DCHECK_LE(_num_vars, MAX_DIM);
    size_t i = 0;
    foreach (const discrete_variable& var, vars)
      _vars[i++] = var;
  }

  discrete_bounds(const discrete_bounds& other) : _num_vars(other._num_vars) {
    *this = other;
  }
  ...
  discrete_bounds& operator+=(const discrete_bounds& other) {
    ...
    _num_vars = 0;
    for (size_t i = 0, j = 0; i < backup.num_vars() || j < other.num_vars();) {
      DCHECK_LE(_num_vars, MAX_DIM);
      // Both
  ...
  // get the index within our domain of each variable in other.
  // other must be a subset
  std::vector<size_t> vars_location(
      const std::vector<discrete_variable>& other) const {
    // ensure that the other domain is a subset of our domain
