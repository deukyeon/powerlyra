...
class dense_table_impl : public table_base<MAX_DIM> {
 public:
  ...
  dense_table_impl(const domain_type& dom) : _args(dom), _data(dom.size()) {}
 ...
 public:
  ...
  void set_domain(const domain_type& args) { set_args(args); }

  inline const domain_type& args() const { return _args; }
  inline const domain_type& domain() const { return args(); }
 ...
 public:
  ...
  void uniform() { std::fill(_data.begin(), _data.end(), log(1.0 / size())); }

  void uniform(double value) { std::fill(_data.begin(), _data.end(), value); }
 ...
 public:
  ...
  void convolve(const dense_table_impl& joint, const dense_table_impl& other) {
  ...
  void condition(const dense_table_impl& other, const assignment_type& asg) {
  ...
  size_t max_index() const { return max_asg().linear_index(); }
  ...
  void set_as_agreement(const double& lambda) {
    ...
    for (; asg != end; ++asg) {
      ...
      if (diff > 0) {
        set_logP(asg->linear_index(), -lambda);
      } else {
        set_logP(asg->linear_index(), 0);
      }
    }
 ...
 private:
  //! The domain of the table (the arity of the table, along with its
  //! cardinality)
  domain_type _args;
...
class dense_table : public dense_table_impl<MAX_DIM> {
  ...
  // "using typename" parses but doesn't work in GCC < 4.7. Use "typedef
  // typename" instead.
 ...
 public:
  ...
  /** Construct a dense table over the given domain */
  dense_table(const domain_type& dom) : dense_table_impl_t(dom) {}
  ...
  /** Construct a unary dense table over the given var */
  dense_table(const variable_type& var, double const* const begin,
              double const* const end)
      : dense_table_impl_t(var, begin, end) {}

  /** Construct a copy */
  dense_table(const dense_table& other) : dense_table_impl_t(other) {}
  ...
  */
  ...
  virtual void times(const table_base_t& base, table_base_t& out_base) const {
    // ensure we are dealing with a dense_table
    dense_table* const out = dynamic_cast<dense_table* const>(&out_base);
    if (out == NULL) {
      std::cout << "ERROR: std::bad_cast" << std::endl;
      // REVIEW should probably raise an exception
      ASSERT_TRUE(false);
 ...
 public:
  virtual void load(graphlab::iarchive& arc) { dense_table_impl_t::load(arc); }
