...
class discrete_domain : public discrete_bounds<MAX_DIM> {
 ...
 public:
  ...
  discrete_domain() : bounds_type() {}

  //! Make a single variable discrete_domain
  discrete_domain(const discrete_variable& v1) : bounds_type(v1) {}
  ...
  discrete_domain(const discrete_domain& other) : bounds_type(other) {}

  discrete_domain(const bounds_type& other) : bounds_type(other) {}
  ...
  class Iterator
   ...
   private:
    ...
    typedef std::iterator<std::forward_iterator_tag, assignment_type>
        iterator_t;
    // "using typename" doesn't actually work in GCC < 4.7, which we don't have
    // installed everywhere. The "typedef typename" construct seems to, so stick
    // with that for now.
   ...
   public:
    ...
    bool operator==(const Iterator& other) const { return _asg == other._asg; }
    bool operator!=(const Iterator& other) const { return !(*this == other); }
    reference_custom operator*() { return _asg; }
  ...
  class ConstIterator
    ...
    typedef std::iterator<std::forward_iterator_tag, const assignment_type>
        const_iterator_t;
   ...
   public:
    ...
    reference_custom operator*() const { return _asg; }
    pointer_custom operator->() const { return &_asg; }
