...
class factor_product {
 ...
 public:
  ...
  //   optimized out
  factor_product(const factor_type& factor = factor_type()) : factor(factor) {}
...
class bp_vertex_program
  ...
  typedef graphlab::ivertex_program<typename graph_type<MAX_DIM>::type,
                                    ...
                                    graphlab::messages::sum_priority>
      ivertex_program_t;
  // NOTE there is a bug in GCC < 4.7 which prevents these using declarations
  // from compiling (http://gcc.gnu.org/bugzilla/show_bug.cgi?id=14258)
  ...
  typedef graphlab::table_factor<MAX_DIM>
      factor_type;  // vertex_data_t::factor_type
 ...
 public:
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    logstream(LOG_DEBUG)
        << "in bp_vertex_program::gather(): compute message to '" << vdata.name
        << "' from vertex '" << ovdata.name << "'" << std::endl;
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    // vdata.belief.table()->copy_onto(*(vdata.potential.table())); // should be
    // faster...
    vdata.belief *= total.factor;
    logstream(LOG_EVERYTHING)
        << "vdata.potential=" << vdata.potential << std::endl;
    logstream(LOG_EVERYTHING) << "total.factor=" << total.factor << std::endl;
    logstream(LOG_EVERYTHING)
        << "vdata.belief=vdata.potential * total.factor = " << vdata.belief
        << std::endl;
    if (vdata.isVariable == true) {
      logstream(LOG_INFO) << "belief-prop variable state = '" << vdata.name
    ...
    // logstream(LOG_DEBUG) << "vdata.shift_normalized=" << vdata.belief <<
    // std::endl;
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    edge_data_t& edata = edge.data();
    logstream(LOG_DEBUG)
        << "in bp_vertex_program::scatter(): compute message from '"
        << vdata.name << "' to vertex '" << ovdata.name << "'" << std::endl;

    // construct the cavity
    // factor_type cavity = vdata.belief;
    factor_type& cavity =
        init_factor(vdata.belief).factor;  // initilizes the cache factor
    logstream(LOG_EVERYTHING) << "factor=" << cavity << std::endl;
    const msg_type& incoming_message =
        edata.old_message(other_vertex.id(), vertex.id());
    cavity /= incoming_message;
    logstream(LOG_DEBUG) << "incoming_message=" << incoming_message
                         << std::endl;
    ...
    msg_type& new_message = edata.message(vertex.id(), other_vertex.id());
    ...
    if (vdata.REGULARIZATION > 0.0) {
      ...
      //  regularize
      logstream(LOG_DEBUG) << "regularization_value=" << vdata.REGULARIZATION
                           << std::endl;
    ...
    // dec: The literature seems to indicate that the l_inf norm is better, so
    // lets try that.
    ...
    logstream(LOG_INFO)
        << "belief-prop message from '" << vdata.name << "' to vertex '"
        << ovdata.name << "':"
        << " raw=" << new_message  // NOTE newlines here can break atomicity...
        << " damped=" << raw_message << std::endl;
 ...
 private:
  ...
  // their domains is null (cf. dense_table::operator*() =>
  // dense_table::logP(asg) => discrete_assignment::restrict() fails), so i
  // repmat the message to cover the domain
 ...
 private:
  // keeping a cache here has very good storage requirements as opposed to in
  // vector_data. NOTE no need to serialize.
