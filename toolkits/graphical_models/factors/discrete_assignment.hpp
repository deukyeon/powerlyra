...
class discrete_assignment {
 ...
 public:
  ...
  explicit discrete_assignment(const domain_type& args)
      : _args(args), _index(0) {
    for (size_t i = 0; i < args.num_vars(); ++i) _asgs[i] = 0;
  }
  ...
  discrete_assignment(const domain_type& args, const std::vector<size_t>& asg)
      : _args(args), _index(0) {
  ...
  inline discrete_assignment operator&(const discrete_assignment& other) const {
    ...
    while (i < num_vars() && j < other.num_vars()) {
      ...
      result._asgs[k] =
          (result.args().var(k) == args().var(i)) ? asg_at(i) : other.asg_at(j);
    ...
    while (i < num_vars()) result._asgs[k++] = asg_at(i++);
    while (j < other.num_vars()) result._asgs[k++] = other.asg_at(j++);
  ...
  size_t asg(const variable_type& var) const { return asg(var.id()); }
 ...
 public:
  ...
  //! Restrict the discrete_assignment to a discrete_assignment over the
  //! sub-domain
  discrete_assignment restrict(const domain_type& sub_domain) const {
    ...
    for (size_t i = 0; i < _args.num_vars() && index < sub_domain.num_vars();
         ++i) {
  ...
  void update(const discrete_assignment& other) {
    ...
    // NOTE this depends on the list of variables in both domains being sorted
    for (size_t i = 0, j = 0; i < num_vars() && j < other.num_vars();) {
      if (_args.var(i) == other._args.var(j)) {
        _asgs[i] = other._asgs[j];
        i++;
        j++;
      }
      while (i < num_vars() && _args.var(i) < other.args().var(j)) i++;
      while (j < other.num_vars() && other.args().var(j) < _args.var(i)) j++;
    }
    recompute_linear_index();
  }
