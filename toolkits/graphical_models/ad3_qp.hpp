...
struct ad3_vertex_program : public admm_vertex_program {
  /**
   * \brief Maximize returns the maximum value and configuration with reference
   * to input additional and variable log potentials. addtional log potential
   * corresponds factor potentials and variable potential corresponds to sum of
   * lagrange multipliers and unary potentials divided by degree of the unary
   * vertex.
   */

  void Maximize(vertex_type& vertex, vec additional_log_potentials,
                vec variable_log_potentials, Configuration& configuration,
                double* value) {
  ...
  /**
   * \brief InvertAfterInsertion function is used to invert the matrix. Used in
   * solveQP
   */
  ...
  bool InvertAfterInsertion(vertex_type& vertex, vector<double>& inverse_A_,
    ...
    for (int i = 0; i < active_set.size(); ++i) {
      ...
      int num_common_values =
          CountCommonValues(vertex, active_set[i], inserted_element);
      r[i + 1] = static_cast<double>(num_common_values);
    }

    double r0 = static_cast<double>(
        CountCommonValues(vertex, inserted_element, inserted_element));
    double s = r0;
  ...
  /**
   * \brief InvertAfterRemoval function is used to invert the matrix. Used in
   * solveQP
   */
  void InvertAfterRemoval(vector<double>& inverse_A_,
                          const vector<Configuration>& active_set,
                          int removed_index) {
    vector<double> inverse_A = inverse_A_;
    int size_A = active_set.size() + 1;
  ...
  void ComputeActiveSetSimilarities(vertex_type& vertex,
    ...
    for (int i = 0; i < active_set.size(); ++i) {
      ...
      for (int j = i + 1; j < active_set.size(); ++j) {
        ...
        int num_common_values =
            CountCommonValues(vertex, active_set[i], active_set[j]);
  ...
  /**
   * \brief  ComputeMarginalsFromSparseDistribution computes marginalvalues for
   * unary factor from given factor distribution.
   */

  void ComputeMarginalsFromSparseDistribution(
      vertex_type& vertex, const vector<Configuration>& active_set,
      const vector<double>& distribution, vec& variable_posteriors,
      vec& additional_posteriors) {
    variable_posteriors.setZero();
    additional_posteriors.setZero();
    for (int i = 0; i < active_set.size(); ++i) {
      UpdateMarginalsFromConfiguration(vertex, active_set[i], distribution[i],
                                       variable_posteriors,
  ...
  void UpdateMarginalsFromConfiguration(vertex_type& vertex,
                                        ...
                                        double weight, vec& variable_posteriors,
                                        vec& additional_posteriors) {
    vector<Configuration> states(vertex.data().nvars, -1);
   ...
   * log potential corresponds factor potentials and variable potential
   * corresponds to sum of lagrange  * multipliers and unary potentials divided
   * by degree of the unary vertex.
   */

  void Evaluate(vertex_type& vertex, vec additional_log_potentials,
                vec variable_log_potentials, const Configuration configuration,
                double* value) {
    vector<Configuration> states(vertex.data().nvars, -1);
    get_configuration_states(vertex, configuration, &states);
    *value = 0.0;
  ...
  void SolveQP_dense(vertex_type& vertex, const gather_type& total,
    ...
    for (int iter = 0; iter < num_max_iterations; ++iter) {
      ...
      if (same_as_before) {
        ...
        // stored in z.
        ComputeMarginalsFromSparseDistribution(vertex, vdata.active_set_, z,
                                               variable_posteriors,
                                               additional_posteriors);
        ...
        Maximize(vertex, additional_log_potentials, scores, configuration,
                 &value);
        double very_small_threshold = 1e-9;
        if (value <= tau + very_small_threshold) {  // value <= tau.
          // We have found the solution;
          // the distribution, active set, and inv(A) are cached for the next
          // round.
          ...
          for (int k = 0; k < vdata.active_set_.size(); ++k) {
            ...
            if (vdata.active_set_[k] == configuration) {
              if (opts.verbose > 2) {
                cout << "Warning: value - tau = " << value - tau << " " << value
                     << " " << tau << endl;
              }
              // We have found the solution;
              ...
              for (int j = 0; j < vdata.active_set_.size(); ++j) {
                if (j == k)
                  continue;  // This configuration was deleted already.
          ...
          // Update inv(A).
          bool singular = !InvertAfterInsertion(
              vertex, vdata.inverse_A_, vdata.active_set_, configuration);
          if (singular) {
            ...
            vector<double> similarities(vdata.active_set_.size() *
                                        vdata.active_set_.size());
            ComputeActiveSetSimilarities(vertex, vdata.active_set_,
                                         &similarities);

            // cout<<"compute active similarities in solveQP .."<<endl;
            vector<double> padded_similarities(
                (vdata.active_set_.size() + 2) * (vdata.active_set_.size() + 2),
                1.0);
            for (int i = 0; i < vdata.active_set_.size(); ++i) {
              for (int j = 0; j < vdata.active_set_.size(); ++j) {
            ...
            for (int i = 0; i < vdata.active_set_.size(); ++i) {
              double value = static_cast<double>(CountCommonValues(
                  vertex, configuration, vdata.active_set_[i]));
              padded_similarities[(i + 1) * (vdata.active_set_.size() + 2) +
                                  (vdata.active_set_.size() + 1)] = value;
              padded_similarities[(vdata.active_set_.size() + 1) *
                                      (vdata.active_set_.size() + 2) +
                                  (i + 1)] = value;
            }
            ...
            for (int i = 0; i < vdata.active_set_.size() + 2; ++i) {
              if (NEARLY_EQ_TOL(eigenvalues[i], 0.0, 1e-9)) {
                if (zero_eigenvalue >= 0) {
                  // If this happens, something failed. Maybe a numerical
                  // problem may cause this. In that case, just give up, clean
                  // the cache and return. Hopefully the next iteration will fix
                  // it.
            ...
            singular = !InvertAfterInsertion(vertex, vdata.inverse_A_,
                                             vdata.active_set_, configuration);
            assert(!singular);
          }

          // Insert configuration to active set.
          if (opts.verbose > 2) {
            cout << "Inserted one element to the active set (iteration " << iter
                 << ")." << endl;
          }
          vdata.active_set_.push_back(configuration);
        ...
        if (!exist_blocking) {
          ...
          // Remove blocking constraint from the active set.
          if (opts.verbose > 2) {
            cout << "Removed one element to the active set (iteration " << iter
                 << ")." << endl;
          }

          // DeleteConfiguration(active_set_[blocking]); // Delete
          // configutation.
  ...
  int project_onto_budget_constraint_cached(vec& x, int d, double budget,
  ...
  void SolveQP_budget(vertex_type& vertex, const gather_type& total,
    ...
    if (s > static_cast<double>(vdata.budget)) {
      ...
      project_onto_budget_constraint_cached(
          variable_posteriors, variable_log_potentials.size(),
  ...
  void SolveMAP_dense(vertex_type& vertex, const gather_type& total,
    ...
    for (int index_configuration = 0; index_configuration < num_configurations;
         ++index_configuration) {
  ...
  void SolveQP_xor(vertex_type& vertex, const gather_type& total,
    ...
    for (int f = 0; f < variable_posteriors.size(); ++f) {
      variable_posteriors[f] =
          total.neighbor_distribution[f] + total.messages[f];
      // cout<<vdata.potentials[f]<<" "<<total.neighbor_distribution[f]<<"
      // "<<total.messages[f]<<endl;
    }
    // cout<<"v "<<variable_posteriors;
    // cout<<endl;
    project_onto_simplex_cached(variable_posteriors, vdata.potentials.size(),
                                1.0, vdata.last_sort_);
  ...
  void compute_beliefs(vertex_type& vertex, const gather_type& total,
    ...
    switch (vertex.data().factor_type) {
      case DENSE:
        SolveQP_dense(vertex, total, variable_posteriors,
                      additional_posteriors);
        break;
      case BUDGET:
        SolveQP_budget(vertex, total, variable_posteriors,
                       additional_posteriors);
        break;

      case XOR:
        SolveQP_xor(vertex, total, variable_posteriors, additional_posteriors);
    }
  };
  ...
  void SolveMAP(vertex_type& vertex, const gather_type& total,
    ...
    switch (vertex.data().factor_type) {
      case XOR:
        SolveMAP_xor(vertex, total, variable_posteriors, additional_posteriors,
                     value);
        break;
      case DENSE:
        SolveMAP_dense(vertex, total, variable_posteriors,
                       additional_posteriors, value);
        break;
      case BUDGET:
        SolveMAP_budget(vertex, total, variable_posteriors,
                        additional_posteriors, value);
    }
  };
};
...
struct bethe_admm_vertex_program : public admm_vertex_program {
  ...
  void compute_grad_phi(vertex_type& vertex, vec& unary_beliefs,
                        vec& factor_beliefs, vec& unary_grad,
                        vec& factor_grad) {
    vertex_data& vdata = vertex.data();
  ...
  void run_bp(vertex_type& vertex, vec& unary_pots, vec& factor_pots,
    ...
    for (int i = 0; i < vdata.nvars; i++) {
      ...
      for (int j = 0; j < factor_pots.size(); j++) {
        ...
        int offset = 0;
        for (int k = 0; k < vdata.nvars; k++) {
          if (k != i) messages[j] *= unary_pots[offset + states[k]];
          offset += vdata.cards[k];
        }
      }
      ...
      for (int j = 0; j < marg_messages.size(); j++) {
        unary_margs[var_offset + j] =
            marg_messages[j] * unary_pots[var_offset + j];
  ...
  void adjust_beliefs(vertex_type& vertex) {
    ...
    for (int i = 0; i < vdata.beliefs.size(); i++) {
      if (vdata.beliefs[i] < 10e-100) vdata.beliefs[i] = 10e-100;
    }
  ...
  void compute_beliefs(vertex_type& vertex, const gather_type& total,
    ...
    unary_eta = total.messages +
                opts.step_size * (total.neighbor_distribution - vdata.beliefs);
    ...
    compute_grad_phi(vertex, vdata.beliefs, vdata.factor_beliefs, unary_eta,
                     factor_eta);
    // running bp on eta
    switch (vdata.factor_type) {
      case DENSE:
        run_bp(vertex, unary_eta, factor_eta, vdata.beliefs,
               vdata.factor_beliefs);
        break;
      case XOR:
        softmax(vertex, unary_eta, vdata.beliefs);
    }
