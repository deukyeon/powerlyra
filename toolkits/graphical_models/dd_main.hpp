...
void loadUAIfile(graphlab::distributed_control& dc, graph_type& graph,
  ...
  in >> name;
  // CHECK(name.compare("MARKOV")==0, "Only Markov networks are supported. Are
  // you sure this is a typeUAI energy file?");
  ...
  in >> nnodes;
  nodes = nnodes;
  // CHECK(nnodes>0, "No. of nodes can't be negative. Are you sure this is a
  // typeUAI energy file?");
  CHECK(nnodes > 0);
  ...
  for (int i = 0; i != nnodes; ++i) {
    ...
    // CHECK(in.good(), "Could not finish reading cardinalities. Are you sure
    // this is a typeUAI energy file?");
    CHECK(in.good());
  }
  ...
  vector<vector<int> > factor_memb;
  factor_memb.resize(nfactors);
  int temp1, temp2;
  ...
  if (opts.verbose > 1)
    cout << "Finished Reading UAI-Preamble:"
         << " #Nodes = " << nnodes << ", #Factors = " << nfactors
         << ", Average Cardinality = "
         << double(sum_of_cardinalities) / nfactors << "\n";

  // Now read factor potentials
  for (int i = 0; i != nfactors; ++i) {
    int cardprod;
    double potential_value;  //, energy;
    in >> cardprod;
    ...
    for (int j = 0; j != factor_size[i]; ++j) {
      ...
      vdata.neighbors[j] =
          factor_memb[i][j];  // afm (check if this was intended!)
    ...
    // CHECK(in.good(), "Could not finish reading factor tables. Are you sure
    // this is a typeUAI energy file?");
    ...
    if (i % dc.numprocs() != dc.procid()) continue;

    // If all is well, add vertex and edge
    graph.add_vertex(vid, vdata);

    if (factor_size[i] > 1 ||
        opts.algorithm > 0)  // if not a unary, add edges to unaries
      for (int j = 0; j != factor_size[i]; ++j)
    ...
    if (opts.verbose > 1) {
      cout << "Machine #" << dc.procid() << ", Vertex Id = " << i << " with "
           << vdata.nvars << " variables.";
      if (factor_size[i] > 1) {
        cout << ", Edges = ";
        for (int j = 0; j != factor_size[i]; ++j)
          cout << ", (" << i << "," << varid[j] << ")";
...
void dist_unary_potentials(graph_type::edge_type& edge) {
  vertex_data& vdata = (edge.source().data().factor_type == VAR)
                           ? edge.source().data()
                           : edge.target().data();
...
struct gather_potentials {
  ...
  void load(graphlab::iarchive& arc) { arc >> degree >> potentials; }
  void save(graphlab::oarchive& arc) const { arc << degree << potentials; }
...
struct compute_degree_budget
  ...
  gather_potentials gather(icontext_type& context, const vertex_type& vertex,
                           edge_type& edge) const {
    const vertex_data& vdata = vertex.data();
    const vertex_type& other_vertex =
    ...
    if (vdata.factor_type == VAR) {
      ...
      switch (other_vertex.data().factor_type) {
        case XOR:
          potentials = other_vertex.data().potentials;
    ...
    if (vdata.factor_type == VAR) {
      if (other_vertex.data().factor_type != BUDGET) {
        for (int i = 0; i < vdata.potentials.size(); i++) {
          edge.data().potentials[i] =
              vdata.potentials[i] / vdata.unary_degree[i];
        }
        ...
        edge.data().potentials[state_index] =
            vdata.potentials[state_index] / vdata.unary_degree[state_index];
      }
    }
...
void run_dd_symmetric(graphlab::distributed_control& dc, graph_type& graph,
  ...
  // Attach an aggregator to compute primal/dual objective, with periodic
  // interval specified in cmdline argument.
