...
void make_data(const uint16_t rows, const uint16_t cols, const size_t ncolors,
               const double error_rate, const std::string& vdata_fn,
               const std::string& edata_fn, const std::string& orig_img_fn,
  ...
  for (size_t r = 0; r < rows; ++r) {
    for (size_t c = 0; c < cols; ++c) {
      ...
      const uint16_t obs_color =
          graphlab::random::rand01() < error_rate
              ? graphlab::random::fast_uniform<uint16_t>(0, ncolors - 1)
              : true_color;
      ...
      for (size_t pred = 0; pred < ncolors; ++pred) {
        const double prior =
            obs_color == pred ? error_rate : (error_rate) / (ncolors - 1);
        vdata_fout << prior << (pred + 1 < ncolors ? '\t' : '\n');
      }

      // Add the edges
      if (r + 1 < rows) edata_fout << vid << '\t' << sub2ind(r + 1, c) << '\n';
      if (c + 1 < cols) edata_fout << vid << '\t' << sub2ind(r, c + 1) << '\n';
...
void read_data(const std::string& pred_img_fn) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    const bool success = qi::phrase_parse(
        ...
        ascii::space);
    if (!success) {
      logstream(LOG_ERROR) << "Error parsing line: " << line_counter
                           << std::endl
                           << "\t\"" << line << "\"";
    }
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("vdata", vdata_fn, "Vertex prior filename");
  clopts.attach_option("edata", edata_fn, "Adjacency information");
  ...
  clopts.attach_option("orig", orig_img_fn, "Original image file name.");
  clopts.attach_option("noisy", noisy_img_fn, "Noisy image file name.");
  clopts.attach_option("pred", pred_img_fn, "Predicted image file name.");
  ...
  if (!pred_img_fn.empty()) {
    ...
    make_data(nrows, ncols, ncolors, error_rate, vdata_fn, edata_fn,
