...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("history", opts.history_file,
                       " for saving objective values");
  ...
  clopts.attach_option("debug_mode", opts.debug,
                       "to activate debug mode set it to true");
  ...
  clopts.attach_option("algorithm", opts.algorithm,
                       "specify type of algorithm: 0 for dd_symmetric, 1 for "
                       "dd_projected, 2 for admm");
  ...
  switch (opts.algorithm) {
    case 1:
      run_dd_projected(dc, graph, opts.exec_type, clopts);
      break;
    case 2:
      run_ad3(dc, graph, opts.exec_type, clopts);
      break;
    case 3:
      run_bethe_admm(dc, graph, opts.exec_type, clopts);
      break;
    default:
      run_dd_symmetric(dc, graph, opts.exec_type, clopts);
  }

  // save predictions
  if (opts.output_dir.find("/", opts.output_dir.size() - 1) ==
      std::string::npos) {
  ...
  // save history
  if (opts.history_file.size() < 4 ||
      opts.history_file.find(".txt", opts.history_file.size() - 4) ==
          std::string::npos) {
  ...
  while (i < global_vars.history[0].size()) {
    file << global_vars.history[0][i] << " " << global_vars.history[1][i] << " "
         << global_vars.history[2][i] << " " << global_vars.history[3][i]
         << endl;
    i++;
  }
  file.close();
