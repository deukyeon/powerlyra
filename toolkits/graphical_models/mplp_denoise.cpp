...
struct vertex_data {
  ...
  // since variables i and j are present in multiple factors, this determines
  // who owns them
  bool iowner, jowner;
  ...
  void save(graphlab::oarchive& arc) const {
    arc << i << j << deg_i << deg_j << obs_color_i << obs_color_j
        << pred_color_i << pred_color_j << maxI << maxJ << maxIJ_i << maxIJ_j
        << vali << valj << valij << pvali << pvalj << pvalij << prvali << prvalj
        << prvalij << iowner << jowner << delf_i << delf_j;
  }
...
struct gather_type {
  ...
  void save(graphlab::oarchive& arc) const { arc << delf_i << delf_j; }
  void load(graphlab::iarchive& arc) { arc >> delf_i >> delf_j; }
};  // end of gather type
...
class mplp_vertex_program
 ...
 public:
  ...
  static void init_vertex_data(icontext_type& context, vertex_type& vertex) {
    ...
    MAPval += vdata.pvalj;
    MAPval += vdata.pvalij;
    MAPrepval += vdata.prvali;
    MAPrepval += vdata.prvalj;
    MAPrepval += vdata.prvalij;
    mutex.unlock();

    // debug code to check in all nodes are inited
    if (vinit[vertex.id()] == 0) vinit[vertex.id()] = 1;
  }
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (vapply[vertex.id()] == 0) vapply[vertex.id()] = 1;
    ...
    vdata.delf_i =
        ...
        (THETA_ij +
         (theta_j + sum.delf_j).transpose().replicate(THETA_ij.rows(), 1))
                .rowwise()
                .maxCoeff() /
            2;
    // Update del fj
    ...
    LPremove += vdata.vali;
    LPremove += vdata.valj;
    LPremove += vdata.valij;
    MAPremove += vdata.pvali;
    MAPremove += vdata.pvalj;
    MAPremove += vdata.pvalij;
    MAPrepremove += vdata.prvali;
    MAPrepremove += vdata.prvalj;
    MAPrepremove += vdata.prvalij;

    // Update dual, primal and rep primal contributions.
    // TODO: if primal labelling changes at a node we own, update it's edge
    // potential too
    if (vdata.iowner) {
    ...
    {
      mutex.lock();
      std::cout << "Applying at vertex: " << vertex.id() << "(" << vdata.i
                << "," << vdata.j << ")\n";
      ...
      for (it = vinit.begin(); it < vinit.end(); ++it) vinitsum += *it;
      for (it = vapply.begin(); it < vapply.end(); ++it) vapplysum += *it;

      // if all vertices have been visited start counting again
      if (vapply.size() == vapplysum)
        for (int i = 0; i != vapply.size(); ++i) vapply[i] = 0;

      std::cout << "Verted Id: " << vertex.id() << " " << vdata.i << ","
                << vdata.j << " ";
    ...
    if (0)  // vdata.i == 1)
    {
      ...
      std::cout << "thetaij + j message\n"
                << (THETA_ij +
                    sum.delf_j.transpose().replicate(THETA_ij.rows(), 1)) /
                       2
                << "\n\n";
      ...
      std::cout << "New del_fi\n"
                << -(theta_i + sum.delf_i) / 2 +
                       (THETA_ij + (theta_j + sum.delf_j)
                                       .transpose()
                                       .replicate(THETA_ij.rows(), 1))
                               .rowwise()
                               .maxCoeff() /
                           2
                << "\n\n";
      ...
      std::cout << "New del_fj\n"
                << -(theta_j + sum.delf_j) / 2 +
                       ((THETA_ij +
                         (theta_i + sum.delf_i).replicate(1, THETA_ij.cols()))
                            .colwise()
                            .maxCoeff())
                               .transpose() /
                           2
 ...
 private:
  ...
  static vector make_unary_potential(const vertex_type& vertex,
    ...
    const double obs =
        varid == 'i' ? vertex.data().obs_color_i : vertex.data().obs_color_j;
...
// typedef graphlab::asynchronous_consistent_engine<mplp_vertex_program>
// engine_type;

/////////////////////////////////////////////////////////////////////////////////////
// Aggregator functions to compute primal & dual values
double get_energy_fun(mplp_vertex_program::icontext_type& context,
                      const mplp_vertex_program::vertex_type& vertex) {
  double tmp = 0;

  const vertex_data& vdata = vertex.data();

  if (vdata.iowner) tmp += vdata.vali;
  if (vdata.jowner) tmp += vdata.valj;
  tmp += vdata.valij;
...
void finalize_fun(mplp_vertex_program::icontext_type& context, double total) {
  if (context.procid() == 0) std::cout << "Dual value: " << total << std::endl;
}

// Helper functions ===========================================================>
graphlab::vertex_id_type pixel_ind(size_t rows, size_t cols, size_t r,
                                   size_t c) {
  return r * cols + c;
};  // end of pixel_ind

graphlab::vertex_id_type factor_ind(size_t rows, size_t cols, size_t i,
                                    size_t j) {
  if (i > j) std::swap(i, j);
  return i * (rows * cols) + j;
};  // end of factor_ind

graphlab::vertex_id_type factor_ind2(size_t rows, size_t cols, size_t i,
                                     size_t j) {
  if (i > j) std::swap(i, j);
...
void create_synthetic_cluster_graph(graphlab::distributed_control& dc,
                                    graph_type& graph, const size_t rows,
                                    const size_t cols) {
  dc.barrier();
  ...
  for (size_t r = 0; r < rows; ++r) {
    for (size_t c = 0; c < cols; ++c) {
      ...
      true_pixels[pixel] = true_color;
      obs_pixels[pixel] = obs_color;
  ...
  if (dc.procid() == 0) {
    ...
    vinit.clear();
    vinit.resize(nedges, 0);
    vapply.clear();
    vapply.resize(nedges, 0);
    ...
    for (size_t r = 0; r < rows; ++r) {
      for (size_t c = 0; c < cols; ++c) {
        ...
        //                for(int pred = 0; pred < potential.size(); ++pred)
        //                    potential[pred] /= sum;
        //                ne << true_pixels[pixel_ind(rows,cols,r,c)] << " " <<
        //                obs << " " << potential << std::endl; ne << 4 -
        //                int((r==0)||(r==(rows-1))) -
        //                int((c==0)||(c==(cols-1))) << " " << potential <<
        //                std::endl;
        ...
        if (r + 1 < rows) {
          ...
          vdata.deg_i = 4 - int((r == 0) || (r == (rows - 1))) -
                        int((c == 0) || (c == (cols - 1)));
          vdata.deg_j = 4 - int(((r + 1) == 0) || ((r + 1) == (rows - 1))) -
                        int((c == 0) || (c == (cols - 1)));
          ...
          // temp code
          max_vid =
              std::max((int)factor_ind2(rows, cols, vdata.i, vdata.j), max_vid);
        }
        if (c + 1 < cols) {
          vertex_data vdata;
          vdata.i = pixel_ind(rows, cols, r, c);
          ...
          vdata.deg_i = 4 - int((r == 0) || (r == (rows - 1))) -
                        int((c == 0) || (c == (cols - 1)));
          vdata.deg_j = 4 - int((r == 0) || (r == (rows - 1))) -
                        int(((c + 1) == 0) || ((c + 1) == (cols - 1)));
          vdata.obs_color_i = obs_pixels[vdata.i];
          vdata.obs_color_j = obs_pixels[vdata.j];
          ...
          ++ownercount;
          if ((c + 1) ==
              (cols - 1))  // and j-ownership too if last node in this row
          {
            vdata.jowner = true;
            ++ownercount;
          }
        ...
        nbrs.clear();
        if (r + 1 < rows)
          nbrs.push_back(factor_ind2(rows, cols, pixel_ind(rows, cols, r, c),
...
void initialize_theta_ij(const std::string& smoothing, const double lambda) {
...
merge_reduce_type pred_map_function(graph_type::vertex_type vertex) {
  ...
  ret.values.insert(
      pred_pair_type(vertex.data().i, vertex.data().pred_color_i));
  ret.values.insert(
      pred_pair_type(vertex.data().j, vertex.data().pred_color_j));
  return ret;
...
std::pair<int, int> ind2sub(size_t rows, size_t cols, size_t ind) {
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("bound", BOUND, "Residual termination bound");
  clopts.attach_option("ncolors", NCOLORS,
                       "The number of colors in the noisy image");
  clopts.attach_option("sigma", SIGMA, "Standard deviation of noise.");
  clopts.attach_option("nrows", nrows, "The number of rows in the noisy image");
  ...
  clopts.attach_option("smoothing", smoothing, "Options are {square, laplace}");
  clopts.attach_option("orig", orig_fn, "Original image file name.");
  clopts.attach_option("noisy", noisy_fn, "Noisy image file name.");
  clopts.attach_option("pred", pred_fn, "Predicted image file name.");
