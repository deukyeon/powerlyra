...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("nfiles", nfiles, "The number of files to generate.");
  clopts.attach_option("D", D, "Number of latent dimensions.");
  clopts.attach_option("nusers", nusers, "The number of users.");
  clopts.attach_option("nmovies", nmovies, "The number of movies.");
  clopts.attach_option("alpha", alpha, "The power-law constant.");
  clopts.attach_option("nvalidate", nvalidate,
                       "The validate ratings pers user");
  clopts.attach_option("npredict", npredict, "The predict ratings pers user");
  ...
  if (!boost::filesystem::create_directory(output_folder)) {
    logstream(LOG_ERROR) << "Error creating directory: " << directory
                         << std::endl;
    return EXIT_FAILURE;
  }
  ...
  for (size_t i = 0; i < nfiles; ++i) {
    ...
    if (!train_files[i]->good()) {
      logstream(LOG_ERROR) << "Error creating file: " << train_fname;
    }
  ...
  foreach (vec_type& factor, user_factors) {
    ...
    for (size_t d = 0; d < D; ++d) factor(d) = gen.gaussian(0, stdev);
  }
  ...
  for (size_t movie_id = 0, user_id = 0; movie_id < nmovies; ++movie_id) {
    ...
    for (size_t i = 0; i < out_degree; ++i) {
      ...
      const double rating = user_factors[user_id].dot(movie_factors[movie_id]);
      *(train_files[file_id])
  ...
  for (size_t i = 0; i < nfiles; ++i) {
    train_files[i]->close();
    delete train_files[i];
    train_files[i] = NULL;
    validate_files[i]->close();
    delete validate_files[i];
    validate_files[i] = NULL;
    predict_files[i]->close();
    delete predict_files[i];
    predict_files[i] = NULL;
  }
  std::cout << "Created " << nedges_train << " training edges." << std::endl;
