...
std::size_t hash_value(vertex_data const& b) { return b.nupdates; }
...
void vertex_delta(graph_type::vertex_type& vtx, const vec_type& delta) {
  if (delta.sum() != 0) vtx.data().pvec.array() *= delta.array() / px->array();
  for (uint i = 0; i < vertex_data::NLATENT; i++)
    if (vtx.data().pvec[i] < epsilon) vtx.data().pvec[i] = epsilon;
}
...
void nmf_function(engine_type::context_type& context,
  ...
  vec_type delta;
  delta = (phase == PHASE1 ? edge.target().data().pvec
                           : edge.source().data().pvec) *
          edge.data().obs / pred;
  context.send_delta(VERTEX_DELTA_TASK_ID,
                     phase == PHASE1 ? edge.source() : edge.target(), delta);
}
...
vec_type count_edges(const graph_type::edge_type& edge) {
  ...
  if (edge.data().obs < 0)
    logstream(LOG_FATAL) << "Found a negative entry in matirx row "
                         << edge.source().id()
                         << " with value: " << edge.data().obs << std::endl;
  return ret;
}

void verify_rows(graph_type::vertex_type& vertex) {
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("iterations", ITERATIONS, "number of NMF iterations");
  ...
  dc.cout() << "Training edges: " << edge_count[0]
            << " validation edges: " << edge_count[1] << std::endl;
