...
mat a, PT;
bool v_vector = false;
int input_file_offset = 0;  // if set to non zero, each row/col id will be
                            // reduced the input_file_offset
...
struct vertex_data {
  ...
  void save(graphlab::oarchive& arc) const { arc << pvec << A_ii; }
  /** \brief Load the vertex data from a binary archive */
  void load(graphlab::iarchive& arc) { arc >> pvec >> A_ii; }
};  // end of vertex data
...
inline graph_type::vertex_type get_other_vertex(
    graph_type::edge_type& edge, const graph_type::vertex_type& vertex) {
  ...
  std::string save_edge(const edge_type& edge) const {
    if (edge.data().role == edge_data::PREDICT) {
      ...
      const double prediction =
          edge.source().data().pvec.head(nconv).transpose() * diagonal_matrix *
          edge.target().data().pvec.head(nconv);
...
struct linear_model_saver_U {
  ...
  */
  std::string save_vertex(const vertex_type& vertex) const {
    if (vertex.id() < rows) {
      std::string ret =
          boost::lexical_cast<std::string>(vertex.id() + input_file_offset) +
          " ";
  ...
  std::string save_edge(const edge_type& edge) const { return ""; }
};
...
struct linear_model_saver_V {
  ...
  */
  std::string save_vertex(const vertex_type& vertex) const {
    if (vertex.id() >= rows) {
      std::string ret = boost::lexical_cast<std::string>(vertex.id() - rows +
                                                         input_file_offset) +
                        " ";
      for (uint i = 0; i < nconv; i++)
        ret += boost::lexical_cast<std::string>(vertex.data().pvec[i]) + " ";
 ...
 */
inline bool graph_loader(graph_type& graph, const std::string& filename,
                         const std::string& line) {
  // no need to parse
  if (filename == vecfile) return true;
  if (boost::ends_with(filename, "singular_values") ||
      boost::ends_with(filename, "_v0"))
    return true;
  if (line.find("#") != std::string::npos) return true;

  // Determine the role of the data
  edge_data::data_role_type role = edge_data::TRAIN;
  if (boost::ends_with(filename, ".predict")) role = edge_data::PREDICT;
  ...
  if (source_id == graph_type::vertex_id_type(-1) ||
      target_id == graph_type::vertex_id_type(-1)) {
    logstream(LOG_WARNING) << "Failed to read input line: " << line
                           << " in file: " << filename
                           << " (or node id is -1). " << std::endl;
    return true;
  }
  ...
  if (source_id > (uint)rows)
    logstream(LOG_FATAL) << "Problem at input line: [ " << line
                         << " ] row id ( = " << source_id + input_file_offset
                         << " ) should be <= than matrix rows (= " << rows
                         << " ) " << std::endl;
  // if (target_id >= (uint)cols)
  if (target_id > (uint)cols)
    logstream(LOG_FATAL) << "Problem at input line: [ " << line
                         << " ] col id ( = " << target_id + input_file_offset
                         << " ) should be <= than matrix cols (= " << cols
                         << " ) " << std::endl;

  if (!binary) strm >> obs;
  if (!info.is_square()) target_id = rows + target_id;
...
void init_lanczos(graph_type* g, bipartite_graph_descriptor& info) {
  ...
  if (info.is_square()) actual_vector_len = 2 * data_size;
  ...
  logstream(LOG_INFO) << "Allocated a total of: "
                      << ((double)actual_vector_len * g->num_vertices() *
                          sizeof(double) / 1e6)
                      << " MB for storing vectors." << std::endl;
}

void swork_vec(graph_type::vertex_type& vertex) {
  vertex.data().pvec[nconv + kk] = 0;
  for (int ttt = nconv; ttt < nconv + n; ttt++) {
    vertex.data().pvec[nconv + kk] +=
        curvec(ttt - nconv) * vertex.data().pvec[ttt];
  }
}

void compute_ritz(graph_type::vertex_type& vertex) {
  if (!info.is_square()) assert(vertex.id() - pcurrent->start >= 0);
  ...
  if (info.is_square() && !v_vector) offset += data_size;
  ...
  if (debug)
    std::cout << "Entered ritz with " << offset << " , v_vector: " << v_vector
              << "data_size: " << data_size << " kk: " << kk << std::endl;
}
...
void lanczos(bipartite_graph_descriptor& info, timer& mytimer, vec& errest,
  ...
  int its = 1;
  DistMat A(info);
  DistSlicedMat U(info.is_square() ? data_size : 0,
                  info.is_square() ? 2 * data_size : data_size, true, info,
                  "U");
  ...
  if (vecfile.size() == 0) v_0 = randu(size(A, 2));
  ...
  while (nconv < nsv && its < max_iter) {
    logstream(LOG_EMPH) << "Starting iteration: " << its
                        << " at time: " << mytimer.current_time() << std::endl;
    int k = nconv;
    n = nv;
    PRINT_INT(k);
    PRINT_INT(n);
    ...
    for (int i = k + 1; i < n; i++) {
      logstream(LOG_EMPH) << "Starting step: " << i
                          << " at time: " << mytimer.current_time()
                          << std::endl;
      PRINT_INT(i);
    ...
    mat T = diag(alpha);
    for (int i = 0; i < n - 1; i++) set_val(T, i, i + 1, beta(i));
    PRINT_MAT2("T", T);
  ...
  if (save_vectors) {
    if (nconv == 0)
      logstream(LOG_FATAL)
          << "No converged vectors. Aborting the save operation" << std::endl;
    if (predictions == "")
      logstream(LOG_FATAL) << "Please specify prediction output fie name using "
                              "the --predictions=filename command"
                           << std::endl;

    BEGIN_TRACEPOINT(svd_vectors);
    std::cout << "Saving singular value triplets to files: " << predictions
              << ".U.* and " << predictions << ".V.*" << std::endl;
    const bool gzip_output = false;
    const bool save_vertices = false;
    const bool save_edges = true;
    const size_t threads_per_machine = 1;
    pgraph->save(predictions + ".U", linear_model_saver_U(), gzip_output,
                 save_edges, save_vertices, threads_per_machine);
    pgraph->save(predictions + ".V", linear_model_saver_V(), gzip_output,
                 save_edges, save_vertices, threads_per_machine);
  ...
  if (!predictions.empty()) {
    ...
    pgraph->save(predictions, prediction_saver(), gzip_output, save_vertices,
...
void write_output_vector(const std::string datafile, const vec& output,
                         bool issparse, std::string comment) {
  FILE* f = fopen(datafile.c_str(), "w");
  if (f == NULL)
    logstream(LOG_FATAL) << "Failed to open file: " << datafile
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option(
      "ortho_repeats", ortho_repeats,
      "orthogonalization iterations. 1 = low accuracy but fast, 2 = medium "
      "accuracy, 3 = high accuracy but slow.");
  clopts.attach_option("nv", nv, "Number of vectors in each iteration");
  clopts.attach_option("nsv", nsv,
                       "Number of requested singular values to comptue");
  clopts.attach_option("tol", tol, "convergence threshold");
  clopts.attach_option("save_vectors", save_vectors,
                       "save output matrices U and V.");
  ...
  clopts.attach_option("binary", binary,
                       "If true, all edges are weighted as one");
  clopts.attach_option("input_file_offset", input_file_offset,
                       "input file node id offset (default 0)");
  ...
  if (rows <= 0 || cols <= 0)
    logstream(LOG_FATAL)
        << "Please specify number of rows/cols of the input matrix"
        << std::endl;

  if (rows == cols) {
    logstream(LOG_WARNING) << "GraphLab SVD does not support square matrices. "
                              "Increasing row size by one."
                           << std::endl;
    rows++;
  }
  info.rows = rows;
  ...
  if (nv < nsv) {
    logstream(LOG_FATAL)
        << "Please set the number of vectors --nv=XX, to be at least the "
           "number of support vectors --nsv=XX or larger"
        << std::endl;
  }
  ...
  dc.cout() << "========== Graph statistics on proc " << dc.procid()
            << " ==============="
            ...
            << (float)graph.num_local_vertices() /
                   graph.num_local_own_vertices()
            << "\n Num local edges: "
            << graph.num_local_edges()
            //<< "\n Begin edge id: " << graph.global_eid(0)
            << "\n Edge balance ratio: "
            << float(graph.num_local_edges()) / graph.num_edges() << std::endl;
  ...
  dc.cout() << "Please send bug reports to danny.bickson@gmail.com"
            << std::endl;
  timer.start();
  ...
  if (graphlab::mpi_tools::rank() == 0)
    write_output_vector(predictions + ".singular_values", singular_values,
                        false,
                        "%GraphLab SVD Solver library. This file contains the "
                        "singular values.");
  ...
  if (unittest == 1) {
    ...
    for (int i = 0; i < errest.size(); i++) assert(errest[i] < 1e-30);
  } else if (unittest == 2) {
    assert(errest.size() == 10);
    for (int i = 0; i < errest.size(); i++) assert(errest[i] < 1e-15);
  } else if (unittest == 4) {
