 ...
 * Advertising in Microsoft’s Bing Search Engine Thore Graepel, Joaquin
 * Quinonero Candela, Thomas Borchert, and Ralf Herbrich ICML 2010 Implemented
 * by Danny Bickson, GraphLab, Inc.
 *
 */
...
double v(double t) {
  ...
  if (std::isinf(ret)) {
    std::cout << "BUG: " << ret << " " << t << " " << exp(-t * t / 2)
              << " phi(t)" << phi(t) << std::endl;
    assert(false);
  }
  return ret;
}
...
float ctr_predict(const vertex_data& data, const float rating,
                  double& prediction, void* extra = NULL) {
  assert(beta > 0);
...
gather_type adpredictor_map(graph_type::edge_type edge,
...
// map function.
void adpredictor_combine2(gather_type& a, const gather_type& b,
                          const vertex_data unused) {
  a.mu += b.mu;
  a.mult_sigma *= b.mult_sigma;
}

gather_type adpredictor_map2(graph_type::edge_type edge,
                             graph_type::vertex_type other,
                             vertex_data vertex) {
  gather_type ret;
  ...
  assert(edge.data().x_ij == 1);
  double product =
      other.data().y * other.data().xT_mu / sqrt(other.data().sigma);
  // assert(product > 0);
  ret.mu = (other.data().y * edge.data().x_ij * vertex.sigma /
            sqrt(other.data().sigma)) *
           v(product);
  if (std::isinf(ret.mu)) {
    std::cout << "BUG: " << ret.mu << " vertex.sigma " << vertex.sigma
              << " other.data().sigma " << other.data().sigma << " v(prod) "
              << v(product);
    assert(false);
  }
  double factor =
      1.0 - (edge.data().x_ij * vertex.sigma / other.data().sigma) * w(product);
  if (factor <= 0) {
    std::cout << "BUG: " << product << " " << ret.mu << " " << factor
              << std::endl;
    assert(factor > 0);
  }
  ret.sigma = factor;
...
void adpredictor_update(graph_type::vertex_type vertex) {
  ...
  if (vertex.num_out_edges() > 0) {
    if (debug)
      printf("Entered vertex %lu role %d \n", vertex.id(), vertex.data().type);
    if (vertex.data().type == TRAIN) {
      ...
      if (debug)
        std::cout << "Entered item " << vertex.id() << " y: " << row.y
                  << std::endl;
      row.sigma = beta * beta;
      row.xT_mu = 0;

      gather_type sum = graphlab::warp::map_reduce_neighborhood<gather_type>(
          vertex, graphlab::OUT_EDGES, adpredictor_map);
      row.sigma = sum.sigma;
      row.xT_mu = sum.mu;
      ...
      if ((predicted_target == -1 && row.y == 1) ||
          (predicted_target == 1 && row.y == -1))
        row.err += 1.0;
      if (debug)
        std::cout << "Prediction was: " << prediction
                  << " real value: " << row.y << std::endl;
      row.likelihood += ret;

      assert(row.sigma > 0);
    } else if (vertex.data().type == VALIDATE ||
               vertex.data().type == PREDICT) {
      vertex_data& row = vertex.data();
      row.likelihood = 0;
      row.err = 0;
      if (vertex.data().type == VALIDATE) assert(row.y == -1 || row.y == 1);
      gather_type sum = graphlab::warp::map_reduce_neighborhood<gather_type>(
          vertex, graphlab::OUT_EDGES, adpredictor_map);
      row.predict = sum.mu;
      double predict = sum.mu > 0 ? 1 : -1;
      if (predict != row.y && vertex.data().type == VALIDATE) row.err++;
    } else
      assert(false);
  }
}
...
void adpredictor_update2(graph_type::vertex_type vertex) {
  if (vertex.num_in_edges() > 0) {
    gather_type sum = graphlab::warp::map_reduce_neighborhood<gather_type>(
        vertex, graphlab::IN_EDGES, vertex.data(), adpredictor_map2,
        adpredictor_combine2);
...
struct model_saver {
  ...
  std::string save_vertex(const vertex_type& vertex) const {
    if (vertex.num_in_edges() == 0 || vertex.data().type != TRAIN) return "";
...
struct prediction_saver {
   ...
   */
  std::string save_vertex(const vertex_type& vertex) const {
    if (vertex.num_out_edges() == 0 || vertex.data().type != PREDICT) return "";
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  strm >> label;
  if (label != -1 && label != 1)
    logstream(LOG_FATAL) << "Each line must have label -1 or 1 as the first "
                            "item in the row. Row was : "
                         << line << " label: " << label << std::endl;
  ...
  while (strm.good()) {
    ...
    if (weight != 1)
      logstream(LOG_FATAL)
          << "Currently we support only binary edges. Line was: " << line
  ...
  if (num_vals == 0)
    logstream(LOG_FATAL) << "Failed to load line: " << line
                         << " in file: " << filename << std::endl;
...
int main(int argc, char** argv) {
  ...
  const std::string description = "adPredictor algorithm";
  ...
  clopts.attach_option("debug", debug, "debug - additional verbose info");
  ...
  gather_type edge_count =
      graph.map_reduce_vertices<gather_type>(count_vertices);
  dc.cout() << "Training rows: " << edge_count.mu
            << " validation rows: " << edge_count.sigma << std::endl;
  if (edge_count.mu <= 0)
    logstream(LOG_FATAL) << "Failed to read training data. Aborting"
                         << std::endl;
  ...
  for (int i = 0; i < MAX_ITER; ++i) {
    ...
    dc.cout() << i << ") Log likelihood: " << std::setw(10) << ret.sigma
              << " Avg error: " << std::setw(10) << ret.mu / edge_count.mu
              << std::endl;
    mode = VALIDATE;
    ret = graph.map_reduce_vertices<gather_type>(calc_error);
    dc.cout() << i << " Avg validation error: " << std::setw(10)
              << ret.mu / edge_count.sigma << std::endl;
  }
  ...
  if (!save_model.empty()) {
    ...
    graph.save(save_model, model_saver(), gzip_output, save_vertices,
               save_edges, threads_per_machine);
    graph.save(save_model + ".predict", prediction_saver(), gzip_output,
               save_vertices, save_edges, threads_per_machine);
  }
