...
gather_type collect_vec(const graph_type::vertex_type& vertex) {
  ...
  if (vertex.id() >= (uint)pcurrent->start &&
      vertex.id() < (uint)pcurrent->start + len)
...
void print_mat(const char* name, const mat& pmat, bool high = false) {
  ...
  if (pmat2.cols() == 1) pmat2 = pmat2.transpose();
...
#define PRINT_INT(a) \
  if (debug) printf("%s: %d\n", #a, a);
#define PRINT_NAMED_INT(a, b) \
  if (debug) printf("%s: %d\n", a, b);
#define PRINT_DBL(a) \
  if (debug) printf("%s: %.5lg\n", #a, a);
#define PRINT_NAMED_DBL(a, b) \
  if (debug) printf("%s: %.5lg\n", a, b);
