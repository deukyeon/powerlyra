...
// algorithm run modes
enum { SPARSE_USR_FACTOR = 1, SPARSE_ITM_FACTOR = 2, SPARSE_BOTH_FACTORS = 3 };
...
struct vertex_data {
  ...
  void load(graphlab::iarchive& arc) { arc >> nupdates >> residual >> factor; }
};  // end of vertex data
...
class als_vertex_program
 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (algorithm == SPARSE_BOTH_FACTORS ||
        (algorithm == SPARSE_USR_FACTOR && isuser) ||
        (algorithm == SPARSE_ITM_FACTOR && !isuser)) {
      double sparsity_level = 1.0;
      if (isuser)
        sparsity_level -= user_sparsity;
      else
        sparsity_level -= movie_sparsity;
      vdata.factor =
          CoSaMP(XtX, Xy, ceil(sparsity_level * (double)vertex_data::NLATENT),
                 10, 1e-4, vertex_data::NLATENT);
    } else
  ...
  if (role == edge_data::TRAIN || role == edge_data::VALIDATE) {
    if (obs < als_vertex_program::MINVAL || obs > als_vertex_program::MAXVAL)
      logstream(LOG_FATAL) << "Rating values should be between "
                           << als_vertex_program::MINVAL << " and "
                           << als_vertex_program::MAXVAL
                           << ". Got value: " << obs << " [ user: " << source_id
...
double extract_l2_error(const graph_type::edge_type& edge) {
  double pred = edge.source().data().factor.dot(edge.target().data().factor);
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("maxval", als_vertex_program::MAXVAL,
                       "max allowed value");
  clopts.attach_option("minval", als_vertex_program::MINVAL,
                       "min allowed value");
  ...
  clopts.attach_option("user_sparsity", user_sparsity,
                       "sparsity of user factors");
  clopts.attach_option("movie_sparsity", movie_sparsity,
                       "sparsity of item factors");
  clopts.attach_option("algorithm", algorithm,
                       "run mode. 1 = SPARSE_USR_FACTOR, 2 = "
                       "SPARSE_ITM_FACTOR, 3 = SPARSE_BOTH_FACTORS");
  ...
  if (user_sparsity < 0.5 || user_sparsity >= 1)
    logstream(LOG_FATAL) << "Sparsity level should be [0.5,1). Please run "
                            "again using --user_sparsity=XX in this range"
                         << std::endl;

  if (movie_sparsity < 0.5 || movie_sparsity >= 1)
    logstream(LOG_FATAL) << "Sparsity level should be [0.5,1). Please run "
                            "again using --movie_sparsity=XX in this range"
                         << std::endl;

  if (algorithm != SPARSE_USR_FACTOR && algorithm != SPARSE_BOTH_FACTORS &&
      algorithm != SPARSE_ITM_FACTOR)
    logstream(LOG_FATAL)
        << "Algorithm should be 1 for SPARSE_USR_FACTOR, 2 for "
           "SPARSE_ITM_FACTOR and 3 for SPARSE_BOTH_FACTORS"
        << std::endl;
