...
struct math_info {
  ...
  math_info() { reset_offsets(); }

  void reset_offsets() {
    increment = 2;
    c = 1.0;
    d = 0.0;
    x_offset = b_offset = y_offset = r_offset = div_offset = prev_offset =
        mat_offset = vec_offset = -1;
    A_offset = false;
    A_transpose = false;
    use_diag = true;
    start = end = -1;
  ...
  int increment_offset() { return increment++; }
};
 ...
 */
class Axb : public graphlab::ivertex_program<graph_type, double>,
 ...
 public:
  ...
  double gather(icontext_type& context, const vertex_type& vertex,
    ...
    if (edge.data().role == edge_data::PREDICT) return 0;

    bool brows = vertex.id() < (uint)info.get_start_node(false);
    if (info.is_square()) brows = !mi.A_transpose;
    if (mi.A_offset && mi.x_offset >= 0) {
      double val =
          edge.data().obs * (brows ? edge.target().data().pvec[mi.x_offset]
  ...
  void apply(icontext_type& context, vertex_type& vertex, const double& total) {
    ...
    if (mi.orthogonalization) {
      for (int i = mi.mat_offset; i < mi.vec_offset; i++) {
        vertex.data().pvec[mi.vec_offset] -=
            alphas.pvec[i - mi.mat_offset] * vertex.data().pvec[i];
      }
      return;
    }
    ...
    if (mi.prev_offset >= 0) user.pvec[mi.prev_offset] = user.pvec[mi.r_offset];

    assert(mi.x_offset >= 0 || mi.y_offset >= 0);
    if (mi.A_offset && mi.x_offset >= 0) {
      if (info.is_square() && mi.use_diag)  // add the diagonal term
        val +=
            (/*mi.c**/ (user.A_ii + regularization) * user.pvec[mi.x_offset]);
...
void init_math(graph_type* _pgraph, bipartite_graph_descriptor& _info,
               double ortho_repeats = 3, bool update_function = false) {
  pgraph = _pgraph;
  info = _info;
  mi.reset_offsets();
...
class DistVec {
 public:
  ...
  DistVec(const bipartite_graph_descriptor& _info, int _offset, bool _transpose,
  ...
  DistVec(const bipartite_graph_descriptor& _info, int _offset, bool _transpose,
          const std::string& _name, int _prev_offset) {
    offset = _offset;
    display_offset = _offset;
    name = _name;
    info = _info;
  ...
  void debug_print(const char* name) {
    if (debug) {
      ...
      vec pvec = this->to_vec(MAX_PRINT_ITEMS,
                              mi.r_offset == -1 ? offset : mi.r_offset);
      for (int i = 0; i < pvec.size(); i++) {
        // TODO printf("%.5lg ",
        // fabs(pgraph->vertex_data(i).pvec[(mi.r_offset==-1)?offset:mi.r_offset]));
...
class DistSlicedMat {
 public:
  ...
  DistSlicedMat(int _start_offset, int _end_offset, bool _transpose,
                const bipartite_graph_descriptor& _info, std::string _name) {
  ...
  int size(int dim) {
    return (dim == 1) ? (end - start) : (end_offset - start_offset);
  }
...
void assign_vec(graph_type::vertex_type& vertex) {
  if (!info.is_square())
    assert(vertex.id() - pcurrent->start >= 0 &&
           vertex.id() - pcurrent->start < curvec.size());
...
gather_type output_vector(const graph_type::vertex_type& vertex) {
  assert(pcurrent && pcurrent->offset >= 0 &&
         pcurrent->offset < vertex.data().pvec.size());
  gather_type ret;
  assert(pcurrent->end - pcurrent->start > 0);
  assert(vertex.id() - pcurrent->start >= 0);
  ret.pvec = vec::Zero(pcurrent->end - pcurrent->start);
  ret.pvec[vertex.id() - pcurrent->start] =
      vertex.data().pvec[pcurrent->offset];
  return ret;
}
bool select_in_range(const graph_type::vertex_type& vertex) {
  return vertex.id() >= (uint)pcurrent->start &&
         vertex.id() < (uint)pcurrent->end;
}
...
DistVec& DistVec::operator=(const vec& pvec) {
  assert(offset >= 0);
  assert(pvec.size() == info.num_nodes(true) ||
         pvec.size() == info.num_nodes(false));
  assert(start < end);
...
vec DistVec::to_vec(int dmax, int doffset) {
  pcurrent = this;
  if (doffset >= 0) pcurrent->offset = doffset;
  if (dmax >= 0)
  ...
  //    }
  gather_type curvec =
      pgraph->map_reduce_vertices<gather_type>(output_vector, nodes);
  return curvec.pvec;
}
...
class DistDouble {
 public:
  ...
  bool operator<(const double other) { return val < other; }
  DistDouble& operator=(const DistDouble& other) {
    val = other.val;
    debug_print(name);
    return *this;
  }
  bool operator==(const double _val) { return val == _val; }
  void debug_print(const char* name) {
    std::cout << name << " " << val << std::endl;
  }
  double toDouble() { return val; }
  void debug_print(std::string name) { return debug_print(name.c_str()); }
};
...
gather_type map_reduce_ortho(const graph_type::vertex_type& vertex) {
  ...
  for (int i = curMat->start_offset; i < pcurrent->offset; i++) {
    ret.pvec[i - curMat->start_offset] =
        vertex.data().pvec[i] * vertex.data().pvec[pcurrent->offset];
  }
...
void transform_ortho(graph_type::vertex_type& vertex) {
  ...
  for (int i = curMat->start_offset; i < pcurrent->offset; i++) {
    ...
    vertex.data().pvec[pcurrent->offset] -=
        alphas.pvec[i - curMat->start_offset] * vertex.data().pvec[i];
  }
}
...
double orthogonalize_vs_all(DistSlicedMat& mat, int _curoffset, double& alpha) {
  ...
  INITIALIZE_TRACER(orthogonalize_vs_alltrace,
                    "orthogonalization step - optimized");
  ...
  if (curoffset > 0) {
    for (int j = 0; j < mi.ortho_repeats; j++) {
      ...
      alphas =
          pgraph->map_reduce_vertices<gather_type>(map_reduce_ortho, nodes);
  ...
  sum_alpha =
      pgraph->map_reduce_vertices<gather_type>(map_reduce_sum_power, nodes);
