 ...
 * \file
 *
 * Matrix factorization with the Alternative Least Squares (ALS)  - parallel
 * coordinate descent algorithm. See the papers: H.-F. Yu, C.-J. Hsieh, S. Si,
 * I. S. Dhillon, Scalable Coordinate Descent Approaches to Parallel Matrix
 * Factorization for Recommender Systems. IEEE International Conference on Data
 * Mining(ICDM), December 2012. Steffen Rendle, Zeno Gantner, Christoph
 * Freudenthaler, and Lars Schmidt-Thieme. 2011. Fast context-aware
 * recommendations with factorization machines. In Proceedings of the 34th
 * international ACM SIGIR conference on Research and development in Information
 * Retrieval (SIGIR '11). ACM, New York, NY, USA, 635-644. Written by Danny
 * Bickson, CMU
 */
...
struct vertex_data {
  ...
  void save(graphlab::oarchive& arc) const { arc << pvec << t << prev << z; }
  /** \brief Load the vertex data from a binary archive */
  void load(graphlab::iarchive& arc) { arc >> pvec >> t >> prev >> z; }
};  // end of vertex data
...
std::size_t hash_value(edge_data const& b) { return boost::hash_value(b.obs); }
...
class gather_type {
 public:
  ...
  gather_type(double numerator, double denominator)
...
gather_type als_coord_map(const graph_type::vertex_type& center,
  ...
  gather_type ret((edge.data().R_ij + center.data().pvec[center.data().t] *
                                          other.data().pvec[center.data().t]) *
                      other.data().pvec[center.data().t],
...
void als_coord_transform(const graph_type::vertex_type& center,
  ...
  // R_ij     -= (z             - w_it         )*h_jt
  edge.data().R_ij -= (center.data().z - center.data().prev[center.data().t]) *
                      other.data().pvec[center.data().t];
}

// sum up two numerators and denomenators
void als_coord_combine(gather_type& v1, const gather_type& v2) { v1 += v2; }

// the main update function
void als_coord_function(engine_type::context_type& context,
                        graph_type::vertex_type& vertex) {
  double regularization = LAMBDA;
  for (vertex.data().t = 0; vertex.data().t < (int)vertex_data::NLATENT;
       vertex.data().t++) {
    gather_type frac = context.map_reduce<gather_type>(ALS_COORD_MAP_REDUCE,
                                                       graphlab::ALL_EDGES);
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("lambda", LAMBDA, "regularization weight");
  clopts.attach_option("max_iter", max_iter, "number of iterations");
  ...
  engine.register_map_reduce(ALS_COORD_MAP_REDUCE, als_coord_map,
                             als_coord_combine);
