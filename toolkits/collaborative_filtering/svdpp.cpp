    ...
    if (vertex.num_in_edges() == 0) {
      ...
      if (phase == PHASE1) {
        ...
        context.signal(other_vertex,
                       gather_type(vec_type::Zero(vertex_data::NLATENT),
                                   vec_type::Zero(vertex_data::NLATENT), 0));
        return gather_type(vec_type::Zero(vertex_data::NLATENT),
                           other_vertex.data().weight, 0);
      } else if (phase == PHASE2) {
        // vertex_data & my_data = my_vertex.data();
        double pred = svdpp_vertex_program::GLOBAL_MEAN +
                      my_vertex.data().bias + other_vertex.data().bias +
                      my_vertex.data().pvec.dot(other_vertex.data().pvec +
                                                other_vertex.data().weight);
        ...
        if (std::isnan(err))
          logstream(LOG_FATAL)
              << "Got into numeric errors.. try to tune step size and "
                 "regularization using command line flags"
              << std::endl;
        if (edge.data().role == edge_data::TRAIN) {
          ...
          other_delta =
              itmFctrStep * (err * (usrFctr + my_vertex.data().weight) -
                             itmFctrReg * other_vertex.data().pvec);
          ...
          if (other_vertex.data().nupdates < MAX_UPDATES)
            context.signal(other_vertex,
                           gather_type(other_delta, step, other_bias));
        }
...
struct error_aggregator : public graphlab::IS_POD_TYPE {
  ...
  static void finalize(icontext_type& context, const error_aggregator& agg) {
    ...
    assert(!std::isnan(train_error));
    context.cout() << std::setw(8) << context.elapsed_seconds() << std::setw(8)
                   << train_error;
 ...
 */
double extract_l2_error(const graph_type::edge_type& edge) {
  double pred = svdpp_vertex_program::GLOBAL_MEAN + edge.source().data().bias +
  ...
  assert(rmse <=
         pow(svdpp_vertex_program::MAXVAL - svdpp_vertex_program::MINVAL, 2));
  return rmse;
...
struct prediction_saver {
  ...
  std::string save_edge(const edge_type& edge) const {
    ...
    double pred = svdpp_vertex_program::GLOBAL_MEAN +
                  edge.target().data().bias + edge.source().data().bias +
                  edge.source().data().pvec.dot(edge.target().data().pvec +
                                                edge.target().data().weight);
    pred = std::min(pred, svdpp_vertex_program::MAXVAL);
    pred = std::max(pred, svdpp_vertex_program::MINVAL);
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  if (role == edge_data::TRAIN || role == edge_data::VALIDATE) {
    strm >> obs;
    if (obs < svdpp_vertex_program::MINVAL ||
        obs > svdpp_vertex_program::MAXVAL) {
      logstream(LOG_WARNING)
          << "Rating values should be between " << svdpp_vertex_program::MINVAL
          << " and " << svdpp_vertex_program::MAXVAL << ". Got value: " << obs
...
int main(int argc, char** argv) {
  ...
  const std::string description = "Compute the ALS factorization of a matrix.";
  ...
  clopts.attach_option("gamma", svdpp_vertex_program::GAMMA, "SGD step size");
  ...
  clopts.attach_option("maxval", svdpp_vertex_program::MAXVAL,
                       "max allowed value");
  clopts.attach_option("minval", svdpp_vertex_program::MINVAL,
                       "min allowed value");
  clopts.attach_option("step_dec", svdpp_vertex_program::STEP_DEC,
  ...
  svdpp_vertex_program::GLOBAL_MEAN =
      graph.map_reduce_edges<double>(calc_global_mean);
  svdpp_vertex_program::NUM_TRAINING_EDGES =
      graph.map_reduce_edges<size_t>(count_edges);
  svdpp_vertex_program::GLOBAL_MEAN /= svdpp_vertex_program::NUM_TRAINING_EDGES;
  dc.cout() << "Global mean is: " << svdpp_vertex_program::GLOBAL_MEAN
            << std::endl;

  // Signal all vertices on the vertices on the left (libersgd)
  engine.map_reduce_vertices<graphlab::empty>(
      svdpp_vertex_program::signal_left);
