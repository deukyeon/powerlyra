...
struct vertex_data {
  ...
  void save(graphlab::oarchive& arc) const { arc << nupdates << pvec << bias; }
  /** \brief Load the vertex data from a binary archive */
  void load(graphlab::iarchive& arc) { arc >> nupdates >> pvec >> bias; }
};  // end of vertex data
    ...
    if (edata.role == edge_data::TRAIN) {
      ...
      if (other_vertex.data().nupdates < MAX_UPDATES)
        context.signal(other_vertex,
                       gather_type(vec_type::Zero(vertex_data::NLATENT), 0));
    }
...
double extract_l2_error(const graph_type::edge_type& edge) {
  ...
  assert(
      rmse <=
      pow(biassgd_vertex_program::MAXVAL - biassgd_vertex_program::MINVAL, 2));
  return rmse;
...
struct prediction_saver {
  ...
  std::string save_edge(const edge_type& edge) const {
    ...
    double prediction =
        biassgd_vertex_program::GLOBAL_MEAN + edge.source().data().bias +
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  if (role == edge_data::TRAIN || role == edge_data::VALIDATE) {
    strm >> obs;
    if (obs < biassgd_vertex_program::MINVAL ||
        obs > biassgd_vertex_program::MAXVAL) {
      logstream(LOG_WARNING)
          << "Rating values should be between "
          << biassgd_vertex_program::MINVAL << " and "
          << biassgd_vertex_program::MAXVAL << ". Got value: " << obs
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("gamma", biassgd_vertex_program::GAMMA, "SGD step size");
  ...
  clopts.attach_option("maxval", biassgd_vertex_program::MAXVAL,
                       "max allowed value");
  clopts.attach_option("minval", biassgd_vertex_program::MINVAL,
                       "min allowed value");
  clopts.attach_option("step_dec", biassgd_vertex_program::STEP_DEC,
