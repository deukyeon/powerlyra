...
ivec sort_union(ivec a, ivec b) {
  ...
  for (int i = 1; i < ab.size(); i++) {
    if (ab[i] == ab[i - 1]) del(ab, i);
  }
  return ab;
}

vec CoSaMP(const mat& Phi, const vec& u, int K, int max_iter, double tol1,
           int D) {
  assert(K <= 2 * D);
  assert(K >= 1);
  ...
  while (t < max_iter) {
    ...
    z3 = reverse(z3);
    Sest = zeros(D);
    for (int i = 0; i < K; i++) set_val(Sest, z3[i], b[z3[i]]);
...
void test_cosamp() {
  mat A = init_mat(
      "0.9528    0.5982    0.8368 ; 0.7041    0.8407    0.5187; 0.9539    "
      "0.4428    0.0222",
      3, 3);
