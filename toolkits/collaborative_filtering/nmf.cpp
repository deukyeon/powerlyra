...
struct vertex_data {
  ...
  vertex_data() {
    if (debug)
      pvec = vec::Ones(NLATENT);
    else
      randomize();
    train_rmse = validation_rmse = 0;
  }
...
class gather_type {
 public:
  ...
  gather_type(const vec& _pvec, double _train_rmse, double _validation_rmse) {
    pvec = _pvec;
    training_rmse = _train_rmse;
    validation_rmse = _validation_rmse;
  }
  void reset() {
    pvec = vec::Zero(vertex_data::NLATENT);
    training_rmse = 0;
    validation_rmse = 0;
  }
  void save(graphlab::oarchive& arc) const {
...
class nmf_vertex_program
 ...
 public:
  ...
  static float nmf_predict(const vertex_data& user, const vertex_data& movie,
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    if (edge.data().role == edge_data::TRAIN ||
      ...
      double prediction = 0;
      double rmse = nmf_predict(vertex.data(), other_vertex.data(),
                                edge.data().weight, prediction);
      if (prediction == 0)
        logstream(LOG_FATAL) << "Got into numerical error!" << std::endl;
      if (edge.data().role == edge_data::TRAIN)
        return gather_type(
            other_vertex.data().pvec * (edge.data().weight / prediction), rmse,
            0);
      else  // validation
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (vdata.pvec.sum() != 0) {
      for (uint i = 0; i < vertex_data::NLATENT; i++) {
        ...
        if (vdata.pvec[i] < epsilon) vdata.pvec[i] = epsilon;
      }
    }
  ...
  static void verify_rows(graph_type::vertex_type& vertex) {
    if (isuser(vertex.id()) && vertex.num_out_edges() == 0)
      logstream(LOG_FATAL) << "NMF algorithm can not work when the row "
                           << vertex.id() << " of the matrix contains all zeros"
                           << std::endl;
  }
...
gather_type count_edges(nmf_vertex_program::icontext_type& context,
  ...
  if (edge.data().weight < 0)
    logstream(LOG_FATAL) << "Found a negative entry in matirx row "
                         << edge.source().id()
                         << " with value: " << edge.data().weight << std::endl;
  return ret;
}
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  if (role == edge_data::TRAIN || role == edge_data::VALIDATE) {
    strm >> weight;
    if (weight < nmf_vertex_program::MINVAL ||
        weight > nmf_vertex_program::MAXVAL)
      logstream(LOG_FATAL) << "Rating values should be between "
                           << nmf_vertex_program::MINVAL << " and "
                           << nmf_vertex_program::MAXVAL
                           << ". Got value: " << weight
                           << " [ user: " << source_id
                           << " to item: " << target_id << " ] " << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("maxval", nmf_vertex_program::MAXVAL,
                       "max allowed value");
  clopts.attach_option("minval", nmf_vertex_program::MINVAL,
                       "min allowed value");
  ...
  dc.cout() << "Training edges: " << edge_count.training_rmse
            << " validation edges: " << edge_count.validation_rmse << std::endl;
  ...
  for (uint j = 0; j < nmf_vertex_program::MAX_UPDATES; j++) {
    x1 = graph.map_reduce_vertices<gather_type>(nmf_vertex_program::pre_iter,
                                                right);
    px = &x1;
    for (int i = 0; i < (int)vertex_data::NLATENT; i++)
      ASSERT_NE(px->pvec[i], 0);

    dc.cout() << std::setw(8) << mytimer.current_time() << " "
              << sqrt(x1.training_rmse / edge_count.training_rmse);
    if (edge_count.validation_rmse > 0)
      dc.cout() << " " << std::setw(8)
                << sqrt(x1.validation_rmse / edge_count.validation_rmse)
                << std::endl;
    else
      dc.cout() << std::endl;
    engine.map_reduce_vertices<graphlab::empty>(
        nmf_vertex_program::signal_left);
    engine.start();
    x1.reset();

    x2 = graph.map_reduce_vertices<gather_type>(nmf_vertex_program::pre_iter,
                                                left);
    px = &x2;

    engine.map_reduce_vertices<graphlab::empty>(
        nmf_vertex_program::signal_right);
    engine.start();
    x2.reset();
  }
