...
class als_vertex_program
 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (REGNORMAL) regularization = LAMBDA * vertex.num_out_edges();
    for (int i = 0; i < XtX.rows(); ++i) XtX(i, i) += regularization;
...
struct error_aggregator : public graphlab::IS_POD_TYPE {
  ...
  static void finalize(icontext_type& context, const error_aggregator& agg) {
    ...
    context.cout() << "Time in seconds: " << context.elapsed_seconds()
                   << "\tiTraining RMSE: " << train_error;
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("regnormal", als_vertex_program::REGNORMAL,
                       "regularization type. 1 = weighted according to "
                       "neighbors num. 0 = no weighting - just lambda");
  ...
  if (!predictions.empty()) {
    ...
    graph.save(predictions, prediction_saver(), gzip_output, false, true,
               threads_per_machine);
    // save the linear model
    graph.save(predictions + ".U", linear_model_saver_U(), gzip_output, true,
               false, threads_per_machine);
    graph.save(predictions + ".V", linear_model_saver_V(), gzip_output, true,
               false, threads_per_machine);
  }
