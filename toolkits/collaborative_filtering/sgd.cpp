...
bool isuser(uint node) { return ((int)node) >= 0; }
...
struct vertex_data {
   ...
   *  data
   */
  vertex_data() : nupdates(0) {
    if (debug)
      pvec = vec_type::Ones(NLATENT);
    else
      randomize();
  }
  ...
  void save(graphlab::oarchive& arc) const { arc << nupdates << pvec; }
  /** \brief Load the vertex data from a binary archive */
  void load(graphlab::iarchive& arc) { arc >> nupdates >> pvec; }
};  // end of vertex data
...
class sgd_vertex_program
 ...
 public:
  ...
  void save(graphlab::oarchive& arc) const { arc << pmsg; }
  /** \brief Load the vertex data from a binary archive */
  void load(graphlab::iarchive& arc) { arc >> pmsg; }
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    if (vertex.num_in_edges() == 0) {
      ...
      // compute the current prediction by computing a dot production of user
      // and item nodes
      ...
      if (debug)
        std::cout << "entering edge " << (int)edge.source().id() << ":"
                  << (int)edge.target().id() << " err: " << err
                  << " rmse: " << err * err << std::endl;
      if (std::isnan(err))
        logstream(LOG_FATAL)
            << "Got into numeric errors.. try to tune step size and "
               "regularization using --lambda and --gamma flags"
            << std::endl;
      ...
      if (edge.data().role == edge_data::TRAIN) {
        ...
        delta = GAMMA *
                (err * other_vertex.data().pvec - LAMBDA * vertex.data().pvec);
        // compute the change in gradient for this item node
        other_delta = GAMMA * (err * vertex.data().pvec -
                               LAMBDA * other_vertex.data().pvec);

        // heuristic: update the current gradient with the change (this change
        // is discarded when this function exists) my_vertex.data().pvec +=
        // delta; other_vertex.data().pvec += other_delta;
        if (debug)
          std::cout << "new val:" << (int)edge.source().id() << ":"
                    << (int)edge.target().id() << " U "
                    << my_vertex.data().pvec.transpose() << " V "
                    << other_vertex.data().pvec.transpose() << std::endl;
        // send the delta gradient for the item node to be updated in the next
        // iteration
        if (std::fabs(err) > TOLERANCE &&
            other_vertex.data().nupdates < MAX_UPDATES)
  ...
  void init(icontext_type& context, const vertex_type& vertex,
    ...
    // if this is an item node, store the change in the gradient (sum of
    // changes) to be applied in the apply() function
    if (vertex.num_in_edges() > 0) {
      pmsg = msg;
    }
  }
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    // this is a user node, update the gradient using the comulative sum of
    // gradient updates computed in gather
    if (sum.pvec.size() > 0) {
      vdata.pvec += sum.pvec;
      assert(vertex.num_in_edges() == 0);
    }
    // if this is an item node, update the gradient using the received sum from
    // the init() function
    else if (pmsg.size() > 0) {
      vdata.pvec += pmsg;
  ...
  static graphlab::empty signal_left(icontext_type& context,
    ...
    if (vertex.num_out_edges() > 0)
      context.signal(vertex, vec_type::Zero(vertex_data::NLATENT));
...
struct error_aggregator : public graphlab::IS_POD_TYPE {
  ...
  error_aggregator() : train_error(0), validation_error(0) {}
  ...
  static error_aggregator map(icontext_type& context,
                              const graph_type::edge_type& edge) {
    error_aggregator agg;
    if (edge.data().role == edge_data::TRAIN) {
      if (isuser_node(edge.source())) agg.train_error = extract_l2_error(edge);
  ...
  static void finalize(icontext_type& context, const error_aggregator& agg) {
    ...
    assert(!std::isnan(train_error));
    context.cout() << std::setw(8) << context.elapsed_seconds() << "  "
                   << std::setw(8) << train_error;
...
double extract_l2_error(const graph_type::edge_type& edge) {
  ...
  assert(rmse <=
         pow(sgd_vertex_program::MAXVAL - sgd_vertex_program::MINVAL, 2));
  return rmse;
...
struct prediction_saver {
   ...
   */
  std::string save_vertex(const vertex_type& vertex) const { return ""; }
  std::string save_edge(const edge_type& edge) const {
    if (edge.data().role != edge_data::PREDICT) return "";

    std::stringstream strm;
    const double prediction =
        edge.source().data().pvec.dot(edge.target().data().pvec);
    strm << edge.source().id() << '\t' << -edge.target().id() - SAFE_NEG_OFFSET
         << '\t' << prediction << '\n';
    return strm.str();
  }
...
struct linear_model_saver_V {
  ...
  std::string save_vertex(const vertex_type& vertex) const {
    if (vertex.num_out_edges() == 0) {
      std::string ret =
          boost::lexical_cast<std::string>(-vertex.id() - SAFE_NEG_OFFSET) +
          ") ";
      for (uint i = 0; i < vertex_data::NLATENT; i++)
        ret += boost::lexical_cast<std::string>(vertex.data().pvec[i]) + " ";
      ret += "\n";
  ...
  if (role == edge_data::TRAIN || role == edge_data::VALIDATE) {
    ...
    if (obs < sgd_vertex_program::MINVAL || obs > sgd_vertex_program::MAXVAL) {
      logstream(LOG_WARNING)
          << "Rating values should be between " << sgd_vertex_program::MINVAL
          << " and " << sgd_vertex_program::MAXVAL << ". Got value: " << obs
          << " [ user: " << source_id << " to item: " << target_id << " ] "
          << std::endl;
      assert(false);
    }
  }
...
int main(int argc, char** argv) {
  ...
  const std::string description = "Compute the SGD factorization of a matrix.";
  ...
  clopts.attach_option("gamma", sgd_vertex_program::GAMMA, "SGD step size");
  ...
  clopts.attach_option("maxval", sgd_vertex_program::MAXVAL,
                       "max allowed value");
  clopts.attach_option("minval", sgd_vertex_program::MINVAL,
                       "min allowed value");
  clopts.attach_option("step_dec", sgd_vertex_program::STEP_DEC,
                       "multiplicative step decrement");
  ...
  const bool success =
      engine.add_edge_aggregator<error_aggregator>(
  ...
  dc.cout() << "Training edges: " << info.training_edges
            << " validation edges: " << info.validation_edges << std::endl;
  ...
  if (!predictions.empty()) {
    ...
    graph.save(predictions, prediction_saver(), gzip_output, save_vertices,
               save_edges, threads_per_machine);
    // save the linear model
    graph.save(predictions + ".U", linear_model_saver_U(), gzip_output,
               save_edges, save_vertices, threads_per_machine);
    graph.save(predictions + ".V", linear_model_saver_V(), gzip_output,
               save_edges, save_vertices, threads_per_machine);
  }
