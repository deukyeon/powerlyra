  ...
  void load_counts(const std::string& fname) {
    ...
    while (fin.good()) {
      ...
      if (fin.good()) {
        ...
        if (doc >= ntokens_in_doc.size()) ntokens_in_doc.resize(doc + 1, 0);
...
class matrix {
 ...
 public:
  ...
  void zeros() { std::fill(data.begin(), data.end(), T(0)); }
...
class collapsed_gibbs {
 public:
  ...
  collapsed_gibbs(const corpus_type& corpus, const size_t& ntopics,
  ...
  void iterate() {
    ...
    for (size_t i = 0; i < corpus.ntokens; ++i) {
      ...
      if (old_topic != NULL_TOPIC) {
        --n_td(old_topic, d);
        --n_wt(w, old_topic), --n_t(old_topic);
      }
      ...
      ++n_td(new_topic, d);
      ++n_wt(w, new_topic), ++n_t(new_topic);
...
void display_top(const corpus_type& corpus, const mat_type& n_wt,
                 const size_t& ntop) {
  assert(ntop > 0);
  const size_t nwords = n_wt.rows();
...
int main(int argc, char** argv) {
  ...
  desc.add_options()("help", "produce help message")(
      "dictionary",
      po::value<std::string>(&dictionary_fname)
          ->default_value(dictionary_fname),
      "Dictionary file")(
      "counts",
      po::value<std::string>(&counts_fname)->default_value(counts_fname),
      "Counts file")("ntopics",
                     po::value<size_t>(&ntopics)->default_value(ntopics),
                     "Number of topics")(
      "nburnin", po::value<size_t>(&nburnin)->default_value(nburnin),
      "Number of iterations")(
      "nsamples", po::value<size_t>(&nsamples)->default_value(nsamples),
      "Number of iterations")(
      "alpha", po::value<double>(&alpha)->default_value(alpha), "Alpha prior")(
      "beta", po::value<double>(&beta)->default_value(beta), "Beta prior")(
      "doctop_fname",
      po::value<std::string>(&doctop_fname)->default_value(doctop_fname),
      "doctop_fname")(
      "wordtop_fname",
      po::value<std::string>(&wordtop_fname)->default_value(wordtop_fname),
      "wordtop_fname")("topk", po::value<size_t>(&topk)->default_value(topk),
                       "number of top k to show");
  ...
  for (size_t i = 0; i < nburnin; ++i) {
    ...
    double llik =
        log_likelihood(gibbs.alpha, gibbs.beta, gibbs.n_td, gibbs.n_wt);
