 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    if (is_doc(vertex)) {
      ...
      for (size_t i = 0; i < vdata.factor.size(); ++i) {
        MIMNO_R +=
            vdata.factor[i] * BETA / (BETA * NWORDS + GLOBAL_TOPIC_COUNT[i]);
      }
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    foreach (topic_id_type& asg, assignment) {
      ...
      ASSERT_GE(MIMNO_Q, 0);
      float f =
          graphlab::random::uniform<float>(0, MIMNO_S + MIMNO_R + MIMNO_Q);
      if (f < MIMNO_S) {
        float ctr = 0;
        ...
        for (size_t t = 0; t < NTOPICS; ++t) {
          if (doc_topic_count[t] > 0) {
            ctr += doc_topic_count[t] * BETA /
                   (BETA * NWORDS + GLOBAL_TOPIC_COUNT[t]);
            if (ctr >= f) {
              asg = t;
              break;
            }
...
struct global_counts_aggregator {
  ...
  static void finalize(icontext_type& context, const factor_type& total) {
    ...
    for (size_t t = 0; t < total.size(); ++t) {
      ...
      NEW_MIMNO_S += ALPHA * BETA /
                     (BETA * NWORDS +
                      (GLOBAL_TOPIC_COUNT[t] > 0 ? GLOBAL_TOPIC_COUNT[t] : 0));
    }
    MIMNO_S = NEW_MIMNO_S;
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("interval", INTERVAL, "statistics reporting interval");
