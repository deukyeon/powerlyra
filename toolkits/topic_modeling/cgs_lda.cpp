...
struct top_words_type {
  ...
  inline std::string json_header_string() const {
    return "\t\"ntopics\": " + graphlab::tostr(NTOPICS) + ",\n" +
 ...
 */
std::pair<std::string, std::string> word_cloud_callback(
    std::map<std::string, std::string>& varmap) {
  TOP_WORDS.lock.lock();
  const std::pair<std::string, std::string> pair("text/html",
...
struct vertex_data {
  ...
  void load(graphlab::iarchive& arc) { arc >> nupdates >> nchanges >> factor; }
};  // end of vertex_data
...
struct edge_data {
  ...
  edge_data(size_t ntokens = 0)
      : nchanges(0), assignment(ntokens, NULL_TOPIC) {}
...
class cgs_lda_vertex_program
 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    // There should be no new edge data since the vertex program has been
    // cleared
  ...
  edge_dir_type scatter_edges(icontext_type& context,
                              const vertex_type& vertex) const {
    return (DISABLE_SAMPLING ||
            (BURNIN > 0 && context.elapsed_seconds() > BURNIN))
               ? graphlab::NO_EDGES
               : graphlab::ALL_EDGES;
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    factor_type& doc_topic_count = is_doc(edge.source())
                                       ? edge.source().data().factor
                                       : edge.target().data().factor;
...
class topk_aggregator {
 ...
 public:
  ...
  static void finalize(icontext_type& context, const topk_aggregator& total) {
    ...
    std::string json =
        "{\n" + TOP_WORDS.json_header_string() + "\t\"values\": [\n";
    for (size_t i = 0; i < total.top_words.size(); ++i) {
      ...
      rev_foreach(cw_pair_type pair, total.top_words[i]) {
        ...
        json += '\n';
        std::cout << DICTIONARY[pair.second] << "(" << pair.first << ")"
                  << ", ";
 ...
 *    sum_d(sum_t(gammaln(n_td + alpha)) - gammaln(sum_t(n_td) + ntopics *
 alpha));
 *
 * Latex formulation:
 *
    \mathcal{L}( w | z) & = T * \left( \log\Gamma(W * \beta) - W *
 \log\Gamma(\beta) \right) + \\ & \sum_{t} \left( \left(\sum_{w}
 \log\Gamma(N_{wt} + \beta)\right) -
    ...
    \\
    \mathcal{L}(z) & = D * \left(\log\Gamma(T * \alpha) - T * \log\Gamma(\alpha)
 \right) + \\ & \sum_{d} \left( \left(\sum_{t}\log\Gamma(N_{td} + \alpha)\right)
 -
...
class likelihood_aggregator : public graphlab::IS_POD_TYPE {
 ...
 public:
  ...
  static likelihood_aggregator map(icontext_type& context,
  ...
  static void finalize(icontext_type& context,
                       const likelihood_aggregator& total) {
    using boost::math::lgamma;
...
struct signal_only {
   ...
   * vertices.
   */
  static graphlab::empty docs(icontext_type& context,
                              const graph_type::vertex_type& vertex) {
   ...
   * vertices.
   */
  static graphlab::empty words(icontext_type& context,
                               const graph_type::vertex_type& vertex) {
...
bool load_and_initialize_graph(graphlab::distributed_control& dc,
                               graph_type& graph, const std::string& corpus_dir,
...
bool load_dictionary(const std::string& fname) {
  ...
  if (boost::starts_with(fname, "hdfs://")) {
    ...
    fin.push(in_file);
    if (!fin.good()) {
      logstream(LOG_ERROR) << "Error loading dictionary: " << fname
                           << std::endl;
      return false;
    }
    std::string term;
...
struct count_saver {
  ...
  std::string save_vertex(const vertex_type& vertex) const {
    ...
    // with the save type
    if ((save_words && is_doc(vertex)) || (!save_words && is_word(vertex)))
      return "";
    // Proceed to save
...
int main(int argc, char** argv) {
  ...
  const std::string description =
      ...
      "(where wordid is indexed starting from zero and docid are positive "
      "integers)\n"
      "\t0\t0\t3\n"
      "\t0\t5\t1\n"
      "\t ...\n\n"
      "For JSON format, make sure docid are negative integers index starting "
      "from -2 \n\n"
  ...
  clopts.attach_option("alpha", ALPHA, "The document hyper-prior");
  clopts.attach_option("beta", BETA, "The word hyper-prior");
  clopts.attach_option("topk", TOPK, "The number of words to report");
  ...
  clopts.attach_option("format", format, "Formats: matrix,json,json-gzip");
  ...
  if (ALPHA <= 0) {
    logstream(LOG_ERROR) << "Alpha must be positive (alpha=" << ALPHA << ")!"
                         << std::endl;
    return EXIT_FAILURE;
  }

  if (BETA <= 0) {
    logstream(LOG_ERROR) << "Beta must be positive (beta=" << BETA << ")!"
                         << std::endl;
    return EXIT_FAILURE;
  }
  ...
  {  // Add the Global counts aggregator
    const bool success = engine.add_vertex_aggregator<factor_type>(
                             "global_counts", global_counts_aggregator::map,
  ...
  {  // Add the likelihood aggregator
    const bool success = engine.add_vertex_aggregator<likelihood_aggregator>(
                             "likelihood", likelihood_aggregator::map,
  ...
  if (!word_dir.empty()) {
    ...
    graph.save(word_dir, count_saver(save_words), gzip_output, save_vertices,
  ...
  if (!doc_dir.empty()) {
    ...
    graph.save(doc_dir, count_saver(save_words), gzip_output, save_vertices,
               save_edges, threads_per_machine);
  }
