...
struct vertex_data {
  ...
  void load(graphlab::iarchive& arc) { arc >> factor >> nupdates >> nchanges; }
};  // end of vertex_data
...
struct edge_data {
  ...
  void save(graphlab::oarchive& arc) const { arc << count << belief; }
  void load(graphlab::iarchive& arc) { arc >> count >> belief; }
};  // end of edge data
...
struct selective_signal {
  static graphlab::empty docs(IContext& context,
                              graph_type::vertex_type& vertex) {
    if (is_doc(vertex)) context.signal(vertex);
    return graphlab::empty();
  }  // end of signal_docs
  static graphlab::empty words(IContext& context,
                               graph_type::vertex_type& vertex) {
