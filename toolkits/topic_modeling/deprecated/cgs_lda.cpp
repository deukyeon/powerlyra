...
struct gather_type {
  ...
  gather_type(const graph_type::edge_type& edge,
    ...
    const graph_type::vertex_type other_vertex = get_other_vertex(edge, vertex);
...
class cgs_lda_vertex_program
 ...
 private:
  typedef std::map<graphlab::vertex_id_type, assignment_type>
      edge_data_map_type;
 ...
 public:
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    if (iter != new_edge_data.end()) {
      ...
      ASSERT_EQ(new_topic_assignment.size(), edge.data().assignment.size());
...
int main(int argc, char** argv) {
  ...
  const std::string description =
      ...
      "\t./fast_cgs_lda --dictionary dictionary.txt --matrix "
      "doc_word_count.tsv\n"
  ...
  clopts.attach_option("alpha", &ALPHA, ALPHA, "The document hyper-prior");
  clopts.attach_option("beta", &BETA, BETA, "The word hyper-prior");
  clopts.attach_option("topk", &TOPK, TOPK, "The number of words to report");
  ...
  ///! Add an aggregator
  success = engine.add_vertex_aggregator<topk_type>("topk", topk_type::map,
                                                    topk_type::finalize) &&
