...
struct message_type : public graphlab::IS_POD_TYPE {
  ...
  message_type& operator+=(const message_type& other) { return *this; }
};

struct extension_update_functor
    : public graphlab::ivertex_program<internal_graph_type, gather_var,
                                       message_type> {
 public:
  ...
  inline void init(icontext_type& context, const vertex_type& vertex,
  ...
  edge_dir_type gather_edges(icontext_type& context,
    ...
    gas_op_descriptor* gas = descriptor_access.query_unsafe(descriptor_id);
  ...
  inline gather_var gather(icontext_type& context, const vertex_type& vertex,
    ...
    gather_var ret;
    ret.v = (*gas->gather_op)(
        vertex.data(), edge.data(), other_vertex.data(),
        edge.source().id() == vertex.id() ? OUT_EDGE : IN_EDGE);
  ...
  inline void save(graphlab::oarchive& oarc) const { oarc << descriptor_id; }

  inline void load(graphlab::iarchive& iarc) { iarc >> descriptor_id; }
};
