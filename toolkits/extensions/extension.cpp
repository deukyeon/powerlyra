...
var& operator+=(var& value, const var& other) {
  ...
  if (double* val = boost::get<double>(&value)) {
    if (other_double != NULL)
      value = (double)(*val) + (*other_double);
    else
      ASSERT_MSG(false, "Type mismatch in operator+=");
...
lockfree_push_back<std::vector<gas_op_descriptor> > descriptor_access(
    descriptor_set, 0);
