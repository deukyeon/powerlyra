...
class extension_graph {
 public:
  ...
  extension_graph()
      : rmi(*dc_impl::get_last_dc(), this),
        internal_graph(*dc_impl::get_last_dc(), __glopts),
        finalized(false) {}
  ...
  void transform_field(FieldType field, TransformType transform_functor) {
    finalize();
    lock.lock();
    transform_field_wrapper<FieldType, TransformType> fw(&transform_functor,
                                                         field);
  ...
  */
  ...
  internal_graph_type& graph() { return internal_graph; }

  void finalize() {
    lock.lock();
    if (!finalized) {
      internal_graph.finalize();
      internal_graph.transform_vertices(
          [](internal_graph_type::vertex_type& v) {
  ...
  template <typename GatherType, typename CombinerType, typename ApplyType,
            typename ScatterType>
  void GAS(GatherType gather, CombinerType combiner, ApplyType apply,
           ScatterType scatter, size_t iterations = 0) {
    finalize();
  ...
  /// Regular GAS
  template <typename GatherSelectType, typename GatherType,
            typename CombinerType, typename ApplyType,
            typename ScatterSelectType, typename ScatterType>
  void GAS(GatherSelectType gatherselect, GatherType gather,
           CombinerType combiner, ApplyType apply,
           ScatterSelectType scatterselect, ScatterType scatter,
           size_t iterations = 0) {
    finalize();
