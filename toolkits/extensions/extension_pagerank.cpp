...
void pagerank(extension_graph& graph, const std::string PR_FIELD_NAME,
              double tolerance) {
  ...
  graph.GAS(
      ...
      [=](const vars&, vars&, const vars& other, edge_direction) {  // gather
        return get<double>(other(PR_FIELD)) / get<double>(other(OUT_DEG));
      },
      ...
      [=](vars& v, const var& result) -> bool {  // apply
        ...
        v(PR_CHANGE) =
            std::fabs(pr - get<double>(v(PR_FIELD))) / get<double>(v(OUT_DEG));
        v(PR_FIELD) = pr;
        return false;
      },
      [=](const vars& v) {  // scatter_edges
        return get<double>(v.field(PR_CHANGE)) > tolerance ? graphlab::OUT_EDGES
                                                           : graphlab::NO_EDGES;
      },
