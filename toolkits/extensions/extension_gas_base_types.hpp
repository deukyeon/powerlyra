...
namespace extension {
// edge direction enum
enum edge_direction { IN_EDGE, OUT_EDGE };
...
struct gather_functor {
  virtual var operator()(const vars& center, vars& edge, const vars& other,
                         edge_direction direction) = 0;
};

// gather select base type
struct gather_select_functor {
  virtual edge_dir_type operator()(const vars& center) { return ALL_EDGES; }
};

// the apply base type
struct apply_functor {
  // return true to schedule self
  virtual bool operator()(vars& center, const var& gather_result) = 0;
};
...
struct scatter_functor {
  ...
  virtual bool operator()(const vars& center, vars& edge, const vars& other,
                          edge_direction direction) = 0;
};
