...
struct vertex_data {
  ...
  vertex_data()
      : A_ii(1) {  //: y(0), Aii(1), pvec[JACOBI_X](0), pvec[JACOBI_REAL_X](0),
...
void verify_values(int unittest, double residual) {
  if (unittest == 1) assert(residual < 1e-5);
}
/**
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  // no need to parse
  if (boost::algorithm::ends_with(filename, vecfile)) return true;
  ...
  if (source_id >= (uint)rows)
    logstream(LOG_FATAL) << "Row number: " << source_id << " sould be < rows "
                         << rows << " [ line: " << line << " ] " << std::endl;
  if (target_id >= (uint)cols)
    logstream(LOG_FATAL) << "Col number: " << target_id << " sould be < cols "
                         << cols << " [ line: " << line << " ] " << std::endl;
  strm >> obs;
...
int main(int argc, char** argv) {
  ...
  const std::string description = "Solve a linear system using Jacobi method";
  ...
  dc.cout() << "Solution converged to residual: " << ret.toDouble()
            << std::endl;
