...
struct vertex_data {
  ...
  explicit vertex_data(const std::vector<float>& x_in) : x(x_in), D_ii(0.0) {}

  void save(graphlab::oarchive& oarc) const {
    oarc << x.size();
    for (size_t i = 0; i < x.size(); ++i) oarc << x[i];
    oarc << D_ii;
  }
...
struct edge_data {
  ...
  void save(graphlab::oarchive& oarc) const { oarc << A_ij << nearest; }
  void load(graphlab::iarchive& iarc) { iarc >> A_ij >> nearest; }
};
...
bool line_parser(graph_type& graph, const std::string& filename,
  ...
  const bool success = qi::phrase_parse(
      ...
      (qi::ulong_[phoenix::ref(id) = qi::_1] >> -qi::char_(",") >>
       (qi::double_[phoenix::push_back(phoenix::ref(vtx.x), qi::_1)] %
        -qi::char_(","))),
      //  End grammar
      ascii::space);
...
void calc_similarities(graph_type::edge_type& edata) {
  edata.data().A_ij =
      similarity(edata.source().data().x, edata.target().data().x);
}
...
struct top_t_similarity {
  ...
  top_t_similarity()
      : ids(number_of_nearest_neighbors, std::numeric_limits<size_t>::max()),
        sims(number_of_nearest_neighbors, -1.0) {}
  top_t_similarity(size_t id, float sim)
      : ids(number_of_nearest_neighbors, std::numeric_limits<size_t>::max()),
        sims(number_of_nearest_neighbors, -1.0) {
    ids[0] = id;
    sims[0] = sim;
  ...
  void save(graphlab::oarchive& oarc) const {
    oarc << ids.size();
    for (size_t i = 0; i < ids.size(); ++i) oarc << ids[i];
    for (size_t i = 0; i < sims.size(); ++i) oarc << sims[i];
  }
  void load(graphlab::iarchive& iarc) {
    ids.clear();
    sims.clear();
...
class t_nearest
    : public graphlab::ivertex_program<graph_type, top_t_similarity>,
 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    //    std::cout << vertex.id() << "\t" << total.ids[0] << "-" <<
    //    total.sims[0] << ", "
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
               edge_type& edge) const {
    if (edge.data().A_ij >= threshold) edge.data().nearest = true;
  }
};

// discard small similarities (Optional)
void make_other_similarities_zero(graph_type::edge_type& edata) {
  if (edata.data().nearest == false) edata.data().A_ij = 0.0;
}

// compute sums over rows and then take inverse square root
class calc_degrees : public graphlab::ivertex_program<graph_type, float>,
                     public graphlab::IS_POD_TYPE {
 public:
  // gather A_ij
  edge_dir_type gather_edges(icontext_type& context,
...
void mult_D(graph_type::edge_type& edata) {
  edata.data().A_ij = edata.data().A_ij * edata.source().data().D_ii *
                      edata.target().data().D_ii;
}
...
struct max_min_similarity {
  ...
  explicit max_min_similarity(float similarity)
      : max_sim(similarity), min_sim(similarity) {}
  ...
  void save(graphlab::oarchive& oarc) const { oarc << max_sim << min_sim; }
  void load(graphlab::iarchive& iarc) { iarc >> max_sim >> min_sim; }
};
...
struct max_vid {
  ...
  void save(graphlab::oarchive& oarc) const { oarc << vid; }
  void load(graphlab::iarchive& iarc) { iarc >> vid; }
};
...
class graph_writer {
 public:
  ...
  std::string save_edge(graph_type::edge_type e) {
    ...
    std::stringstream strm;
    if (A_ij > 0.0) {
      strm << e.source().id() << " " << e.target().id() << " " << A_ij << "\n";
      strm << e.target().id() << " " << e.source().id() << " " << A_ij << "\n";
    }
    return strm.str();
  }
};
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("similarity-thres",
                       threshold_to_discard_small_similarities,
                       "Threshold to discard small similarities. ");
  clopts.attach_option(
      "t-nearest", number_of_nearest_neighbors,
      "Number of nearest neighbors (=t). Will use only the t-nearest "
      "similarities "
  ...
  time(&start);
  size_t data_num =
      graph.map_reduce_vertices<max_vid>(absolute_vertex_data).vid;

  // calculate similarities
  graph.transform_edges(calc_similarities);

  // show the max similarity less than 1 and the min similarity grater than 0
  max_min_similarity stat =
      graph.map_reduce_edges<max_min_similarity>(absolute_edge_data);
