...
bool call_eigen_vector_normalization(const std::string& mpi_args,
                                     const std::string& filename,
                                     const std::string& graph_analytics_dir,
...
bool call_kmeans_as_preprocess(const std::string& mpi_args,
                               const std::string& filename,
...
void read_pairs_with_prefix(std::vector<std::vector<size_t> >& ret,
  ...
  graphlab::fs_util::list_files_with_prefix(directory_name, search_prefix,
                                            files);
  if (files.size() == 0) {
    logstream(LOG_WARNING) << "No files found matching " << original_path
                           << std::endl;
  }
...
int recover_labels(const std::string& prefix) {
  ...
  for (size_t i = 0; i < spectral_result.size(); ++i) {
    label_map.insert(
        std::make_pair(spectral_result[i][0], spectral_result[i][1]));
  }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options clopts(
      ...
      "comma or white-space separated list of numeric values representing a "
      "vector. "
      "Every line must have the same number of values. The required "
      "--clusters=N "
  ...
  clopts.attach_option("sigma", sigma, "Scale parameter for Gaussian kernel");
  clopts.attach_option(
      "t-nearest", num_nearests,
  ...
  clopts.attach_option("graph-analytics-dir", graph_analytics_dir,
                       "Path to the directory where Graphlab "
                       "eigen_vector_normalization is located");
  clopts.attach_option("pre-kmeans-clusters", pre_kmeans_clusters,
                       "If set, will perform kmeans as a preprocess with the "
  ...
  if (pre_kmeans_clusters > 0) {
    if (pre_kmeans_clusters < num_clusters) {
      std::cout << "the number of --pre-kmeans-clusters must be bigger than "
                   "the number of clusters\n";
      return EXIT_FAILURE;
    }
    time(&mid);
    if (call_kmeans_as_preprocess(mpi_args, datafile, kmeans_dir,
                                  pre_kmeans_clusters, other_args) == false)
      return EXIT_FAILURE;
    // modify settings
    ...
    time(&end);
    times.push_back(
        std::pair<std::string, time_t>("kmeans preprocess", (end - mid)));
  }
  ...
  time(&end);
  times.push_back(
      std::pair<std::string, time_t>("graph laplacian", (end - mid)));
  ...
  if (sv == 0) {
    ...
    if (sv < num_clusters) sv = num_clusters;
  }
  time(&mid);
  ...
  if (call_eigen_vector_normalization(mpi_args, datafile, graph_analytics_dir,
                                      num_clusters, sv, num_data,
                                      other_args) == false) {
    return EXIT_FAILURE;
  }
  time(&end);
  times.push_back(
      std::pair<std::string, time_t>("eigen decomposition", (end - mid)));
  ...
  for (size_t i = 0; i < times.size(); ++i) {
    std::cout << "process " << i + 1 << "\t" << times[i].first << "\t"
              << times[i].second << " sec\n";
  }
  std::cout << "Overall processing time of spectral clustering is "
            << (end - start) << " sec\n";
