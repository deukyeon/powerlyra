...
std::map<size_t, double>& scale_vector(std::map<size_t, double>& a, double d) {
  for (std::map<size_t, double>::iterator iter = a.begin(); iter != a.end();
       ++iter) {
    size_t id = (*iter).first;
    double val = (*iter).second;
    a[id] = val * d;
...
bool vertex_loader(graph_type& graph, const std::string& fname,
  ...
  const bool success = qi::phrase_parse(
      line.begin(), line.end(),
      //  Begin grammar
      ((qi::double_[phoenix::push_back(phoenix::ref(vtx.point), qi::_1)] %
        -qi::char_(","))),
      //  End grammar
      ascii::space);
...
struct max_point_size_reducer : public graphlab::IS_POD_TYPE {
  ...
  static max_point_size_reducer get_max_point_size(
      const graph_type::vertex_type& v) {
...
struct min_point_size_reducer : public graphlab::IS_POD_TYPE {
  ...
  static min_point_size_reducer get_min_point_size(
      const graph_type::vertex_type& v) {
...
void kmeans_pp_initialization(graph_type::vertex_type& v) {
  double d =
      sqr_distance(v.data().point, CLUSTERS[KMEANS_INITIALIZATION].center);
...
struct random_sample_reducer {
  ...
  random_sample_reducer(const std::vector<double>& vtx, double weight)
      : vtx(vtx), weight(weight) {}
  ...
  static random_sample_reducer get_weight(const graph_type::vertex_type& v) {
    if (v.data().best_cluster == (size_t)(-1)) {
      ...
      return random_sample_reducer(v.data().point, v.data().best_distance);
    }
  }
  ...
  void save(graphlab::oarchive& oarc) const { oarc << vtx << weight; }

  void load(graphlab::iarchive& iarc) { iarc >> vtx >> weight; }
};
...
struct random_sample_reducer_sparse {
  ...
  static random_sample_reducer_sparse get_weight(
      const graph_type::vertex_type& v) {
    if (v.data().best_cluster == (size_t)(-1)) {
  ...
  random_sample_reducer_sparse& operator+=(
      const random_sample_reducer_sparse& other) {
    double totalweight = weight + other.weight;
    // if any weight is too small, just quit
    if (totalweight <= 0) return *this;
  ...
  if (CLUSTERS[v.data().best_cluster].changed) {
    ...
    for (size_t i = 0; i < NUM_CLUSTERS; ++i) {
      if (CLUSTERS[i].center.size() > 0 ||
          CLUSTERS[i].center_sparse.size() > 0) {
        double d = 0.0;
        if (IS_SPARSE == true)
...
struct neighbor_info {
  ...
  neighbor_info() : cw_map() {}
  neighbor_info(size_t clst, double weight) : cw_map() {
  ...
  void save(graphlab::oarchive& oarc) const { oarc << cw_map; }
  void load(graphlab::iarchive& iarc) { iarc >> cw_map; }
};

// used when edge weight file is given
class cluster_assignment
    : public graphlab::ivertex_program<graph_type, neighbor_info>,
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    for (size_t i = 0; i < NUM_CLUSTERS; ++i) {
      if (CLUSTERS[i].center.size() > 0 ||
        ...
        double d = 0.0;
        if (IS_SPARSE == true)
          d = sqr_distance(vertex.data().point_sparse,
                           CLUSTERS[i].center_sparse);
        else
        ...
        for (std::map<size_t, double>::const_iterator iter = cw_map.begin();
          ...
          if (i == neighbor_cluster) d -= total_wieght;
        }
...
struct cluster_center_reducer {
  ...
  cluster_center_reducer()
      : new_clusters(NUM_CLUSTERS), num_changed(0), cost(0) {}
  ...
  static cluster_center_reducer get_center(const graph_type::vertex_type& v) {
    ...
    if (IS_SPARSE == true)
      cc.new_clusters[v.data().best_cluster].center_sparse =
          v.data().point_sparse;
    else
  ...
  cluster_center_reducer& operator+=(const cluster_center_reducer& other) {
    for (size_t i = 0; i < NUM_CLUSTERS; ++i) {
      if (new_clusters[i].count == 0)
        ...
        if (IS_SPARSE == true)
          plus_equal_vector(new_clusters[i].center_sparse,
                            other.new_clusters[i].center_sparse);
        else
          plus_equal_vector(new_clusters[i].center,
                            other.new_clusters[i].center);
...
struct vertex_writer_sparse {
  std::string save_vertex(graph_type::vertex_type v) {
    std::stringstream strm;
    for (std::map<size_t, double>::iterator iter =
             v.data().point_sparse.begin();
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options clopts(
      ...
      "argument denotes the number of clusters to generate. To store the "
      "output "
  ...
  clopts.attach_option("data", datafile,
                       "Input file. Each line holds a white-space or comma "
                       "separated numeric vector");
  ...
  NEXT_VID =
      (((graphlab::vertex_id_type)1 << 31) / dc.numprocs()) * dc.procid();
  if (IS_SPARSE == true) {
    if (use_id) {
  ...
  if (IS_SPARSE == false) {
    size_t max_p_size = graph
                            .map_reduce_vertices<max_point_size_reducer>(
                                max_point_size_reducer::get_max_point_size)
                            .max_point_size;
    size_t min_p_size = graph
                            .map_reduce_vertices<min_point_size_reducer>(
                                min_point_size_reducer::get_min_point_size)
                            .min_point_size;
    if (max_p_size != min_p_size) {
      dc.cout() << "Data has dimensionality ranging from " << min_p_size
                << " to " << max_p_size << "! K-means cannot proceed!"
                << std::endl;
      return EXIT_FAILURE;
    }
    // allocate clusters
  ...
  for (KMEANS_INITIALIZATION = 0; KMEANS_INITIALIZATION < NUM_CLUSTERS;
    ...
    if (IS_SPARSE == true) {
      random_sample_reducer_sparse rs =
          graph.map_reduce_vertices<random_sample_reducer_sparse>(
      ...
      random_sample_reducer rs =
          graph.map_reduce_vertices<random_sample_reducer>(
  ...
  while (clusters_changed) {
    if (MAX_ITERATION > 0 && iteration_count >= MAX_ITERATION) break;

    cluster_center_reducer cc =
        graph.map_reduce_vertices<cluster_center_reducer>(
    ...
    if (edgedata_file.size() > 0) {
      ...
      graphlab::omni_engine<cluster_assignment> engine(dc, graph, "async",
                                                       clopts);
      engine.signal_all();
      engine.start();
    } else {
  ...
  if (!outcluster_file.empty() && dc.procid() == 0) {
    ...
    if (IS_SPARSE) {
      for (size_t i = 0; i < NUM_CLUSTERS; ++i) {
        if (use_id) fout << i + 1 << "\t";
        for (std::map<size_t, double>::iterator iter =
                 CLUSTERS[i].center_sparse.begin();
