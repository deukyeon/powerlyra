...
struct evec_vertex_data {
  ...
  evec_vertex_data() : vec(num_clusters, 0.0) {}
  void save(graphlab::oarchive& oarc) const { oarc << vec; }
  void load(graphlab::iarchive& iarc) { iarc >> vec; }
};

struct evec_edge_data {
  float val;
  evec_edge_data() : val(0.0) {}
  explicit evec_edge_data(float in_val) : val(in_val) {}
  void save(graphlab::oarchive& oarc) const { oarc << val; }
  void load(graphlab::iarchive& iarc) { iarc >> val; }
};

typedef graphlab::distributed_graph<evec_vertex_data, evec_edge_data>
    evec_graph_type;
...
bool evec_line_parser(evec_graph_type& graph, const std::string& filename,
  ...
  while (strm >> val) {
    ...
    graph.add_edge(vid, num_data + current_cluster + colcount + 1,
                   evec_edge_data(val));
    colcount++;
  }
...
struct id_and_val {
  ...
  id_and_val() : ids(), vals() {}
  id_and_val(size_t id, float val) : ids(), vals() {
    ids.push_back(id);
    vals.push_back(val);
  }
  ...
  void save(graphlab::oarchive& oarc) const {
    size_t num = ids.size();
    oarc << num;
    for (size_t a = 0; a < num; ++a) oarc << ids[a];
    for (size_t a = 0; a < num; ++a) oarc << vals[a];
  }
...
class aggregate_values
    : public graphlab::ivertex_program<evec_graph_type, id_and_val>,
...
class evec_graph_writer {
 public:
  ...
  std::string save_edge(evec_graph_type::edge_type e) { return ""; }
};
