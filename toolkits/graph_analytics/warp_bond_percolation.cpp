 ...
 * See algorithm description in Wikipedia:
...
std::size_t hash_value(vertex_data const& b) { return b.comp_id; }
...
std::size_t hash_value(edge_data const& b) { return b.comp_id; }
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  if (edge_id == (uint)-1)
    logstream(LOG_FATAL) << "Input file must contains line with the following "
                            "format: [from] [ to] [edge_id]\n "
                         << std::endl
                         << " where edge_id is a consecutive integer "
                         << std::endl;
...
size_t count_component(const graph_type::edge_type& edge) {
  ...
  if (debug && diff)
    std::cout << "Adding diff between node: " << edge.source().id()
              << " to: " << edge.target().id()
              << " compA: " << edge.source().data().comp_id
              << " compB: " << edge.target().data().comp_id << std::endl;
  return diff;
}
...
unsigned int bond_percolation_map(const graph_type::vertex_type& center,
  ...
  if (debug)
    std::cout << "Comparing: " << center.data().comp_id << " : "
              << edge.data().id << " : " << other.data().comp_id << std::endl;
  edge.data().comp_id = std::min(
      std::min(center.data().comp_id, edge.data().id), other.data().comp_id);
  if (debug)
    std::cout << "Setting edge id to: " << edge.data().comp_id
              << " from: " << center.id() << " to: " << other.id() << std::endl;
...
void bond_percolation_function(engine_type::context_type& context,
  ...
  vertex.data().comp_id = context.map_reduce<unsigned int>(
      BOND_PERCOLATION_MAP_REDUCE, graphlab::ALL_EDGES);
  if (debug && comp_id != vertex.data().comp_id)
    std::cout << "node: " << vertex.id()
              << " min edge component found: " << vertex.data().comp_id
              << std::endl;
#ifndef SYNC
...
struct model_saver {
  ...
  std::string save_edge(const edge_type& edge) const {
    return boost::lexical_cast<std::string>(edge.data().id) + " " +
           boost::lexical_cast<std::string>(edge.data().comp_id) +
           std::string("\n");
  }
};
...
struct label_counter {
  ...
  explicit label_counter(size_t labelId) { counts[labelId] = 1; }
...
int main(int argc, char** argv) {
  ...
  const std::string description = "Compute connected component - by edges";
  ...
  */
...
#endif
  engine.register_map_reduce(BOND_PERCOLATION_MAP_REDUCE, bond_percolation_map,
                             bond_percolation_combine);
  ...
  if (!output_file.empty()) {
    ...
    // save the output
    graph.save(output_file, model_saver(), gzip_output, save_vertices,
  ...
  if (p > 0) dc.cout() << "site fraction p= " << p << std::endl;
  if (n > 0) {
    dc.cout() << "n=" << n * p << std::endl;
    dc.cout() << "isolated sites: " << p * n - graph.num_vertices()
              << std::endl;
  }
  dc.cout() << "Number of sites: " << graph.num_vertices() << std::endl;
  dc.cout() << "Number of bonds: " << graph.num_edges() << std::endl;
  if (n) {
    dc.cout() << "Percentage of sites: " << (double)graph.num_vertices() / n
              << std::endl;
    dc.cout() << "Percentage of bonds: "
              << (double)graph.num_edges() / (2.0 * n) << std::endl;
  }
