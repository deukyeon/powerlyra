...
class node {
 public:
  ...
  void load(graphlab::iarchive& infile) { infile >> V >> w >> TSC >> prev; }
...
class edge {
 public:
  ...
  void save(graphlab::oarchive& outfile) const { outfile << weight; }

  void load(graphlab::iarchive& infile) { infile >> weight; }
};
...
// This is just a little class to be used to find the maximum change in TSC
// values.
class max_finder {
 public:
  double data;
  ...
  void load(graphlab::iarchive& infile) { infile >> data; }
  void save(graphlab::oarchive& outfile) const { outfile << data; }
};

// These three functions compute the A*w step of the
// Arnoldi iteration.
double arnoldi_map(graph_type::edge_type e, graph_type::vertex_type v) {
  return v.data().V.back();
}
void arnoldi_combine(double& v1, const double& v2) { v1 += v2; }
void AVj_to_w(graph_type::vertex_type& v) {
  v.data().w = graphlab::warp::map_reduce_neighborhood(
      v, graphlab::IN_EDGES, arnoldi_map, arnoldi_combine);
  return;
}

// Pushes the current w onto the V matrix
void w_to_v(graph_type::vertex_type& v) { v.data().V.push_back(v.data().w); }
// Scales w by a constant factor.  This is meant to be called through
// transform_vertices with a boost::bind to take care of the argument list
...
void print_w(graph_type::vertex_type& v) {
  std::cout << v.id() << " " << v.data().w << " " << v.data().V.size() << " "
            << v.data().V.back() << std::endl;
}
...
void accumulate_hleft(graph_type::vertex_type& v) {
  ...
  if (verbose) {
    if (v.id() < 10) {
      std::cout << "TSC " << v.id() << " " << Hleft.size() << " "
                << v.data().TSC << " " << v.data().prev << " "
                << (v.data().TSC - v.data().prev) / (1e-15 + v.data().TSC)
                << std::endl;
    }
  }
  return;
...
max_finder max_error(const graph_type::vertex_type& v) {
  return max_finder(
      fabs((v.data().TSC - v.data().prev) / (1e-15 + v.data().TSC)));
}
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option(
      "m", m,
      "Maximum number of orthogonal vectors to approximate with. Default 100.");
  clopts.attach_option(
      "column", column,
      "Column of exponential to calculate (instead of row-sum)");
  ...
  for (int j = 0; j < m; j++) {
    ...
    // We should be able to move this loop inside the map-reduce and transform
    // steps
    for (int i = 0; i <= j; i++) {
    ...
    // prep for the next iteration.  Otherwise, make the data we'll need next
    // time.
    ...
    if (j > 0) {
      ...
      if (j > 1) {
        max_finder largest_error =
            graph.map_reduce_vertices<max_finder>(max_error);
        double all_error = graph.map_reduce_vertices<double>(total_error);
        if (verbose) {
          std::cerr << "ARNOLDI STEP FINISHED " << j << std::endl;
          std::cout << "MAX ERROR: " << largest_error.data
                    << " TOTAL ERROR: " << all_error << std::endl;
        }
        if (largest_error.data < 1e-15) {
          break;
        };
        if (all_error < 1e-15) {
          break;
    ...
    if (std::isnan(H(j + 1, j))) {
      break;
    }
    if (fabs(H(j + 1, j)) < 1e-15) {
      break;
    }
  }
