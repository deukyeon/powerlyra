...
graphlab::empty signal_vertices_at_saturation(
    engine_type::icontext_type& ctx, const graph_type::vertex_type& vertex) {
  if (vertex.data().saturation == current_saturation &&
      vertex.data().color == UNCOLORED) {
    ctx.signal(vertex);
  }
    ...
    for (int x = max_saturation; x > 0; x--) {
      if (sats.find(x) != sats.end()) {
        ...
        engine.map_reduce_vertices<graphlab::empty>(
            signal_vertices_at_saturation);
      }
    }
    engine.start();

    /*
     *Colors the component with the highest degree and then colors all other
     *components randomly if the graph has more than one component.
     */
    if (next_component) {
