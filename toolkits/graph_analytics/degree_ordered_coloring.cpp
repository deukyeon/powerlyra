...
typedef struct {
  ...
  // serialize
  void save(graphlab::oarchive& oarc) const { oarc << color << degree; }

  // deserialize
  void load(graphlab::iarchive& iarc) { iarc >> color >> degree; }
 ...
 public:
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    color_type other_color = edge.source().id() == vertex.id()
                                 ? edge.target().data().color
                                 : edge.source().data().color;
  ...
  void scatter(icontext_type& context, const vertex_type& vertex,
    ...
    if (edge.source().data().color == edge.target().data().color) {
      context.signal(edge.source().id() == vertex.id() ? edge.target()
                                                       : edge.source());
    }
  }
};
...
void initialize_vertex_values(graph_type::vertex_type& v) {
  ...
  if (v.data().degree > max_degree) max_degree = v.data().degree;
  if (v.data().degree < low_degree) low_degree = v.data().degree;
}
...
struct save_colors {
  std::string save_vertex(graph_type::vertex_type v) {
    return graphlab::tostr(v.id()) + "\t" + graphlab::tostr(v.data().color) +
           "\n";
  }
...
graphlab::empty signal_vertices_at_degree(
    engine_type::icontext_type& ctx, const graph_type::vertex_type& vertex) {
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("output", output, "A prefix to save the output.");
  clopts.attach_option("powerlaw", powerlaw,
                       "Generate a synthetic powerlaw out-degree graph. ");
  clopts.attach_option("alpha", alpha, "Alpha in powerlaw distrubution");
  clopts.attach_option("trade", trade,
                       "Execute tradeoff version. Probability of degree "
                       "execution for node (0.0 to 1.0)");
  clopts.attach_option("edgescope", EDGE_CONSISTENT, "Use Locking. ");
  ...
  dc.cout() << "Degrees range from " << low_degree << " to " << max_degree
            << std::endl;
  ...
  if (TRADE) {
    ...
    dc.cout() << "Degree ordered coloring for " << fraction << " in "
              << graph_size << " vertices." << std::endl;
  }
  ...
  if (output != "") {
    graph.save(output, save_colors(), false, /* no compression */
               true,                         /* save vertex */
