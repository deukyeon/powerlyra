...
struct vdata {
  float degree;
  vdata() : degree(0.0) {}
  void save(graphlab::oarchive& oarc) const { oarc << degree; }
  void load(graphlab::iarchive& iarc) { iarc >> degree; }
};

struct edata {
  float weight;
  edata() : weight(1.0) {}
  explicit edata(const float in_w) : weight(in_w) {}
...
bool line_parser(graph_type& graph, const std::string& filename,
  ...
  strm.ignore(1);
  strm >> weight;
  if (source != target) graph.add_edge(source, target, edata(weight));

  return true;
}

// calculate vertex degree
class add_rows : public graphlab::ivertex_program<graph_type, float>,
...
int main(int argc, char** argv) {
  ...
  if (normalized_cut == true) {
    ...
    //    normalize_factor =
    //    graph.map_reduce_vertices<max_degree>(create_max_degree).degree;
  ...
  if (normalized_cut == true)
    graph.save(outputname, graph_writer_normalized_cut(),
    ...
    graph.save(outputname, graph_writer_ratio_cut(),
               false,  // set to true if each output file is to be gzipped
               true,   // whether vertices are saved
               true);  // whether edges are saved
