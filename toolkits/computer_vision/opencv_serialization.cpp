...
graphlab::oarchive& operator<<(graphlab::oarchive& arc,
  ...
  arc << keypoint.pt << keypoint.size << keypoint.angle << keypoint.response
      << keypoint.octave << keypoint.class_id;
  return arc;
}

graphlab::iarchive& operator>>(graphlab::iarchive& arc,
...
graphlab::oarchive& operator<<(graphlab::oarchive& arc, const cv::Mat& mat) {
  ...
  arc << mat.cols << mat.rows << elem_size << elem_type;
...
graphlab::iarchive& operator>>(graphlab::oarchive& arc, const cv::Mat& mat) {
  ...
  arc >> cols >> rows >> elem_size >> elem_type;
...
graphlab::oarchive& operator<<(graphlab::oarchive& arc,
  ...
  arc << features.img_idx << features.img_size << features.keypoints
      << features.descriptors;
  return arc;
}

graphlab::iarchive& operator>>(graphlab::oarchive& arc,
                               const cv::detail::ImageFeatures& features) {
  arc >> features.img_idx >> features.img_size >> features.keypoints >>
      features.descriptors;
  return arc;
...
graphlab::oarchive& operator<<(graphlab::oarchive& arc,
  ...
  arc << match.queryIdx << match.trainIdx << match.imgIdx << match.distance;
  return arc;
}
...
// For MatchesInfo
graphlab::oarchive& operator<<(graphlab::oarchive& arc,
                               const cv::detail::MatchesInfo& matchesinfo) {
  arc << matchesinfo.src_img_idx << matchesinfo.dst_img_idx
      << matchesinfo.matches << matchesinfo.inliers_mask
      << matchesinfo.num_inliers << matchesinfo.H << matchesinfo.confidence;
  return arc;
}
...
// For CameraParams
graphlab::oarchive& operator<<(graphlab::oarchive& arc,
                               const cv::detail::CameraParams& camera) {
  arc << camera.focal << camera.aspect << camera.ppx << camera.ppy << camera.R
      << camera.t;
  return arc;
}
...
graphlab::iarchive& operator>>(graphlab::oarchive& arc,
                               const cv::detail::CameraParams& camera) {
  arc >> camera.focal >> camera.aspect >> camera.ppx >> camera.ppy >>
      camera.R >> camera.t;
  return arc;
}
