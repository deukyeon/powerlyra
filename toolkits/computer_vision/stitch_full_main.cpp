...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("img", img_dir, "The directory containing the images");
  ...
  clopts.attach_option(
      "work_megapix", opts.work_megapix,
  ...
  if (opts.work_megapix > 10) {
    logstream(LOG_ERROR) << "Inappropriate value for work_megapix."
                         << std::endl;
    return EXIT_FAILURE;
  }

  // display settings
  dc.cout() << "ncpus:          " << clopts.get_ncpus() << std::endl
  ...
  engine_feat.add_vertex_aggregator<ImgArea>("find_largest_img",
                                             find_largest_img, set_scales);
  ...
  LOGLN("\nPairwise matching, time: "
        << ((getTickCount() - t1) / getTickFrequency()) << " sec");
  ...
  // vector<MatchesInfo> pairwise_matches(edlist.size()); //not needed for
  ...
  vector<int> indices =
      leaveBiggestComponent(features, pairwise_matches, opts.conf_thresh);
  ...
  logstream(LOG_EMPH) << "Homography-based init, time: "
  ...
  if (opts.ba_cost_func == "reproj")
    adjuster = new detail::BundleAdjusterReproj();
  else if (opts.ba_cost_func == "ray")
  ...
  if (dc.procid() == 0)
    logstream(LOG_EMPH) << "Bundle Adjustment, time: "
                        << ((getTickCount() - t) / getTickFrequency())
                        << " sec\n";
  ...
  if (focals.size() % 2 == 1)
    ...
    opts.warped_image_scale = static_cast<float>(focals[focals.size() / 2 - 1] +
  ...
  // Wave-Correction
  vector<Mat> rmats;
  WaveCorrectKind wave_correct;  // in future it needs to be added in clopts to
                                 // change it in runtime
  if (opts.wave_correct_type == "horiz")
    wave_correct = detail::WAVE_CORRECT_HORIZ;
  if (opts.wave_correct_type == "vert")
    wave_correct = detail::WAVE_CORRECT_VERT;
  for (size_t i = 0; i < cameras.size(); ++i) rmats.push_back(cameras[i].R);
  ...
  // load the graph
  if (dc.procid() == 0)
    graph_loader(graph_cam, img_dir, cameras, img_path, indices,
                 pairwise_matches);
  ...
  for (size_t i = 0; i != veclist.size(); ++i) {
    ...
    if (opts.verbose > 2)
      LOGLN("Blending corners x : " << veclist[i].corner.x << "   y : "
                                    << veclist[i].corner.y << "\n");
    ...
    if (opts.verbose > 2)
      LOGLN("Blending sizes height : " << size[i].height << "   width : "
                                       << size[i].width << "\n");
  }
  veclist.clear();
  ...
  int blend_type;
  if (opts.blending_type == "no") blend_type = Blender::NO;
  if (opts.blending_type == "feather") blend_type = Blender::FEATHER;
  if (opts.blending_type == "multiband") blend_type = Blender::MULTI_BAND;
  ...
  if (blender.empty()) {
    ...
    float blend_width =
        sqrt(static_cast<float>(dst_sz.area())) * opts.blend_strength / 100.f;
