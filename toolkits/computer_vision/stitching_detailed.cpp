...
int parseCmdArgs(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "--help" || string(argv[i]) == "/?") {
      ...
      features = argv[i + 1];
      if (features == "orb") match_conf = 0.3f;
      i++;
...
int main(int argc, char* argv[]) {
  ...
  if (save_graph) {
    ...
    //            f << pairwise_matches[i].src_img_idx << " " <<
  ...
  LOGLN("Homography-based init, time: "
        << ((getTickCount() - t) / getTickFrequency()) << " sec");
  ...
  for (int i = 0; i < num_images; ++i) {
    ...
    // cout << "corners x : " << corners[i].x << "   y : " << corners[i].y <<
    // endl;
    sizes[i] = images_warped[i].size();
    // cout << "sizes height : " << sizes[i].height << "   width : " <<
    // sizes[i].width << endl;
  ...
  if (seam_find_type == "no")
    ...
    if (try_gpu && gpu::getCudaEnabledDeviceCount() > 0)
      seam_finder =
          new detail::GraphCutSeamFinderGpu(GraphCutSeamFinderBase::COST_COLOR);
    else
#endif
      seam_finder =
          new detail::GraphCutSeamFinder(GraphCutSeamFinderBase::COST_COLOR);
  ...
  for (int img_idx = 0; img_idx < num_images; ++img_idx) {
    ...
    if (!is_compose_scale_set) {
      ...
      for (int i = 0; i < num_images; ++i) {
        ...
        corners[i] = roi.tl();
        // cout << "corner x : " << corners[i].x << "   y : " << corners[i].y <<
        // endl;
        sizes[i] = roi.size();
        // cout << "size height : " << sizes[i].height << "   width : " <<
        // sizes[i].width << endl;
      }
    }
