...
bool graph_loader(graphlab::distributed_control &dc, graph_type &graph,
                  string img_dir) {
  ...
  if (opts.verbose > 2)
    logstream(LOG_EMPH) << "Total number of images: " << graph_files.size()
                        << "\n";
  ...
  // Adding edges between every pair of vertices to create a fully connected
  // graph no duplicate edges are added
...
bool graph_loader(graph_type &graph, string img_dir,
                  vector<CameraParams> &cameras, vector<string> &img_path,
                  vector<int> indices, vector<MatchesInfo> &pairwise_matches) {
  // force a "/" at the end of the path
  // make sure to check that the path is non-empty. (you do not
  // want to make the empty path "" the root path "/" )
  string path = img_dir;
  ...
  if (opts.verbose > 2)
    logstream(LOG_EMPH) << "Total number of vertices in the second graph: "
                        << indices.size() << "\n";
  ...
  // Adding edges between selected pair of vertices to create a subgraph of the
  // fully connected graph no duplicate edges are added

  if (opts.verbose > 2)
    logstream(LOG_EMPH) << "Pairwise_matches size : " << pairwise_matches.size()
                        << "\n";

  for (size_t i = 0; i < pairwise_matches.size(); ++i) {
    if (opts.verbose > 2)
      logstream(LOG_EMPH) << "Pairwise_matches : ("
                          << pairwise_matches[i].src_img_idx << ","
                          << pairwise_matches[i].dst_img_idx << ")\n";

    if (pairwise_matches[i].src_img_idx >= 0 &&
        pairwise_matches[i].dst_img_idx >= 0) {
      if (pairwise_matches[i].src_img_idx <
          pairwise_matches[i].dst_img_idx)  // no duplicate edges are allowed
      {
...
bool vertex_loader(graphlab::distributed_control &dc, graph_type &graph,
                   string img_path) {
  // force a "/" at the end of the path
  // make sure to check that the path is non-empty. (you do not
  // want to make the empty path "" the root path "/" )
...
bool vertex_loader(graph_type &graph, string img_path,
...
bool edge_loader(graph_type &graph, const std::string &fname,
  ...
  if (opts.verbose > 0)
    logstream(LOG_EMPH) << "Here's the input: " << textline << "\n"
                        << vid << "\n";

  // Line should contain at least 1 more number (degree of node)
  if (!strm.good()) {
    logstream(LOG_ERROR) << "The following ajacency list line is "
                            "incomplete(check adj_list standard):\n"
                         << textline << std::endl;
    return EXIT_FAILURE;
  }
...
struct ImgArea {
  ...
  void save(graphlab::oarchive &arc) const { arc << full_img_area; }
  void load(graphlab::iarchive &arc) { arc >> full_img_area; }

  ImgArea operator+=(ImgArea other)  // computes max
  {
    ImgArea max;
    max.full_img_area = (full_img_area > other.full_img_area)
                            ? full_img_area
                            : other.full_img_area;
    return max;
  }
...
void set_scales(engine_type::icontext_type &context, ImgArea &largestimg) {
  if (opts.work_megapix > 0)
    opts.work_scale =
        min(1.0, sqrt(opts.work_megapix * 1e6 / largestimg.full_img_area));

  if (opts.seam_megapix > 0)
    opts.seam_scale =
        min(1.0, sqrt(opts.seam_megapix * 1e6 / largestimg.full_img_area));

  if (opts.compose_megapix > 0)
    opts.compose_scale =
        min(1.0, sqrt(opts.compose_megapix * 1e6 / largestimg.full_img_area));

  if (opts.output_megapix > 0)
    opts.output_scale =
        min(1.0, sqrt(opts.output_megapix * 1e6 / largestimg.full_img_area));
...
void warp_images(graph_type::vertex_type &vertex) {
  ...
  double seam_scale =
      min(1.0, sqrt(opts.seam_megapix * 1e6 / full_img.size().area()));
...
void composite_images(graph_type::vertex_type &vertex) {
  ...
  Ptr<RotationWarper> warper = warper_creator->create(
      static_cast<float>(opts.warped_image_scale * opts.compose_work_aspect));
...
void find_seams(graph_type::edge_type &edge) {
  ...
  // seam_finder = new
  // detail::GraphCutSeamFinder(GraphCutSeamFinderBase::COST_COLOR);

  // Code from PairwiseSeamFinder::Impl::findInPair()
  // Mat img1 = images_[first], img2 = images_[second];
  Mat &img1 = vdata1.img_warped_f;
  Mat &img2 = vdata2.img_warped_f;
  vector<Mat> src;
  src.push_back(img1);
  src.push_back(img2);
  ...
  Mat &mask1 = vdata1.mask_warped;
  Mat &mask2 = vdata2.mask_warped;
  // Point tl1 = corners_[first], tl2 = corners_[second];
  Point2f &tl1 = vdata1.corner;
  Point2f &tl2 = vdata2.corner;
    ...
    for (int y = 0; y < img_size.height; ++y) {
      for (int x = 0; x < img_size.width; ++x) {
        ...
        graph.addTermWeights(
            v, submask1.at<uchar>(y, x) ? opts.terminal_cost : 0.f,
    ...
    for (int y = 0; y < img_size.height; ++y) {
      for (int x = 0; x < img_size.width; ++x) {
        ...
        if (x < img_size.width - 1) {
          float weight =
              normL2(subimg1.at<Point3f>(y, x), subimg2.at<Point3f>(y, x)) +
              normL2(subimg1.at<Point3f>(y, x + 1),
                     subimg2.at<Point3f>(y, x + 1)) +
              weight_eps;
        ...
        if (y < img_size.height - 1) {
          float weight =
              normL2(subimg1.at<Point3f>(y, x), subimg2.at<Point3f>(y, x)) +
              normL2(subimg1.at<Point3f>(y + 1, x),
                     subimg2.at<Point3f>(y + 1, x)) +
              weight_eps;
      ...
      for (int x = 0; x < img_size.width; ++x) {
        ...
        if (x < img_size.width - 1) {
          float grad = subdx1.at<float>(y, x) + subdx1.at<float>(y, x + 1) +
                       subdx2.at<float>(y, x) + subdx2.at<float>(y, x + 1) +
                       weight_eps;
          float weight =
              (normL2(subimg1.at<Point3f>(y, x), subimg2.at<Point3f>(y, x)) +
               normL2(subimg1.at<Point3f>(y, x + 1),
        ...
        if (y < img_size.height - 1) {
          float grad = subdy1.at<float>(y, x) + subdy1.at<float>(y + 1, x) +
                       subdy2.at<float>(y, x) + subdy2.at<float>(y + 1, x) +
                       weight_eps;
          float weight =
              (normL2(subimg1.at<Point3f>(y, x), subimg2.at<Point3f>(y, x)) +
               normL2(subimg1.at<Point3f>(y + 1, x),
...
vector<edge_data> compile_edges(engine_type::icontext_type &context,
  ...
  int pair_idx =
      edge.source().id() * context.num_vertices() + edge.target().id();
