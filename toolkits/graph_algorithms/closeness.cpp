 ...
 *       can become inaccurate in the course of calculations and must be
 * recalculated by traversing the shortest path tree to get an accurate result.
...
class Gather {
 public:
  ...
  void load(graphlab::iarchive& iarc) { iarc >> id >> cost >> edge_count; }
};
...
class GatherMultiTree {
 public:
  ...
  GatherMultiTree() { edge_count = 0; }

  GatherMultiTree& operator+=(const GatherMultiTree& other) { return *this; }

  void save(graphlab::oarchive& oarc) const { oarc << content << edge_count; }

  void load(graphlab::iarchive& iarc) { iarc >> content >> edge_count; }
};
 ...
 */
class ClearBooleans : public graphlab::ivertex_program<graph_type, gather_type>,
 ...
 public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    for (std::map<long, DjikstraNode>::const_iterator iter =
             vertex.data().djikstra_pieces.begin();
 ...
 *
 * The process terminates when all nodes active have no neighbors that are not
 * done.
 *
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    for (std::map<long, DjikstraNode>::const_iterator iter =
      ...
      if (vertex.data().djikstra_pieces[key].launched == false) {
        ...
        if (vertex.data().djikstra_pieces[key].cost >
            total.content.find(key)->second.cost) {
          vertex.data().djikstra_pieces[key].cost =
              total.content.find(key)->second.cost;
          vertex.data().djikstra_pieces[key].id =
              total.content.find(key)->second.id;
        } else {
    ...
    for (std::map<long, DjikstraNode>::const_iterator iter =
         ...
         iter != vertex.data().djikstra_pieces.end(); ++iter) {
      long key = iter->first;
      if (vertex.data().djikstra_pieces.find(key) ==
          vertex.data().djikstra_pieces.end()) {
        vertex.data().djikstra_pieces[key].launched = true;
        vertex.data().edge_count = total.edge_count;
        vertex.data().djikstra_pieces[key].cost =
            total.content.find(key)->second.cost;
...
/*
 * For every node, print the previous node in its spanning tree for all spanning
 * trees this node is in.
 *
 */
...
/*
 * Select ~3000 root nodes or an exact count which gives up around +/-3%
 * accuracy in prestige measures. It is a constant memory random selector.
 */
...
class ClosenessGather {
 public:
  ...
  void save(graphlab::oarchive& oarc) const { oarc << counts << edge_count; }

  void load(graphlab::iarchive& iarc) { iarc >> counts >> edge_count; }
 ...
 * and walk back towards the leafs.  Record the shortest path costs for every
 * node along the way, halting when no signaled node has a neighbor that is not
 * done.
 *
 * This generates a random sample of the set of all spanning trees for the graph
 * of a about 3000 trees (or exact value if fewer than 3000 nodes) that gives
 * the closeness metric of each node to +/-3%. It is needed to sum the closeness
 * for each djikstra id to get the value without using the output function.
 *
 */
class ClosenessAlgorithm
 ...
 public:
  ...
  ClosenessGather gather(icontext_type& context, const vertex_type& vertex,
                         edge_type& edge) const {
    ClosenessGather g;
    for (std::map<long, DjikstraNode>::const_iterator iter =
             vertex.data().djikstra_pieces.begin();
      ...
      if (edge.target().data().djikstra_pieces[key].id == vertex.id()) {
        if (edge.source().data().djikstra_pieces[key].launched == true) {
          g.counts[key] =
              edge.source().data().djikstra_pieces[key].cost + edge.data();
          g.edge_count[key] = 1;
        }
      }
    }
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    for (std::map<long, DjikstraNode>::const_iterator iter =
      ...
      if ((vertex.data().djikstra_pieces[key].launched == true) &&
          (vertex.data().djikstra_pieces[key].done == false) &&
          (((long)vertex.data().djikstra_pieces[key].cost) ==
           total.edge_count.find(key)->second)) {
        vertex.data().djikstra_pieces[key].done = true;
        vertex.data().djikstra_pieces[key].cost =
            (double)total.edge_count.find(key)->second;
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option(
      "samplesize", desired_sample_size,
      "(Sample size) the number of spanning trees to calculate");
  ...
  graphlab::omni_engine<DjikstraAlgorithm> engine(dc, graph, "asynchronous",
                                                  clopts);
  ...
  dc.cout() << "Finished Djikstra engine in " << runtime << " seconds."
            << std::endl;

  graphlab::omni_engine<ClearBooleans> engine2(dc, graph, "asynchronous",
                                               clopts);
  engine2.signal_all();
  engine2.start();

  const float runtime2 = engine.elapsed_seconds();
  dc.cout() << "Finished resetting the graph in " << runtime2 << " seconds."
            << std::endl;

  graphlab::omni_engine<ClosenessAlgorithm> engine3(dc, graph, "asynchronous",
                                                    clopts);
  engine3.signal_vset(start_set);
  engine3.start();

  const float runtime3 = engine.elapsed_seconds();
  dc.cout() << "Finished the closeness engine in " << runtime3 << " seconds."
            << std::endl;
