...
class PrestigeAlgorithm
 ...
 public:
  ...
  PrestigeGather gather(icontext_type& context, const vertex_type& vertex,
                        edge_type& edge) const {
    PrestigeGather g;
    for (std::map<long, DjikstraNode>::const_iterator iter =
             vertex.data().djikstra_pieces.begin();
...
int main(int argc, char** argv) {
  ...
  dc.cout() << "Finished graph reset in " << runtime2 << " seconds."
            << std::endl;

  graphlab::omni_engine<PrestigeAlgorithm> engine3(dc, graph, "asynchronous",
                                                   clopts);
  engine3.signal_vset(start_set);
  engine3.start();

  const float runtime3 = engine.elapsed_seconds();
  dc.cout() << "Finished prestige calculations in " << runtime3 << " seconds."
            << std::endl;
