 ...
 * the gather step). Each signals the node referenced in its internal spanning
 * tree record. This is performed simultaneously for each spanning tree in the
 * set.
 *
...
class BetweenessAlgorithm
 ...
 public:
  ...
  BetweenessGather gather(icontext_type& context, const vertex_type& vertex,
                          edge_type& edge) const {
    BetweenessGather g;
    for (std::map<long, DjikstraNode>::const_iterator iter =
             vertex.data().djikstra_pieces.begin();
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    for (std::map<long, DjikstraNode>::const_iterator iter =
      ...
      if ((vertex.data().djikstra_pieces[key].launched == true) &&
        ...
        vertex.data().djikstra_pieces[key].done = true;
        vertex.data().djikstra_pieces[key].cost =
            fmax(1.0, (double)total.edge_count.find(key)->second);
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("samplesize", desired_vertices_count,
                       "(Sample Size) Number of spanning trees to use");
  ...
  dc.cout() << "Finished resetting graph engine in " << runtime2 << " seconds."
            << std::endl;

  graphlab::omni_engine<BetweenessAlgorithm> engine3(dc, graph, "asynchronous",
                                                     clopts);
  engine3.signal_all();
  engine3.start();
  const float runtime3 = engine.elapsed_seconds();
  dc.cout() << "Finished Betweeness engine in " << runtime3 << " seconds."
            << std::endl;
