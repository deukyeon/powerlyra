 ...
 *
 *
 * 2014.02  implement word search application for testing bipartite-aware
 * partitiong with affinity
 *
 */
...
struct vertex_t {
  ...
  void load(graphlab::iarchive& arc) { arc >> is_doc >> count >> str_vec; }
...
inline bool graph_loader(graph_type& graph, const std::string& filename,
  ...
  const bool success =
      qi::parse(line.begin(), line.end(),
                //  Begin grammar
                (qi::omit[qi::ulong_[phoenix::ref(source_id) = qi::_1]] >>
                 qi::omit[+qi::space] >>  //-qi::char_(',') >>
...
edge_data_type signal_target(
    graphlab::omni_engine<wsearch>::icontext_type& context,
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options clopts(
      "Word serach application for data affinity.");
