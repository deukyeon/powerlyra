...
typedef graphlab::distributed_graph<vertex_data_type, graphlab::empty>
    graph_type;
...
class cascades : public graphlab::ivertex_program<graph_type, gather_type>,
 ...
 public:
  ...
  gather_type gather(icontext_type& context, const vertex_type& vertex,
    ...
    vertex_data_type neighbor_status =
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    // occurs
    // if vertex.data == SUSCEPTIBLE, then do (total) dice rolls (each time
    // comparing the result to infection_chance). if any of them show up as
    // positive, set vertex.data to INFECTED.
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option(
      ...
      "Chance of recovery for an infected individual at each step. Required.");
  clopts.attach_option("infection chance", infection,
                       "Chance of infection for a susceptible individual per "
                       "person at each step. Required.");

  clopts.attach_option(
      "iterations", iterations,
      "If set, will force the use of synchronous engine overriding any engine "
      "option set by the --engine parameter. Runs cascades for a fixed number "
      "of iterations. Also overrides the max_iterations option in the engine.");
  ...
  if (iterations != -1) {
    ...
    dc.cout()
        << "--iterations set. Forcing Synchronous engine, and running for "
