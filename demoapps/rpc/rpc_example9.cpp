...
struct test_struct {
  ...
  test_struct(distributed_control& dc) : rmi(dc, this) { dc.barrier(); }

  void test_blob(size_t len, wild_pointer w) {
    assert(len == sizeof(procid_t));
    std::cout << "split call from : "
              << *reinterpret_cast<const procid_t*>(w.ptr) << "\n";
  }

  void print(int val) {
    std::cout << rmi.procid() << ": Receiving print with value : " << val
              << std::endl;
  }
