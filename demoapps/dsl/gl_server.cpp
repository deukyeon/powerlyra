...
//  void edge_get_reduce(std::vector<graph_type::edge_data_type>& ev, const
//  graph_type::edge_data_type ed) {
//      ev.push_back(ed);
//  }

// void vertex_get_reduce(std::vector<graph_type::vertex_data_type>& vv, const
// graph_type::edge_data_type vd) {
//     vv.push_back(vd);
// }

// std::vector<edge_data_type> get_neighboring_edges() {
//     return current_context->map_reduce<std::vector<edge_data_type>
//     >(GET_EDGE_DATA, ALL_EDGES);
// }
// std::vector<vertex_data_type> get_neighboring_vertices() {
//     return current_context->map_reduce<std::vector<vertex_data_type>
//     >(GET_VERTEX_DATA, ALL_EDGES);
// }

vertex_data_type reduce_neighbors(edge_dir_type d,
                                  engine_type::context_type* ctx) {
...
//     return current_context->map_reduce<edge_data_type>(GET_EDGE_DATA,
//     ALL_EDGES);
// }
...
void server_program(engine_type::context_type& context,
  ...
  f._reduce_neighbors =
      (vertex_data_type(*)(edge_dir_type, void*))reduce_neighbors;
  ...
  // float last_change = std::fabs((vertex.data()- prev) /
  // vertex.num_out_edges()); if (last_change > TOLERANCE) {
...
int main(int argc, char** argv) {
  ...
  void* handle = dlopen(
      "/home/emullen/graphlab/graphlab2.2/graphlabapi/debug/demoapps/dsl/"
      "libgen_impl.so",
      RTLD_LAZY);
  if (handle == NULL) {
  ...
  // pagerank_map = (float (*)(const graphlab::distributed_graph<float,
  // graphlab::empty>::vertex_type&))dlsym(pagerank_handle, "pagerank_map");
  // assert(pagerank_map != NULL);
  // pagerank_combine = (void (*)(float&, const float&))dlsym(pagerank_handle,
  // "pagerank_combine"); assert(pagerank_combine != NULL);
  user_program = (void (*)(user_funs*))dlsym(handle, "user_program");
  assert(user_program != NULL);
  vertex_reduce = (void (*)(vertex_data_type&, const vertex_data_type&))dlsym(
      handle, "vertex_reduce");
  assert(vertex_reduce != NULL);
  // edge_reduce = (void (*)(edge_data_type&,const
  // edge_data_type&))dlsym(handle, "edge_reduce"); assert(edge_reduce != NULL);
  ...
  // 			     edge_get_map,
  // 			     edge_get_reduce);
  engine.register_map_reduce(GET_VERTEX_DATA, vertex_get_map,
                             vertex_get_reduce);
