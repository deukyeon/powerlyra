...
unsigned List::size() const {
  unsigned count = 0;
  for (const Link *l = head(); l != 0; l = l->next()) ++count;
  return count;
}
...
void List::activateAll() {
  for (Link *l = _head; l != 0; l = l->justNext()) l->setActive(true);
}
...
Link *Link::next() {
  Link *l = _next;
  while (l && !l->_active) l = l->_next;
  return l;
}

Link *Link::prev() {
  Link *l = _prev;
  while (l && !l->_active) l = l->_prev;
  return l;
}
